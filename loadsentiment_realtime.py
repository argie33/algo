    ...
    def analyze_financial_text(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            if TEXTBLOB_AVAILABLE:
                ...
                results["textblob_subjectivity"] = safe_float(
                    blob.sentiment.subjectivity, 0.0
                )

            # VADER analysis (good for social media)
            if self.vader_analyzer:
                vader_scores = self.vader_analyzer.polarity_scores(text)
                results["vader_compound"] = safe_float(
                    vader_scores.get("compound", 0.0), 0.0
                )
                results["vader_positive"] = safe_float(
                    vader_scores.get("pos", 0.0), 0.0
                )
                results["vader_negative"] = safe_float(
                    vader_scores.get("neg", 0.0), 0.0
                )
...
class RealTimeSentimentCollector:
    ...
    def get_real_time_news_sentiment(self) -> Dict:
        ...
        try:
            ...
            if sentiment_scores:
                ...
                if finbert_scores:
                    result["finbert_sentiment"] = np.mean(
                        [s for s in finbert_scores if s != 0]
                    )
                if vader_scores:
                    result["vader_sentiment"] = np.mean(
                        [s for s in vader_scores if s != 0]
                    )
            ...
            result["news_volume_trend"] = min(
                len(recent_news) / 5.0, 2.0
            )  # Normalize to ~5 articles baseline
    ...
    def get_social_media_sentiment(self) -> Dict:
        ...
        try:
            ...
            if abs(daily_change) > 0.05:  # 5% move
                ...
                mention_multiplier = (
                    1 + abs(daily_change) * 10
                )  # More mentions on big moves
            else:
            ...
            result.update(
                {
                    ...
                    "reddit_sentiment": social_sentiment
                    * 0.9,  # Reddit slightly more negative
    ...
    def get_analyst_sentiment_changes(self) -> Dict:
        ...
        try:
            ...
            logging.error(
                f"Error collecting analyst sentiment changes for {self.symbol}: {e}"
            )
    ...
    def get_alternative_sentiment_sources(self) -> Dict:
        ...
        try:
            ...
            logging.error(
                f"Error collecting alternative sentiment for {self.symbol}: {e}"
            )
    ...
    try:
        ...
        if sentiment_components and weights:
            weighted_sentiment = sum(
                s * w for s, w in zip(sentiment_components, weights)
            )
            total_weight = sum(weights)
...
def load_realtime_sentiment_batch(
    symbols: List[str], conn, cur, batch_size: int = 8
) -> Tuple[int, int]:
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        logging.info(
            f"Processing real-time sentiment batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
        ...
        if sentiment_data:
            try:
                ...
                for item in sentiment_data:
                    insert_data.append(
                        (
                            ...
                            item["date"],
                            # News sentiment
                            item.get("news_article_count", 0),
                            item.get("news_sentiment_score", 0),
                            item.get("news_sentiment_strength", 0),
                            item.get("news_source_diversity", 0),
                            item.get("news_volume_trend", 0),
                            item.get("positive_news_ratio", 0),
                            item.get("negative_news_ratio", 0),
                            item.get("finbert_sentiment", 0),
                            item.get("vader_sentiment", 0),
                            # Social sentiment
                            item.get("social_mention_count", 0),
                            item.get("social_sentiment_score", 0),
                            item.get("social_sentiment_volume", 0),
                            item.get("reddit_sentiment", 0),
                            item.get("social_momentum", 0),
                            item.get("viral_score", 0),
                            # Analyst sentiment
                            item.get("recent_upgrades", 0),
                            item.get("recent_downgrades", 0),
                            item.get("analyst_momentum", 0),
                            item.get("recommendation_change", 0),
                            item.get("estimate_revisions_up", 0),
                            item.get("estimate_revisions_down", 0),
                            item.get("price_target_change", 0),
                            # Alternative sources
                            item.get("sec_filing_sentiment", 0),
                            item.get("earnings_call_sentiment", 0),
                            item.get("management_sentiment", 0),
                            item.get("industry_sentiment", 0),
                            item.get("economic_sentiment", 0),
                            # Composite
                            item.get("composite_sentiment", 0),
                            item.get("sentiment_momentum", 0),
                        )
                    )
                ...
                logging.info(
                    f"Sentiment batch {batch_num} inserted {len(sentiment_data)} symbols successfully"
                )

            except Exception as e:
                logging.error(
                    f"Database insert error for sentiment batch {batch_num}: {e}"
                )
                conn.rollback()
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process sentiment for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
