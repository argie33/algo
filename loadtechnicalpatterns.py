...
class TechnicalPatternDetector:
    ...
    def _cluster_levels(
        self, levels: np.ndarray, tolerance: float = 0.02
    ) -> List[float]:
    ...
    def _calculate_level_strength(
        self, levels: List[float], price_series: np.ndarray
    ) -> Dict:
        """Calculate strength of support/resistance levels"""
        strength = {}
        for level in levels:
            touches = sum(
                1 for price in price_series if abs(price - level) / level <= 0.01
            )
        ...
        if len(peaks) >= 3:
            for i in range(len(peaks) - 2):
                ...
                if (
                    ...
                    and abs(highs[left_shoulder] - highs[right_shoulder]) / highs[head]
                    < 0.05
                ):
            ...
            for i in range(len(peaks) - 1):
                ...
                if abs(highs[peak1] - highs[peak2]) / highs[peak1] < 0.03:
                    ...
                    pattern = {
                        ...
                        "breakout_confirmed": self.data["Close"].iloc[-1]
                        < lows[trough],
                        "volume_confirmation": self._check_volume_confirmation(
                            peak1, peak2
                        ),
    ...
    def _detect_triangles(self) -> List[Dict]:
        ...
        if len(peaks) >= 3 and len(troughs) >= 3:
            ...
            peak_slope, peak_intercept, peak_r, _, _ = linregress(
                peaks, recent_highs[peaks]
            )
            trough_slope, trough_intercept, trough_r, _, _ = linregress(
                troughs, recent_lows[troughs]
            )
            ...
            pattern = {
                ...
                "breakout_level": self._calculate_triangle_breakout_level(
        ...
        for i in range(len(self.data)):
            ...
            if total_range > 0 and body_size / total_range < 0.1:
                ...
                pattern = {
                    ...
                    "confidence": 0.6,
                    "signal_strength": self._calculate_doji_strength(
    ...
    def _detect_engulfing(self) -> List[Dict]:
        ...
        for i in range(1, len(self.data)):
            ...
            if (
                ...
                and curr_close > curr_open  # Current candle bullish
                and curr_open < prev_close  # Current opens below previous close
                and curr_close > prev_open  # Current closes above previous open
            ...
            # Bearish Engulfing
                ...
                and curr_close < curr_open  # Current candle bearish
                and curr_open > prev_close  # Current opens above previous close
                and curr_close < prev_open  # Current closes below previous open
                and curr_body > prev_body
            ):  # Current body engulfs previous
    ...
    def _calculate_hs_confidence(
        self, left: int, head: int, right: int, neckline: float
    ) -> float:
    ...
    def _calculate_triangle_breakout_level(
    ...
    def _get_market_context(self, index: int) -> str:
        ...
        trend = (
            "Uptrend" if recent_prices.iloc[-1] > recent_prices.iloc[0] else "Downtrend"
        )
...
def create_patterns_table(cursor):
    ...
    # Create indices
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_patterns_symbol ON technical_patterns(symbol)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_patterns_type ON technical_patterns(pattern_type)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_patterns_date ON technical_patterns(end_date DESC)"
    )
    ...
    try:
        ...
        hist = stock.history(
            ...
            interval=(
                "1d"
                if timeframe == "daily"
                else "1wk" if timeframe == "weekly" else "1mo"
            ),
        )
...
def save_patterns_to_db(cursor, pattern_results: List[Dict]):
    ...
    if insert_data:
        ...
        if symbols:
            cursor.execute(
                "DELETE FROM technical_patterns WHERE symbol = ANY(%s)", (symbols,)
            )
    ...
    try:
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(process_symbol_patterns, symbol) for symbol in batch
                ]
        ...
        logging.info(
            f"Processing complete: {len(successful_results)}/{len(symbols)} symbols processed"
        )
