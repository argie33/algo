            ...
            if last_update:
                ...
                if minutes_since_update > 60:  # More than 1 hour
                    alert = Alert(
                        ...
                        severity=AlertSeverity.HIGH if minutes_since_update > 120 else AlertSeverity.MEDIUM,
    ...
    def check(self) -> List[Alert]:
        ...
        for metric, current_value in self.portfolio_metrics.items():
            ...
            if threshold is not None:
                ...
                elif metric == 'var_95' and current_value < threshold:  # VaR is negative
                    alert = Alert(
                        id=f"portfolio_{metric}_{int(time.time())}",
                        timestamp=datetime.now(),
                        alert_type=AlertType.PORTFOLIO,
...
class AlertManager:
    ...
    def get_alert_summary(self, hours_back: int = 24) -> Dict:
        ...
        return {
            ...
            'most_recent': recent_alerts[0].timestamp.isoformat() if recent_alerts else None
        }
...
class MonitoringSystem:
    ...
    def __init__(self, email_config: Optional[Dict] = None):
        ...
        performance_thresholds = [
            MetricThreshold('cpu_usage', 80.0, 'gt', AlertSeverity.HIGH, AlertType.SYSTEM_HEALTH),
            MetricThreshold('memory_usage', 85.0, 'gt', AlertSeverity.HIGH, AlertType.SYSTEM_HEALTH),
            MetricThreshold('api_response_time', 1000.0, 'gt', AlertSeverity.MEDIUM, AlertType.PERFORMANCE),
            MetricThreshold('data_freshness', 30.0, 'gt', AlertSeverity.MEDIUM, AlertType.DATA_QUALITY)
        ]
        
        # Initialize monitors
        self.performance_monitor = PerformanceMonitor(performance_thresholds)
        self.data_quality_monitor = DataQualityMonitor(['price_data', 'financial_data', 'news_data'])
        self.portfolio_monitor = PortfolioMonitor({'max_drawdown': 0.15, 'var_95': -50000})
        self.trading_signal_monitor = TradingSignalMonitor({'momentum': 0.8, 'pattern': 0.7})
...
def main():
    ...
    monitoring_system.data_quality_monitor.update_quality_score('price_data', 45.0)  # Low quality
    monitoring_system.portfolio_monitor.update_portfolio_metrics({'max_drawdown': 0.18})  # High drawdown
