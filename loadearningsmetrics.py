...
MAX_WORKERS = min(
    os.cpu_count() or 1, 4
)  # Limit to available CPUs or 4, whichever is smaller
...
def safe_numeric_convert(value):
    ...
    if isinstance(value, str):
        try:
            ...
            return (
                float(value)
                if value.strip() not in ["", "N/A", "null", "NULL", "nan", "NaN"]
                else None
            )
...
def calculate_annual_eps_growth(eps_series, years):
    ...
    if len(eps_series) >= 4:
        ...
        current_annual = (
            sum(x for x in current_values if x is not None)
            if any(x is not None for x in current_values)
            else None
        )
    else:
    ...
    if len(eps_series) >= quarters + 4:
        past_values = [
            safe_numeric_convert(x) for x in eps_series.iloc[-quarters - 4 : -quarters]
        ]
        past_annual = (
            sum(x for x in past_values if x is not None)
            if any(x is not None for x in past_values)
            else None
        )
    else:
...
def calculate_consecutive_eps_growth_years(eps_series):
    ...
    for i in range(4, len(eps_series), 4):
        if i + 4 <= len(eps_series):
            # Safely convert all values before summing
            current_values = [
                safe_numeric_convert(x) for x in eps_series.iloc[i : i + 4]
            ]
            ...
            if (
                current_annual is not None
                and past_annual is not None
                and past_annual > 0
                and current_annual > past_annual
            ):
                years += 1
            else:
                break
...
def process_symbol(symbol, conn_pool):
    ...
    try:
        ...
        for i, row in enumerate(earnings_df.iterrows()):
            ...
            eps_surprise_last_q = (
                row[1]["surprise_percent"]
                if row[1]["surprise_percent"] is not None
                else None
            )

            # Calculate estimate revisions from earnings_estimates table
            eps_estimate_revision_1m = calculate_estimate_revisions_from_db(
                symbol, 1, earnings_estimates
            )
            eps_estimate_revision_3m = calculate_estimate_revisions_from_db(
                symbol, 3, earnings_estimates
            )
            eps_estimate_revision_6m = calculate_estimate_revisions_from_db(
                symbol, 6, earnings_estimates
            )
            ...
            consecutive_eps_growth_years = calculate_consecutive_eps_growth_years(
                eps_series
            )

            # Calculate estimated change this year from earnings_estimates
            eps_estimated_change_this_year = calculate_estimated_change_from_db(
                symbol, earnings_estimates
            )
            ...
            data.append(
                (
                    ...
                    (
                        ...
                        if consecutive_eps_growth_years is not None
                        else None
                    ),
...
def calculate_estimate_revisions_from_db(symbol, months, earnings_estimates):
    ...
    try:
        ...
        if len(earnings_estimates) >= 2:
            latest_estimate = safe_numeric_convert(
                earnings_estimates[0][1]
            )  # avg_estimate from most recent
            earliest_estimate = safe_numeric_convert(
                earnings_estimates[-1][1]
            )  # avg_estimate from oldest

            if (
                earliest_estimate is not None
                and latest_estimate is not None
                and earliest_estimate != 0
            ):
                return (
                    (latest_estimate - earliest_estimate) / abs(earliest_estimate)
                ) * 100
...
def calculate_estimated_change_from_db(symbol, earnings_estimates):
    ...
    try:
        ...
        if (
            current_year_estimate is not None
            and previous_year_estimate is not None
            and previous_year_estimate != 0
        ):
            return (
                (current_year_estimate - previous_year_estimate)
                / abs(previous_year_estimate)
            ) * 100
...
def main():
    ...
    try:
        ...
        logging.info(
            f"Summary: Processed {symbols_processed + symbols_failed} symbols in {elapsed:.2f} seconds"
        )
        logging.info(
            f"Success: {symbols_processed} symbols ({total_inserted} rows inserted)"
        )
        if symbols_failed > 0:
