...
const checkForCommonIssues = async () => {
  ...
  try {
    const healthResponse = await fetch(
      "https://ye9syrnj8c.execute-api.us-east-1.amazonaws.com/dev/health"
    );
    const healthData = await healthResponse.json();
    console.log("✅ API health check successful:", healthData.status);

    const perfResponse = await fetch(
      "https://ye9syrnj8c.execute-api.us-east-1.amazonaws.com/dev/portfolio/performance?timeframe=1Y"
    );
    const perfData = await perfResponse.json();
    console.log(
      "✅ Portfolio performance API working, data points:",
      perfData.data?.performance?.length || 0
    );
  } catch (error) {
...
const investigateIssue = async () => {
  ...
  console.log("\n💡 Recommendations:");
  console.log(
    "1. Open browser to: http://localhost:3000/portfolio/performance"
  );
  ...
  console.log(
    "3. If that works, the issue is specific to PortfolioPerformance component"
  );
