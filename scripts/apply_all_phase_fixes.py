            ...
            if 'from utils' in modified:
                ...
                for i, line in enumerate(lines):
                    if line.startswith('from utils'):
                        import_end = i
                lines.insert(import_end + 1, 'from utils.contexts import LoaderErrorContext')
                modified = '\n'.join(lines)
                changes += 1
        ...
        if original.count('cur.execute(') > 1 and 'try:' in original:
            ...
            if 'from utils.decorators import' not in modified:
                if 'from utils' in modified:
                    ...
                    for i, line in enumerate(lines):
                        if line.startswith('from utils'):
                            lines.insert(i + 1, 'from utils.decorators import transactional')
                            modified = '\n'.join(lines)
                            break
                    changes += 1
        ...
        if 'requests.' in modified and '@external_api_handler' not in modified:
            if 'from utils.decorators import' not in modified:
                if 'from utils' in modified:
                    ...
                    for i, line in enumerate(lines):
                        if line.startswith('from utils'):
                            lines.insert(i + 1, 'from utils.decorators import external_api_handler')
                            modified = '\n'.join(lines)
                            break
...
def apply_phase6_utilities():
    ...
    for filepath in all_files:
        ...
        if 'try:' in modified and 'except' in modified:
            if 'from utils.error_handlers import' not in modified and 'classify_exception' not in modified:
                ...
                if 'except Exception' in modified:
                    ...
                    for i, line in enumerate(lines):
                        if line.startswith('from') or line.startswith('import'):
                            if not import_added:
                                lines.insert(i + 1, 'from utils.error_handlers import classify_exception, make_error_response')
                                import_added = True
                                changes += 1
                                break
