...
def main():
    ...
    # Suggest dismissals
    logger.info("\n" + "=" * 80)
    logger.info("SUGGESTED DISMISSALS")
    logger.info("=" * 80)

    dismissals = [r for r in results if r["status"] == "MISSING_ENTIRELY"]
    if dismissals:
        logger.info(f"\nTo dismiss these {len(dismissals)} gap(s):")
        for r in dismissals:
            logger.info(
                f"\npython scripts/xbrl_concept_continuity_scan.py --dismiss '{r['cik']}:us-gaap:{r['concept']}' \\"
            )
            logger.info(
                f"  --reason 'Final filing or legitimate absence (no synonym found, company: {r['entity_name']})'"
            )
    else:
