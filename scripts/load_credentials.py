...
def load_credentials_from_database() -> dict[str, str]:
    ...
    try:
        ...
        for db_key, env_var in config_keys.items():
            cur.execute("SELECT value FROM algo_config WHERE key = %s", (db_key,))
            row = cur.fetchone()
            if row and row[0]:
        ...
        if "APCA_API_SECRET_KEY" not in credentials:
            logger.error(
                "[CREDS] APCA_API_SECRET_KEY not found in database (checked: alpaca_api_secret, alpaca_api_secret_key)"
            )
