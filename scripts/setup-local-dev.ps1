...
//
// API_URL is intentionally EMPTY for local dev. The Vite dev server proxies
// /api/* to VITE_PROXY_TARGET (the real API Gateway). This avoids CORS errors
// that occur when the browser makes direct cross-origin requests to AWS.
...
window.__CONFIG__ = {
  ...
  USER_POOL_DOMAIN: '$PoolId'.split('_')[0] + '.auth.us-east-1.amazoncognito.com',
  BUILD_TIME: new Date().toISOString(),
  VERSION: '1.0.0-dev',
  ENVIRONMENT: environment,
};

console.log('[Config] Frontend environment loaded:', window.__CONFIG__.ENVIRONMENT, 'API: (Vite proxy → $ApiUrl)');
"@
