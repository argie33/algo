...
def standardize_api_routes(dry_run: bool = True) -> Tuple[int, int]:
    ...
    for filepath in files:
        ...
        if pattern in original or "handle_db_error" in original:
            print(
                f"  [PHASE 2] {filepath.name}: Found {count_try_except_blocks(original)} try/except blocks"
            )
...
def standardize_loaders(dry_run: bool = True) -> Tuple[int, int]:
    ...
    for filepath in files:
        ...
        if block_count > 0:
            ...
            if not dry_run:
                modified_content = _apply_loader_standardization(
...
def standardize_db_operations(dry_run: bool = True) -> Tuple[int, int]:
    ...
    for filepath in files:
        ...
        if any(
            x in filepath.name for x in ["signal", "metrics", "config", "monitoring"]
        ):
            continue
        ...
        if "cur.execute(" in original and original.count("cur.execute(") > 1:
            ...
            if block_count > 0:
                print(
                    f"  [PHASE 4] {filepath.name}: Found {block_count} try/except blocks (multi-statement)"
                )
                blocks += block_count
                modified += 1
...
def standardize_external_apis(dry_run: bool = True) -> Tuple[int, int]:
    ...
    for filepath in files_to_check:
        ...
        if "requests.get" in original or "requests.post" in original:
            if "timeout=" not in original or original.count(
                "timeout="
            ) < original.count("requests."):
                block_count = count_try_except_blocks(original)
                if block_count > 0:
                    print(
                        f"  [PHASE 5] {filepath.name}: Found {block_count} try/except blocks (external API)"
                    )
                    blocks += block_count
                    modified += 1
...
def main():
    ...
    parser.add_argument(
        "--phase", type=int, choices=[2, 3, 4, 5, 6], help="Run specific phase"
    )
    parser.add_argument("--all", action="store_true", help="Run all phases")
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't modify files, just report"
    )
    ...
    print("=" * 70)
    print(
        f"Mode: {'DRY RUN (no modifications)' if dry_run else 'LIVE MODIFICATIONS'}\n"
    )
