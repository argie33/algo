#!/usr/bin/env python3
"""
Queue only FAILING loaders (skip 10 that already succeeded).
Runs sequentially to prevent API rate limiting cascade.
"""
...
# Loaders that ALREADY SUCCEEDED (skip these)
ALREADY_SUCCEEDED = {
...
FAILING_LOADERS = [l for l in ALL_LOADERS if l not in ALREADY_SUCCEEDED]
...
class SequentialLoaderQueue:
    ...
    def run_sequential(self) -> None:
        log.info(f"\n{'='*60}")
        log.info(f"Queue FAILING LOADERS ONLY: {len(FAILING_LOADERS)} to fix")
        log.info(f"(Skipping {len(ALREADY_SUCCEEDED)} that already succeeded)")
        log.info(f"{'='*60}\n")

        for i, loader in enumerate(FAILING_LOADERS, 1):
            try:
                log.info(f"\n[{i}/{len(FAILING_LOADERS)}] {loader}...")
        ...
        log.info(f"{'='*60}")
        log.info(f"Fixed: {len(self.completed)}/{len(FAILING_LOADERS)}")
        log.info(f"Still failing: {len(self.failed)}")
