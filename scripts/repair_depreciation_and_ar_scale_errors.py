#!/usr/bin/env python3
"""Repair severely-corrupt depreciation_expense and accounts_receivable values.

These fields show >100x divergences from yfinance cross-check data, indicating
systematic SEC XBRL scale/unit conversion errors. Fixes records where the error
ratio is > 100x (clearly corrupt, not plausible reporting differences).

Usage:
    python scripts/repair_depreciation_and_ar_scale_errors.py --dry-run  # preview
    python scripts/repair_depreciation_and_ar_scale_errors.py --fix      # apply fixes
"""
...
def get_repair_candidates():
    """Get fields with >100x divergences."""
    ...
    cursor.execute("""
      ...
      WHERE divergent = true
        AND our_field IN ('depreciation_expense', 'accounts_receivable')
        AND ratio > 100
...
def main():
    ...
    if updated > 0:
        print(f"Successfully repaired {updated} records")
