...
def create_init_file(dir_path: Path):
    ...
    py_files = [f for f in dir_path.glob('*.py') if f.name != '__init__.py' and f.name != '__pycache__']
    if not py_files:
        return
...
def main():
    ...
    # Algo submodules
    algo_submodules = ['signals', 'risk', 'trading', 'monitoring', 'reporting', 'orchestration', 'infrastructure']
    ...
    # Utils submodules
    utils_submodules = ['db', 'logging', 'validation', 'data', 'signals', 'trading', 'loaders', 'external', 'infrastructure', 'ops']
