        ...
        if positions:
            ...
            for pos in positions:
                ...
                if avg_fill_raw is None:
                    logger.warning(
                        f"⚠️  {symbol}: Alpaca returned NO avg_fill_price — position data incomplete"
                    )
                ...
                if current_raw is None:
                    logger.warning(
                        f"⚠️  {symbol}: Alpaca returned NO current_price — position data incomplete"
                    )
                ...
                if value_raw is None:
                    logger.warning(
                        f"⚠️  {symbol}: Alpaca returned NO market_value — position data incomplete"
                    )
                ...
                logger.info(
                    f"{symbol:<8} {qty:<8} {avg_fill_display:<12} {current_display:<12} {value_display:<15}"
                )
        else:
        ...
        with DatabaseContext("read") as cur:
            ...
            if rows:
                ...
                for symbol, qty, avg_entry, current, status in rows:
                    ...
                    logger.info(
                        f"{symbol:<8} {qty:<8} ${avg_entry:<11.2f} ${current:<11.2f} {status_marker:<12}"
                    )
            else:
...
def check_position_drift():
    ...
    try:
        ...
        db_symbols = {
            row[0] for row in db_pos if row[4] == "open"
        }  # Only count 'open', not 'orphaned'
