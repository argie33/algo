#!/usr/bin/env python3
"""Repair moderate scale errors (10-50x divergence).

Fixes records where values diverge 10-50x from yfinance cross-check data.
These are clearly wrong (unit/scale conversion errors), likely due to:
- Stock split adjustments not applied
- Thousands vs. ones unit confusion
- Share count scaling errors

Usage:
    python scripts/repair_moderate_10_50x_errors.py --dry-run  # preview
    python scripts/repair_moderate_10_50x_errors.py --fix      # apply fixes
"""
...
def get_repair_candidates():
    """Get records with 10-50x divergences."""
    ...
    cursor.execute("""
      ...
      WHERE divergent = true AND ratio > 10 AND ratio <= 50
...
def apply_repairs(candidates, dry_run=True):
    ...
    print(f"\n{'=' * 70}")
    print(f"REPAIR PLAN: {total_records} moderate errors (10-50x divergence)")
    print(f"{'=' * 70}\n")
...
def main():
    ...
    if not candidates:
        print("No moderate errors found (10-50x divergence)")
        sys.exit(0)
    ...
    if updated > 0:
        print(f"Successfully repaired {updated} moderate records")
