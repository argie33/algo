...
LOADERS = {
    ...
    "metrics": {
        ...
        "loaders": [
            ...
            "load_analyst_upgrade_downgrade.py",  # RESTORED 2026-07-27: analyst_upgrade_downgrade had no writer since Session 275 (yfinance_snapshot removal); algo/signals/advanced_filters.py::_analyst_score() silently scored every symbol 0 for ~2 months. yfinance's Ticker.upgrades_downgrades is a real, working feed - no free official source exists (SEC doesn't publish analyst ratings), same accepted tradeoff as put_call_ratio.
            "load_analyst_sentiment_analysis.py",  # RESTORED 2026-07-27: same gap class as above, separate table - lambda/api/routes/sentiment.py's /api/sentiment/analyst/* endpoints have been correctly fail-fasting on stale data for ~2 months. yfinance's recommendations_summary + analyst_price_targets restores a real feed.
