...
from scripts.scheduler_log_management import (
    _SCHEDULER_LOG_MAX_BYTES,  # noqa: F401 - re-exported, test suite reaches it via this module
    _SCHEDULER_LOG_RETENTION_DAYS,  # noqa: F401 - re-exported, same reason
    _cleanup_old_loader_run_logs,
    _cleanup_old_scheduler_logs,
    _dated_scheduler_log_path,
    _rotate_scheduler_log_if_oversized,
)
...
# Log rotation/retention for scheduler_invocations.log and per-loader tee logs - extracted to
# scripts/scheduler_log_management.py 2026-09-15 to stay under the file-size ratchet's 2000-line
# hard ceiling. See that module's own docstring for the incident history (11GB unbounded
# scheduler log, Windows open-file-rename failure, 21GB of never-cleaned-up per-loader logs).
...
def main() -> int:
    ...
    logs_dir.mkdir(exist_ok=True)
    _cleanup_old_scheduler_logs(logs_dir)
    _cleanup_old_loader_run_logs(logs_dir)
    _scheduler_log_path = _dated_scheduler_log_path(logs_dir)
