...
def run_pipeline(pipeline_name: str) -> int:
    ...
    LOADER_TIMEOUTS = {  # noqa: N806
        ...
        * 60,  # 900 min (15h) - SESSION 93: measured 761m actual (Session 92 comment), 600m insufficient (NEGATIVE margin). 900m provides ~140m safety buffer
        ...
        "valuations": 60
        * 60,  # 60 min - SESSION 92+ fix: SEC API @ 2 req/sec needs 41min base + retry backoff overhead
        ...
        * 60,  # 60 min - SESSION 93: audit found lowest safety margin (32%), max observed 20.3m. Increased for rate-limit variance (was 45m with insufficient margin)
        ...
        # exponential backoff (3.0s, 4 retries), adequate runtime is 35-40m with safety margin.
        "analyst_upgrades": 45 * 60,  # 45 min - SESSION 92+ fix: match sentiment/earnings timeout for consistency
        # Sector/industry
        ...
        * 60,  # 45 min - SESSION 92+ fix: SEC Form 4/5 bulk downloads + rate limit backoff (was failing at 30m)
        ...
        # symbol processing and database writes).
        # SESSION 92+ FIX: Further increased to 45m - bulk downloads under rate limiting take
        # longer than static 12m base. Real runtime likely 35-40m with backoff overhead.
        "insider_velocity": 45
        * 60,  # 45 min - SESSION 92+ fix: depends on insider_holdings, needs same buffer (was 30m)
        ...
        * 60,  # 60 min - 8-K report scanning (SEC API rate-limited, ~4900 symbols @ 2 req/sec = 2450s+ base)
        ...
        # SESSION 92+ FIX: Increased to 60 min. Like valuations, rate-limited SEC calls
        # hitting 2 req/sec cap with parallel loaders. 41m base + overhead.
        "segment_info": 60 * 60,  # 60 min - SESSION 92+ fix: SEC API rate limiting (was failing at 45m)
