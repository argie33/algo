#!/usr/bin/env python3
# Updated: 2025-10-01 - Calendar data loader with optimized health checks
import functools
import json
import logging
import math
...
def process_symbol(symbol, conn):
    ...
    if "Earnings Date" in calendar_data:
        ...
        if isinstance(earnings_date, list):
            ...
            end_date = (
                pd.to_datetime(earnings_date[1])
        ...
        earnings_title = (
            "Q" + str(calendar_data.get("Earnings Quarter", "")) + " Earnings"
        )
        events_to_insert.append(
            (symbol, "earnings", start_date, end_date, earnings_title)
        )
    ...
    if "Dividend Date" in calendar_data:
        ...
        if ex_date is not None:
            events_to_insert.append(
                (symbol, "dividend", ex_date, ex_date, f"Ex-Dividend ${div_amount:.4f}")
            )
    ...
    logger.info(
        f"Successfully processed {len(events_to_insert)} calendar events for {symbol}"
    )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Completed processing {processed}/{total_symbols} symbols with {failed} failures"
        )
    except Exception:
