...
class InstitutionalHoldingsAnalyzer:
    ...
    def get_institutional_holdings_analysis(self) -> Dict:
        ...
        try:
                ...
                if (
                    ...
                    if "Shares" in institutional_holders.columns:
                        ...
                        if total_shares > 0:
                            ...
                            top_10_shares = institutional_holders.head(10)[
                                "Shares"
                            ].sum()
                            result["top_10_institutions_pct"] = (
                    ...
                    quality_score = self._calculate_institutional_quality(
            ...
            logging.error(
                f"Error analyzing institutional holdings for {self.symbol}: {e}"
            )
        ...
        try:
            ...
            if not hist.empty:
                ...
                if performance_3m > 0.1:  # Strong performance
                    ...
                    momentum_buying = momentum_selling = contrarian_buying = (
                        contrarian_selling
                    ) = 0
                ...
                result.update(
                    {
                        ...
                        "net_institutional_flow": (net_buying - net_selling)
                        * size_factor,
                        "institutional_momentum": (net_buying - net_selling)
                        * size_factor
                        * 0.5,
                    }
                )
                ...
                base_ownership = safe_float(
                    result.get("institutional_ownership_pct", 0.5)
                )
                quality_score = safe_float(
                    result.get("institutional_quality_score", 0.5)
                )
...
class InsiderTradingAnalyzer:
    ...
    def get_insider_trading_analysis(self) -> Dict:
        ...
        try:
            ...
            try:
                        ...
                        try:
                            ...
                            if shares and shares > 0:
                                if (
                                    ...
                                    or "purchase" in transaction_type
                                ):
                                    buys += 1
                                    buy_value += (
                                        value if value else shares * 100
                                    )  # Estimate value
                                elif (
                                    "sell" in transaction_type
                                    or "sale" in transaction_type
                                ):
                                    sells += 1
                ...
                logging.warning(
                    f"Could not get insider transactions for {self.symbol}: {e}"
                )

            # Simulate insider trading analysis if no direct data
            if (
                result["recent_insider_buys"] == 0
                and result["recent_insider_sells"] == 0
            ):
    ...
    def _simulate_insider_trading(self) -> Dict:
        ...
        try:
            ...
            result["ceo_trading_activity"] = (
                1 if buys > sells else -1 if sells > buys else 0
            )
            result["director_trading_activity"] = np.random.choice(
                [-1, 0, 1], p=[0.3, 0.4, 0.3]
            )
                    ...
                    if not calls.empty and not puts.empty:
                        ...
                        if current_price > 0:
                            ...
                            if not near_money_calls.empty and not near_money_puts.empty:
                                ...
                                strikes = sorted(
                                    set(
                                        list(near_money_calls["strike"])
                                        + list(near_money_puts["strike"])
                                    )
                                )
                                ...
                                for strike in strikes:
                                    call_pain = (
                                        near_money_calls[
                                            near_money_calls["strike"] <= strike
                                        ]["openInterest"]
                                        .fillna(0)
                                        .sum()
                                    )
                        ...
                        avg_volume = result["options_volume"] / max(
                            len(calls) + len(puts), 1
                        )
                        if avg_volume > 1000:  # High volume threshold
                            result["unusual_options_activity"] = min(
                                avg_volume / 1000, 3.0
                            )
                        ...
                        result["large_options_trades"] = (
                            large_call_trades + large_put_trades
                        )

                except Exception as e:
                    logging.warning(
                        f"Could not analyze options chain for {self.symbol}: {e}"
                    )
            ...
            result.update(
                {
                    ...
                    - put_call_ratio / 2,  # Convert to sentiment
        ...
        try:
            ...
            if info:
                ...
                if short_percent:
                    ...
                    result["borrow_rate"] = min(
                        base_rate * rate_multiplier, 0.5
                    )  # Cap at 50%

                    # Lower availability with higher short interest
                    availability = max(
                        1 - (short_percent * 2), 0.1
                    )  # Minimum 10% availability
...
def process_symbol_positioning(symbol: str) -> Optional[Dict]:
    ...
    try:
        ...
        institutional_data = (
            institutional_analyzer.get_institutional_holdings_analysis()
        )
...
def calculate_composite_positioning_score(data: Dict) -> float:
    ...
    try:
        ...
        options_score = ((options_sentiment + 1) / 2 * 0.7) + (
            min(unusual_activity / 2, 1) * 0.3
        )
...
def load_positioning_batch(
    symbols: List[str], conn, cur, batch_size: int = 5
) -> Tuple[int, int]:
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        logging.info(
            f"Processing positioning batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
        ...
        if positioning_data:
            try:
                ...
                for item in positioning_data:
                    insert_data.append(
                        (
                            ...
                            item["date"],
                            # Institutional
                            item.get("institutional_ownership_pct", 0),
                            item.get("institutional_holders_count", 0),
                            item.get("top_10_institutions_pct", 0),
                            item.get("institutional_concentration", 0),
                            item.get("recent_institutional_buying", 0),
                            item.get("recent_institutional_selling", 0),
                            item.get("net_institutional_flow", 0),
                            item.get("institutional_momentum", 0),
                            item.get("smart_money_score", 0),
                            item.get("institutional_quality_score", 0),
                            # Insider
                            item.get("insider_ownership_pct", 0),
                            item.get("recent_insider_buys", 0),
                            item.get("recent_insider_sells", 0),
                            item.get("insider_buy_value", 0),
                            item.get("insider_sell_value", 0),
                            item.get("net_insider_trading", 0),
                            item.get("insider_sentiment_score", 0),
                            item.get("ceo_trading_activity", 0),
                            item.get("director_trading_activity", 0),
                            item.get("insider_concentration", 0),
                            # Options
                            item.get("put_call_ratio", 0),
                            item.get("options_volume", 0),
                            item.get("unusual_options_activity", 0),
                            item.get("gamma_exposure", 0),
                            item.get("options_sentiment", 0),
                            item.get("large_options_trades", 0),
                            item.get("options_skew", 0),
                            item.get("max_pain_level", 0),
                            # Short interest
                            item.get("short_interest_pct", 0),
                            item.get("short_ratio", 0),
                            item.get("days_to_cover", 0),
                            item.get("short_squeeze_score", 0),
                            item.get("borrow_rate", 0),
                            item.get("short_availability", 0),
                            # Composite
                ...
                logging.info(
                    f"Positioning batch {batch_num} inserted {len(positioning_data)} symbols successfully"
                )

            except Exception as e:
                logging.error(
                    f"Database insert error for positioning batch {batch_num}: {e}"
                )
                conn.rollback()
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process positioning for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
...
if __name__ == "__main__":
    ...
    for row in cur.fetchall():
        logging.info(
            f"  {row['symbol']} ({row['company_name'][:25]}): "
