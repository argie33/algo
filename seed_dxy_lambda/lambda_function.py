...
def lambda_handler(event, context):
    ...
    try:
        ...
        return {
            ...
            'body': json.dumps({
                'success': True,
                'message': f'Seeded {inserted_count} real DXY_ICE values from Yahoo Finance'
            })
        }
...
def _fetch_dxy_from_yahoo() -> list[dict]:
    ...
    try:
        ...
        raise RuntimeError('[CRITICAL] yfinance library not available. Cannot fetch real DXY data.')
    except Exception as e:
        raise RuntimeError(f'[CRITICAL] Failed to fetch DXY from Yahoo Finance: {e}')
