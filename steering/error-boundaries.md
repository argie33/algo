...
### React Frontend
- [x] Global error handler (window.addEventListener)
- [x] Global unhandledrejection handler
- [x] ErrorBoundary component (full page)
- [x] Route-level ErrorBoundaries (all routes wrapped)
- [x] ApiErrorBanner component
- [x] QuerySection component
- [x] DataSectionErrorBoundary component (NEW)
- [x] Error logging service (createComponentLogger)
- [ ] Error logging backend integration (Sentry/LogRocket)
- [ ] Add DataSectionErrorBoundary to high-risk pages

### Python Dashboard
- [x] Fetcher error handling with retry logic
- [x] error_boundary.py module (NEW)
- [x] error_summary_panel() showing failed fetchers
- [x] render_dashboard() wrapped in try-catch
- [x] Safe data access functions (safe_get, safe_list)
- [x] Panel functions check for _error keys
...
**To test DataSectionErrorBoundary:**
```jsx
<DataSectionErrorBoundary section="Test Section">
  {Math.random() < 0.5 && <Component />}
  {Math.random() >= 0.5 && (() => { throw new Error('Test'); })()}
</DataSectionErrorBoundary>
```
...
2. Monitor error logging service (when configured)
3. Check error IDs in support tickets
4. Alert on error spike (e.g., 10+ errors per minute)

## Best Practices

### When Writing Components

1. **Always wrap data sections:**
   ```jsx
   <DataSectionErrorBoundary section="Data Name" onRetry={refetch}>
     <Component data={data} />
   </DataSectionErrorBoundary>
   ```

2. **Always use QuerySection for queries:**
   ```jsx
   <QuerySection loading={isLoading} error={error} isEmpty={isEmpty}>
     <Component data={data} />
   </QuerySection>
   ```

3. **Never suppress errors** - Let them bubble up or show them explicitly

4. **Always provide retry capability** - User should be able to recover

5. **Always log context** - Help debugging with error metadata

### When Adding New Pages

1. Check that page is wrapped in ErrorBoundary (App.jsx does this)
2. Add DataSectionErrorBoundary to each data section
3. Use QuerySection for each API query
4. Test error rendering in development
5. Test error recovery (retry, navigate, reload)

### When Debugging Errors

1. Check browser DevTools Console for error logs
2. Look for error ID in error UI
3. Check component stack in dev mode
4. Reproduce error in isolation
5. Check if error is in data validation or rendering
6. Add defensive coding (null checks, safe access) as needed

## Files Changed

### Python Dashboard
- **NEW:** `tools/dashboard/error_boundary.py` - Error boundary utilities
- **MODIFIED:** `tools/dashboard/dashboard.py` - Added error panel display and try-catch wrapper

### React Frontend
- **NEW:** `webapp/frontend/src/components/DataSectionErrorBoundary.jsx` - Section-level error boundary
- **NEW:** `webapp/frontend/src/components/README.ErrorBoundaries.md` - Error boundary usage guide
- **EXISTING (no changes needed):** main.jsx, App.jsx, ErrorBoundary.jsx, QueryErrorBoundary.jsx
