...
## Strategy Across All Docs

This codebase is governed by a **lean but complete** steering system:

**Quick Start:** Read `CLAUDE.md` (quick reference + pre-commit rules)

**Full Architecture:** See `steering/` documents:
- `GOVERNANCE.md` (this file) — Architecture, safety, rules
- `OPERATIONS.md` — CI/CD, hotload config, circuit breaker monitoring, troubleshooting
- `DATA_LOADERS.md` — Loader pipeline timing, parallelism tuning, freshness thresholds
- `DEPLOYMENT.md` — Full deployment workflow (code, infrastructure, migrations)
- `API_ARCHITECTURE.md` — Exception hierarchy, error boundaries, validation patterns
- `LINT_POLICY.md` — Type safety, linting enforcement, audit procedures

**Key Principles:**
1. **Fail-fast on critical data** — No silent fallbacks, no degraded datasets
2. **Configuration hotload** — Change thresholds via database, no restart needed
3. **Type-safe everywhere** — mypy strict + Pylint enforcement on critical rules
4. **Trading safety first** — 8 circuit breakers halt on drawdown/loss/volatility
5. **Operational transparency** — Missing data logged at WARNING+, visible to ops

**Onboarding Path:**
- Day 1: Read CLAUDE.md + GOVERNANCE.md (architecture)
- Day 2: Read OPERATIONS.md + DATA_LOADERS.md (how things run)
- Day 3: Read DEPLOYMENT.md + API_ARCHITECTURE.md (how to modify)
- Day 4+: Reference specific docs as needed, audit weekly with `steering/LINT_POLICY.md`
...
| Deploy code to production | DEPLOYMENT.md (code deployment), OPERATIONS.md (verification) |
| Deploy infrastructure change | DEPLOYMENT.md (infrastructure deployment) |
| Add API endpoint | API_ARCHITECTURE.md (exception hierarchy), LINT_POLICY.md (type safety) |
...
| Rotate credentials | CLAUDE.md (AWS credential refresh script) |
