...
5. **Exposure Policy Actions** — Gates new entries only (`halt_new_entries`, `max_new_positions_today`, `max_concentration_pct` at entry); as of 2026-08-24 no longer force-exits, partial-exits, or tightens stops on existing positions purely off the market-wide exposure tier — an open position's own exit logic (`exit_engine.py`'s 11-step hierarchy) decides its fate independent of exposure (see `exposure_policy_no_longer_sells_existing_positions_20260824` in memory). `always_run=True` (same correction as Phase 4).
