...
5. **Exposure Policy Actions** — Gates new entries only (`halt_new_entries`, `max_new_positions_today`, `max_concentration_pct` at entry); as of 2026-08-24 no longer force-exits, partial-exits, or tightens stops on existing positions purely off the market-wide exposure tier — an open position's own exit logic (`exit_engine.py`'s 11-step hierarchy) decides its fate independent of exposure (see `exposure_policy_no_longer_sells_existing_positions_20260824` in memory). `always_run=True` (same correction as Phase 4). The active tier's live entry constraints (`min_composite_score`, `max_concentration_pct`, `max_new_positions_today`, `risk_multiplier`, `halt_new_entries` — `algo/risk/exposure_policy.py`'s `EXPOSURE_TIERS`) are surfaced end-to-end as of 2026-08-25 (TUI compact/expanded panels + web `MarketsHealth.jsx`, via `lambda/api/routes/algo_handlers/signals.py`'s `_TIER_CONFIG`, itself derived from `EXPOSURE_TIERS` — see `exposure_policy_tier_dashboard_gap_fixed_20260824` in memory); previously only `exposure_pct`/`regime`/pillar scores were visible, not what the score actually does to that day's trading.
...
**Signal generation pipeline:** Fetch buy_sell_daily BUY signals → Filter: close > SMA_50, not bottom 40% range, composite_score >= floor threshold, signal_quality_score present (still required, gates Phase 8's `min_signal_quality_score` floor) → Rank by `composite_score` descending (RESTORED 2026-08-27, real-money-readiness review; see `phase7_signal_generation.py`'s module docstring for the full history) → Liquidity check top `LIQUIDITY_CHECK_LIMIT` (20) → Phase 8 re-sorts the survivors by the same `composite_score`, now one consistent ranking end-to-end. History: Session 377 switched the sort key to `signal_quality_score` on a short-term-predictive-power hypothesis; a 2026-08-26 empirical check of live signals found the opposite (near-zero/inverse correlation with forward returns, non-monotonic quintiles) and that `run_backtest.py`'s cited 1.42 Sharpe used the same flawed ranking so wasn't independent confirmation either - see `signal_quality_score_pooled_check_no_predictive_power_found_20260825` and `phase7_sqs_ranking_contradicts_own_predictive_power_finding_20260826` in memory. `run_backtest.py` still ranks by `signal_quality_score` (not composite_score) intentionally - `stock_scores` has no historical date dimension, so ranking backtest history by today's composite_score would be look-ahead-biased; `stock_scores_history` (migration 1221) will eventually have enough depth to backtest this change properly.
