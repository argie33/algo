# Operations: CI/CD Pipeline & Dashboard Diagnostics

## Overview

This document covers two critical operational areas:

1. **CI/CD Pipeline (ci-fast-gates.yml):** 27 min runtime, all gates blocking. Runs on every commit to `main` and every pull request. Blocks merges if any gate fails.
2. **Dashboard Diagnostics:** Real-time data validation for the algo dashboard. Surfaces data issues instead of masking them with placeholder values.

---

# Part 1: CI/CD Pipeline & Quality Gates
...
7. **Linting & Type Checking** (~5 min)
   - Import validation: Runs `scripts/ci_validation.py` to ensure all `.py` files are importable (catches broken imports, incomplete refactors). Critical because MyPy only does static analysis and doesn't execute imports, so broken function references can slip through linting.
...
Run `make install-hooks` to install pre-commit hooks that run on every commit. Hooks are faster (~10s) and give immediate feedback.
...
3. Force-commit without pre-commit hooks (last resort): `git commit --no-verify` (NOT RECOMMENDED)
...
Fix: Run `make format` to auto-fix, then `make lint` and `make type-check` to see remaining errors (type errors require manual fix).

### [FAIL] Tests failed

Cause: Test assertion failed or test crashed

Fix: Run `make test` / `make test-unit` / `pytest tests/test_*.py` / `pytest -k test_name`
...
Common issues: Variable type mismatch, missing required variables, undefined resource references

### [FAIL] Coverage analysis failed

Cause: Code coverage dropped below minimum threshold (75%)

Fix: Run `make coverage` to generate HTML report. Open `htmlcov/index.html` to see coverage, then write tests for uncovered lines. Don't lower thresholds; new code should maintain or improve coverage.
...
**Bottlenecks:** Container build (Trivy, SBOM, Grype, DAST all rebuild image), test execution (full test suite + coverage), Terraform plan (requires AWS credentials, slow without them)
...
# Part 2: Dashboard Data Diagnostics
...
2. Check if NULL is expected (some fields legitimately can be NULL, e.g., performance.equity_vals if no history)
...
- **Code quality standards:** GOVERNANCE.md → "Code Cleanliness"
- **Security baseline:** GOVERNANCE.md → "Security Baseline"
- **System architecture:** GOVERNANCE.md → "System Architecture"
