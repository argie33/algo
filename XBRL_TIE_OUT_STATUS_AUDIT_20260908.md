...
- **XBRL Continuity:** ⚠️ 9 active gaps (mostly small-cap/trust filings, requires triage)
...
- ✅ `check_stock_based_compensation_nonnegative` (annual)
...
- **Test Breakdown:**
  - 5 balance-sheet identity tests (annual + quarterly)
  - 7 cashflow reconciliation tests
  - 4 EPS reconciliation tests (eps + basic)
  - 4 gross profit identity tests (annual + quarterly)
  - 4 pretax-to-net-income tests (annual + quarterly)
  - 5 diluted-vs-basic tests (shares and eps, annual + quarterly)
  - 7 retained earnings tests
  - 3 quick ratio tests
  - 5 current assets tests (annual + quarterly)
  - 4 current liabilities tests (annual + quarterly)
  - 5 long-term debt tests (annual + quarterly)
  - 4 operating income bound tests (annual + quarterly)
  - 5 goodwill tests (annual + quarterly)
  - 5 accounts payable tests (annual + quarterly)
  - 5 cash tests (annual + quarterly)
  - 5 inventory tests (annual + quarterly)
  - 5 accounts receivable tests (quarterly)
  - 5 PPE tests (quarterly)
  - 5 short-term debt tests (annual + quarterly)
  - 5 operating/finance lease tests (annual + quarterly)
  - 5 stock-based compensation tests (annual + quarterly)
  - 5 common stock repurchased tests (annual + quarterly)
  - 5 shares outstanding DEI tests (annual + quarterly)
  - 5 stock scores bounds tests
  - 4 quarterly revenue annual duplicate tests
  - 1 aggregation test (verifies all 55 checks are called)

### Known Exclusions (Deliberately Not Checked)
1. **Segment-sum-to-consolidated** (revenue) — REJECTED for two reasons:
   - FX normalization now fixed (Sept 2026), but
   ...
   - Requires parser rewrite to identify and tag XBRL axis, separate work
...
2. **Full operating-income identity** (revenue - operating_expenses = operating_income) — REJECTED:
   ...
   - R&D, D&A, restructuring, impairments also reduce operating_income
   - One-directional upper-bound check used instead

3. **Quarterly cash-flow reconciliation** (prior_cash + OCF+ICF+FCF = curr_cash) — REJECTED:
   - Live feasibility showed p50=22%, p90=144%, p99=1,532% relative error
   - Root cause not tracked (restatement/re-presentation between filings? quarter-boundary mismatch?)
   - Needs separate investigation, not a tolerance-tuning problem

4. **Retained earnings quarterly port** — DEFERRED:
   - `retained_earnings` column only added to annual table (migration 1234)
   - Concurrent session's migration 1266 may add quarterly variant
   - Will re-evaluate once confirmed live
...
- The ~1,619 dismissed concepts are intentionally excluded (e.g., dimensional details, bookkeeping, schedule line-items)
- No new major-adoption XBRL tags are missing

#### How It Works
- Scans on-disk SEC EDGAR companyfacts cache (auto-built by loaders)
- Diffs real filer-tagged concepts against our source files (sec_income_statement.py, sec_balance_sheet.py, etc.)
- min_companies=50 threshold (production DataPatrol check runs at 50, showing fewer false positives)
- WARN severity (not ERROR) — validates gap existence, not root cause

#### Related Tests
**File:** `tests/unit/test_xbrl_new_concept_checker_20260907.py`
- ✅ `test_no_cache_means_no_finding` — gracefully handles fresh ECS task (no cache yet)
- ✅ `test_undismissed_gap_above_threshold_warns` — detects genuine undismissed concept
- ✅ `test_no_gaps_means_no_finding` — clean state produces no alert
- ✅ `test_scan_failure_logs_error_not_raise` — exception handling
...
Manual triage:
```bash
# Option A: Dismiss as legitimate (e.g., company gone private)
python scripts/xbrl_concept_continuity_scan.py --dismiss "0000350894:us-gaap:Assets" \
  --reason "SEI Investments went dark in 2025 10-K, check if it's legitimate or a filing issue"

# Option B: Add synonym concept to fetch allowlist if it's a taxonomy migration
# (requires investigation of raw SEC companyfacts JSON)
```

#### Test Coverage
**File:** `tests/unit/test_xbrl_concept_continuity_checker_20260907.py`
- ✅ `test_no_cache_means_no_finding` — gracefully handles fresh ECS task (no cache yet)
- ✅ `test_undismissed_gap_warns` — detects genuine continuity gap
- ✅ `test_no_gaps_means_no_finding` — clean state produces no alert
- ✅ `test_scan_failure_logs_error_not_raise` — exception handling
...
| `xbrl_concept_continuity.py` | Filers dropping previously-tagged concepts | ⚠️ 9 gaps (triage needed) |
...
### OPEN / ACTIVE (Current Blockers)
1. **Continuity gaps (9 filers)** — Requires manual triage to separate legitimate from fixable
...
1. **Triage the 9 continuity gaps** (30 min, semi-automated)
   - Run SEC companyfacts API for each CIK:concept pair
   - Determine if it's a synonym concept (fixable) or legitimate absence (dismissable)
   - Update dismiss log or add new concept to allowlist

2. **Confirm migration 1266 landed** (5 min, verification)
...
3. **Segment-sum-to-consolidated parser rewrite** (4-8 hours, complexity TBD)
   - Identify XBRL axis for each segment row (ASC 280 vs. product disaggregation)
   - Add `segment_axis` column (or reuse semantic tagging)
   - Filter segment sum to ASC 280 only
   - Add tie-out check once axis tagging is reliable

### Low Priority (Technical Debt)
4. **Quarterly cash-flow reconciliation root-cause analysis** (4-8 hours, exploratory)
   - Sample 20-30 failing rows across multiple filers
   - Check for restatement/re-presentation between quarterly and annual filings
   - Investigate quarter-boundary cash concept collisions
   - Document findings; decide if check is viable with tighter concept definition
...
scripts/xbrl_continuity_dismissed.json          [Manual dismissals for continuity gaps]
...
3. ✅ Run DataPatrol integration test (if one exists) to confirm alerts flow correctly
4. ⚠️ Continuity gaps may have changed if new data loaded (re-run scan if needed)
