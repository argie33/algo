...
const testEndpoints = [
  ...
  { path: "/health", method: "GET", description: "Main API health check" },
  {
    ...
    method: "GET",
    description: "WebSocket route health check",
  },
  ...
  // Public stock endpoints (no auth required)
  {
    ...
    method: "GET",
    description: "Stock sectors endpoint (public)",
  },
  ...
  // Core stock endpoints (require auth)
  {
    path: "/api/stocks",
    method: "GET",
    description: "Stock listing endpoint (requires auth)",
    expectAuth: true,
  },
  {
    path: "/api/stocks/AAPL",
    method: "GET",
    description: "Individual stock data (requires auth)",
    expectAuth: true,
  },
  ...
  // Screening endpoints (require auth)
  {
    path: "/api/screener",
    method: "GET",
    description: "Stock screening endpoint (requires auth)",
    expectAuth: true,
  },
  ...
  // WebSocket/Live data endpoints
  {
    ...
    method: "GET",
    description: "Live data service status",
  },
];
...
async function makeRequest(endpoint) {
  return new Promise((resolve) => {
      ...
      res.on("end", () => {
        try {
          ...
          const result = {
            ...
            error:
              res.statusCode >= 400 ? parsed.error || parsed.message : null,
          ...
          if (result.success) {
            if (authSuccess) {
              console.log(
                `✅ ${endpoint.path}: ${res.statusCode} (${duration}ms) - Auth Required (Correct)`
              );
            } else {
              console.log(
                `✅ ${endpoint.path}: ${res.statusCode} (${duration}ms) - ${result.hasData ? "Has Data" : "No Data"}`
              );
            }
          } else {
            console.log(
              `❌ ${endpoint.path}: ${res.statusCode} (${duration}ms) - ${result.error || "Error"}`
            );
          }
          ...
          console.log(
            `⚠️  ${endpoint.path}: ${res.statusCode} (${duration}ms) - Invalid JSON response`
          );
          resolve({
            endpoint: endpoint.path,
            description: endpoint.description,
    ...
    req.on("error", (error) => {
      const duration = Date.now() - startTime;
      console.log(
        `❌ ${endpoint.path}: Network Error (${duration}ms) - ${error.message}`
      );
      resolve({
        endpoint: endpoint.path,
        description: endpoint.description,
...
async function runTests() {
  ...
  console.log(
    `✅ Successful: ${successful.length}/${results.length} (${Math.round((successful.length / results.length) * 100)}%)`
  );
  ...
  const routeResults = {
    ...
    stocks: results.filter(
      (r) => r.endpoint.startsWith("/stocks") && !r.endpoint.includes("health")
    ),
    screener: results.filter(
      (r) =>
        r.endpoint.startsWith("/screener") && !r.endpoint.includes("health")
    ),
    websocket: results.filter(
      (r) =>
        r.endpoint.startsWith("/websocket") && !r.endpoint.includes("health")
    ),
  };
  ...
  for (const [routeName, routeResults] of Object.entries(routeResults)) {
    ...
    const status =
      routeSuccess === routeTotal ? "✅" : routeSuccess > 0 ? "⚠️" : "❌";
    console.log(
      `   ${status} ${routeName.toUpperCase()}: ${routeSuccess}/${routeTotal} endpoints working`
    );
  }
  ...
  // Overall assessment
  const healthChecksPassing = routeResults.health.filter(
    (r) => r.success
  ).length;
  const totalHealthChecks = routeResults.health.length;

  if (
    healthChecksPassing === totalHealthChecks &&
    successful.length >= results.length * 0.8
  ) {
    console.log(
      "🎉 ROUTE RESTORATION SUCCESS! All critical endpoints are working."
    );
  } else if (healthChecksPassing === totalHealthChecks) {
    console.log(
      "✅ Health checks passing, but some advanced endpoints may need attention."
    );
  } else {
