...
class RecessionPredictor:
    ...
    def calculate_recession_probability(self) -> Dict:
        ...
        recession_prob = sum(
            indicator_values[name] * self.weights[name] 
    ...
    def _identify_key_risks(self, indicators: Dict[str, EconomicIndicator]) -> List[str]:
        ...
        for name, indicator in indicators.items():
            if indicator.signal_strength >= 0.6:
                if name == 'yield_curve':
                    ...
                    risks.append("Credit market stress indicating financial instability")
...
class MarketRegimeDetector:
    ...
    def detect_regime(self) -> Dict:
        ...
        try:
            ...
            for period_name, period_days in self.lookback_periods.items():
                ...
                regime_indicators[f'{period_name}_volatility'] = period_returns.std() * np.sqrt(252)
    ...
    def _classify_regime(self, indicators: Dict) -> str:
        ...
        if (short_return > 0.1 and medium_return > 0.05 and 
            long_return > 0.0 and short_vol < 0.25):
            return 'bull'
        
        # Bear market criteria  
        elif (short_return < -0.1 and medium_return < -0.05 and
              short_vol > 0.3):
            return 'bear'
...
def main():
    ...
    for name, details in result['indicator_breakdown'].items():
        print(f"  {name}: {details['signal_strength']:.2f} (weight: {details['weight']:.1%}, contribution: {details['contribution']:.3f})")
