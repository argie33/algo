...
def process_symbol_incremental(cur, symbol, timeframe="daily"):
    ...
    if last_signal_date:
        ...
        start_date = last_signal_date - timedelta(
            days=2
        )  # 2-day buffer for recalculation
    ...
    for signal in signals:
        signal_rows.append(
            (
                ...
                signal["macd"],
                signal["volume"],
                signal["volume_avg_10d"],
                signal["price_vs_ma20"],
                signal["price_vs_ma50"],
...
def main():
    ...
    try:
        ...
        for symbol in symbols:
            try:
                ...
                if processed_count % 10 == 0:
                    conn.commit()
                    logging.info(
                        f"Progress: {processed_count}/{len(symbols)} symbols processed"
                    )
        ...
        if deleted_count > 0:
            logging.info(
                f"Cleaned up {deleted_count} old signals before {cleanup_date}"
            )
