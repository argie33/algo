    ...
    try {
        ...
        for (const tableRow of tables.rows) {
            ...
            const dateColumns = structure.rows
                .filter(col => col.data_type === 'date' || col.column_name.toLowerCase().includes('date'))
            ...
            if (dateColumns.length > 0) {
                ...
                console.log(`Date range: ${range.min_date} to ${range.max_date} (${range.unique_dates} unique dates)`);
            }
            ...
            if (symbolColumns.length > 0) {
                ...
                console.log(`Symbols: ${symbols.unique_symbols} unique symbols, ${symbols.total_records} total records`);
        ...
        try {
            ...
            if (apiTest.rows.length === 0) {
                ...
                apiTest.rows.forEach(row => {
                    console.log(`  - ${row.symbol} (${row.company_name}) on ${row.date}: RSI=${row.rsi}, MACD=${row.macd}, SMA20=${row.sma_20}`);
                });
            }
        } catch (error) {
