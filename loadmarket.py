...
class MarketDataCollector:
    ...
    def get_market_data(self, period: str = "1y") -> Optional[Dict]:
        ...
        try:
            ...
            high_52w = (
                hist["High"].tail(252).max() if len(hist) >= 252 else hist["High"].max()
            )
            low_52w = (
                hist["Low"].tail(252).min() if len(hist) >= 252 else hist["Low"].min()
            )
                ...
                try:
                    ...
                    if not spy.empty and len(spy) == len(hist):
                        ...
                        if len(market_returns) > 30 and len(asset_returns) > 30:
                            covariance = np.cov(
                                asset_returns.tail(252), market_returns.tail(252)
                            )[0][1]
            ...
            result = {
                ...
                "price_vs_sma_20": (
                    safe_float((latest_price - sma_20) / sma_20) if sma_20 else None
                ),
                "price_vs_sma_50": (
                    safe_float((latest_price - sma_50) / sma_50) if sma_50 else None
                ),
                "price_vs_sma_200": (
                    safe_float((latest_price - sma_200) / sma_200) if sma_200 else None
                ),
                # High/Low metrics
                "high_52w": safe_float(high_52w),
                "low_52w": safe_float(low_52w),
                "distance_from_high": (
                    safe_float((latest_price - high_52w) / high_52w)
                    if high_52w
                    else None
                ),
                "distance_from_low": (
                    safe_float((latest_price - low_52w) / low_52w) if low_52w else None
                ),
                # Volume and beta
                "avg_volume_30d": safe_float(avg_volume_30d),
                "volume_ratio": (
                    safe_float(latest_volume / avg_volume_30d)
                    if avg_volume_30d
                    else None
                ),
...
def load_market_data_batch(
    symbols_dict: Dict[str, str], conn, cur, batch_size: int = 10
) -> Tuple[int, int]:
    ...
    for i in range(0, len(symbols_list), batch_size):
        ...
        logging.info(
            f"Processing market data batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
        ...
        if market_data:
            try:
                ...
                for item in market_data:
                    insert_data.append(
                        (
                            ...
                            item.get("high_52w"),
                            item.get("low_52w"),
                            item.get("distance_from_high"),
                            item.get("distance_from_low"),
                            # Volume and risk
                ...
                logging.error(
                    f"Database insert error for market batch {batch_num}: {e}"
                )
                conn.rollback()
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process market data for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
...
if __name__ == "__main__":
    ...
    for row in cur.fetchall():
        logging.info(
            f"  {row['asset_class']}: {row['count']} symbols, "
