...
# Updated: 2025-10-01 - Trigger fresh data load for relative strength score calculation
import gc
import json
import logging
import math
...
def load_prices(table_name, symbols, cur, conn):
    ...
    for batch_idx in range(batches):
        ...
        for attempt in range(1, MAX_BATCH_RETRIES + 1):
            logging.info(
                f"{table_name} – batch {batch_idx+1}/{batches}, download attempt {attempt}"
            )
            ...
            try:
                ...
                logging.info(
                    f"Downloaded dataframe for {yq_batch}: {df.shape if hasattr(df, 'shape') else type(df)}"
                )
                break
            ...
            logging.error(
                f"{table_name} batch {batch_idx+1} failed after {MAX_BATCH_RETRIES} attempts"
            )
            failed += batch
            continue
        ...
        try:
            for yq_sym, orig_sym in mapping.items():
                ...
                for idx, row in sub.iterrows():
                    rows.append(
                        [
                            ...
                            (
                                None
                                if math.isnan(row.get("Adj Close", row["Close"]))
                            ...
                            (
                                0.0
                                if (
                                    "Dividends" not in row
                                    or math.isnan(row["Dividends"])
                                )
                            ...
                            (
                                0.0
                                if (
                                    "Stock Splits" not in row
                                    or math.isnan(row["Stock Splits"])
                                )
                ...
                inserted += len(rows)
                logging.info(
                    f"{table_name} — {orig_sym}: batch-inserted {len(rows)} rows"
                )
            if inserted == 0:
                logging.warning(
                    f"{table_name}: No rows inserted for batch {batch_idx+1}"
                )
        finally:
            gc.enable()
...
if __name__ == "__main__":
    ...
    logging.info(f"Price Daily — total: {t_s}, inserted: {i_s}, failed: {len(f_s)}")
