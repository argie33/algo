...
def insert_etf_price(cursor, symbol, rec):
    vals = [symbol] + [rec[col] for col in PRICE_COLUMNS]
    sql = f"INSERT INTO etf_price_daily ({COL_LIST}) VALUES ({PLACEHOLDERS});"
    cursor.execute(sql, vals)
...
if __name__ == "__main__":
    ...
    t_stock, i_stock, f_stock = load_prices("price_daily", stock_syms, insert_stock_price)

    # 4) Fetch & load ETF symbols
    cur.execute("SELECT symbol FROM etf_symbols;")
    etf_syms = [row["symbol"] for row in cur.fetchall()]
    t_etf, i_etf, f_etf = load_prices("etf_price_daily", etf_syms, insert_etf_price)
