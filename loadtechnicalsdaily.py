#!/usr/bin/env python3
# Updated: 2025-01-16 - Trigger full technical data loading for stock APIs
...
MEMORY_THRESHOLD_MB = int(
    os.environ.get("MEMORY_THRESHOLD_MB", "400")
)  # Warning threshold
    ...
    for i in range(1, n):
        if not np.isnan(pivot_highs[i - 1]) and high_series[i] > pivot_highs[i - 1]:
            ...
            if pivot_highs[i - 1] != last_triggered_pivot:
                triggered[i] = pivot_highs[
                    i - 1
                ]  # Return the pivot level that was broken
                last_triggered_pivot = pivot_highs[
                    i - 1
                ]  # Mark this pivot as triggered
...
def process_symbol(symbol, conn_pool):
    ...
    try:
        ...
        if len(df) < 20:  # Reduced from 30 to 20 bars for faster processing
            logging.warning(
                f"Insufficient data for {symbol}: {len(df)} bars (need at least 20)"
            )
            conn_pool.putconn(conn)
            return 0
        ...
        if USE_ULTRA_LOW_MEMORY and len(df) > 500:
            ...
            df = df.tail(500)
            logging.info(
                f"{symbol}: Ultra low memory mode - processing only {len(df)} recent bars"
            )

        # Final check for sufficient data after cleaning (reduced minimum)
        if len(df) < 15:  # Reduced from 20 to 15 bars for efficiency
            logging.warning(
                f"Insufficient data after cleaning for {symbol}: {len(df)} bars"
            )
            conn_pool.putconn(conn)
            return 0
        ...
        if final_memory > MEMORY_THRESHOLD_MB:
            logging.warning(
                f"High memory usage: {final_memory:.1f}MB/{ECS_MEMORY_MB}MB after {symbol} (+{memory_used:.1f}MB)"
            )
        ...
        if final_memory > ECS_MEMORY_MB * 0.9:  # Using >90% of available memory
            logging.error(
                f"CRITICAL: Memory usage {final_memory:.1f}MB exceeds 90% of {ECS_MEMORY_MB}MB"
            )
                ...
                for i in range(0, len(symbols), BATCH_SIZE):
                    ...
                    for symbol in batch:
                        try:
                            ...
                            if (
                                current_memory > ECS_MEMORY_MB * 0.85
                            ):  # >85% memory usage
                                logging.warning(
                                    f"High memory before {symbol}: {current_memory:.1f}MB"
                                )
                                gc.collect()
