...
if not DB_SECRET_ARN:
    logging.error("DB_SECRET_ARN environment variable is not set")
    sys.exit(1)
...
# -------------------------------
def process_symbols(conn, cur):
    # Load and process stock symbols
    ...
    for batch_idx in range(batches):
        ...
        for symbol in batch:
            yf_sym = symbol.upper().replace(".", "-").replace("$", "-")
            logging.info(f"→ {symbol} (YF={yf_sym})")

            try:
                info = yf.Ticker(yf_sym).get_info() or {}
            except Exception as e:
                logging.warning(f"yfinance.get_info() failed for {symbol}: {e}")
...
def main():
    ...
    # First connection - rebuild tables
    cfg = get_db_config()
    logging.info("Rebuilding tables...")
...
""")
    ...
    # Second connection - process symbols
    cfg = get_db_config()
    conn = psycopg2.connect(
        host=cfg["host"], port=cfg["port"],
        user=cfg["user"], password=cfg["password"],
    ...
    total, inserted, failed = process_symbols(conn, cur)
    ...
    logging.info(f"Total: {total}, Inserted: {inserted}, Failed: {len(failed)}")
    if failed:
        logging.warning(f"Failed symbols: {', '.join(failed)}")
