#!/usr/bin/env python3
"""
Cloud-Native ETF Price Daily Loader - AWS Best Practices
...
def load_etf_data(symbol: str) -> List[tuple]:
    """Fetch daily price data for one ETF"""
    try:
        ...
        for date, row in hist.iterrows():
            # Skip zero-volume rows (common for ETFs with limited trading)
            if pd.notna(row["Volume"]) and int(row["Volume"]) > 0:
                rows.append((
                    symbol,
                    date.date(),
        ...
        logging.debug(f"[OK] {symbol}: {len(rows)} ETF daily rows")
        return rows
...
def main():
    logging.info("Starting loadetfpricedaily_cloud (PARALLEL + S3 BULK)")
    logging.info("Expected time: 3-15 minutes (vs 20-40 minutes with standard inserts)")

    db_config = get_db_config()
    rds_role = get_rds_s3_role()
    s3_bucket = os.environ.get("S3_STAGING_BUCKET", "stocks-app-data")

    # Connect to get ETF symbols
    try:
        conn = psycopg2.connect(**db_config)
        cur = conn.cursor()
        cur.execute("SELECT DISTINCT symbol FROM stock_symbols WHERE symbol LIKE '%ETF%' OR symbol IN (SELECT symbol FROM price_daily WHERE symbol IN (SELECT symbol FROM stock_symbols)) ORDER BY symbol")
        symbols = [row[0] for row in cur.fetchall()]
        cur.close()
        conn.close()
    except Exception as e:
        logging.error(f"Failed to fetch ETF symbols: {e}")
        return False

    if not symbols:
        logging.warning("No ETF symbols found")
        return False

    total_symbols = len(symbols)
    logging.info(f"Loading daily ETF prices for {total_symbols} ETFs...")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        future_to_symbol = {
            executor.submit(load_etf_data, symbol): symbol
    ...
    if all_rows:
        try:
            ...
            inserted = bulk_inserter.insert_bulk("etf_price_daily", columns, all_rows, rds_role)
