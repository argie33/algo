...
# Configuration
DEFAULT_SYMBOLS = [
    'BTC', 'ETH', 'BNB', 'XRP', 'SOL', 'USDT', 'USDC', 'ADA', 'AVAX', 'DOGE',
    'MATIC', 'DOT', 'LINK', 'UNI', 'AAVE', 'CRV', 'MKR', 'COMP', 'YFI', 'SUSHI'
]
...
class CryptoDataLoader:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    self.logger.error(f"API request failed after {MAX_RETRIES} attempts: {url}")
                    return None
    ...
    def fetch_coingecko_prices(self, symbols: List[str]) -> List[CryptoPrice]:
        ...
        symbol_to_id = {
            ...
            'XRP': 'ripple', 'SOL': 'solana', 'USDT': 'tether',
        ...
        for coin_data in data:
            try:
                ...
                price = CryptoPrice(
                    ...
                    price_change_percentage_24h=Decimal(str(coin_data.get('price_change_percentage_24h', 0))),
                    price_change_7d=None,  # Not available in this endpoint
                    price_change_percentage_7d=Decimal(str(coin_data.get('price_change_percentage_7d_in_currency', 0))),
                ...
                self.logger.warning(f"Failed to parse price data for {coin_data.get('id', 'unknown')}: {e}")
                continue
        ...
        try:
            with self.get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    data_tuples = [
                        (
                            price.symbol, price.price_usd, price.market_cap_usd, price.volume_24h_usd,
                            price.circulating_supply, price.price_change_24h, price.price_change_percentage_24h,
                            price.price_change_7d, price.price_change_percentage_7d, price.high_24h, price.low_24h,
                            price.ath, price.ath_date, price.atl, price.atl_date, price.last_updated
                        ) for price in prices
                    ]
    ...
    def store_historical_data(self, historical_data: List[tuple]) -> bool:
        ...
        if self.dry_run:
            self.logger.info(f"DRY RUN: Would store {len(historical_data)} historical records")
            return True
        ...
        try:
            with self.get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    conn.commit()
                    self.logger.info(f"Successfully stored {len(historical_data)} historical records")
                    return True
            ...
            with self.get_db_connection() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    ...
                    return {
                        ...
                        'sma_50': sma_50,
                        'sma_200': sma_200,
                        'volume_sma': sum([float(row['volume_usd'] or 0) for row in data[-20:]]) / 20 if len(data) >= 20 else None
                    }
        ...
        defi_protocols = [
            {
                ...
                'symbol': 'UNI',
                'category': 'dex',
                'tvl_usd': 4250000000 + int((time.time() % 1000) * 1000000),  # Simulate changes
    ...
    def load_crypto_data(self, symbols: List[str] = None, include_historical: bool = False, include_news: bool = False, include_defi: bool = False):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Load cryptocurrency data into database')
    parser.add_argument('--symbols', type=str, help='Comma-separated list of symbols to load')
    ...
    if args.schedule:
        ...
        schedule.every().hour.do(lambda: loader.load_crypto_data(symbols=symbols, include_defi=True))  # DeFi hourly
        schedule.every().day.at("06:00").do(lambda: loader.load_crypto_data(symbols=symbols, include_historical=True))  # Historical daily
