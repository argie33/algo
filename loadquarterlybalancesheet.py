...
"""
Quarterly Balance Sheet Loader - Optimal Pattern using SEC EDGAR.
...
Run:
    python3 loadquarterlybalancesheet.py [--symbols AAPL,MSFT] [--parallelism 8]
"""
...
class QuarterlyBalanceSheetLoader(OptimalLoader):
    ...
    def fetch_incremental(self, symbol: str, since: Optional[date]):
        ...
        try:
            rows = client.get_balance_sheet(symbol, period="quarterly")
            if not rows:
                return None
...
def main():
    parser = argparse.ArgumentParser(description="Optimal quarterly_balance_sheet loader")
    ...
    loader = QuarterlyBalanceSheetLoader()
    try:
