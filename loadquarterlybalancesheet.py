...
# Updated: 2025-10-04 16:45 - Quarterly balance sheet loader - trigger rebuild
import gc
import json
import logging
import os
...
def get_quarterly_balance_sheet_data(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if balance_sheet is None or balance_sheet.empty:
            logging.warning(
                f"No quarterly balance sheet data returned by any method for {symbol}"
            )
            return None
        ...
        valid_columns = [
            col for col in balance_sheet.columns if not balance_sheet[col].isna().all()
        ]
        if not valid_columns:
            logging.warning(
                f"No valid quarterly balance sheet columns found for {symbol}"
            )
            return None

        # Sort columns by date (most recent first)
        balance_sheet = balance_sheet.reindex(
            sorted(balance_sheet.columns, reverse=True), axis=1
        )

        logging.info(
            f"Retrieved quarterly balance sheet data for {symbol}: {len(balance_sheet.columns)} periods, {len(balance_sheet.index)} line items"
        )
        ...
        for symbol in batch:
            ...
            for attempt in range(1, MAX_BATCH_RETRIES + 1):
                try:
                    ...
                    # Process the data
                    balance_sheet_data = process_balance_sheet_data(
                    ...
                    if balance_sheet_data:
                        ...
                        processed += 1
                        logging.info(
                            f"✓ Successfully processed {symbol} ({len(balance_sheet_data)} records)"
                        )
                        success = True
                        break
                    else:
