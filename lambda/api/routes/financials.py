...
def handle(  # noqa: C901
    ...
    try:
        ...
        if endpoint == "ownership":
            ...
            # Segment Count/Concentration HHI/Diversified blank ("--") site-wide, even
            # though sec_segment_metrics is populated and fresh.
            #
            # REMOVED 2026-08-24: the insider_holdings_sec LEFT JOIN (insider_ownership_pct/
            # number_of_insiders/recent_buys/recent_sells/net_insider_transactions/
            # latest_insider_filing_date) - insider_ownership_pct was removed entirely
            # (static governance metric, near-zero information content for this system's
            # weeks-scale swing trading, recurring source of real bugs). See
            # loaders/DEPRECATED_LOADERS.md.
            rows = execute_with_timeout(
                cur,
                """
                SELECT
            ...
            # if the source table has no data for this symbol) - the frontend already
            # renders "--" for each null field individually, so there is no "no data" error
            # case here.
