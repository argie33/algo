...
def get_price_coverage(cur: cursor) -> dict[str, Any]:
    ...
    try:
        ...
        if not row:
            return cast(dict[str, Any], error_response(503, "no_data", "Price data not yet available")  # type: ignore[no-any-return])
...
def get_technical_coverage(cur: cursor) -> dict[str, Any]:
    ...
    try:
        ...
        row = cur.fetchone()
        if not row:
            return cast(dict[str, Any], error_response(503, "no_data", "Technical data not yet available")  # type: ignore[no-any-return])
...
def get_loader_health(cur: cursor) -> dict[str, Any]:
    ...
    try:
        ...
        if not rows:
            ...
            logger.error(error_msg)
            return cast(dict[str, Any], error_response(503, "no_loader_status", error_msg)  # type: ignore[no-any-return])
...
def _safe_call(cur: cursor, fn: Any) -> dict[str, Any]:
...
def handle(
    ...
    if method != "GET":
        return cast(dict[str, Any], error_response(405, "method_not_allowed", "Method not allowed. Only GET is supported.")  # type: ignore[no-any-return])

    try:
        summary = get_overall_coverage_summary(cur)
        # Return data wrapped in standard response format
        return cast(dict[str, Any], json_response(200, summary)  # type: ignore[no-any-return])
    except (
        psycopg2.errors.UndefinedTable,
        psycopg2.errors.UndefinedColumn,
        psycopg2.OperationalError,
