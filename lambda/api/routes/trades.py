        ...
        if path == "/api/trades":
            ...
            if status_filter:
                if status_filter.lower() not in valid_statuses:
                    return cast(dict[str, Any], error_response(400, "bad_request", f"Invalid status value: {status_filter}")  # type: ignore[no-any-return])
            ...
            if not is_valid:
                logger.error(f"Endpoint response validation failed: {error_msg}")
                return cast(dict[str, Any], error_response(500, "response_validation_error", error_msg)  # type: ignore[no-any-return])
            return cast(dict[str, Any], json_response(200, summary_result)  # type: ignore[no-any-return])
