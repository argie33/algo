...
def _get_algo_config_key(cur: cursor, key: str) -> dict[str, Any]:
    ...
    row = cur.fetchone()
    return cast(dict[str, Any], json_response(200, safe_json_serialize(safe_dict_convert(row)) if row else {})  # type: ignore[no-any-return])
...
def _reset_algo_config_key(cur: cursor, key: str, actor: str) -> dict[str, Any]:
    ...
    if key not in AlgoConfig.DEFAULTS:
        return cast(dict[str, Any], error_response(404, "not_found", f"Config key not found: {key}")  # type: ignore[no-any-return])
...
def _update_algo_config_key(cur: cursor, key: str, body: dict[str, Any], actor: str) -> dict[str, Any]:
    ...
    if not body or "value" not in body:
        return cast(dict[str, Any], error_response(400, "bad_request", "value required in request body")  # type: ignore[no-any-return])
    ...
    try:
        if not AlgoConfig.DEFAULTS.get(key):
            return cast(dict[str, Any], error_response(400, "bad_request", f"Unknown config key: {key}")  # type: ignore[no-any-return])
        ...
        return cast(dict[str, Any], error_response(400, "bad_request", f"Invalid value for {key}: {e!s}")  # type: ignore[no-any-return])
