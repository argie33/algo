...
def _get_orchestrator_execution_details(cur: cursor, run_id: str) -> dict[str, Any]:
    ...
    return success_response(result)  # type: ignore[no-any-return]
...
def _get_orchestrator_execution_failed(cur: cursor, days: int = 30) -> dict[str, Any]:
    ...
    rows = cur.fetchall()
    return list_response([safe_json_serialize(safe_dict_convert(r)) for r in rows], total=len(rows))  # type: ignore[no-any-return]
...
def _get_orchestrator_execution_patterns(cur: cursor, days: int = 30) -> dict[str, Any]:
    ...
    return success_response({"patterns": patterns, "period_days": days})  # type: ignore[no-any-return]
...
def _get_orchestrator_execution_recent(cur: cursor, days: int = 7, limit: int = 50) -> dict[str, Any]:
    ...
    try:
        ...
        rows = cur.fetchall()
        if not rows:
            return list_response([], total=0, limit=limit)  # type: ignore[no-any-return]
        try:
            items = [safe_json_serialize(safe_dict_convert(r)) for r in rows]
            return list_response(items, total=len(rows), limit=limit)  # type: ignore[no-any-return]
