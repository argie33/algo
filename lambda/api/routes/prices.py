    ...
    try:
        ...
        if len(parts) == 3 and parts[2] in ["prices", "prices?symbols"]:
            ...
            if symbols_raw:
                ...
                if not symbols:
                    return cast(dict[str, Any], error_response(400, "bad_request", "symbols parameter required")  # type: ignore[no-any-return])
                if len(symbols) > 20:
                    return cast(dict[str, Any], error_response(400, "bad_request", "maximum 20 symbols per batch")  # type: ignore[no-any-return])
                for sym in symbols:
                    if not all(c.isalnum() or c in ("-", ".", "^") for c in sym):
                        return cast(dict[str, Any], error_response(400, "bad_request", f"Invalid symbol: {sym}")  # type: ignore[no-any-return])
                ...
                return cast(dict[str, Any], json_response(200, price_result)  # type: ignore[no-any-return])
            else:
                return cast(dict[str, Any], error_response(400, "bad_request", "symbols parameter required")  # type: ignore[no-any-return])
        ...
        if len(parts) >= 5 and parts[3] == "history":
            ...
            if not symbol or not all(c.isalnum() or c in ("-", ".", "^") for c in symbol):
                return cast(dict[str, Any], error_response(400, "bad_request", "Invalid symbol")  # type: ignore[no-any-return])
        ...
        # Replaces N concurrent per-symbol calls with one batch query.
            ...
            if not DatabaseResultValidator.validate_rows_not_empty(rows, "prices batch-history query"):
                return cast(dict[str, Any], json_response(200, result3)  # type: ignore[no-any-return])
            found_symbols = set()
            for row in rows:
                sym = row["symbol"]
            ...
            if not is_valid:
                logger.error(f"Endpoint response validation failed: {error_msg}")
                return cast(dict[str, Any], error_response(500, "response_validation_error", error_msg)  # type: ignore[no-any-return])
            return cast(dict[str, Any], json_response(200, batch_result)  # type: ignore[no-any-return])

        return cast(dict[str, Any], error_response(404, "not_found", f"No prices handler for {path}")  # type: ignore[no-any-return])
    except (
