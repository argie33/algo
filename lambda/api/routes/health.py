...
def _handle_basic(cur: cursor) -> dict[str, Any]:
    ...
    try:
        ...
        if not result:
            return error_response(503, "connection_error", "Database connection failed")  # type: ignore[no-any-return]
...
def _handle_cognito(cur: cursor) -> dict[str, Any]:
    ...
    try:
        ...
        if not configured_client_id:
            ...
            return error_response(503, "misconfigured", "Cognito client ID not configured")  # type: ignore[no-any-return]
        ...
        if not cognito_user_pool_id:
            ...
            return error_response(503, "misconfigured", "Cognito user pool ID not configured")  # type: ignore[no-any-return]
        ...
        return error_response(503, "health_check_error", sanitized)  # type: ignore[no-any-return]
