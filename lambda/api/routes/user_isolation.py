...
def _validate_credentials_structure(creds: Any) -> bool:
    ...
    if not isinstance(creds, dict):
        logger.error(f"[ALPACA] Credentials must be dict, got {type(creds).__name__}")
        return False
...
def requires_auth(handler_func: Callable[..., T]) -> Callable[..., T]:
    ...
    def wrapper(cur: cursor, path: str, method: str, params: Any, body: Any = None, jwt_claims: Any = None) -> T:
        try:
            ...
            return error_response(401, "unauthorized", str(e))  # type: ignore[return-value]  # type: ignore[no-any-return]
