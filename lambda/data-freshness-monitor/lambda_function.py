    ...
    try:
        ...
        for table_name, description in critical_tables.items():
            try:
                ...
                if age_days > max_age:
                    logger.warning(
                        f"[FRESHNESS] {description}: {age_days} days old (max {max_age})"
                    )
                ...
                if table_name in ["swing_trader_scores", "signal_quality_scores"]:
                    ...
                    if max_created:
                        ...
                        if age_hours > 24:
                            logger.critical(
                                f"[FRESHNESS] {description}: {age_hours:.1f}h old (max 24h)"
                            )
                ...
                logger.warning(
                    f"[FRESHNESS] Could not check {description}: {table_err}"
                )
                age_details[table_name] = {
                    "status": "error",
                    "reason": str(table_err)[:50],
                }
...
def lambda_handler(event, context):
    ...
    if freshness["status"] in ["degraded", "critical"]:
        logger.critical(
            f"[FRESHNESS] Data quality degraded: {freshness['stale_tables']}"
        )
        ...
        try:
            ...
            reason = (
                f"Data freshness degraded: {'; '.join(freshness['stale_tables'][:3])}"
            )
