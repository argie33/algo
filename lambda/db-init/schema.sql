...
-- Base signals before evaluation/trade execution
...
CREATE INDEX IF NOT EXISTS idx_data_patrol_log_created_at ON data_patrol_log(created_at DESC);
...
INSERT INTO algo_config (key, value, value_type, description, updated_by) VALUES ('patrol_corporate_action_lookback_days', '90', 'int', 'Days of lookback for corporate action detection in data patrol', 'schema-seed') ON CONFLICT (key) DO NOTHING;
INSERT INTO algo_config (key, value, value_type, description, updated_by) VALUES ('patrol_corporate_action_drop_ratio', '-0.30', 'float', 'Price drop ratio threshold that triggers corporate action alert', 'schema-seed') ON CONFLICT (key) DO NOTHING;
