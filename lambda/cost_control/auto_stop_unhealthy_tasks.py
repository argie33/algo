...
def lambda_handler(event, context):
    ...
    try:
        ...
        for task in details["tasks"]:
            ...
            if should_stop:
                # Stop the task
                ecs.stop_task(cluster=cluster, task=task_arn, reason=f"[AUTO-COST-CONTROL] {reason}")
        ...
        if alerts:
            alerts_str = "\n".join(alerts)
            message = (
                f"Cost Control Alert:\n\n{alerts_str}\n\n"
                f"Total stopped: {len(alerts)} tasks\n"
                f"Cluster: {cluster}\n"
                f"Time: {datetime.now(timezone.utc).isoformat()}"
            )

            sns.publish(TopicArn=alert_topic, Subject=f"ECS Auto-Stop: {len(alerts)} Tasks Terminated", Message=message)

            return {
                "statusCode": 200,
                "body": json.dumps({"action": "stopped_unhealthy_tasks", "count": len(alerts), "tasks": alerts}),
            }
