#!/usr/bin/env python3
# Updated: 2025-01-16 - Trigger news data loading for AWS RDS
import gc
import json
import logging
import math
...
def load_news_data(symbols, cur, conn):
    ...
    logging.info(
        f"Loading news for {total} symbols in {num_batches} batches of {BATCH_SIZE}"
    )
    ...
    for batch_idx in range(num_batches):
        ...
        logging.info(
            f"Processing batch {batch_idx+1}/{num_batches}: symbols {start_idx+1}-{end_idx}"
        )
        ...
        for symbol in batch:
            ...
            try:
                ...
                for news_item in news_data:
                    ...
                    if publish_time:
                        try:
                            ...
                            logging.warning(
                                f"Invalid timestamp for {orig_sym}: {publish_time}"
                            )

                    # Get thumbnail and related tickers
                    thumbnail = None
                    if (
                        "thumbnail" in news_item
                        and "resolutions" in news_item["thumbnail"]
                    ):
                    ...
                    if "relatedTickers" in news_item:
                        related_tickers = [
                            ticker for ticker in news_item["relatedTickers"] if ticker
                        ]
                ...
                if news_items:
                    ...
                    logging.info(
                        f"Inserted {len(news_items)} news items for {orig_sym}"
                    )
