...
try:
    ...
    print("[OK] BLOCKER #1 FIXED: get_pool_health() exists and imports successfully")
except ImportError as e:
    print(f"[ERROR] BLOCKER #1 NOT FIXED: {e}")
    sys.exit(1)
...
try:
    ...
    if 'cur' in sig.parameters:
        print("[OK] BLOCKER #2 FIXED: _with_cursor accepts cur parameter")
    else:
        print("[ERROR] BLOCKER #2 NOT FIXED: _with_cursor missing cur parameter")
        sys.exit(1)
except Exception as e:
    print(f"[ERROR] BLOCKER #2 NOT FIXED: {e}")
    sys.exit(1)

# BLOCKER #3: Exit retry logic
try:
    from algo.orchestrator.phase6_exit_execution import _retry_exit_trade
    print("[OK] BLOCKER #3 FIXED: _retry_exit_trade function exists")
except ImportError as e:
    print(f"[ERROR] BLOCKER #3 NOT FIXED: {e}")
    sys.exit(1)
...
try:
    ...
    print("[OK] BLOCKER #4 FIXED: config_validator accepts Decimal types")
except Exception as e:
    print(f"[ERROR] BLOCKER #4 NOT FIXED: {e}")
    sys.exit(1)
...
try:
    ...
    if "if not checks:" in content:
        print("[OK] BLOCKER #5 FIXED: Circuit breaker empty check is present")
    else:
        print("[ERROR] BLOCKER #5 NOT FIXED: Circuit breaker empty check not found")
        sys.exit(1)
except Exception as e:
    print(f"[ERROR] BLOCKER #5 NOT FIXED: {e}")
    sys.exit(1)
...
try:
    ...
    if "data_unavailable" in content and "WHERE data_unavailable = false" in content:
        print("[OK] BLOCKER #6 FIXED: stock_scores checks data_unavailable flag")
    else:
        print("[ERROR] BLOCKER #6 NOT FIXED: stock_scores missing data_unavailable checks")
        sys.exit(1)
except Exception as e:
    print(f"[ERROR] BLOCKER #6 NOT FIXED: {e}")
    sys.exit(1)
