...
class NewsSentimentAnalyzer(BaseSentimentAnalyzer):
    ...
    def _fetch_news(
        self, symbol: str, company_name: str, days_back: int = 7
    ) -> List[Dict]:
    ...
    def _analyze_article_sentiment(self, article: Dict) -> Dict:
        ...
        confidence = min(
            1.0, base_sentiment.subjectivity + abs(keyword_sentiment) * 0.5
        )
        ...
        try:
            ...
            if sentiment_scores:
                ...
                if total_engagement > 0:
                    weighted_score = (
                        sum(s["score"] * s["engagement"] for s in sentiment_scores)
                        / total_engagement
                    )
                else:
            ...
            return SentimentResult(
                ...
                raw_data={
                    ...
                    "total_engagement": sum(
                        s.get("engagement", 0) for s in sentiment_scores
                    ),
...
class AnalystSentimentAnalyzer(BaseSentimentAnalyzer):
    ...
    def get_sentiment(self, symbol: str) -> SentimentResult:
        ...
        try:
            ...
            combined_score = (
                rec_sentiment["score"] * 0.6 + estimate_sentiment["score"] * 0.4
            )
            combined_confidence = (
                rec_sentiment["confidence"] * 0.6
    ...
    def _get_estimate_revisions(self, symbol: str) -> Dict:
        ...
        return {"score": np.clip(revision_trend, -1.0, 1.0), "confidence": 0.6}
...
class SentimentAnalyzer:
    ...
    def _calculate_weighted_sentiment(
        self, sentiments: Dict[str, SentimentResult]
    ) -> float:
...
def main():
    ...
    for symbol in test_symbols:
        ...
        for source, data in result["components"].items():
            print(
                f"  {source.capitalize()}: {data['score']:.3f} "
