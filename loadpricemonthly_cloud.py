#!/usr/bin/env python3
"""
Cloud-Native Price Monthly Loader - AWS Best Practices
...
def load_symbol_data(symbol: str) -> List[tuple]:
    ...
    try:
        ...
        logging.debug(f"[OK] {symbol}: {len(rows)} monthly rows")
        return rows
...
def main():
    logging.info("Starting loadpricemonthly_cloud (PARALLEL + S3 BULK)")
    ...
    logging.info(f"Loading monthly prices for {total_symbols} stocks...")
    ...
    if all_rows:
        try:
            ...
            inserted = bulk_inserter.insert_bulk("price_monthly", columns, all_rows, rds_role)
