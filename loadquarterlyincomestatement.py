...
SCRIPT_NAME = "loadquarterlyincomestatement.py"
logging.basicConfig(
    level=logging.INFO,
...
def get_quarterly_income_statement_data(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if income_statement is None or income_statement.empty:
            logging.warning(
                f"No quarterly income statement data returned by any method for {symbol}"
            )
            return None
        ...
        valid_columns = [
            ...
            if not income_statement[col].isna().all()
        ]
        if not valid_columns:
            logging.warning(
                f"No valid quarterly income statement columns found for {symbol}"
            )
            return None
        ...
        logging.info(
            f"Retrieved quarterly income statement data for {symbol}: {len(income_statement.columns)} periods, {len(income_statement.index)} line items"
        )
...
def process_income_statement_data(
    symbol: str, income_statement: pd.DataFrame
) -> List[Tuple]:
...
def load_quarterly_income_statement(
    symbols: List[str], cur, conn
) -> Tuple[int, int, List[str]]:
    """Load quarterly income statement data for given symbols"""
    total = len(symbols)
    logging.info(f"Loading quarterly income statement for {total} symbols")
    processed, failed = 0, []
        ...
        for symbol in batch:
            ...
            for attempt in range(1, MAX_BATCH_RETRIES + 1):
                try:
                    ...
                    income_statement = get_quarterly_income_statement_data(yf_symbol)
                    if income_statement is None:
                        break

                    # Process the data
                    income_statement_data = process_income_statement_data(
                    ...
                    if income_statement_data:
                        ...
                        processed += 1
                        logging.info(
                            f"✓ Successfully processed {symbol} ({len(income_statement_data)} records)"
                        )
                        success = True
                        break
                    else:
...
if __name__ == "__main__":
    ...
    if stock_syms:
        t_s, p_s, f_s = load_quarterly_income_statement(stock_syms, cur, conn)
    ...
    try:
        ...
        if etf_syms:
            t_e, p_e, f_e = load_quarterly_income_statement(etf_syms, cur, conn)
