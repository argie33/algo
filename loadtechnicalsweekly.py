#!/usr/bin/env python3
"""
Technical Indicators Loader - WEEKLY
Calculates and populates technical_data_weekly table with RSI, MACD, and other technical indicators.
...
def load_technical_indicators(cur, conn):
    ...
    try:
        logging.info("Loading price data for technical calculations (WEEKLY)...")
        ...
        logging.info(f"Processing {len(symbols)} symbols with weekly price data")
        ...
        for symbol in symbols:
            try:
                # Get last 200 weeks of price data (roughly 4 years)
                cur.execute("""
                    SELECT date, close
                    FROM price_weekly
                    WHERE symbol = %s
                ...
                roc_10w = calculate_roc(prices, period=10) if len(prices) >= 11 else None
                roc_20w = calculate_roc(prices, period=20) if len(prices) >= 21 else None
                roc_40w = calculate_roc(prices, period=40) if len(prices) >= 41 else None
                roc_52w = calculate_roc(prices, period=52) if len(prices) >= 53 else None
        ...
        cur.execute("DELETE FROM technical_data_weekly WHERE date >= CURRENT_DATE - INTERVAL '7 days'")
        ...
        if indicators_data:
            execute_values(
                ...
                INSERT INTO technical_data_weekly (
                    ...
                    mom, roc, roc_10w, roc_20w, roc_40w, roc_52w, roc_200w,
                ...
                ON CONFLICT (symbol, date) DO UPDATE SET
                    ...
                    mom = EXCLUDED.mom,
                    roc = EXCLUDED.roc,
                    roc_10w = EXCLUDED.roc_10w,
                    roc_20w = EXCLUDED.roc_20w,
                    roc_40w = EXCLUDED.roc_40w,
                    roc_52w = EXCLUDED.roc_52w
                """,
                indicators_data,
                page_size=1000
            )
...
def main():
    ...
    try:
        ...
        logging.info(f"[OK] Technical indicators (WEEKLY) loaded successfully ({count} records)")
        log_mem("finished")
