#!/usr/bin/env python3
# Updated: 2025-10-03 17:35 - StockDetail page fixed, populate AWS with weekly signals
...
def prepare_db():
    ...
    cursor.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_technical_weekly_symbol_date_desc
        ON technical_data_weekly(symbol, date DESC);
        """
    )
    logging.info("Table 'technical_data_weekly' ready with performance indexes.")
...
def process_symbol(symbol, conn_pool):
    ...
    try:
        ...
        if len(df) < 30:  # Need at least 30 bars for meaningful technical analysis
            logging.warning(
                f"Insufficient data for {symbol}: {len(df)} bars (need at least 30)"
            )
            conn_pool.putconn(conn)
            return 0
