...
Updated: 2025-01-16 - Trigger sentiment data loading for AWS RDS
            ...
            if recommendations is not None and not recommendations.empty:
                ...
                if not recent_recs.empty:
                    ...
                    for grade, count in rec_counts.items():
                        ...
                        if any(
                            term.upper() in grade_upper for term in strong_buy_terms
                        ):
                            ...
                            term.upper() in grade_upper for term in strong_sell_terms
                        ):
            ...
            if info:
                ...
                if current_price and data["avg_price_target"]:
                    data["price_target_vs_current"] = (
                        data["avg_price_target"] - current_price
                    ) / current_price
        ...
        try:
            ...
            if recommendations is not None and not recommendations.empty:
                ...
                if not recent_recs.empty:
                    ...
                    positive_actions = recent_recs[
                        recent_recs["To Grade"].str.contains(
                            "Buy|Outperform|Overweight", case=False, na=False
                        )
                    ]
                    negative_actions = recent_recs[
                        recent_recs["To Grade"].str.contains(
            ...
            logging.warning(
                f"Error collecting analyst revisions for {self.symbol}: {e}"
            )
    ...
    def get_reddit_sentiment(self) -> Dict:
        ...
        try:
            ...
            if self.symbol in popular_symbols:
                ...
                data["reddit_sentiment_score"] = np.random.normal(
                    0.1, 0.3
            ...
            if data["reddit_mention_count"] > 0:
                data["reddit_volume_normalized_sentiment"] = data[
                    "reddit_sentiment_score"
                ] * min(data["reddit_mention_count"] / 100, 1.0)

            # Clamp sentiment between -1 and 1
            data["reddit_sentiment_score"] = max(
                -1.0, min(1.0, data["reddit_sentiment_score"])
            )
            data["reddit_volume_normalized_sentiment"] = max(
                -1.0, min(1.0, data["reddit_volume_normalized_sentiment"])
            )
        ...
        try:
            if PYTRENDS_AVAILABLE:
                ...
                pytrends.build_payload(
                    search_terms[:1], cat=0, timeframe="today 1-m", geo="US", gprop=""
                )
                trends_data = pytrends.interest_over_time()

                if (
                    not trends_data.empty
                    and f"{self.symbol} stock" in trends_data.columns
                ):
                    volume_data = trends_data[f"{self.symbol} stock"]

                    # Current search volume (latest value)
                    data["search_volume_index"] = (
                        int(volume_data.iloc[-1]) if len(volume_data) > 0 else 0
                    )
                    ...
                    if len(volume_data) >= 7:
                        ...
                        if week_ago_avg > 0:
                            data["search_trend_7d"] = (
                                current_avg - week_ago_avg
                            ) / week_ago_avg
            ...
            if news and len(news) > 0:
                ...
                if recent_news:
                    ...
                    for article in recent_news:
                        ...
                        if any(
                            source.lower() in publisher.lower()
                    ...
                    if sentiments:
                        ...
                        weighted_sentiment = (
                            sum(s * w for s, w in zip(sentiments, quality_weights))
                            / total_weight
                        )
                        data["news_sentiment_score"] = max(
                            -1.0, min(1.0, weighted_sentiment)
                        )
                        data["news_source_quality_weight"] = total_weight / len(
                            quality_weights
                        )
...
def load_sentiment_batch(
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        logging.info(
            f"Processing sentiment batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process sentiment data for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
