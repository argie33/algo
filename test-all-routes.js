  ...
  stocks: [
    {
      path: "/api/stocks/",
      method: "GET",
      description: "Stocks main endpoint",
    },
    {
      path: "/api/stocks/sectors",
      method: "GET",
      description: "Stock sectors",
    },
    {
      path: "/api/stocks/public/sample",
      method: "GET",
      description: "Public stock sample",
    },
  ],
  ...
  screener: [
    {
      path: "/api/screener/",
      method: "GET",
      description: "Stock screener main",
      requiresAuth: true,
    },
    {
      path: "/api/screener/health",
      method: "GET",
      description: "Screener health",
    },
  ],
...
async function testEndpoint(endpoint, category) {
  ...
  try {
    console.log(
      `${colors.blue}Testing:${colors.reset} ${endpoint.method} ${endpoint.path} - ${endpoint.description}`
    );
    ...
    console.log(
      `  ${statusColor}Status:${colors.reset} ${response.status} ${response.statusText}`
    );
    console.log(`  ${colors.cyan}Duration:${colors.reset} ${duration}ms`);
    console.log(
      `  ${colors.cyan}Size:${colors.reset} ${responseText.length} bytes`
    );

    if (TEST_CONFIG.verbose && responseData) {
      if (responseData.success !== undefined) {
        console.log(
          `  ${colors.cyan}Success:${colors.reset} ${responseData.success}`
        );
      }
      if (responseData.message) {
        console.log(
          `  ${colors.cyan}Message:${colors.reset} ${responseData.message}`
        );
      }
      if (responseData.error) {
        console.log(
          `  ${colors.yellow}Error:${colors.reset} ${responseData.error}`
        );
      }
    }
...
async function testCategory(categoryName, endpoints) {
  console.log(
    `${colors.bold}${colors.blue}=== Testing ${categoryName.toUpperCase()} Endpoints ===${colors.reset}\n`
  );
...
async function runAllTests() {
  console.log(
    `${colors.bold}${colors.cyan}Financial Platform API Route Testing${colors.reset}\n`
  );
  console.log(`${colors.cyan}API Base URL:${colors.reset} ${API_BASE}`);
  console.log(
    `${colors.cyan}Timeout:${colors.reset} ${TEST_CONFIG.timeout}ms\n`
  );
  ...
  console.log(
    `${colors.cyan}Success rate:${colors.reset} ${Math.round((testResults.passed / testResults.total) * 100)}%\n`
  );
  ...
  if (testResults.errors.length > 0) {
    ...
    testResults.errors.forEach((error) => {
      console.log(
        `  ${colors.red}✗${colors.reset} ${error.endpoint}: ${error.error}`
      );
    });
    console.log("");
  }
  ...
  for (const [category, results] of Object.entries(allResults)) {
    ...
    console.log(
      `  ${colors.cyan}${category}:${colors.reset} ${Math.round(avgDuration)}ms average`
    );
  }
