...
class PaperModeReconciliationMixin:
    ...
    def _paper_mode_compute_state(self, reconcile_date: _date_type | None) -> PaperModeState:
        ...
        with DatabaseContext("read") as cur:
            open_position_count, total_unrealized_pnl, total_invested, total_cost_basis = (
                self._paper_mode_query_open_positions(cur)
            )
            ...
            realized_pnl_today = self._paper_mode_query_realized_pnl_today(cur, reconcile_date)

            initial_capital, baseline_equity, prev_unrealized_pnl = self._paper_mode_query_baseline(cur, reconcile_date)
    ...
    def _paper_mode_query_open_positions(self, cur: Any) -> tuple[int, float, float, float]:
        """Open position count + unrealized P&L/invested/cost-basis aggregates.

        Verbatim relocation of the first block of `_paper_mode_compute_state`.
        Returns (open_position_count, total_unrealized_pnl, total_invested, total_cost_basis).
        """
        ...
        if position_count == 0:
            ...
            if pnl_row["total_pnl"] is None or pnl_row["total_invested"] is None or pnl_row["total_cost_basis"] is None:
                missing_fields = [f for f in ["total_pnl", "total_invested", "total_cost_basis"] if pnl_row[f] is None]
                raise RuntimeError(
        ...
        return open_position_count, total_unrealized_pnl, total_invested, total_cost_basis

    def _paper_mode_query_realized_pnl_today(self, cur: Any, reconcile_date: _date_type) -> float:
        """Realized P&L for trades closed on `reconcile_date`.

        Verbatim relocation of the second block of `_paper_mode_compute_state`.
        """
    ...
    def _paper_mode_query_baseline(self, cur: Any, reconcile_date: _date_type) -> tuple[float, float, float]:
        """Initial capital + the prior-snapshot roll-forward baseline.

        Verbatim relocation of the third block of `_paper_mode_compute_state`.
        Returns (initial_capital, baseline_equity, prev_unrealized_pnl).
        """
        ...
        return float(initial_capital), baseline_equity, prev_unrealized_pnl
    ...
    def _paper_mode_write_snapshot(self, state: PaperModeState) -> PaperModeSnapshotWrite:
        ...
        try:
            ...
            with DatabaseContext("write") as cur:
                ...
                running_peak, drawdown_pct = self._paper_mode_compute_drawdown(cur, reconcile_date, portfolio_value)

                winning_count, losing_count, breakeven_count = self._paper_mode_query_win_loss_breakeven(
                    cur, open_position_count
                )
                ...
                largest_position_pct_paper, avg_position_size_pct_paper = self._paper_mode_compute_concentration(
                    cur, portfolio_value, open_position_count
                )

                herfindahl_paper = self._paper_mode_compute_herfindahl(cur, portfolio_value)
    ...
    def _paper_mode_compute_drawdown(
        self, cur: Any, reconcile_date: _date_type, portfolio_value: float
    ) -> tuple[float, float]:
        """Running peak and drawdown pct against all snapshots up to `reconcile_date`.

        Verbatim relocation from `_paper_mode_write_snapshot`. Returns (running_peak, drawdown_pct).
        """
        ...
        return running_peak, drawdown_pct

    def _paper_mode_query_win_loss_breakeven(self, cur: Any, open_position_count: int) -> tuple[int, int, int]:
        """Position win/loss/breakeven counts from actual open positions.

        Verbatim relocation from `_paper_mode_write_snapshot`.
        Returns (winning_count, losing_count, breakeven_count).
        """
        winning_count = 0
        losing_count = 0
        breakeven_count = 0
        ...
        return winning_count, losing_count, breakeven_count

    def _paper_mode_compute_concentration(
        self, cur: Any, portfolio_value: float, open_position_count: int
    ) -> tuple[float, float]:
        """Largest/average position size as % of portfolio, from actual position data.

        Verbatim relocation from `_paper_mode_write_snapshot`.
        Returns (largest_position_pct_paper, avg_position_size_pct_paper).
        """
        ...
        return largest_position_pct_paper, avg_position_size_pct_paper

    def _paper_mode_compute_herfindahl(self, cur: Any, portfolio_value: float) -> float:
        """Herfindahl concentration index (sum of squared position pcts) from open positions.
