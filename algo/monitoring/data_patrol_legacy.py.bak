...
class DataPatrol:
    ...
    def _load_configuration(self, cur):
        ...
        config = {
            "staleness_windows": {
                "price_daily": self._get_config_value(
                    cur, "patrol_staleness_price_daily", STALENESS_WINDOW_PRICE_DAILY
                ),
                "technical_data_daily": self._get_config_value(
                    cur,
                    "patrol_staleness_technical_daily",
                    STALENESS_WINDOW_TECHNICAL_DATA,
                ),
                "buy_sell_daily": self._get_config_value(
                    cur,
                    "patrol_staleness_buy_sell_daily",
                    STALENESS_WINDOW_BUY_SELL_DAILY,
                ),
                "signal_quality_scores": self._get_config_value(
                    cur,
                    "patrol_staleness_signal_quality_scores",
                    STALENESS_WINDOW_SIGNAL_QUALITY,
                ),
                "stock_scores": self._get_config_value(
                    cur, "patrol_staleness_stock_scores", STALENESS_WINDOW_STOCK_SCORES
                ),
                "earnings_history": self._get_config_value(
                    cur,
            ...
            "coverage_thresholds": {
                "min_universe_pct": self._get_config_value(
                    cur, "patrol_min_universe_pct", COVERAGE_RATIO_MIN_UNIVERSE_PCT
                ),
                "min_coverage_ratio": self._get_config_value(
                    cur, "patrol_min_coverage_ratio", COVERAGE_RATIO_MIN_NORMAL
                ),
            },
            "price_sanity": {
            ...
            "volume_sanity": {
                "low_volume_threshold": self._get_config_value(
                    cur, "patrol_low_volume_threshold", VOLUME_LOW_THRESHOLD
                ),
                "high_volume_threshold": self._get_config_value(
                    cur, "patrol_high_volume_threshold", VOLUME_HIGH_THRESHOLD
                ),
                "new_low_volume_alert": self._get_config_value(
                    cur, "patrol_new_low_volume_alert", NEW_LOW_VOLUME_ALERT_COUNT
                ),
            },
            "loader_contracts": {
                "price_daily_14d_min": self._get_config_value(
                    cur, "patrol_price_daily_14d_min", 40000
                ),
                "buy_sell_daily_14d_min": self._get_config_value(
                    cur, "patrol_buy_sell_daily_14d_min", 800
                ),
                "coverage_ratio_min": self._get_config_value(
                    cur, "patrol_coverage_ratio_min", COVERAGE_RATIO_MIN_NORMAL
                ),
            },
        }
        return config
    ...
    def check_staleness(self, cur):
        ...
        for tbl, col, freq, config_key, sev_on_stale in sources:
            try:
                ...
                max_days = self._get_config_value(
                    cur, config_key, 7
                )  # Default to 7 days

                tbl_safe = assert_safe_table(tbl)
                col_safe = assert_safe_column(col)
                count, latest_str = safe_select_count(
                    cur, tbl_safe, date_column=col_safe
                )
                if not latest_str:
                ...
                try:
                    latest = datetime.strptime(
                        latest_str.split()[0] if latest_str else "", "%Y-%m-%d"
                    ).date()
    ...
    def check_null_anomalies(self, cur):
        ...
        try:
            ...
            max_null_pct = self._get_config_value(
                cur, "patrol_max_null_pct_threshold", NULL_ANOMALY_MAX_PCT
            )
            ...
            self.log(
                cur, "null_anomaly", ERROR, "price_daily", f"Check failed: {e}", None
            )
    ...
    def check_zero_or_identical(self, cur):
        ...
        try:
            ...
            new_zeros_error = self._get_config_value(
                cur, "patrol_new_zero_symbols_error", ZERO_SYMBOLS_ERROR_THRESHOLD
            )
            new_zeros_warn = self._get_config_value(
                cur, "patrol_new_zero_symbols_warn", ZERO_SYMBOLS_WARN_THRESHOLD
            )
            ident_threshold = self._get_config_value(
                cur, "patrol_identical_ohlc_threshold", IDENTICAL_OHLC_THRESHOLD
            )
    ...
    def check_volume_sanity(self, cur):
        ...
        try:
            # Get thresholds from config (constants define defaults)
            low_vol_threshold = self._get_config_value(
                cur, "patrol_low_volume_threshold", VOLUME_LOW_THRESHOLD
            )
            high_vol_threshold = self._get_config_value(
                cur, "patrol_high_volume_threshold", VOLUME_HIGH_THRESHOLD
            )
            new_low_alert = self._get_config_value(
                cur, "patrol_new_low_volume_alert", NEW_LOW_VOLUME_ALERT_COUNT
            )
            ...
            if low_new > new_low_alert:
                issues.append(
                    f"{low_new} symbols NEW low-volume (<{low_vol_threshold}) â€” possible data source issue"
                )
                self.log(
                    cur,
                    "volume_sanity",
            ...
            if high_vol > 5:
                issues.append(
                    f"{high_vol} symbols with extreme volume (>{high_vol_threshold})"
                )
                self.log(
                    cur,
                    "volume_sanity",
            ...
            self.log(
                cur, "volume_sanity", ERROR, "price_daily", f"Check failed: {e}", None
            )
    ...
    def check_ohlc_sanity(self, cur):
        ...
        try:
            ...
            self.log(
                cur, "ohlc_sanity", ERROR, "price_daily", f"Check failed: {e}", None
            )
    ...
    def check_price_sanity(self, cur):
        ...
        try:
            ...
            self.log(
                cur, "price_sanity", ERROR, "price_daily", f"Check failed: {e}", None
            )
    ...
    def check_loader_coverage(self, cur):
        ...
        """
        try:
            coverage_error_pct = self._get_config_value(
                cur, "patrol_coverage_error_threshold_pct", 95
            )
            coverage_warn_pct = self._get_config_value(
                cur, "patrol_coverage_warning_threshold_pct", 90
            )
            ...
            for table_name in critical_tables:
                try:
                    ...
                    self.log(
                        cur, "coverage", ERROR, table_name, f"Check failed: {e}", None
                    )
        except Exception as e:
            self.log(
                cur, "coverage", ERROR, "patrol_coverage", f"Check failed: {e}", None
            )
    ...
    def check_yahoo_cross_validate(self, cur, top_n=None):
        ...
        if mismatches:
            ...
            self.log(
                ...
                INFO,
                "price_daily",
                f"All {len(symbols)} top symbols match Yahoo within {PRICE_XVAL_MISMATCH_PCT}%",
                None,
            )
    ...
    def check_alpaca_cross_validate(self, cur, top_n=None):
        ...
        try:
            ...
            self.log(
                ...
                INFO,
                "alpaca",
                "No Alpaca creds â€” skipping cross-validate",
                None,
            )
            return
        get_alpaca_base_url()
        ...
        if mismatches:
            ...
            self.log(
                ...
                INFO,
                "price_daily",
                f"All {len(symbols)} top-volume symbols match Alpaca within {PRICE_XVAL_MISMATCH_PCT}%",
                None,
            )
        ...
        for tbl, cond, min_rows, severity, desc in contracts:
            try:
                ...
                if actual < min_rows:
                    ...
                    self.log(
                        cur, "loader_contract", INFO, tbl, f"{actual:,} rows OK", None
                    )
            except Exception as e:
    ...
    def check_db_constraints(self, cur):
        ...
        try:
            self.log(
                ...
                INFO,
                "all",
                "Constraint check skipped (too many false positives)",
                None,
            )
        except Exception as e:
    ...
    def check_etf_data(self, cur):
        ...
        try:
            ...
            self.log(
                cur, "etf_prices", WARN, "etf_price_daily", f"Check skipped: {e}", None
            )
    ...
    def check_cross_table_alignment(self, cur):
        ...
        for tbl, where, min_ratio, sev in checks:
            try:
                tbl_safe = assert_safe_table(tbl)
                cur.execute(
                    f"SELECT COUNT(DISTINCT symbol) FROM {tbl_safe} WHERE {where}"
                )
        ...
        try:
            ...
            self.log(
                cur, "cross_align", INFO, "date_alignment", f"Check skipped: {e}", None
            )
    ...
    def check_fundamental_data(self, cur):
        ...
        for tbl, col, max_days, sev in table_checks:
            try:
                ...
                self.log(
                    cur, "fundamental_data", WARN, tbl, f"Check skipped: {e}", None
                )
    ...
    def summarize(self, cur, elapsed_seconds=None):
        ...
        if flagged:
            ...
            for r in flagged:
                ...
                logger.info(
                    f"  [{sev_pad}] {r['check']:20s} {r['target']:28s} : {r['message']}"
                )
        else:
        ...
        try:
            ...
            status = "ready" if ready else "completed_with_findings"
            logger.info(
                f"[PATROL] ✓ Completed successfully. Updated DynamoDB for grace period tracking (status={status})"
            )
        except Exception as e:
            logger.warning(
                f"[PATROL] Could not update completion status in DynamoDB: {e}"
            )
...
if __name__ == "__main__":
    try:
        ...
        with ExecutionTimeout(max_seconds=600, label="data_patrol"):
            ...
            parser.add_argument(
                "--quick", action="store_true", help="Critical checks only"
            )
            parser.add_argument(
                "--validate-alpaca",
                action="store_true",
