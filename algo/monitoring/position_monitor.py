...
class PositionMonitor:
    ...
    def _fetch_current_market(
        ...
        Args:
            ...
            cur: REQUIRED cursor to use. Must not be None - prevents nested context issues.
                 CRITICAL: Never opens new DatabaseContext - caller must provide cursor
                 to avoid nested context closing the outer cursor (causes "cursor already closed" errors).

        Raises:
            ValueError: If price data is missing or cur is None
        """
        if cur is None:
            raise ValueError(
                f"[POSITION_MONITOR CRITICAL] _fetch_current_market called without cursor. "
                f"This would require opening nested DatabaseContext which closes parent cursor. "
                f"Caller must provide cursor. This is a programming error - Phase 3 must pass cursor."
            )
    ...
    def _check_sector_health(self, symbol: str, current_date: _date | datetime, cur: PsycopgCursor[Any] | None = None) -> str:
        ...
        Args:
            ...
            cur: REQUIRED cursor to use. Must not be None.
                 CRITICAL: Never opens new DatabaseContext - caller must provide cursor
                 to avoid nested context closing the outer cursor (causes "cursor already closed" errors).
        """
        ...
        # CRITICAL FIX: Require cursor - no nested contexts allowed
        if cur is None:
            raise ValueError(
                f"[POSITION_MONITOR CRITICAL] _check_sector_health called without cursor. "
                f"This would require opening nested DatabaseContext which closes parent cursor. "
                f"Caller must provide cursor."
            )
