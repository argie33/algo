...
class Orchestrator:
    ...
    def __init__(
        ...
        # Apply critical pending database migrations
    ...
    def _apply_pending_migrations(self) -> None:
        """Apply critical pending database migrations (idempotent).

        Migration 083: Fix sector enrichment in algo_positions_with_risk view
        - Implements COALESCE hierarchy: algo_trades.sector → company_profile.sector → 'Unknown'
        - Adds company_profile join for proper sector fallback
        - Critical for dashboard sector allocation display
        """
        try:
            with DatabaseContext("write") as cur:
                logger.info("[MIGRATION 083] Applying sector enrichment fix...")

                # Drop and recreate view with proper sector enrichment
