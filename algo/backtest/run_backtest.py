...
Simulates the live algo strategy historically:
  ...
  - Ranking: signal_quality_score (contemporaneous, from buy_sell_daily) - matches live Phase 7's
    actual ranking (see phase7_signal_generation.py's "CRITICAL FIX (Session 377)" note). This
    docstring previously (incorrectly) claimed composite_score was the ranking signal; it never
    was in this script's code, only informational (min_composite is accepted for CLI/API
    compatibility but unused - see _get_daily_buy_signals).
  - Exit: first of - SELL signal in buy_sell_daily, profit target, stop loss, max hold

Note: stock_scores has no date dimension (current snapshot) - composite_score here is always
TODAY's value regardless of the historical signal_date being simulated, so it's informational
only, not a valid historical ranking input. signal_quality_score IS contemporaneous (stored
per-date on buy_sell_daily), which is why it's the real ranking key, matching live Phase 7.
...
numbers are generated by ranking on the same score shown not to work live, so they should NOT
be treated as independent confirmation the strategy works (same underlying mechanism, same
untested-until-recently assumption). Momentum sub-score is price-based (1m/3m/6m/12m) so
