...
This backtest script deliberately was NOT switched to match - it can't be, without introducing
worse look-ahead bias than the SQS-vs-composite_score question it would be trying to answer.
stock_scores has no historical date dimension (current snapshot only) - composite_score here
is always TODAY's value regardless of the historical signal_date being simulated, so ranking
backtest history by it would use future information a live run on that historical date never
had. signal_quality_score IS contemporaneous (stored per-date on buy_sell_daily), which is why
it remains this script's ranking key even though live Phase 7 no longer uses it that way -
the two are now deliberately different mechanisms, not a bug to reconcile blindly.
...
pooled correlation ~0, non-monotonic quintiles) - meaning this backtest's historical Sharpe
numbers are generated by ranking on a score shown not to work live, so they should NOT
be treated as confirmation that live Phase 7's CURRENT (composite_score-based) ranking works
either - this script cannot currently validate that change one way or the other. A
stock_scores_history table (migration 1221, point-in-time composite_score by symbol+date)
exists but only has snapshots from 2026-08-24 onward - once it accumulates enough history,
this script should be rewired to join it by (symbol, score_date) and rank by composite_score
there, to actually test the live ranking without look-ahead bias, rather than assuming either
score works from partial evidence. Momentum sub-score is price-based (1m/3m/6m/12m) so
recency bias is expected for stocks that gained since signal date.
