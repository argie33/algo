...
def run(
    ...
    # Sequential guard pipeline - the 4 guards live in phase8_guards.py (2026-09-05,
    # file-size ratchet: this file is already over its baseline and may not grow further).
    # Each guard's body is the original code verbatim; see phase8_guards.py's module
    # docstring for why it accesses `datetime`/`DatabaseContext` via this module rather
    # than importing them directly.
    if (guard_result := _check_market_hours_guards(config, execution_mode, log_phase_result_fn)) is not None:
        return guard_result
    if (guard_result := _check_pending_orders_guard(execution_mode, run_date, log_phase_result_fn)) is not None:
        return guard_result
    if (guard_result := _check_signal_freshness_guard(log_phase_result_fn)) is not None:
        return guard_result
    if (guard_result := _check_price_freshness_guard(run_date, log_phase_result_fn)) is not None:
        return guard_result
