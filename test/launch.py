...
def run_simple_test():
    ...
    return run_command(["docker-compose", "run", "--rm", "test-runner", "python", "simple_test.py"])

def run_full_tests():
    """Run the full test suite"""
    logger.info("Running full test suite...")
    return run_command(["docker-compose", "run", "--rm", "test-runner", "python", "run_direct_test.py"])

def run_health_check():
    """Run health check"""
    logger.info("Running health check...")
    return run_command(["docker-compose", "run", "--rm", "test-runner", "python", "test_config.py"])
...
def connect_to_db():
    ...
    return run_command(["docker-compose", "exec", "postgres", "psql", "-U", "testuser", "-d", "testdb"])

def main():
    parser = argparse.ArgumentParser(description="ECS Container Test Environment Launcher")
    parser.add_argument("action", choices=[
        "start", "stop", "test", "full-test", "health", "logs", "db", "all"
