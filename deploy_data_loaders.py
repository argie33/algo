...
class DataLoaderDeployment:
    ...
    def __init__(self, region: str = None, dry_run: bool = False):
        ...
        self.logger.info(f"Data Loader Deployment initialized for region: {self.region}")
    
    def build_and_push_docker_image(self, loader_name: str, dockerfile_path: str = None) -> str:
        """
        ...
        try:
            ...
            process = subprocess.run(login_cmd, input=password, text=True, capture_output=True)
            if process.returncode != 0:
                raise RuntimeError(f"ECR login failed: {process.stderr}")
            
            # Build image
            image_tag = f"{loader_name}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            image_uri = f"{registry.replace('https://', '')}/{self.ecr_repository}:{image_tag}"
            
            if dockerfile_path is None:
                # Use default Dockerfile
                dockerfile_path = os.path.join(os.path.dirname(__file__), 'Dockerfile.dataloader')
            
            build_cmd = [
                'docker', 'build',
                '-t', image_uri,
                '-f', dockerfile_path,
                '--build-arg', f'LOADER_SCRIPT={loader_name}.py',
    ...
    def create_or_update_task_definition(self, loader_name: str, image_uri: str) -> str:
            ...
            'containerDefinitions': [
                {
                    ...
                    'environment': [
                        ...
                        {'name': 'DB_SECRET_ARN', 'value': os.getenv('DB_SECRET_ARN', '')},
                        {'name': 'FRED_API_KEY', 'value': os.getenv('FRED_API_KEY', '')}
                    ],
                    'cpu': 512,
                    'memory': 1024,
        ...
        if self.dry_run:
            self.logger.info(f"DRY RUN: Would create/update task definition for {loader_name}")
        ...
        try:
            ...
            self.logger.error(f"Failed to create task definition for {loader_name}: {e}")
            raise
    
    def run_ecs_task(self, loader_name: str, task_definition_arn: str, timeout_minutes: int = 30) -> Dict:
        """
        ...
        try:
            ...
            while datetime.now() < timeout_time:
                ...
                if last_status == 'STOPPED':
                    ...
                    if success:
                        self.logger.info(f"Task {loader_name} completed successfully in {execution_time:.1f}s")
                    else:
                        self.logger.error(f"Task {loader_name} failed with exit code {exit_code}: {stop_reason}")
            ...
            # Task timed out
            self.logger.error(f"Task {loader_name} timed out after {timeout_minutes} minutes")
    ...
    def get_task_logs(self, loader_name: str, task_arn: str, lines: int = 100) -> List[str]:
        """
    ...
    def deploy_and_run_loader(self, loader_name: str, timeout_minutes: int = 30) -> Dict:
        """
        ...
        try:
            ...
            # Step 3: Run the task
            execution_result = self.run_ecs_task(loader_name, task_def_arn, timeout_minutes)
            ...
            if result['success']:
                self.logger.info(f"Successfully deployed and ran {loader_name} in {total_time:.1f}s")
            else:
    ...
    def deploy_multiple_loaders(self, loader_names: List[str], parallel: bool = False) -> List[Dict]:
        """
        ...
        if parallel:
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                futures = {
                    executor.submit(self.deploy_and_run_loader, loader_name): loader_name 
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        self.logger.error(f"Parallel deployment failed for {loader_name}: {e}")
                        results.append({
                            'loader_name': loader_name,
                            'success': False,
                            'error': str(e),
...
def main():
    ...
    parser.add_argument('--parallel', action='store_true', help='Run deployments in parallel')
    parser.add_argument('--timeout', type=int, default=30, help='Execution timeout in minutes')
