...
def panel_performance_spark(
    ...
    # CRITICAL: Check explicitly for None to distinguish 0.0 (valid) from missing data
    avg_win_v = perf.get("avg_win_pct")
    if avg_win_v is None:
        avg_win_v = perf.get("avg_win")
    avg_loss_v = perf.get("avg_loss_pct")
    if avg_loss_v is None:
