...
def _build_scores_table(top_scores: list[Any], limit: int = 15, show_company: bool = False) -> list[Text | Table]:
    ...
    # Wt% column REMOVED 2026-09-18 (composite_tilted_weight no longer exists anywhere -
    # /api/algo/scores ranks by the real composite_score column now, so there's no separate
    # sort key to explain; see lambda/api/routes/algo_handlers/dashboard/scores.py's module
    # docstring).
