...
class ValueMetricsCalculator:
    ...
    def calculate_traditional_multiples_metric(self, financial_data: Dict, market_data: Dict) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            current_price = market_data.get('currentPrice', market_data.get('regularMarketPrice', 0))
            shares_outstanding = financial_data.get('sharesOutstanding', 0)
            market_cap = financial_data.get('marketCap', current_price * shares_outstanding if current_price and shares_outstanding else 0)
    ...
    def calculate_intrinsic_value_metric(self, financial_data: Dict, market_data: Dict) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            dcf_metric, dcf_components = self._calculate_dcf_metric(financial_data, current_price)
            components.update(dcf_components)
            
            # 2. Dividend Discount Model (25% of intrinsic value metric)
            ddm_metric, ddm_components = self._calculate_ddm_metric(financial_data, current_price)
            components.update(ddm_components)
            
            # 3. Residual Income Model (25% of intrinsic value metric)
            rim_metric, rim_components = self._calculate_residual_income_metric(financial_data, current_price)
    ...
    def _calculate_dcf_metric(self, financial_data: Dict, current_price: float) -> Tuple[float, Dict]:
        ...
        try:
            ...
            return 0.5, {'dcf_metric': 0.5, 'dcf_error': 'Insufficient data for DCF calculation'}
            
        except Exception as e:
            logger.error(f"Error in DCF calculation: {str(e)}")
            return 0.5, {'dcf_error': str(e)}
    
    def _calculate_ddm_metric(self, financial_data: Dict, current_price: float) -> Tuple[float, Dict]:
        ...
        try:
            ...
            if dividend_yield and dividend_yield > 0:
                ...
                dividend_growth_rate = min(0.15, max(0, roe * retention_ratio)) if roe else 0.03
                
                # Gordon Growth Model
                cost_of_equity = self.risk_free_rate + financial_data.get('beta', 1.0) * self.market_risk_premium
                
                if cost_of_equity > dividend_growth_rate:
                    expected_dividend = dividend_rate * (1 + dividend_growth_rate) if dividend_rate else current_price * dividend_yield * (1 + dividend_growth_rate)
                    ddm_value = expected_dividend / (cost_of_equity - dividend_growth_rate)
            ...
            return 0.5, {'ddm_metric': 0.5, 'ddm_note': 'No dividend or insufficient data'}
            
        except Exception as e:
            logger.error(f"Error in DDM calculation: {str(e)}")
            return 0.5, {'ddm_error': str(e)}
    
    def _calculate_residual_income_metric(self, financial_data: Dict, current_price: float) -> Tuple[float, Dict]:
    ...
    def calculate_relative_value_metric(self, financial_data: Dict, symbol: str) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            if sector_multiples and pe_ratio:
                ...
                if sector_pe_median > 0:
                    ...
                    components['sector_pe_percentile'] = ((sector_pe_median - pe_ratio) / sector_pe_median + 1) * 50
            
            components['sector_relative_metric'] = sector_metric
            
            # Historical valuation analysis (40% of relative value metric)
            historical_metric = self._calculate_historical_valuation_metric(financial_data, symbol)
            components['historical_metric'] = historical_metric
            
            # Calculate weighted relative value metric
            relative_value_metric = (
                sector_metric * 0.60 +
                historical_metric * 0.40
            )
...
def main():
    ...
    try:
        ...
        logger.info(f"Value metrics calculation completed. Success: {successful_calculations}, Failed: {failed_calculations}")
