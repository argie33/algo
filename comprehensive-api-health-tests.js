...
async function testAllAPIs() {
  ...
  const endpoints = [
    // Core Health & System
    {
      ...
      tests: [
        { name: 'Health Check (Quick)', url: '/health?quick=true', method: 'GET' },
        { name: 'Health Check (Full)', url: '/health', method: 'GET' },
        { name: 'Database Health', url: '/health/database', method: 'GET' },
        { name: 'Database Diagnostics', url: '/health/database/diagnostics', method: 'GET' },
        { name: 'Health Status Summary', url: '/health/status-summary', method: 'GET' }
      ]
    },
    ...
    // Stock Data & Screening
    {
      ...
      tests: [
        ...
        { name: 'Stock Prices (AAPL)', url: '/stocks/AAPL/prices', method: 'GET' }
      ]
    },
    ...
    // Metrics & Scoring
    {
      ...
      tests: [
        ...
        { name: 'Sector Analysis', url: '/metrics/sectors/analysis', method: 'GET' },
        { name: 'Top Quality Stocks', url: '/metrics/top/quality', method: 'GET' },
        { name: 'Top Value Stocks', url: '/metrics/top/value', method: 'GET' },
        { name: 'Top Composite Stocks', url: '/metrics/top/composite', method: 'GET' },
    ...
    // Technical Analysis
    {
      ...
      tests: [
        ...
        { name: 'Technical Detail (AAPL)', url: '/technical/AAPL', method: 'GET' }
      ]
    },
    ...
    // Financial Data
    {
      ...
      tests: [
        ...
        { name: 'Income Statement (AAPL)', url: '/financials/AAPL/income', method: 'GET' },
        { name: 'Balance Sheet (AAPL)', url: '/financials/AAPL/balance', method: 'GET' },
        { name: 'Cash Flow (AAPL)', url: '/financials/AAPL/cashflow', method: 'GET' },
        { name: 'Key Metrics (AAPL)', url: '/financials/AAPL/metrics', method: 'GET' }
      ]
    },
    ...
    // Trading & Signals
    {
      ...
      tests: [
        ...
        { name: 'Buy Signals Daily', url: '/trading/signals/daily', method: 'GET' },
        { name: 'Buy Signals Weekly', url: '/trading/signals/weekly', method: 'GET' },
        { name: 'Buy Signals Monthly', url: '/trading/signals/monthly', method: 'GET' },
    ...
    // Analyst Data
    {
      ...
      tests: [
        ...
        { name: 'Analyst Estimates (AAPL)', url: '/analysts/AAPL', method: 'GET' },
        { name: 'Analyst Upgrades/Downgrades', url: '/analysts/upgrades', method: 'GET' },
        { name: 'Analyst Recommendations', url: '/analysts/recommendations', method: 'GET' }
      ]
    },
    ...
    // Portfolio Management
    {
      ...
      tests: [
        ...
        { name: 'Portfolio Holdings', url: '/portfolio/holdings', method: 'GET' },
        { name: 'Portfolio Performance', url: '/portfolio/performance', method: 'GET' },
        { name: 'Portfolio Analytics', url: '/portfolio/analytics', method: 'GET' }
      ]
    },
    ...
    // Backtesting
    {
      ...
      tests: [
        ...
        { name: 'Backtest Strategies', url: '/backtest/strategies', method: 'GET' },
        { name: 'Backtest Performance', url: '/backtest/performance', method: 'GET' }
      ]
    },
    ...
    // Dashboard APIs
    {
      ...
      tests: [
        ...
        { name: 'Market Summary', url: '/dashboard/market-summary', method: 'GET' },
        { name: 'Earnings Calendar (AAPL)', url: '/dashboard/earnings-calendar?symbol=AAPL', method: 'GET' },
        { name: 'Analyst Insights (AAPL)', url: '/dashboard/analyst-insights?symbol=AAPL', method: 'GET' },
        { name: 'Financial Highlights (AAPL)', url: '/dashboard/financial-highlights?symbol=AAPL', method: 'GET' },
    ...
    for (const test of category.tests) {
      try {
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          console.log(`   ⚠️  ${response.status} - Endpoint not implemented (${responseTime}ms)`);
          categoryResults.failed++;
          results.failed++;
          categoryResults.errors.push(`${test.name}: Not implemented (404)`);
          results.failedEndpoints.push(`${category.category}/${test.name}: 404 Not Found`);
        } else if (response.status === 401 || response.status === 403) {
          console.log(`   🔐 ${response.status} - Authentication required (${responseTime}ms)`);
          ...
          categoryResults.failed++;
          results.failed++;
          categoryResults.errors.push(`${test.name}: ${response.status} ${response.statusText}`);
          results.failedEndpoints.push(`${category.category}/${test.name}: ${response.status} ${response.statusText}`);
        }
        ...
        results.failedEndpoints.push(`${category.category}/${test.name}: ${error.message}`);
      }
    }
  ...
  console.log(`📊 Overall: ${results.passed}/${results.total} endpoints passed (${overallPassRate}%)`);
  ...
  for (const [category, data] of Object.entries(results.categories)) {
    ...
    console.log(`   ${status} ${category}: ${data.passed}/${data.total} (${rate}%)`);
  }
  
  if (results.failedEndpoints.length > 0) {
    console.log('\n❌ Failed Endpoints:');
    results.failedEndpoints.forEach(endpoint => console.log(`   - ${endpoint}`));
  }
  ...
  if (overallPassRate >= 90) {
    console.log('   🎉 Excellent API coverage! Most endpoints are working correctly.');
  } else if (overallPassRate >= 75) {
    console.log('   👍 Good API coverage with some areas for improvement.');
  } else {
    console.log('   ⚠️  API coverage needs improvement. Many endpoints are failing.');
  }
