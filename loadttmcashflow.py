#!/usr/bin/env python3
# Updated: 2025-10-04 - TTM cash flow loader
import gc
import json
import logging
import os
...
def calculate_ttm_cash_flow(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if cash_flow is None or cash_flow.empty:
            logging.warning(
                f"No quarterly cash flow data returned by any method for TTM calculation for {symbol}"
            )
            return None
        ...
        if cash_flow.isna().all().all():
            logging.warning(
                f"Quarterly cash flow data is all NaN for TTM calculation for {symbol}"
            )
            return None
        ...
        if len(cash_flow.columns) < 4:
            logging.warning(
                f"Insufficient quarterly data for TTM calculation for {symbol} (only {len(cash_flow.columns)} quarters available)"
            )
            return None
        ...
        logging.info(
            f"Calculated TTM cash flow for {symbol}: {len(ttm_df)} line items from {valid_quarters} quarters"
        )
        return ttm_df
        ...
        for symbol in batch:
            ...
            for attempt in range(1, MAX_BATCH_RETRIES + 1):
                try:
                    ...
                    # Process the data
                    ttm_cash_flow_data = process_ttm_cash_flow_data(
                        symbol, ttm_cash_flow
                    )
                    ...
                    if ttm_cash_flow_data:
                        ...
                        processed += 1
                        logging.info(
                            f"✓ Successfully processed {symbol} ({len(ttm_cash_flow_data)} TTM records)"
                        )
                        success = True
                        break
                    else:
