...
def load_ttm_cashflow(symbols, cur, conn):
    total = len(symbols)
    ...
    for batch_idx in range(batches):
        ...
        for yq_sym, orig_sym in mapping.items():
            ...
            if cashflow is None or cashflow.empty:
                logging.error(f"Skipping {orig_sym} - failed to retrieve TTM cash flow after {MAX_BATCH_RETRIES} attempts")
                continue
            ...
            try:
                ...
                if data_to_insert:
                    execute_values(cur, """
                        INSERT INTO ttm_cashflow (symbol, date, item_name, value)
                        VALUES %s
...
if __name__ == "__main__":
    ...
    # Recreate table
    logging.info("Recreating ttm_cashflow table...")
    ...
    t_s, p_s, f_s = load_ttm_cashflow(stock_syms, cur, conn)
