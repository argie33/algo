#!/usr/bin/env python3
"""
Annual Income Statement Loader - Optimal Pattern using SEC EDGAR.
...
Run:
    python3 loadannualincomestatement.py [--symbols AAPL,MSFT] [--parallelism 8]
"""
...
class AnnualIncomeStatementLoader(OptimalLoader):
    ...
    def fetch_incremental(self, symbol: str, since: Optional[date]):
        ...
        try:
            rows = client.get_income_statement(symbol, period="annual")
            if not rows:
                return None
...
def main():
    parser = argparse.ArgumentParser(description="Optimal annual_income_statement loader")
    ...
    loader = AnnualIncomeStatementLoader()
    try:
