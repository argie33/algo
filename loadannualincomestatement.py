#!/usr/bin/env python3
# Updated: 2025-10-04 02:00 - Annual income statement loader
import gc
import json
import logging
import os
...
def get_income_statement_data(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if income_statement is None or income_statement.empty:
            logging.warning(
                f"No income statement data returned by any method for {symbol}"
            )
            return None
        ...
        logging.info(
            f"Retrieved income statement data for {symbol}: {len(income_statement.columns)} periods, {len(income_statement.index)} line items"
        )
...
def load_annual_income_statement(
