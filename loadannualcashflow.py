...
"""
Annual Cash Flow Loader - Optimal Pattern using SEC EDGAR.
...
Run:
    python3 loadannualcashflow.py [--symbols AAPL,MSFT] [--parallelism 8]
"""
...
class AnnualCashFlowLoader(OptimalLoader):
    ...
    def fetch_incremental(self, symbol: str, since: Optional[date]):
        ...
        try:
            rows = client.get_cash_flow(symbol, period="annual")
            if not rows:
                return None
    ...
    def transform(self, rows):
        ...
        schema_cols = {'financing_cash_flow', 'investing_cash_flow', 'fiscal_year', 'operating_cash_flow', 'symbol', 'free_cash_flow'}
        filtered = [
...
def main():
    parser = argparse.ArgumentParser(description="Optimal annual_cash_flow loader")
