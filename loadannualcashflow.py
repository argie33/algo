...
def load_annual_cash_flow(symbols, cur, conn):
    total = len(symbols)
        ...
        for yq_sym, orig_sym in mapping.items():
            ...
            try:
                ...
                if cash_flow_data:
                    ...
                    insert_sql = f"""
                        INSERT INTO annual_cash_flow ({column_names})
                        VALUES ({placeholders})
...
if __name__ == "__main__":
    ...
    sample_cash_flow = sample_ticker.cashflow
    ...
    if sample_cash_flow is not None and not sample_cash_flow.empty:
        ...
        conn.commit()
        logging.info("Created annual cash flow table with dynamic columns")
    else:
