...
def get_cash_flow_data(symbol: str) -> Optional[pd.DataFrame]:
    """Get annual cash flow data using proper yfinance API"""
    try:
        ticker = yf.Ticker(symbol)
        
        # Get annual cash flow (cashflow property gives annual data)
        cash_flow = ticker.cashflow
        
        if cash_flow is None or cash_flow.empty:
            logging.warning(f"No cash flow data returned by yfinance for {symbol}")
            return None
