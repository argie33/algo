        ...
        try:
            ...
            try:
                ...
                if dfs:
                    ...
                    if len(data.columns) >= 7:  # Expected columns
                        ...
                        logging.info(
                            f"Successfully downloaded NAAIM data: {len(data)} records"
                        )
                        return data
                    else:
                        logging.warning(
                            f"Table found but doesn't look like NAAIM data. Columns: {list(data.columns)}"
                        )
            ...
            try:
                ...
                for i, table in enumerate(tables):
                    logging.info(
                        f"Table {i}: {table.get('class', 'no-class')} - {table.get('id', 'no-id')}"
                    )
                    ...
                    if rows:
                        ...
                        if cells:
                            logging.info(
                                f"  First cell content: {cells[0].get_text(strip=True)}"
                            )
                ...
                if tables:
                    ...
                    logging.info(
                        f"Attempting to parse largest table with {len(largest_table.find_all('tr'))} rows"
                    )
                    ...
                    if rows:
                        ...
                        if len(data.columns) >= 7:
                            ...
                            logging.info(
                                f"Successfully parsed NAAIM data: {len(data)} records"
                            )
                            return data
            ...
            if attempt < MAX_DOWNLOAD_RETRIES:
                ...
                raise Exception(
                    f"Failed to download NAAIM data after {MAX_DOWNLOAD_RETRIES} attempts: {e}"
                )
            ...
            try:
                rows.append(
                    [
                        ...
                        (
                            None
                            if pd.isna(row["NAAIM Number Mean/Average"])
                            else float(row["NAAIM Number Mean/Average"])
                        ),
