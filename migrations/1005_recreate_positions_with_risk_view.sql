-- MIGRATION: 1005_recreate_positions_with_risk_view
-- CRITICAL FIX: Recreate algo_positions_with_risk materialized view
--
-- Issue: Phase 9 (Final Reconciliation) tries to REFRESH MATERIALIZED VIEW algo_positions_with_risk
-- but the view doesn't exist in the database, causing orchestrator Phase 9 to fail.
--
-- Root cause: Migration 040 that creates this view was never applied to production database.
-- This materialized view is CRITICAL for:
-- 1. Dashboard positions panel (enriched position data with stops, targets, risk metrics)
-- 2. Phase 9 reconciliation (refreshes after trade execution)
-- 3. Position health monitoring (distance to stops/targets, R-multiple calculations)
--
-- Status: Applied 2026-07-06 (CRITICAL FIX)
...
'Enriched positions with stops, targets, sector, technical scores, and risk metrics. Single source of truth for dashboard and orchestrator Phase 9 reconciliation.';
