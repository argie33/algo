...
apply time and the Python import failed, or if rows were deleted after seeding).

Uses ON CONFLICT DO NOTHING so existing custom values are always preserved.
...
_DEFAULTS = [
    ...
    (
        ...
        "0.0",
        "float",
        "Min % from 52w low (Minervini allows 52w lows with RS strength)",
    ),
    ...
    (
        ...
        "false",
        "bool",
        "Require 2%+ pullback before partial profit exits",
    ),
    ...
    (
        ...
        "200",
        "int",
        "Min trading days of price history (IPO age gate)",
    ),
    ...
    (
        ...
        "100000.0",
        "float",
        "Bootstrap portfolio value when Alpaca unreachable",
    ),
    # Feature Flags
    ...
    (
        ...
        "180",
        "int",
        "Max seconds to wait for ECS task to reach RUNNING state",
    ),
    (
        "failsafe_grace_period_minutes",
        "240",
        "int",
        "Grace period before triggering second failsafe (issue #10)",
    ),
]
...
def down():
    with DatabaseContext("write") as cur:
        cur.execute("DELETE FROM algo_config WHERE updated_by = 'migration-013'")
