-- Migration 077: Fix algo_positions_with_risk stop price reference
-- algo_positions.stop_loss_price is never written — only current_stop_price is set.
-- View used ap.stop_loss_price which was always NULL, making R-multiple, distance-to-stop,
-- and stop column all show NULL/zero in the dashboard.
-- Fix: use COALESCE(ap.stop_loss_price, ap.current_stop_price) everywhere in the view.
...
SELECT
  ...
  -- Use the best available stop: ap.stop_loss_price (if set), fallback to current_stop_price
