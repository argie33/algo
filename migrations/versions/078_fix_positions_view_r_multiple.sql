-- Migration 078: Fix r_multiple in algo_positions_with_risk view
-- Previous formula computed (entry - stop) / entry which is always positive
-- and equals the initial risk fraction, NOT the R-multiple.
-- Correct formula: (current_price - entry) / (entry - stop)
-- This gives positive R when in profit, negative when at a loss.
