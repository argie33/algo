...
async function testFrontendAPI() {
  ...
  try {
    ...
    const perfResponse = await axios.get(
      `${API_URL}/portfolio/performance?timeframe=1Y`
    );
    console.log(`✅ Portfolio Performance: ${perfResponse.status}`);
    console.log(
      `📊 Performance data points: ${perfResponse.data.data.performance.length}`
    );
    console.log(
      `💰 Total return: ${perfResponse.data.data.metrics.totalReturnPercent}%`
    );

    // Test portfolio analytics (main issue endpoint)
    console.log("\n3. Testing Portfolio Analytics...");
    const analyticsResponse = await axios.get(
      `${API_URL}/portfolio/analytics?timeframe=1Y`
    );
    console.log(`✅ Portfolio Analytics: ${analyticsResponse.status}`);
    console.log(
      `📈 Holdings count: ${analyticsResponse.data.data.holdings.length}`
    );
    console.log(
      `🏢 Portfolio value: $${analyticsResponse.data.data.summary.totalValue.toLocaleString()}`
    );

    console.log("\n🎉 All API tests passed! The AWS API is working correctly.");
    console.log(
      "💡 The issue might be with frontend authentication or configuration."
    );
  } catch (error) {
