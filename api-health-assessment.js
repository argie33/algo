...
async function assessAPIHealth() {
  ...
  const healthEndpoints = [
    { name: 'Quick Health Check', url: '/api/health?quick=true', expected: 200 },
    { name: 'Full Health Check', url: '/api/health', expected: 200 },
    { name: 'Database Health', url: '/api/health/database', expected: 200 },
    { name: 'Database Diagnostics', url: '/api/health/database/diagnostics', expected: 200 },
  ...
  const stockEndpoints = [
    ...
    { name: 'Stock Public Sample', url: '/api/stocks/public/sample', expected: 200 },
  ...
  const portfolioEndpoints = [
    { name: 'Portfolio Analytics', url: '/api/portfolio/analytics', expected: 401 }, // Should return 401 without auth
    { name: 'Portfolio Holdings', url: '/api/portfolio/holdings', expected: 401 }
  ];
  ...
  for (const test of allTests) {
    try {
      ...
      if (isHealthy) {
        ...
        console.log(`⚠️  ${test.name} - ${status} (${duration}ms) - Auth bypassed`);
      } else if (isBroken) {
        results.broken.push({
          name: test.name,
          url: test.url,
        ...
        console.log(`❌ ${test.name} - ${status} (${duration}ms) - ${response.data?.error || 'Server error'}`);
      } else {
        results.broken.push({
          name: test.name,
          url: test.url,
        ...
        console.log(`❌ ${test.name} - ${status} (${duration}ms) - Unexpected status`);
      }
  ...
  const healthyHealthCheck = results.healthy.find(r => r.name === 'Full Health Check');
  ...
  const stockSample = results.healthy.find(r => r.name === 'Stock Public Sample');
  if (stockSample) {
