...
def ensure_tables(conn):
    """Drop & recreate quarterly_cash_flow tables and ensure last_updated exists."""
    with conn.cursor() as cur:
        # quarterly_cash_flow table
        cur.execute("DROP TABLE IF EXISTS quarterly_cash_flow;")
        cur.execute("""
        ...
        cur.execute("""
            CREATE INDEX idx_quarterly_cash_flow_symbol 
            ON quarterly_cash_flow (symbol);
        """)
        # Create index on date for faster lookups
        cur.execute("""
            CREATE INDEX idx_quarterly_cash_flow_date 
            ON quarterly_cash_flow (date);
        """)
        # last_updated
        cur.execute("""
...
def process_symbol(symbol, conn):
    """Fetch quarterly cash flow via yfinance and insert into PostgreSQL."""
    yf_symbol = symbol.upper().replace(".", "-")
    ticker = yf.Ticker(yf_symbol)

    # Try to get quarterly cash flow data
    try:
        quarterly_cash_flow = ticker.quarterly_cash_flow
        if quarterly_cash_flow is None or quarterly_cash_flow.empty:
            logger.warning(f"No quarterly cash flow data for {symbol}")
            return
    except Exception as e:
        logger.error(f"Error fetching quarterly cash flow data for {symbol}: {e}")
        raise

    # Process each item in the quarterly cash flow dataframe
    data_to_insert = []
    
    # Iterate through rows (items) and columns (dates)
    for item_name, row in quarterly_cash_flow.iterrows():
        for date_col in quarterly_cash_flow.columns:
    ...
    if not data_to_insert:
        logger.info(f"No quarterly cash flow data found for {symbol}")
        return
    ...
    logger.info(f"Successfully processed {len(data_to_insert)} quarterly cash flow records for {symbol}")
...
def main():
    ...
    try:
        ...
        logger.info("loadquarterlycashflow complete.")
