...
def get_quarterly_cash_flow_data(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if cash_flow is None or cash_flow.empty:
            logging.warning(
                f"No quarterly cash flow data returned by any method for {symbol}"
            )
            return None
        ...
        logging.info(
            f"Retrieved quarterly cash flow data for {symbol}: {len(cash_flow.columns)} periods, {len(cash_flow.index)} line items"
        )
        return cash_flow
