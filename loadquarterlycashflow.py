#!/usr/bin/env python3
"""
Quarterly Cash Flow Loader - Optimal Pattern using SEC EDGAR.
...
Run:
    python3 loadquarterlycashflow.py [--symbols AAPL,MSFT] [--parallelism 8]
"""
...
class QuarterlyCashFlowLoader(OptimalLoader):
    ...
    def fetch_incremental(self, symbol: str, since: Optional[date]):
        ...
        try:
            rows = client.get_cash_flow(symbol, period="quarterly")
            if not rows:
                return None
...
def main():
    parser = argparse.ArgumentParser(description="Optimal quarterly_cash_flow loader")
    ...
    loader = QuarterlyCashFlowLoader()
    try:
