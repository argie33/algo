        ...
        for period in [5, 10, 20, 50, 100, 200]:
            if len(close) >= period:
                ...
                if indicators[f"sma_{period}"]:
                    indicators[f"price_vs_sma_{period}"] = (
                        close.iloc[-1] - indicators[f"sma_{period}"]
    ...
    def calculate_volatility_indicators(self) -> Dict:
        ...
        if len(close) >= 20:
            ...
            if all(indicators.get(k) for k in ["bb_upper", "bb_lower"]):
                bb_position = (close.iloc[-1] - indicators["bb_lower"]) / (
        ...
        if len(close) >= 21:
            ...
            if len(returns) >= 20:
                volatility = returns.rolling(window=20).std() * math.sqrt(
                    252
                )  # Annualized
                indicators["historical_volatility_20d"] = safe_float(
                    volatility.iloc[-1]
                )
        ...
        if len(volume) >= 20:
            ...
            if indicators["volume_ma_20"]:
                indicators["volume_ratio"] = (
                    volume.iloc[-1] / indicators["volume_ma_20"]
                )
    ...
    def calculate_trend_indicators(self) -> Dict:
        ...
        if len(close) >= 10:
            if TALIB_AVAILABLE:
                ...
                # SAR signal
                indicators["sar_signal"] = (
                    1 if close.iloc[-1] > indicators["parabolic_sar"] else -1
                )
    ...
    def get_latest_technical_indicators(self) -> Dict:
        ...
        result = {
            ...
            "date": (
                ...
                if hasattr(self.data.index[-1], "date")
                else date.today()
            ),
...
def load_technicals_batch(
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        logging.info(
            f"Processing technicals batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
        ...
        if technical_data:
            try:
                ...
                for item in technical_data:
                    insert_data.append(
                        (
                            ...
                            item.get("vwap"),
                            # Price vs MAs
                            item.get("price_vs_sma_20"),
                            item.get("price_vs_sma_50"),
                            item.get("price_vs_sma_200"),
                            item.get("price_vs_vwap"),
                            # Momentum
                            item.get("rsi_14"),
                            item.get("macd"),
                            ...
                            item.get("bb_middle"),
                            item.get("bb_lower"),
                            item.get("bb_position"),
                            item.get("bb_width"),
                            item.get("atr_14"),
                            item.get("atr_percent"),
                            item.get("historical_volatility_20d"),
                            # Volume
                            item.get("obv"),
                            item.get("volume_ma_20"),
                ...
                logging.info(
                    f"Technicals batch {batch_num} inserted {len(technical_data)} symbols successfully"
                )

            except Exception as e:
                logging.error(
                    f"Database insert error for technicals batch {batch_num}: {e}"
                )
                conn.rollback()
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process technical indicators for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
