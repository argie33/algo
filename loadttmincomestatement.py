...
def calculate_ttm_income_statement(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if income_statement is None or income_statement.empty:
            logging.warning(f"No quarterly income statement data for TTM calculation for {symbol}")
            return None
            ...
            for attempt in range(1, MAX_BATCH_RETRIES + 1):
                try:
                    ...
                    if ttm_income_statement_data:
                        ...
                        conn.commit()
                        processed += 1
                        logging.info(f"Successfully processed {symbol} ({len(ttm_income_statement_data)} records)")
                        success = True
                        break
                    else:
