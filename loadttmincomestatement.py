#!/usr/bin/env python3
# Updated: 2025-10-04 - Financial statement loader (TTM)
import gc
import json
import logging
import os
...
def calculate_ttm_income_statement(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        for method_name, description in methods_to_try:
            try:
                if hasattr(ticker, method_name):
                    logging.info(
                        f"Trying {method_name} for TTM calculation for {symbol}"
                    )
                    income_statement = getattr(ticker, method_name)

                    if income_statement is not None and not income_statement.empty:
                        logging.info(
                            f"✓ Successfully got data using {method_name} for TTM calculation for {symbol}"
                        )
                        break
                    else:
                        logging.warning(
                            f"{method_name} returned empty data for TTM calculation for {symbol}"
                        )
                else:
                    logging.warning(
                        f"Method {method_name} not available for TTM calculation for {symbol}"
                    )
            except Exception as e:
                logging.warning(
                    f"Error with {method_name} for TTM calculation for {symbol}: {e}"
                )
                continue

        if income_statement is None or income_statement.empty:
            logging.warning(
                f"No quarterly income statement data returned by any method for TTM calculation for {symbol}"
            )
            return None
        ...
        if income_statement.isna().all().all():
            logging.warning(
                f"Quarterly income statement data is all NaN for TTM calculation for {symbol}"
            )
            return None

        # Sort columns by date (most recent first)
        income_statement = income_statement.reindex(
            sorted(income_statement.columns, reverse=True), axis=1
        )

        # Take the most recent 4 quarters for TTM calculation
        if len(income_statement.columns) < 4:
            logging.warning(
                f"Insufficient quarterly data for TTM calculation for {symbol} (only {len(income_statement.columns)} quarters available)"
            )
            return None
        ...
        valid_quarters = sum(
            1 for col in ttm_quarters.columns if not ttm_quarters[col].isna().all()
        )
        if valid_quarters < 3:  # Need at least 3 quarters with some data
            logging.warning(
                f"Insufficient valid quarterly data for TTM calculation for {symbol} (only {valid_quarters} quarters with data)"
            )
            return None
        ...
        logging.info(
            f"Calculated TTM income statement for {symbol}: {len(ttm_df)} line items from {valid_quarters} quarters"
        )
        return ttm_df
...
def process_ttm_income_statement_data(
    symbol: str, ttm_income_statement: pd.DataFrame
) -> List[Tuple]:
    ...
    logging.info(
        f"Processed TTM {symbol}: {valid_dates} valid dates, {valid_values}/{total_values} valid values, {len(processed_data)} records"
    )
    return processed_data


def load_ttm_income_statement(
    symbols: List[str], cur, conn
        ...
        for symbol in batch:
            ...
            for attempt in range(1, MAX_BATCH_RETRIES + 1):
                try:
                    ...
                    # Process the data
                    ttm_income_statement_data = process_ttm_income_statement_data(
                        symbol, ttm_income_statement
                    )
                    ...
                    if ttm_income_statement_data:
                        ...
                        processed += 1
                        logging.info(
                            f"✓ Successfully processed {symbol} ({len(ttm_income_statement_data)} TTM records)"
                        )
                        success = True
                        break
                    else:
                        logging.warning(
                            f"✗ No valid TTM data found for {symbol} after processing"
                        )
                        break
