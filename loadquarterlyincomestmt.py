...
# -------------------------------
SCRIPT_NAME = "loadquarterlyincomestmt.py"
...
def ensure_tables(conn):
    """Drop & recreate quarterly_income_stmt tables and ensure last_updated exists."""
    with conn.cursor() as cur:
        # quarterly_income_stmt table
        cur.execute("DROP TABLE IF EXISTS quarterly_income_stmt;")
        cur.execute("""
            CREATE TABLE quarterly_income_stmt (
        ...
        cur.execute("""
            CREATE INDEX idx_quarterly_income_stmt_symbol 
            ON quarterly_income_stmt (symbol);
        """)
        # Create index on date for faster lookups
        cur.execute("""
            CREATE INDEX idx_quarterly_income_stmt_date 
            ON quarterly_income_stmt (date);
        """)
        # last_updated
        cur.execute("""
...
def process_symbol(symbol, conn):
    """Fetch quarterly income statement via yfinance and insert into PostgreSQL."""
    yf_symbol = symbol.upper().replace(".", "-")
    ticker = yf.Ticker(yf_symbol)

    # Try to get quarterly income statement data
    try:
        quarterly_income = ticker.quarterly_income_stmt
        if quarterly_income is None or quarterly_income.empty:
            logger.warning(f"No quarterly income statement data for {symbol}")
            return
    except Exception as e:
        logger.error(f"Error fetching quarterly income statement data for {symbol}: {e}")
        raise

    # Process each item in the quarterly income statement dataframe
    data_to_insert = []
    
    # Iterate through rows (items) and columns (dates)
    for item_name, row in quarterly_income.iterrows():
    ...
    if not data_to_insert:
        logger.info(f"No quarterly income statement data found for {symbol}")
        return
    ...
    with conn.cursor() as cur:
        cur.executemany(
            """
            INSERT INTO quarterly_income_stmt
    ...
    logger.info(f"Successfully processed {len(data_to_insert)} quarterly income statement records for {symbol}")
...
def main():
    ...
    try:
        ...
        logger.info("loadquarterlyincomestmt complete.")
