...
def load_quarterly_income_statements(symbols, cur, conn):
    total = len(symbols)
    logging.info(f"Loading quarterly income statements for {total} symbols")
    processed, failed = 0, []
    ...
    for batch_idx in range(batches):
        ...
        for yq_sym, orig_sym in mapping.items():
            ...
            if income_stmt is None or income_stmt.empty:
                logging.error(f"Skipping {orig_sym} - failed to retrieve quarterly income statement after {MAX_BATCH_RETRIES} attempts")
                continue
            ...
            try:
                ...
                if data_to_insert:
                    execute_values(cur, """
                        INSERT INTO quarterly_income_stmt (symbol, date, item_name, value)
                        VALUES %s
...
if __name__ == "__main__":
    ...
    # Recreate table
    logging.info("Recreating quarterly_income_stmt table...")
    cur.execute("DROP TABLE IF EXISTS quarterly_income_stmt CASCADE;")
    cur.execute("""
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_quarterly_income_stmt_symbol_date ON quarterly_income_stmt(symbol, date);")
    ...
    t_s, p_s, f_s = load_quarterly_income_statements(stock_syms, cur, conn)
