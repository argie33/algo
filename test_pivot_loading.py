...
and to run a quick test of the technical data loading for pivots
"""
...
def test_pivot_calculations():
    ...
    for symbol in test_symbols:
        try:
            ...
            # Get recent data (more data = better pivot detection)
            ...
            # Calculate pivots with shunt=1 (Pine Script style)
            ...
            if high_count > 0 or low_count > 0:
                ...
                logging.warning(f"⚠️  No pivots found for {symbol} - this might indicate an issue")
                  # Debug: show some raw data
                logging.info("🔍 Sample price data for debugging:")
                sample = df.tail(10)
                for i in range(len(sample)):
                    date = sample.index[i].strftime('%Y-%m-%d')
                    high = sample['High'].iloc[i]
                    low = sample['Low'].iloc[i]
                    close = sample['Close'].iloc[i]
                    logging.info(f"  {date}: H={high:.2f} L={low:.2f} C={close:.2f}")
...
def main():
    ...
    try:
        ...
        if success:
            logging.info("✅ Pivot calculation test completed successfully!")
            logging.info("💡 If you see pivot data above, the calculations are working correctly.")
            logging.info("💡 If pivots are not showing in your database, the issue is likely in:")
            logging.info("   1. Database connection")
            logging.info("   2. Data insertion process")
            logging.info("   3. Script execution/scheduling")
        else:
