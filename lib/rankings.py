...
def calculate_rankings(
    ...
    Returns:
        List of items with overall_rank and sector_rank added
    """
    if not items:
        return items

    # Calculate overall ranking by performance
    sorted_overall = sorted(
        items, key=lambda x: x.get(performance_key, 0), reverse=True
    )
    ...
    for sector, sector_items in by_sector.items():
        sorted_sector = sorted(
            sector_items, key=lambda x: x.get(performance_key, 0), reverse=True
        )
