...
def panel_performance_spark(perf, rec, perf_anl=None, pos=None):
    ...
    if unrlzd is not None:
        ...
        grid_rows.append((cast(Text, cell("Realized P&L:", pnl_s)), cast(Text, cell("Unrealized P&L:", unrlzd_s))))
    else:
        grid_rows.append((cast(Text, cell("Realized P&L:", pnl_s)), Text("")))

    # Ratios row
    grid_rows.append(
        (
            cast(Text, cell("Profit Factor:", f"[{pf_c}]{pf_s}[/]")),
            cast(Text, cell("Sharpe Ratio:", f"[white]{sharpe_s}[/]")),
        )
    )
    ...
    grid_rows.append(
        (
            cast(Text, cell("Expectancy (R):", f"[{exp_c}]{exp_s}[/]")),
            cast(Text, cell("Avg Win / Loss:", f"[{G}]{avg_win_s}[/][dim]/[/][{R}]{avg_loss_s}[/]")),
        )
    )
    ...
    if perf_anl and not perf_anl.get("_error"):
        ...
        if calmar is not None and calmar != 0.0:
            ...
            calmar_cell = cast(Text, cell("Calmar Ratio:", f"[{cc}]{calmar:.2f}[/]"))
        if wr50 is not None and wr50 != 0.0 and (total_trades >= 10 or wr50 > 0):
            wc = G if wr50 >= 50 else (Y if wr50 >= 42 else R)
            wr50_cell = cast(Text, cell("Win Rate (50 trades):", f"[{wc}]{wr50:.0f}%[/]"))
