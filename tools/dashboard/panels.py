...
@register_panel(
    ...
    optional=True,
    description="Exposure Score Expanded",
)
...
def panel_algo_health_expanded(
    ...
    rows: list = [
        Text.from_markup(
            "[dim]press [/][bold yellow]h[/][dim] to return to dashboard[/]"
        ),
        Rule(style="dim"),
    ]
    ...
    if valid_hist:
        ...
        rows.append(
            Text.from_markup(
                f"[dim]Run history ({len(valid_hist)} runs):[/]  [{wc}]{n_ok}/{len(valid_hist)} success[/]"
            )
        )
        for r in valid_hist:
            s = (r.get("overall_status") or "").lower()
