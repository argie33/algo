...
logging.info("✅ Using Pure NumPy/Pandas Implementation - Latest Weekly Technical Indicators Loader!")
...
def identify_symbols_needing_updates(cur, lookback_weeks=4):
    """
    Identify symbols that have new weekly price data and need technical indicators updated.
    Uses the same smart approach as the latest price loader - looks for deltas.
    """
    logging.info("🔍 Identifying symbols with recent weekly price updates that need technical indicators...")
    ...
    logging.info(f"🎯 Found {len(symbols_to_update)} symbols needing weekly technical indicators updates")
    return symbols_to_update

def process_symbol_incremental(symbol_info, db_config, lookback_periods=100):
    """
    Process weekly technical indicators for a single symbol incrementally.
    ...
    try:
        ...
        # For weekly data, use fewer periods but still enough for proper calculation
        start_date = latest_price_date - timedelta(weeks=lookback_periods)
        
        # Load weekly price data with sufficient lookback for technical indicators
        cur.execute("""
        ...
        if not price_rows:
            logging.warning(f"⚠️  {symbol}: No weekly price data found for date range {start_date} to {latest_price_date}")
            cur.close()
            conn.close()
            return False
        
        logging.info(f"📈 {symbol}: Loaded {len(price_rows)} weekly price records for technical analysis")
        ...
        if df_tech.empty:
            logging.warning(f"❌ {symbol}: Failed to calculate weekly technicals - empty result")
            cur.close()
            conn.close()
            return False
        
        logging.info(f"⚡ {symbol}: Calculated weekly technical indicators in {tech_time:.2f}s")
        ...
        if df_new.empty:
            logging.info(f"✅ {symbol}: No new weekly technical data to process - already up to date")
            cur.close()
            conn.close()
            return True
        
        logging.info(f"📊 {symbol}: {operation} - Processing {len(df_new)} new weekly technical indicator records")
        
        # Delete existing data for the date range we're updating (like price loader)
        if operation == "UPDATE":
            cur.execute("""
                DELETE FROM technical_data_weekly 
                WHERE symbol = %s 
                AND date >= %s
        ...
        logging.error(f"❌ {symbol}: Error during weekly incremental processing - {str(e)}")
        if 'cur' in locals():
            cur.close()
        if 'conn' in locals():
            conn.close()
...
def load_latest_technicals_optimized(symbols_to_update):
    """Optimized incremental weekly technical indicators loader with batch processing and parallel execution"""
    total = len(symbols_to_update)
    logging.info(f"🚀 Starting incremental weekly technical indicators calculation for {total} symbols")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        logging.info(f"📋 Submitted {len(chunk_futures)} weekly symbol processing tasks")
    ...
    logging.info(f"🎯 Incremental weekly technical indicators update complete!")
...
def main():
    ...
    try:
        ...
        # Identify symbols that need weekly technical indicators updates
        symbols_to_update = identify_symbols_needing_updates(cur, lookback_weeks=4)
        
        if not symbols_to_update:
            logging.info("✅ No symbols need weekly technical indicators updates - all up to date!")
            ...
            logging.info("🏁 Latest weekly technical indicators loader completed - no updates needed")
            return exit_code

        logging.info(f"📊 Found {len(symbols_to_update)} symbols needing weekly technical indicators updates")

        # Process weekly technical indicators incrementally
        ...
        try:
            ...
            logging.info("Reconnecting to database after weekly technical indicators processing...")
            cur.close()
            conn.close()
            conn = psycopg2.connect(
                host=cfg["host"], port=cfg["port"],
        ...
        logging.info("✅ Latest weekly technical indicators processing completed successfully")
