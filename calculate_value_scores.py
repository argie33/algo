...
class ValueScoreCalculator:
    ...
    def calculate_traditional_multiples_score(self, financial_data: Dict, market_data: Dict) -> Tuple[float, Dict]:
        """
    ...
    def calculate_intrinsic_value_score(self, financial_data: Dict, market_data: Dict) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            dcf_score, dcf_components = self._calculate_dcf_value(financial_data, current_price)
            components.update(dcf_components)
            
            # 2. Dividend Discount Model (25% of intrinsic value score)
            ddm_score, ddm_components = self._calculate_ddm_value(financial_data, current_price)
            components.update(ddm_components)
            
            # 3. Residual Income Model (25% of intrinsic value score)
            rim_score, rim_components = self._calculate_residual_income_value(financial_data, current_price)
    ...
    def _calculate_dcf_value(self, financial_data: Dict, current_price: float) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            # Growth assumptions
            high_growth_rate = min(0.25, max(0.02, revenue_growth or 0.05))  # Cap at 25%, min 2%
            ...
            return 50.0, {'dcf_score': 50.0, 'dcf_error': 'Insufficient data for DCF calculation'}
            
        except Exception as e:
            logger.error(f"Error in DCF calculation: {str(e)}")
            return 50.0, {'dcf_error': str(e)}
    
    def _calculate_ddm_value(self, financial_data: Dict, current_price: float) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            return 50.0, {'ddm_score': 50.0, 'ddm_note': 'No dividend or insufficient data'}
            
        except Exception as e:
            logger.error(f"Error in DDM calculation: {str(e)}")
            return 50.0, {'ddm_error': str(e)}
    
    def _calculate_residual_income_value(self, financial_data: Dict, current_price: float) -> Tuple[float, Dict]:
        """
    ...
    def calculate_relative_value_score(self, financial_data: Dict, symbol: str) -> Tuple[float, Dict]:
        """
        ...
        try:
            ...
            historical_score = self._calculate_historical_valuation_score(financial_data, symbol)
...
def main():
    ...
    try:
        ...
        logger.info(f"Value score calculation completed. Success: {successful_calculations}, Failed: {failed_calculations}")
