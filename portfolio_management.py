...
class Portfolio:
        ...
        try:
            ...
            if remaining_shares > 0:
                self.positions[symbol] = Position(
                    ...
                    weight=0.0,
                    unrealized_pnl=(self._get_current_price(symbol) - position.avg_cost) * remaining_shares,
    ...
    def get_portfolio_metrics(self) -> PortfolioMetrics:
        ...
        try:
            ...
            if len(benchmark_returns) == len(returns) and len(returns) > 1:
                ...
                information_ratio = (mean_return - np.mean(benchmark_returns)) / np.std(returns - benchmark_returns) * np.sqrt(252) if np.std(returns - benchmark_returns) > 0 else 0
            else:
    ...
    def performance_attribution(self, benchmark_weights: Dict[str, float],
        ...
        try:
            ...
            current_weights = {pos.symbol: pos.weight for pos in self.positions.values()}
            ...
            for symbol in set(list(current_weights.keys()) + list(benchmark_weights.keys())):
            ...
            portfolio_return = sum(current_weights.get(s, 0) * portfolio_returns.get(s, 0) 
                                 for s in current_weights.keys())
            benchmark_return = sum(benchmark_weights.get(s, 0) * benchmark_returns.get(s, 0)
    ...
    def get_risk_metrics(self) -> Dict:
        ...
        try:
            ...
            if portfolio_vol > 0 and individual_vol:
                diversification_ratio = sum(w * vol for w, vol in zip(position_weights, individual_vol)) / portfolio_vol
            else:
        ...
        try:
            ...
            for symbol, target_weight in target_weights.items():
                ...
                if abs(weight_diff) > 0.01:  # Only rebalance if difference > 1%
                    ...
                    current_value_symbol = current_weights.get(symbol, 0.0) * current_value
    ...
    def _update_weights(self):
        ...
        for position in self.positions.values():
            ...
            position.unrealized_pnl = (position.current_price - position.avg_cost) * position.shares
    ...
    def _update_performance_data(self):
        ...
        try:
            ...
            if len(spy_data) >= 2:
                benchmark_return = (spy_data['Close'].iloc[-1] - spy_data['Close'].iloc[-2]) / spy_data['Close'].iloc[-2]
    ...
    def _calculate_beta_alpha(self, returns: np.array, benchmark_returns: np.array, risk_free_rate: float) -> Tuple[float, float]:
    ...
    def _get_period_returns(self, start_date: datetime, end_date: datetime) -> Dict[str, float]:
    ...
    def _get_benchmark_returns(self, start_date: datetime, end_date: datetime) -> Dict[str, float]:
        ...
        for symbol in self.positions.keys():
            ...
            returns[symbol] = 0.08 / 252 * (end_date - start_date).days  # 8% annual return assumption
...
def main():
    ...
    for symbol, position in portfolio.positions.items():
        ...
        print(f"    Current: ${position.current_price:.2f}, Weight: {position.weight:.1%}")
