...
echo ""
echo "📊 STEP 5: Calculate Stock Scores (using real data from all metrics: quality, risk, value, growth)"
python3 loadstockscores.py 2>&1 &
SCORES_PID=$!

echo ""
echo "⏳ Waiting for all loaders to complete..."
wait $QUALITY_PID $RISK_PID $VALUE_PID $GROWTH_PID $SCORES_PID 2>/dev/null || true
