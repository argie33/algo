#!/usr/bin/env python3  
"""
Latest Monthly Buy/Sell Signal Generator (Incremental)
...
BUYSELL_TABLE = "buy_sell_monthly"
...
logging.info("🚀 Latest Monthly Buy/Sell Signal Generator - Incremental Processing!")
...
# -------------------------------
def identify_symbols_needing_updates(cur, lookback_months=6):
    """
    Identify symbols that have new monthly technical indicators data and need buy/sell signals updated.
    Similar to the technical indicators approach - looks for symbols with technical data newer than buy/sell data.
    """
    logging.info("🔍 Identifying symbols with recent monthly technical indicators updates that need buy/sell signals...")
    ...
    logging.info(f"🎯 Found {len(symbols_to_update)} symbols needing monthly buy/sell signal updates")
    return symbols_to_update

# -------------------------------
# Data fetching for incremental processing
# -------------------------------
def fetch_symbol_data_incremental(symbol_info, db_config, lookback_periods=60):
    """
    Fetch monthly price and technical data for a single symbol with sufficient lookback for signal generation.
    ...
    try:
        ...
        # Determine the date range - need enough lookback for signal generation (fewer periods for monthly)
        start_date = latest_tech_date - timedelta(days=lookback_periods*30)  # Approximate months to days
        
        # Load monthly price and technical data with sufficient lookback
        cur.execute(f"""
            SELECT
        ...
        if not data_rows:
            logging.warning(f"⚠️  {symbol}: No monthly data found for date range {start_date} to {latest_tech_date}")
            cur.close()
            conn.close()
            return None
        
        logging.info(f"📈 {symbol}: Loaded {len(data_rows)} monthly records for buy/sell signal analysis")
        ...
        logging.error(f"❌ {symbol}: Error fetching monthly data - {str(e)}")
        if 'cur' in locals():
            cur.close()
        if 'conn' in locals():
            conn.close()
...
# -------------------------------
# Ultra-fast signal generation (same as daily/weekly but for monthly data)
# -------------------------------
def generate_signals_vectorized(df, atr_mult=1.0, use_adx=True, adx_threshold=25):
    """Ultra-fast monthly signal generation using pure NumPy vectorization"""
    if df.empty:
        return df
...
def process_symbol_incremental(symbol_info, db_config):
    """
    Process monthly buy/sell signals for a single symbol incrementally.
    ...
    try:
        ...
        if df is None or df.empty:
            logging.warning(f"❌ {symbol}: No monthly data available for signal generation")
            return False
        ...
        if df_signals.empty:
            logging.warning(f"❌ {symbol}: Failed to generate monthly signals - empty result")
            return False
        
        logging.info(f"⚡ {symbol}: Generated monthly buy/sell signals in {signal_time:.2f}s")
        ...
        if df_new.empty:
            logging.info(f"✅ {symbol}: No new monthly buy/sell signals to process - already up to date")
            return True
        
        logging.info(f"📊 {symbol}: {operation} - Processing {len(df_new)} new monthly buy/sell signal records")
        ...
        try:
            ...
            if insert_data:
                ...
                logging.info(f"🚀 {symbol}: Inserted {len(insert_data)} monthly records in {bulk_insert_time:.2f}s ({records_per_sec:.0f} records/sec)")
            else:
                logging.info(f"📊 {symbol}: No actionable monthly signals to insert (all 'None' signals filtered out)")
            ...
            logging.error(f"❌ {symbol}: Database error during monthly incremental processing - {str(e)}")
            cur.close()
            conn.close()
            return False
        
    except Exception as e:
        logging.error(f"❌ {symbol}: Error during monthly incremental signal processing - {str(e)}")
        return False
...
def load_latest_buysell_optimized(symbols_to_update):
    """Optimized incremental monthly buy/sell signal loader with parallel execution"""
    total = len(symbols_to_update)
    logging.info(f"🚀 Starting incremental monthly buy/sell signal generation for {total} symbols")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        logging.info(f"📋 Submitted {len(symbol_futures)} monthly buy/sell signal processing tasks")
    ...
    logging.info(f"🎯 Incremental monthly buy/sell signal update complete!")
...
def main():
    ...
    try:
        ...
        # Identify symbols that need monthly buy/sell signal updates
        symbols_to_update = identify_symbols_needing_updates(cur, lookback_months=6)
        
        if not symbols_to_update:
            logging.info("✅ No symbols need monthly buy/sell signal updates - all up to date!")
            ...
            logging.info("🏁 Latest monthly buy/sell signal loader completed - no updates needed")
            return exit_code

        logging.info(f"📊 Found {len(symbols_to_update)} symbols needing monthly buy/sell signal updates")

        # Process monthly buy/sell signals incrementally
        ...
        try:
            ...
            logging.info("Reconnecting to database after monthly buy/sell signal processing...")
            cur.close()
            conn.close()
            conn = psycopg2.connect(
                host=cfg["host"], port=cfg["port"],
        ...
        logging.info("✅ Latest monthly buy/sell signal processing completed successfully")
