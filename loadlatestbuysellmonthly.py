...
def main():
    ...
    try:
        ...
        logging.info(f"Processing {len(symbols)} symbols for latest monthly buy/sell signals")
        ...
        for symbol in symbols:
            try:
                ...
                if processed_count % 10 == 0:
                    conn.commit()
                    logging.info(f"Monthly progress: {processed_count}/{len(symbols)} symbols processed")
        ...
        if deleted_count > 0:
            logging.info(f"Cleaned up {deleted_count} old monthly signals before {cleanup_date}")
