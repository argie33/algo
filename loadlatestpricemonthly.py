...
def ensure_table(conn):
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS latest_price_monthly (
...
def fetch_and_store(symbol, conn):
    ...
    try:
        df = ticker.history(period="1mo", interval="1mo")
        if not df.empty:
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO latest_price_monthly (symbol, price, currency, price_date, fetched_at)
                    VALUES (%s, %s, %s, %s, NOW())
