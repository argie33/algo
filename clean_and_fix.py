...
def fix_file(filepath: str) -> int:
    ...
    for line in lines:
        # First, completely remove old type: ignore comments
        line = re.sub(r'\s*#\s*type:\s*ignore\[no-any-return\]\s*\)', ')', line)
        
        # Look for return statements that need wrapping
        if line.strip().startswith('return ') and 'cast(' not in line:
            match = re.match(r'^(\s*)return\s+(.+)$', line)
            if match:
                indent, return_val = match.groups()
                # Wrap if returning from specific functions
    ...
    # Ensure cast is imported if we made fixes
    if fixes > 0:
        if not has_cast:
            if typing_import_line >= 0:
                # Add to existing import
                line = new_lines[typing_import_line]
