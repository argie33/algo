...
def _timeout_handler_unix(signum, frame):
    ...
    raise TimeoutError(f"Execution timeout: exceeded maximum allowed time")
...
def ExecutionTimeout(max_seconds: int = 5400, label: str = "loader"):
    ...
    try:
        ...
        # Windows / Other: Use threading-based timeout
            ...
            def _timeout_func():
                logger.critical(
                    ...
                    f"Aborting execution to prevent RDS connection leaks."
                )
        ...
        logger.error(
            ...
            f"This indicates either slow API responses or resource contention. "
            f"Raising TimeoutError to halt loader gracefully."
        )
