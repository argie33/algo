                ...
                if self.conn:
                    if not self._externally_managed:
                        ...
                        if exc_type is None and self.role == "write":
                            ...
                            self.conn.commit()
                            logger.debug("[DB_CONTEXT] __exit__: WRITE COMMIT COMPLETE")
                        else:
                            logger.debug(f"[DB_CONTEXT] __exit__: ROLLBACK (role={self.role}, exc_type={exc_type})")
                            self.conn.rollback()
                        self.conn.close()
                    else:
                        # Still commit/rollback, but don't close the connection
                        if exc_type is None and self.role == "write":
                            logger.debug(f"[DB_CONTEXT] __exit__: COMMIT (externally-managed, role={self.role})")
                            self.conn.commit()
                            logger.debug("[DB_CONTEXT] __exit__: COMMIT COMPLETE (externally-managed)")
                        else:
                            logger.debug(f"[DB_CONTEXT] __exit__: ROLLBACK (externally-managed, exc_type={exc_type})")
                            self.conn.rollback()
