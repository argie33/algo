"""Single-segment-shaped revenue extraction fallbacks (Ares-style, single-segment) for
XBRLSegmentParser, extracted from utils/external/sec_xbrl_segments.py (2026-09-05). Sibling
module sec_xbrl_segment_revenue.py holds the discover-then-reconcile-against-a-consolidated-
anchor fallbacks (cross-tab, component-sum, alt-asset-manager); this one holds the fallbacks
for filers that only ever report a single segment. Bodies are verbatim, no logic changed -
only converted from staticmethods to plain module functions and moved file. Called by
XBRLSegmentParser.extract_segment_revenue_from_xbrl_xml, which tries each fallback in sequence.
