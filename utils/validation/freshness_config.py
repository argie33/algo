...
FRESHNESS_RULES = {
    ...
    "buy_sell_daily": {
        "critical": False,
        "max_age_days": 1,
        "description": "Buy/sell signals generated by Phase 5 (technical analysis, pattern recognition)",
        "purpose": "Signal generation for entry/exit, trade execution",
    ...
    "technical_data_daily": {
        ...
        "applies_to": ["orchestrator_phase1", "orchestrator_phase5", "dashboard"],
        "note": "Legacy table - no longer part of critical EOD pipeline, relaxed 7-day threshold",
    },
    # === IMPORTANT TABLES (Warning if stale, may still use) ===
    "signal_quality_scores": {
        "critical": True,
        "max_age_days": 1,
        "description": "Signal quality scores (Sharpe, win_rate, correlation) for universe",
        "purpose": "Signal quality filtering, risk-adjusted signal ranking, portfolio construction",
        "applies_to": ["orchestrator_phase5", "dashboard"],
    },
    "trend_template_data": {
        "critical": True,
        "max_age_days": 1,
        "description": "Trend template stage (1-4) and base patterns for each symbol",
    ...
    "economic_data": {
        "critical": False,
        "max_age_days": 7,
        "description": "Economic data (interest rates, inflation, GDP)",
        "purpose": "Macro factor analysis, market regime assessment",
        "applies_to": ["orchestrator_phase1", "dashboard", "data_patrol"],
    },
    "sector_ranking": {
        "critical": False,
        "max_age_days": 7,
        "description": "Sector rankings and rotation signals",
...
def is_critical_table(table_name: str) -> bool:
    ...
    if not isinstance(table_name, str):
        raise TypeError(f"table_name must be string, got {type(table_name).__name__}")

    rule = get_freshness_rule(table_name)
    if not rule:
        # No rule found - return False rather than raising to maintain backward compatibility
        logger.debug(f"[DATA_FRESHNESS] No freshness rule found for {table_name}")
        return False
