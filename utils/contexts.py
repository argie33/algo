...
class CircuitBreaker:
    ...
    Tracks consecutive failures and stops accepting requests after threshold.
    ...
    def is_open(self) -> bool:
        ...
        # Circuit is tripped, check if recovery timeout expired
        if self.last_failure_time:
            elapsed = time.time() - self.last_failure_time
            if elapsed < self.recovery_timeout_sec:
                return True
            # Recovery timeout expired, close circuit
            self.failures = 0
            self.last_failure_time = None
    ...
    def guard(self, operation: str = "operation"):
        ...
        Usage:
            breaker = CircuitBreaker('my_api', failure_threshold=3)
            with breaker.guard('fetch_data'):
                data = expensive_fetch()
        """
