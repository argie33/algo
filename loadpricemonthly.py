#!/usr/bin/env python3
# TRIGGER: 20260501_084523 - Phase 2: Monthly price data
...
def get_db_config():
    ...
    if db_secret_arn and aws_region:
        try:
            ...
            logging.warning(f"AWS Secrets Manager failed: {str(e)[:100]}. Falling back to environment variables.")
...
def load_prices(table_name, symbols, cur, conn):
            ...
            for yq_sym, orig_sym in mapping.items():
                ...
                for idx, row in sub.iterrows():
                    try:
                        ...
                        # Skip records with zero or missing volume - these represent non-trading months or delisted stocks
    ...
    if timeout_failures:
        ...
        for orig_sym in timeout_failures:
            ...
            for sym_attempt in range(1, MAX_SYMBOL_RETRIES + 1):
                try:
                    single_df = yf.download(orig_sym, period="3mo", interval="1mo", auto_adjust=False, actions=True, progress=False, timeout=60)
                    if not single_df.empty:
...
def load_prices_smart(table_name, symbols, cur, conn):
    ...
    For monthly data: typically just 1 API call per group after initial load.
    """
    ...
    def _download_and_insert(batch_syms, start_date=None, period=None, pause=1.0):
        ...
        for attempt in range(1, 4):
            try:
                kwargs = dict(tickers=yq_batch, interval="1mo", auto_adjust=False,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Load monthly price data for stocks')
    parser.add_argument('--smart', action='store_true',
                       help='Smart incremental: only fetch missing data per symbol. FAST.')
    args = parser.parse_args()

    if args.smart:
        logging.info("Running in SMART mode - incremental per-symbol fetch")
    else:
        logging.info("Running in DEFAULT mode - loading recent data")
    ...
    try:
        ...
        if args.smart:
            t_s, i_s, f_s = load_prices_smart("price_monthly", stock_syms, cur, conn)
        else:
