...
def load_income_statements(symbols, cur, conn):
    total = len(symbols)
    logging.info(f"Loading income statements for {total} symbols")
    processed, failed = 0, []
    ...
    for batch_idx in range(batches):
        ...
        for yq_sym, orig_sym in mapping.items():
            ...
            for attempt in range(1, MAX_BATCH_RETRIES+1):
                try:
                    ...
                    if income_stmt is None or income_stmt.empty:
                        raise ValueError("No income statement data received")
                    break
            ...
            if income_stmt is None or income_stmt.empty:
                logging.error(f"Skipping {orig_sym} - failed to retrieve income statement after {MAX_BATCH_RETRIES} attempts")
                continue
            ...
            try:
                ...
                if data_to_insert:
                    execute_values(cur, """
                        INSERT INTO income_stmt (symbol, date, item_name, value)
                        VALUES %s
...
if __name__ == "__main__":
    ...
    # Recreate table
    logging.info("Recreating income_stmt table...")
    cur.execute("DROP TABLE IF EXISTS income_stmt CASCADE;")
    cur.execute("""
    ...
    t_s, p_s, f_s = load_income_statements(stock_syms, cur, conn)
