...
def main():
    ...
    all_records = (nas + oth)[:10]  # Only keep first 10 records

    # parse ETF symbols
    nas_etf = parse_nasdaq_etf(nas_text)
    oth_etf = parse_other_etf(oth_text)
    all_etf_records = (nas_etf + oth_etf)[:10]  # Only keep first 10 records

    logger.info("Test stock symbols: %s", [r['symbol'] for r in all_records])
    logger.info("Test ETF symbols: %s", [r['symbol'] for r in all_etf_records])
