...
logging.info("✅ Using Pure NumPy/Pandas Implementation - Latest Monthly Technical Indicators Loader!")
...
def identify_symbols_needing_updates(cur, lookback_months=6):
    """
    Identify symbols that have new monthly price data and need technical indicators updated.
    Uses the same smart approach as the latest price loader - looks for deltas.
    """
    logging.info("🔍 Identifying symbols with recent monthly price updates that need technical indicators...")
    ...
    logging.info(f"🎯 Found {len(symbols_to_update)} symbols needing monthly technical indicators updates")
    return symbols_to_update

def process_symbol_incremental(symbol_info, db_config, lookback_periods=60):
    """
    Process monthly technical indicators for a single symbol incrementally.
    ...
    try:
        ...
        # For monthly data, use fewer periods but still enough for proper calculation
        start_date = latest_price_date - timedelta(days=lookback_periods*30)  # Approximate months to days
        
        # Load monthly price data with sufficient lookback for technical indicators
        cur.execute("""
        ...
        if not price_rows:
            logging.warning(f"⚠️  {symbol}: No monthly price data found for date range {start_date} to {latest_price_date}")
            cur.close()
            conn.close()
            return False
        
        logging.info(f"📈 {symbol}: Loaded {len(price_rows)} monthly price records for technical analysis")
        ...
        if df_tech.empty:
            logging.warning(f"❌ {symbol}: Failed to calculate monthly technicals - empty result")
            cur.close()
            conn.close()
            return False
        
        logging.info(f"⚡ {symbol}: Calculated monthly technical indicators in {tech_time:.2f}s")
        ...
        if df_new.empty:
            logging.info(f"✅ {symbol}: No new monthly technical data to process - already up to date")
            cur.close()
            conn.close()
            return True
        
        logging.info(f"📊 {symbol}: {operation} - Processing {len(df_new)} new monthly technical indicator records")
        ...
        logging.error(f"❌ {symbol}: Error during monthly incremental processing - {str(e)}")
        if 'cur' in locals():
            cur.close()
        if 'conn' in locals():
            conn.close()
...
def load_latest_technicals_optimized(symbols_to_update):
    """Optimized incremental monthly technical indicators loader with batch processing and parallel execution"""
    total = len(symbols_to_update)
    logging.info(f"🚀 Starting incremental monthly technical indicators calculation for {total} symbols")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        logging.info(f"📋 Submitted {len(chunk_futures)} monthly symbol processing tasks")
    ...
    logging.info(f"🎯 Incremental monthly technical indicators update complete!")
...
def main():
    ...
    try:
        ...
        # Identify symbols that need monthly technical indicators updates
        symbols_to_update = identify_symbols_needing_updates(cur, lookback_months=6)
        
        if not symbols_to_update:
            logging.info("✅ No symbols need monthly technical indicators updates - all up to date!")
            ...
            logging.info("🏁 Latest monthly technical indicators loader completed - no updates needed")
            return exit_code

        logging.info(f"📊 Found {len(symbols_to_update)} symbols needing monthly technical indicators updates")

        # Process monthly technical indicators incrementally
        ...
        try:
            ...
            logging.info("Reconnecting to database after monthly technical indicators processing...")
            cur.close()
            conn.close()
            conn = psycopg2.connect(
                host=cfg["host"], port=cfg["port"],
        ...
        logging.info("✅ Latest monthly technical indicators processing completed successfully")
