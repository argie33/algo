...
class FinancialMetricsCalculator:
        ...
        try:
            ...
            if not self.balance_sheet.empty and len(self.balance_sheet.columns) > 0:
                ...
                total_assets = safe_float(
                    self.balance_sheet.loc["Total Assets", latest_col]
                    if "Total Assets" in self.balance_sheet.index
                    else None
                )
                total_equity = safe_float(
                    self.balance_sheet.loc["Total Stockholder Equity", latest_col]
                    if "Total Stockholder Equity" in self.balance_sheet.index
                    else None
                )

                if total_assets and total_equity:
                    metrics["equity_multiplier"] = safe_divide(
            ...
            logging.warning(
                f"Error calculating profitability metrics for {self.symbol}: {e}"
            )
    ...
    def calculate_roic(self) -> Optional[float]:
        ...
        try:
            ...
            operating_income = safe_float(
                self.financials.loc["Operating Income", latest_fin]
                if "Operating Income" in self.financials.index
                else None
            )
            income_before_tax = safe_float(
                self.financials.loc["Income Before Tax", latest_fin]
                if "Income Before Tax" in self.financials.index
                else None
            )
            tax_provision = safe_float(
                self.financials.loc["Tax Provision", latest_fin]
                if "Tax Provision" in self.financials.index
                else None
            )

            if operating_income and income_before_tax and tax_provision:
                tax_rate = safe_divide(
                    tax_provision, income_before_tax, 0.25
                )  # Default 25% if calculation fails
            ...
            total_assets = safe_float(
                self.balance_sheet.loc["Total Assets", latest_bs]
                if "Total Assets" in self.balance_sheet.index
                else None
            )
            cash = safe_float(
                self.balance_sheet.loc["Cash And Cash Equivalents", latest_bs]
    ...
    def calculate_balance_sheet_strength(self) -> Dict:
        ...
        try:
            ...
            if not self.financials.empty and len(self.financials.columns) > 0:
                latest_col = self.financials.columns[0]
                ebit = safe_float(
                    self.financials.loc["EBIT", latest_col]
                    if "EBIT" in self.financials.index
                    else None
                )
                interest_expense = safe_float(
                    self.financials.loc["Interest Expense", latest_col]
                    if "Interest Expense" in self.financials.index
                    else None
                )

                if ebit and interest_expense and interest_expense != 0:
                    metrics["interest_coverage"] = safe_divide(
            ...
            logging.warning(
                f"Error calculating balance sheet metrics for {self.symbol}: {e}"
            )
    ...
    def calculate_piotroski_score(self) -> Dict:
        ...
        try:
            ...
            if not self.cashflow.empty and len(self.cashflow.columns) > 0:
                cfo_current = safe_float(
                    self.cashflow.loc["Operating Cash Flow", current_year]
    ...
    def calculate_altman_zscore(self) -> Dict:
        ...
        try:
            ...
            current_assets = safe_float(
                self.balance_sheet.loc["Current Assets", latest_bs]
                if "Current Assets" in self.balance_sheet.index
                else None
            )
            current_liabilities = safe_float(
                self.balance_sheet.loc["Current Liabilities", latest_bs]
                if "Current Liabilities" in self.balance_sheet.index
                else None
            )
            total_liabilities = safe_float(
                self.balance_sheet.loc["Total Liab", latest_bs]
                if "Total Liab" in self.balance_sheet.index
                else None
            )
            retained_earnings = safe_float(
                self.balance_sheet.loc["Retained Earnings", latest_bs]
                if "Retained Earnings" in self.balance_sheet.index
                else None
            )

            # Get income statement items
            ebit = safe_float(
                self.financials.loc["EBIT", latest_fin]
                if "EBIT" in self.financials.index
                else None
            )
            total_revenue = safe_float(
                self.financials.loc["Total Revenue", latest_fin]
                if "Total Revenue" in self.financials.index
                else None
            )
            ...
            if total_assets:
                ...
                if current_assets and current_liabilities:
                    ...
                    components["working_capital_to_assets"] = safe_divide(
                        working_capital, total_assets
                    )

                # Retained Earnings / Total Assets
                if retained_earnings:
                    components["retained_earnings_to_assets"] = safe_divide(
                ...
                if market_cap and total_liabilities:
                    components["market_value_equity_to_liabilities"] = safe_divide(
                        market_cap, total_liabilities
                    )

                # Sales / Total Assets
                if total_revenue:
                    components["sales_to_assets"] = safe_divide(
    ...
    def calculate_valuation_metrics(self) -> Dict:
        ...
        try:
            ...
            metrics["price_to_sales"] = safe_float(
                self.info.get("priceToSalesTrailing12Months")
            )
            ...
            if current_price:
                ...
                if tangible_book:
                    metrics["price_to_tangible_book"] = safe_divide(
                        current_price, tangible_book
                    )

        except Exception as e:
            logging.warning(
                f"Error calculating valuation metrics for {self.symbol}: {e}"
            )
    ...
    def calculate_growth_metrics(self) -> Dict:
        ...
        try:
            ...
            metrics["earnings_quarterly_growth"] = safe_float(
                self.info.get("earningsQuarterlyGrowth")
            )
            ...
            if not self.financials.empty and len(self.financials.columns) >= 2:
                current_revenue = safe_float(
                    self.financials.loc["Total Revenue", self.financials.columns[0]]
                    if "Total Revenue" in self.financials.index
                    else None
                )
                prior_revenue = safe_float(
                    self.financials.loc["Total Revenue", self.financials.columns[1]]
                    if "Total Revenue" in self.financials.index
                    else None
                )
                ...
                current_income = safe_float(
                    self.financials.loc["Net Income", self.financials.columns[0]]
                    if "Net Income" in self.financials.index
                    else None
                )
                prior_income = safe_float(
                    self.financials.loc["Net Income", self.financials.columns[1]]
                    if "Net Income" in self.financials.index
                    else None
                )
...
def get_financial_data(symbol: str, max_retries: int = 3) -> Optional[Dict]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.warning(
                f"Error fetching financial data for {symbol} on attempt {attempt + 1}: {e}"
            )
            if attempt < max_retries - 1:
                time.sleep(2)
            else:
...
def load_financial_metrics_batch(
    symbols: List[str], conn, cur, batch_size: int = 10
) -> Tuple[int, int]:
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        logging.info(
            f"Processing financial metrics batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
        ...
        if financial_data:
            try:
                ...
                for item in financial_data:
                    ...
                    profitability_data.append(
                        (
                            ...
                            prof.get("return_on_assets"),
                            prof.get("return_on_invested_capital"),
                            prof.get("net_profit_margin"),
                            prof.get("operating_margin"),
                            prof.get("gross_margins"),
                            prof.get("ebitda_margin"),
                            prof.get("asset_turnover"),
                            prof.get("equity_multiplier"),
                        )
                    )
                    ...
                    balance_sheet_data.append(
                        (
                            symbol,
                            calc_date,
                            bs.get("current_ratio"),
                            bs.get("quick_ratio"),
                            bs.get("debt_to_equity"),
                            bs.get("interest_coverage"),
                            bs.get("roa_positive"),
                            bs.get("cfo_positive"),
                            bs.get("roa_improvement"),
                            bs.get("accruals_quality"),
                            bs.get("leverage_decrease"),
                            bs.get("current_ratio_improvement"),
                            bs.get("shares_outstanding_decrease"),
                            bs.get("gross_margin_improvement"),
                            bs.get("asset_turnover_improvement"),
                            bs.get("piotroski_f_score"),
                            bs.get("working_capital_to_assets"),
                            bs.get("retained_earnings_to_assets"),
                            bs.get("ebit_to_assets"),
                            bs.get("market_value_equity_to_liabilities"),
                            bs.get("sales_to_assets"),
                            bs.get("altman_z_score"),
                        )
                    )
                    ...
                    valuation_data.append(
                        (
                            ...
                            val.get("forward_pe"),
                            val.get("peg_ratio"),
                            val.get("price_to_book"),
                            val.get("price_to_sales"),
                            val.get("price_to_tangible_book"),
                            val.get("market_to_book"),
                            val.get("enterprise_value"),
                            val.get("ev_to_revenue"),
                            val.get("ev_to_ebitda"),
                            val.get("current_price"),
                        )
                    )
                    ...
                    growth_data.append(
                        (
                            ...
                            growth.get("earnings_quarterly_growth"),
                            growth.get("sustainable_growth_rate"),
                        )
                    )
                ...
                logging.info(
                    f"Financial batch {batch_num} inserted {len(financial_data)} symbols successfully"
                )

            except Exception as e:
                logging.error(
                    f"Database insert error for financial batch {batch_num}: {e}"
                )
                conn.rollback()
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process financial data for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
...
if __name__ == "__main__":
    ...
    cur.execute(
        "SELECT symbol FROM stock_symbols_enhanced WHERE is_active = TRUE ORDER BY market_cap DESC NULLS LAST LIMIT 200"
    )
    symbols = [row["symbol"] for row in cur.fetchall()]

    if not symbols:
        logging.warning(
            "No symbols found in stock_symbols_enhanced table. Run loadsymbols.py first."
        )
        sys.exit(1)
    ...
    for row in cur.fetchall():
        logging.info(
            f"  {row['symbol']} ({row['company_name'][:30]}): ROE={row['return_on_equity']:.1%}, "
