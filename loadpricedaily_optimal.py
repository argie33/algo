#!/usr/bin/env python3
"""
Reference implementation: optimal price_daily loader.

Demonstrates the new pattern. Compare to the existing loadpricedaily.py
(~400 lines) — this is ~40 lines and does strictly more:
...
Run:
    python3 loadpricedaily_optimal.py [--symbols AAPL,MSFT] [--parallelism 4]
"""
...
class PriceDailyLoader(OptimalLoader):
    ...
    def fetch_incremental(self, symbol: str, since: Optional[date]):
        ...
        - If we have a watermark, fetch from (watermark + 1 day) to today.
        - If no watermark (first run), fetch last 5 years.
        """
        end = date.today()
        if since is None:
    ...
    def _validate_row(self, row: dict) -> bool:
        ...
        if not super()._validate_row(row):
            return False
        # Reject rows where high < low or any price is non-positive
        try:
            return (
                row["high"] >= row["low"]
                and row["close"] > 0
