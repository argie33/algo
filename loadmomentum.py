...
class MomentumCalculator:
    ...
    def get_price_momentum_data(self, period: str = "2y") -> Optional[Dict]:
        ...
        try:
            ...
            result = {
                "symbol": self.symbol,
                "date": (
                    latest_date.date() if hasattr(latest_date, "date") else date.today()
                ),
        ...
        try:
            ...
            if len(hist) >= 252:
                ...
                if len(returns_11m) > 20:
                    ...
                    momentum_sharpe = (
                        jt_momentum / momentum_vol if momentum_vol > 0 else 0
                    )
            ...
            if len(hist) >= 252:
                ...
                if len(returns_12m) > 20:
                    ...
                    risk_adj_momentum = (
                        mean_return / vol_return if vol_return > 0 else 0
                    )
                    ...
                    if len(negative_returns) > 5:
                        ...
                        sortino_momentum = (
                            mean_return / downside_vol if downside_vol > 0 else 0
                        )
                        result["sortino_momentum"] = safe_float(sortino_momentum)

        except Exception as e:
            logging.error(
                f"Error in Jegadeesh-Titman calculation for {self.symbol}: {e}"
            )
    ...
    def calculate_momentum_metrics(self, hist: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(hist) >= 21:
                ...
                result["momentum_1w"] = safe_float(
                    (current_price - price_1w) / price_1w
                )
                result["momentum_1m"] = safe_float(
            ...
            if len(hist) >= 63:
                ...
                for i in range(21, len(hist), 21):  # Every ~month
                    if i < len(hist):
                        ret = (
                            hist["Close"].iloc[i] - hist["Close"].iloc[i - 21]
                        ) / hist["Close"].iloc[i - 21]
                ...
                if len(monthly_returns) >= 3:
                    ...
                    if monthly_returns.std() > 0:
                        momentum_consistency = -abs(
                            monthly_returns.std() / monthly_returns.mean()
                        )
                        result["momentum_consistency"] = safe_float(
    ...
    def calculate_volume_momentum(self, hist: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(hist) >= 63:
                ...
                older_volume = (
                    volume.iloc[-126:-63] if len(hist) >= 126 else recent_volume
                )
                ...
                if len(recent_returns) > 20:
                    ...
                    recent_volume_clean = recent_volume.iloc[
                        -len(recent_returns_clean) :
                    ]

                    if len(recent_returns_clean) > 20 and len(
                        recent_volume_clean
                    ) == len(recent_returns_clean):
                        vol_price_corr = np.corrcoef(
                            recent_returns_clean, recent_volume_clean
                        )[0, 1]
                        if not np.isnan(vol_price_corr):
                            result["volume_price_correlation"] = safe_float(
                                vol_price_corr
                            )
    ...
    def calculate_momentum_quality(self, hist: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(returns) >= 252:
                ...
                if len(returns_12m) >= 63:
                    ...
                    if len(rolling_monthly) >= 3:
                        ...
                        momentum_smoothness = (
                            1 / (1 + monthly_vol) if monthly_vol >= 0 else 0
                        )
            ...
            if len(hist) >= 126:
                ...
                mom_1m = (
                    (prices.iloc[-21] - prices.iloc[-42]) / prices.iloc[-42]
                    if len(hist) >= 42
                    else 0
                )
                mom_2m = (
                    (prices.iloc[-42] - prices.iloc[-63]) / prices.iloc[-63]
                    if len(hist) >= 63
                    else 0
                )
                mom_3m = (
                    (prices.iloc[-63] - prices.iloc[-84]) / prices.iloc[-84]
                    if len(hist) >= 84
                    else 0
                )
    ...
    def get_fundamental_momentum(self) -> Optional[Dict]:
        ...
        result = {"symbol": self.symbol, "date": date.today()}
                ...
                if len(recent_earnings) >= 4:
                    ...
                    if not any(pd.isna(earnings_values)):
                        ...
                        if qoq_growth:
                            ...
                            if len(qoq_growth) >= 2:
                                recent_growth = np.mean(
                                    qoq_growth[-2:]
                                )  # Last 2 quarters
                                older_growth = np.mean(
                                    qoq_growth[:-2]
                                )  # Earlier quarters

                                earnings_acceleration = recent_growth - older_growth
                                result["earnings_acceleration"] = safe_float(
                                    earnings_acceleration
                                )

        except Exception as e:
            logging.error(
                f"Error calculating fundamental momentum for {self.symbol}: {e}"
            )
...
def load_momentum_batch(
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        logging.info(
            f"Processing momentum batch {batch_num}/{total_batches}: {len(batch)} symbols"
        )
        ...
        if momentum_data:
            try:
                ...
                for item in momentum_data:
                    insert_data.append(
                        (
                            ...
                            item.get("jt_momentum_12_1"),
                            item.get("jt_momentum_sharpe"),
                            item.get("jt_momentum_volatility"),
                            # Alternative horizons
                            item.get("momentum_6_1"),
                            item.get("momentum_9_1"),
                            item.get("momentum_3_1"),
                            item.get("momentum_12_3"),
                            # Risk-adjusted
                            item.get("risk_adjusted_momentum"),
                            item.get("sortino_momentum"),
                            # Short to medium term
                            item.get("momentum_1w"),
                            item.get("momentum_1m"),
                            item.get("momentum_3m"),
                            item.get("momentum_6m"),
                            # Quality metrics
                            item.get("momentum_persistence"),
                            item.get("momentum_consistency"),
                            item.get("momentum_max_drawdown"),
                            item.get("momentum_recovery_factor"),
                            item.get("momentum_skewness"),
                            item.get("momentum_kurtosis"),
                            item.get("momentum_strength"),
                            item.get("momentum_smoothness"),
                            item.get("momentum_acceleration"),
                            # Volume-based
                            item.get("volume_weighted_momentum"),
                            item.get("obv_momentum"),
                            item.get("volume_trend"),
                            item.get("volume_price_correlation"),
                            # Fundamental
                            item.get("expected_eps_growth"),
                            item.get("recommendation_momentum"),
                            item.get("price_target_momentum"),
                            item.get("price_target_dispersion"),
                            item.get("earnings_momentum_qoq"),
                            item.get("earnings_acceleration"),
                        )
                    )
                ...
                logging.info(
                    f"Momentum batch {batch_num} inserted {len(momentum_data)} symbols successfully"
                )

            except Exception as e:
                logging.error(
                    f"Database insert error for momentum batch {batch_num}: {e}"
                )
                conn.rollback()
    ...
    if failed_symbols:
        logging.warning(
            f"Failed to process momentum for {len(failed_symbols)} symbols: {failed_symbols[:5]}..."
        )
