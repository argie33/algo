#!/usr/bin/env python3
# Updated: 2025-10-04 02:05 - Annual balance sheet loader
import gc
import json
import logging
import os
...
def get_balance_sheet_data(symbol: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if balance_sheet is None or balance_sheet.empty:
            logging.warning(
                f"No balance sheet data returned by any method for {symbol}"
            )
            return None
        ...
        logging.info(
            f"Retrieved balance sheet data for {symbol}: {len(balance_sheet.columns)} periods, {len(balance_sheet.index)} line items"
        )
