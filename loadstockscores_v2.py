...
def get_db_connection(script_name="loader"):
    ...
    if DB_HOST and DB_HOST != 'localhost':
        try:
            ...
            logger.info(f"Connected via environment variables to {DB_HOST}")
            return conn
        except Exception as e:
            logger.warning(f"Env var connection failed: {e}, trying Secrets Manager...")
    ...
    try:
        if DB_SECRET_ARN:
            ...
            logger.info("Connected via AWS Secrets Manager")
            return conn
    except Exception as e:
        logger.warning(f"AWS Secrets failed: {e}")
