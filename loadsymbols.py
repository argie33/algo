...
# Stock symbol lists
MAJOR_INDICES = [
    'SPY', 'QQQ', 'DIA', 'VTI', 'VXUS', 'BND', 'VNQ', 'GDX', 'TLT', 'EEM'
]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'firstTradeDateEpochUtc' in info and info['firstTradeDateEpochUtc']:
                try:
                    result['listing_date'] = datetime.fromtimestamp(info['firstTradeDateEpochUtc']).date()
                except:
                    pass
...
def load_symbols_batch(symbols: List[str], conn, cur, batch_size: int = 20) -> Tuple[int, int]:
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        if symbol_data:
            try:
                ...
                logging.info(f"Batch {batch_num} inserted {len(symbol_data)} symbols successfully")
    ...
    if failed_symbols:
        logging.warning(f"Failed to process {len(failed_symbols)} symbols: {failed_symbols[:10]}...")
...
if __name__ == "__main__":
    ...
    all_symbols = list(set(MAJOR_INDICES + SP500_SYMBOLS + POPULAR_ETFS + CRYPTO_SYMBOLS))
    ...
    logging.info(f"Average time per symbol: {(end_time - start_time) / processed:.2f} seconds")
    log_mem("completion")
    ...
    for row in cur.fetchall():
        logging.info(f"  {row['sector']}: {row['count']} stocks, "
