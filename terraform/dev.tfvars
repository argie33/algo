...
api_lambda_provisioned_concurrency  = 5     # ENABLED: Pre-warmed instances eliminate VPC cold-start (15-40s) that was exceeding API Gateway 29s timeout. Fixes 503 errors. Cost ~$5/month (acceptable for dev).
...
algo_lambda_ephemeral_storage       = 2048  # RESTORED: Orchestrator needs space for intermediate data processing during all 9 phases
algo_lambda_provisioned_concurrency = 2     # ENABLED: Pre-warm 2 instances for morning + evening orchestrator runs (prevents occasional cold-start delays during scheduled execution)
algo_lambda_reserved_concurrency    = 3     # RIGHT-SIZED: Handles up to 9 EventBridge targets (premarket/morning/afternoon/preclose/evening/weight-opt/3 prewarms) with buffer for manual triggers
