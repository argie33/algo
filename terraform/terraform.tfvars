...
api_lambda_provisioned_concurrency  = 1     # CRITICAL FIX: Enable provisioned concurrency to eliminate VPC cold-start timeouts (15-40s exceeds API Gateway 29s limit). Reserved concurrency alone is insufficient; Lambda cold starts cause 503 errors on every new instance. Cost: ~$12/month, but eliminates dashb board data loading failures. Session 60: Enabled to fix Lambda 503 errors.
...
algo_lambda_reserved_concurrency    = 10    # FIXED: Must be >= provisioned concurrency. Set to 10 to match existing provisioned (9) in AWS. Once terraform apply succeeds and removes provisioned config, can reduce to 5. Temporary unblock for deployment.
