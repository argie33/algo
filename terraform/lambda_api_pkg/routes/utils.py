...
def json_response(code, data, data_freshness=None):
    ...
    Ensures all error responses include _error field for consistent client-side error detection.
    """
    if code == 200:
...
def safe_serialize_row(row):
    """Convert database row to JSON-serializable dict in one step.

    Combines safe_dict_convert() and safe_json_serialize() to handle both
    row conversion and type serialization consistently.

    Args:
        row: Database row (tuple, dict, or RealDictCursor)

    Returns:
        JSON-serializable dictionary with Decimal→float, datetime→ISO, UUID→string
    """
    return safe_json_serialize(safe_dict_convert(row))
