...
async function testDatabaseInit() {
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      console.log(
        `   - No Plaintext Logging: ${result.security.noPlaintextLogging}`
      );
      return true;
    } else {
...
async function testPriceDataInit() {
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      console.log(
        `   - Sample data inserted: ${result.details.sampleDataInserted} rows`
      );
...
function testApiKeyStorage() {
  ...
  console.log(
    `   - Test data: ${JSON.stringify({ ...mockApiKeyData, apiKey: "[ENCRYPTED]", apiSecret: "[ENCRYPTED]" })}`
  );
...
function testPortfolioImport() {
  ...
  console.log(
    `   - Import result: ${mockImportResult.holdingsCount} holdings, $${mockImportResult.totalValue.toLocaleString()} total value`
  );
