...
def _residualize_on_log_market_cap(values: dict[str, float], log_mktcap: dict[str, float]) -> dict[str, float]:
    """Regress `values` on log(market_cap) within this single group via simple OLS and return
    the residual (val - predicted) - the Barra/Axioma/MSCI-style size-neutralization transform
    (regress each pillar on sector dummies + log market cap, score the RESIDUAL), with the
    sector dummy half of that handled by the caller's own peer-group split (this function only
    does the log-mktcap half, one group at a time).

    Needs >=5 points with both a value AND a log_mktcap entry to fit a trustworthy slope - same
    "too few peers to trust" bar `_winsorize_group` already uses. Symbols missing a market_cap,
    or groups too small/degenerate (zero variance in log_mktcap) to regress, pass through
    unchanged (no size adjustment applied) rather than being dropped.
    """
    paired = [(s, v, log_mktcap[s]) for s, v in values.items() if s in log_mktcap]
    if len(paired) < 5:
        return dict(values)
    xs = [p[2] for p in paired]
    ...
    residualized = dict(values)
    for symbol, val, x in paired:
        residualized[symbol] = val - (intercept + slope * x)
    return residualized


def sector_size_neutral_zscore(
    values: dict[str, float],
    sectors: dict[str, str],
    ...
    """Barra-style neutralization: within each of `sector_neutral_zscore`'s own peer groups
    (per-sector / FPI pool / residual pool - identical split, same `min_sector_size` bar),
    regress the raw value on log(market_cap) and z-score the RESIDUAL instead of the raw value.

    This is the size half of the standard Barra/Axioma/MSCI construction (regress each pillar
    on sector dummies + log market cap, score the residual) - `sector_neutral_zscore`'s
    per-sector grouping already provides the sector-dummy half. Unlike a hard market-cap floor,
    nothing is excluded here: a small-cap symbol is still scored, just relative to its peers'
    OWN size-explained baseline rather than compared raw against megacaps in the same sector -
    see `_residualize_on_log_market_cap` for the regression itself.

    See `market_caps` here as log-scale-internal - callers pass raw market_cap (dollars); this
    function takes the log itself so callers don't each need to remember to.
    """
    import math

    log_mktcap = {s: math.log(mc) for s, mc in market_caps.items() if mc and mc > 0}
    ...
    for symbols in groups.values():
        if len(symbols) < min_sector_size:
            ...
            continue
        sector_values = {symbol: values[symbol] for symbol in symbols}
        sector_resid = _residualize_on_log_market_cap(sector_values, log_mktcap)
        result.update(_zscore_group(_winsorize_group(sector_resid)))

    if fpi_pool:
        fpi_resid = _residualize_on_log_market_cap(fpi_pool, log_mktcap)
        result.update(_zscore_group(_winsorize_group(fpi_resid)))
    if residual:
        residual_resid = _residualize_on_log_market_cap(residual, log_mktcap)
        result.update(_zscore_group(_winsorize_group(residual_resid)))
    return result
