...
class SecEdgarStatementLoader(SecLoaderBase):
    ...
    # capex_never_tagged_in_recent_filings/missing_cash_flow_data continuation): mirrors
    # load_sec_valuations.py's SecValuationsLoader.LARGE_BATCH_CAPEX_EXEMPT_SYMBOLS (same
    # duplication convention as _INSURANCE_CAPEX_EXEMPT_SYMBOLS/_FINANCIAL_CAPEX_EXEMPT_SYMBOLS
    # above) - see that class attribute's own docstring for the full verification methodology
    # (programmatic live companyfacts scan across the full capex-concept family, foreign
    # private issuers/funds/ETFs/SPACs/physical-property equity REITs deliberately excluded).
    _LARGE_BATCH_CAPEX_EXEMPT_SYMBOLS = frozenset(
        {
            "ACR",
            "ACT",
            "ACTU",
        ...
        for r in rows:
            ...
            if self.statement_type == "cashflow":
                ...
                if capex is None and (
                    ...
                    or r.get("symbol") in self._LARGE_BATCH_CAPEX_EXEMPT_SYMBOLS
                ):
                    capex = 0
