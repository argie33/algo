...
class Q4DerivationSweepMixin:
    ...
    def _sweep_correct_cumulative_ytd_field(self, field: str, sign_scoped: bool = True) -> None:
        ...
        DataPatrol-flagged negative-Q4-stock_based_compensation finding). Live-confirmed
        against RITM's real cached SEC companyfacts JSON (CIK 0001556593):
        `AllocatedShareBasedCompensationExpense` is filed exclusively as year-to-date
        cumulative facts (every instance's `start` is the fiscal year's January 1st, e.g.
        FY2018 Q2 start=2018-01-01 end=2018-06-30 val=1,019,000, Q3 start=2018-01-01
        end=2018-09-30 val=1,019,000 - IDENTICAL to Q2, meaning zero real incremental Q3
        spend, not a data error) - a genuine cumulative-only reporter, not a sign-flip or
        extraction-layer bug. The extraction layer correctly stores whatever fact it finds,
        but stores the raw cumulative value as if it were the discrete quarter - so Q1 (whose
        cumulative-to-date always equals its own discrete value) is fine, but Q2/Q3 are
        inflated, and the existing Q4 = FY - (Q1+Q2+Q3) sweep then double/triple-subtracts
        the cumulative overlap, producing a deeply negative Q4 "plug" (RITM FY2018:
        1,020,000 - (420,000+1,019,000+1,019,000) = -1,438,000, exactly matching the
        corrupted value live in the DB before this fix).

        SECOND FIELD VERIFIED 2026-09-13: common_stock_repurchased shows the identical shape,
        live-confirmed against Agilent's (symbol A, CIK 0001090872) real cached SEC
        companyfacts JSON for `PaymentsForRepurchaseOfCommonStock`, FY2017 (fiscal year ends
        Oct 31): Q1 start=2016-11-01 end=2017-01-31 val=111,000,000 (discrete, matches its own
        YTD); Q2 start=2016-11-01 end=2017-04-30 val=194,000,000 (YTD-cumulative, start = FY
        start not Q2 start); Q3 start=2016-11-01 end=2017-07-31 val=194,000,000 - IDENTICAL to
        Q2 (no incremental Q3 repurchases); FY start=2016-11-01 end=2017-10-31 val=194,000,000
        (also no Q4 repurchases). The pre-existing Q4 sweep would compute
        194,000,000 - (111,000,000+194,000,000+194,000,000) = -305,000,000, the same
        deeply-negative-plug corruption as RITM's stock_based_compensation.

        Detection is the exact-duplicate fingerprint verified against both symbols' real
        source data: Q2 == Q3 (bit-for-bit) AND Q1 < Q2 AND Q2 > 0. This is a conservative,
        low-false-positive signal - a coincidental exact-dollar match between two
        genuinely-discrete quarters is implausible at real magnitudes for either field, and
        requiring Q1 strictly less than Q2 rules out an all-flat-zero coincidence. Once
        matched, the true discrete values telescope cleanly: true_Q2 = stored_Q2 - stored_Q1,
        true_Q3 = stored_Q3 - stored_Q2 (always 0 in the exact-duplicate case, consistent with
        "no incremental Q3 activity"). Q1 is left unchanged. Marked with a distinct
        `data_source` so this correction never re-fires on its own already-corrected output
        (a corrected Q2/Q3 pair essentially never independently satisfies the same cumulative
        fingerprint again).

        THIRD FIELD VERIFIED 2026-09-13 (algo-fa, cross-session collaboration): capex shows the
        same shape on two independent real filers. AAT (CIK 0001500217, FY2019,
        PaymentsToAcquireRealEstate): Q1 start=2019-01-01 end=2019-03-31 val=20,932,000
        (discrete, matches stored Q1); Q2 start=2019-01-01 end=2019-06-30 val=507,780,000
        (cumulative); Q3 start=2019-01-01 end=2019-09-30 val=507,780,000 - identical to Q2 (one
        real-estate acquisition in Q2, none in Q3). ABAT (CIK 0001576873, non-calendar FY
        July-June, PaymentsToAcquireMiningAssets): Q2 start=2022-07-01 end=2022-12-31
