...
class MarketStatusDailyLoader(OptimalLoader):
    ...
    def _fetch_market_health(self, eval_date: date) -> dict[str, Any]:
        ...
        try:
            ...
            advance_decline = breadth_data.get("advance_decline_ratio") if breadth_data else None
            new_highs = breadth_data.get("new_52wk_highs") if breadth_data else None
            new_lows = breadth_data.get("new_52wk_lows") if breadth_data else None

            # Fetch yield curve (10Y-2Y spread)
            yield_data = self._yield_curve_fetcher.fetch(eval_date - timedelta(days=1), eval_date)
            yield_spread = yield_data.get("yield_10y_2y_spread") if yield_data else None
