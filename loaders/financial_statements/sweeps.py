"""Q4-derivation/copy sweeps mixed into ConsolidatedFinancialStatementsLoader.

Split out of load_financial_statements.py (see that module's top docstring): these are the
10 `_sweep_*` methods post_run() calls, table-wide recompute/derivation passes that run
independent of what a given run's SEC fetch actually returned (Q4 is never separately
filed by any US GAAP domestic filer - see each method's own docstring for the specific
accounting identity/copy it relies on).
"""
...
class _Q4SweepMixin:
    """Mixin providing the Q4-derivation/copy post_run() sweeps.

    Assumes the concrete class also inherits from SecEdgarStatementLoader (self.table_name,
    self.statement_type, module `logger` above are all it relies on) - the two class-level
    annotations below exist purely so mypy can check this file in isolation; the real values
    come from SecEdgarStatementLoader once mixed into ConsolidatedFinancialStatementsLoader.
    """
    ...
    def _sweep_derive_missing_q4_cash_flow(self) -> None:
        ...
        """
        # Local import (not module-level): several existing unit tests patch
        # "loaders.load_financial_statements.DatabaseContext" and expect it to intercept
        # every sweep this run's post_run() call reaches - re-resolving through the
        # facade module at call time (rather than a top-of-file import into this
        # module's own namespace) keeps that patch target working unchanged.
