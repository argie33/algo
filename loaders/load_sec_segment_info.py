...
class SecSegmentInfoLoader(SecLoaderBase):
    ...
    def _unavailable_marker(self, symbol: str, reason: str) -> dict[str, Any] | None:
        ...
        BUG FOUND 2026-08-24 (goal session logic-soundness audit), REVERTED THEN RE-FIXED
        SAME DAY by a concurrent session's mistaken "simplification" commit (b32839ab2,
        which claimed to preserve this guard but actually deleted it from every call site
        except _handle_symbol_not_found's own pre-existing 2026-08-21 check - restored
        here): the 2026-08-21 fix above guards against a data_unavailable marker's
        fiscal_year=today().year permanently outranking real historical segment data in a
        naive ORDER BY fiscal_year DESC read - but that guard was only ever wired into the
        symbol_not_found call site. Four other call sites in fetch_incremental()
        (no_segment_data, no_segments_found, filing_date_unavailable, extraction_error:*)
        constructed this same marker directly, unguarded - so a symbol with real,
        multi-year segment data already on record could still get a fresh current-year
        marker written on any transient failure (an SEC API hiccup, a temporary XBRL parse
        error), which then outranks the real data in any naive
        `ORDER BY fiscal_year DESC LIMIT 1` read. The guard lives here, in the one place
        every marker gets built, so every caller is protected uniformly instead of relying
        on each one remembering to check - and so a future "simplification" pass can't
        silently drop it from four of five call sites while believing it preserved the
        behavior.
        """
