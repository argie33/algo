...
class EtfPriceAggregateLoader(OptimalLoader):
    ...
    def fetch_incremental(self, symbol: str, since: Optional[date]):
        ...
        if start >= end:
            return None
        # FIX: Read from etf_price_daily (RDS) instead of re-fetching from API
        rows = self._fetch_etf_price_daily(symbol, start, end)
        return self._aggregate(rows) if rows else None

    def _fetch_etf_price_daily(self, symbol: str, start: date, end: date) -> Optional[List[dict]]:
        """Read daily ETF prices from RDS (not API) to avoid double-fetching."""
        conn = self._connect()
        cur = conn.cursor()
        try:
            cur.execute(
                "SELECT date, open, high, low, close, volume FROM etf_price_daily "
            ...
            logger.error(f"Error reading etf_price_daily for {symbol}: {e}")
            return None
        finally:
            cur.close()
