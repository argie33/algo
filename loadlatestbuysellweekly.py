    ...
    for signal in signals:
        signal_rows.append(
            (
                ...
                signal["macd"],
                signal["volume"],
                signal["volume_avg_4w"],
                signal["price_vs_ma10w"],
                signal["price_vs_ma26w"],
...
def main():
    ...
    try:
        ...
        logging.info(
            f"Processing {len(symbols)} symbols for latest weekly buy/sell signals"
        )
        ...
        for symbol in symbols:
            try:
                ...
                if processed_count % 10 == 0:
                    conn.commit()
                    logging.info(
                        f"Weekly progress: {processed_count}/{len(symbols)} symbols processed"
                    )
        ...
        if deleted_count > 0:
            logging.info(
                f"Cleaned up {deleted_count} old weekly signals before {cleanup_date}"
            )
