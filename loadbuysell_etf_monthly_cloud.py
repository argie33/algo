#!/usr/bin/env python3
"""
Cloud-Native ETF Buy/Sell Monthly Signals Loader
...
def load_etf_signals(symbol: str) -> List[tuple]:
    """Load monthly buy/sell signals for ETF"""
    try:
        ...
        hist['volume_avg'] = hist['Volume'].rolling(3).mean()
        hist['volume_surge'] = (hist['Volume'] / hist['volume_avg']).fillna(1)
        hist['price_change'] = hist['Close'].pct_change()
        hist['ma_12'] = hist['Close'].rolling(12).mean()

        rows = []
        for date, row in hist.iterrows():
            signal = None
            if row['volume_surge'] > 1.3 and row['price_change'] > 0.03:
                signal = 'Buy'
            elif row['volume_surge'] > 1.3 and row['price_change'] < -0.03:
                signal = 'Sell'
            ...
            if signal:
                rows.append((
                    ...
                    signal,
                    None, None, None, False, 100, 'monthly',
        ...
        logging.debug(f"[OK] {symbol}: {len(rows)} monthly signal rows")
        return rows
...
def main():
    logging.info("Starting loadbuysell_etf_monthly_cloud (PARALLEL + S3 BULK)")
    ...
    logging.info(f"Loading monthly ETF signals for {total_symbols} ETFs...")
    ...
    if all_rows:
        try:
            ...
            inserted = bulk_inserter.insert_bulk("buy_sell_etf_monthly", columns, all_rows, rds_role)
