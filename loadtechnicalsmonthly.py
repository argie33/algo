...
def validate_prerequisites(cur):
    ...
    try:
        logging.info("🔍 Step 1: Checking if price_monthly table exists...")
        
        # Check if price_monthly table exists and has data
        cur.execute("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
                AND table_name = 'price_monthly'
            ) as table_exists
        """)
        result = cur.fetchone()
        price_table_exists = result['table_exists']
        logging.info(f"📊 price_monthly table exists: {price_table_exists}")
        
        if not price_table_exists:
            logging.error("❌ price_monthly table does not exist. Technical data requires price data.")
            logging.error("💡 Hint: Run the price data loader first (pricemonthly-loader) to populate price_monthly table")
            return False
        
        logging.info("🔍 Step 2: Checking if price_monthly table has data...")
        
        # Check total number of rows first
        cur.execute("SELECT COUNT(*) as total_rows FROM price_monthly")
        result = cur.fetchone()
        total_rows = result['total_rows']
        logging.info(f"📊 Total rows in price_monthly: {total_rows}")
        
        if total_rows == 0:
            logging.error("❌ price_monthly table exists but is empty (0 rows)")
            logging.error("💡 Hint: Run the price data loader first (pricemonthly-loader) to populate price_monthly table")
            return False
        
        # Check if we have price data for symbols
        logging.info("🔍 Step 3: Counting distinct symbols in price_monthly...")
        cur.execute("SELECT COUNT(DISTINCT symbol) as symbol_count FROM price_monthly")
        result = cur.fetchone()
        price_symbol_count = result['symbol_count'] if result else 0
        logging.info(f"📊 Distinct symbols in price_monthly: {price_symbol_count}")
        
        if price_symbol_count == 0:
            logging.error("❌ No distinct symbols found in price_monthly table")
            logging.error("💡 This is unusual - table has rows but no distinct symbols")
            return False
        
        logging.info(f"✅ Prerequisites met: price_monthly table exists with {price_symbol_count} symbols")
        return True
...
def delete_existing_table(cur):
    """Delete existing technical_data_monthly table and confirm deletion"""
    try:
        logging.info("🗑️ Step 1: Checking if technical_data_monthly table exists...")
        ...
        if table_exists:
            logging.info("📊 technical_data_monthly table exists, checking row count...")
            
            # Count existing rows
            cur.execute("SELECT COUNT(*) as row_count FROM technical_data_monthly")
            result = cur.fetchone()
            existing_rows = result['row_count']
            logging.info(f"📊 Existing table has {existing_rows} rows")
            
            # Drop the table
            logging.info("🗑️ Dropping existing technical_data_monthly table...")
            cur.execute("DROP TABLE IF EXISTS technical_data_monthly CASCADE")
            logging.info("✅ Successfully dropped technical_data_monthly table")
            ...
            if still_exists:
                ...
                logging.info("✅ Confirmed: technical_data_monthly table has been deleted")
                return True
        else:
            logging.info("📋 technical_data_monthly table does not exist (first run)")
            return True
...
def create_technical_table(cur):
    """Create technical_data_monthly table with comprehensive schema"""
    try:
        logging.info("🔧 Creating technical_data_monthly table...")
        ...
        logging.info("✅ Created technical_data_monthly table")
        
        # Create indexes for performance
        indexes = [
            "CREATE INDEX IF NOT EXISTS idx_technical_monthly_symbol ON technical_data_monthly(symbol)",
            "CREATE INDEX IF NOT EXISTS idx_technical_monthly_date ON technical_data_monthly(date)",
            "CREATE INDEX IF NOT EXISTS idx_technical_monthly_symbol_date ON technical_data_monthly(symbol, date)",
            "CREATE INDEX IF NOT EXISTS idx_technical_monthly_created_at ON technical_data_monthly(created_at)"
        ]
    ...
    try:
        ...
        for i, symbol in enumerate(symbol_chunk):
            ...
            try:
                ...
                logging.info(f"🔄 Processing {symbol} ({i+1}/{len(symbol_chunk)}) - monthly technicals...")
...
def main():
    ...
    try:
        ...
        logging.info("📊 ULTRA-FAST Monthly Technical Indicators Loading")
        logging.info("=" * 60)
