#!/usr/bin/env python3
"""
Technical Indicators Loader - MONTHLY
Calculates and populates technical_data_monthly table with RSI, MACD, and other technical indicators.

Data Calculated:
- RSI (14-month) - Relative Strength Index
- MACD (12/26/9) - Moving Average Convergence Divergence
- Momentum indicators (ROC, momentum)
- Trend indicators (ADX, +DI, -DI)

Data Sources:
- price_monthly table (OHLCV data)
...
def load_technical_indicators(cur, conn):
    ...
    try:
        logging.info("Loading price data for technical calculations (MONTHLY)...")
        ...
        logging.info(f"Processing {len(symbols)} symbols with monthly price data")
        ...
        for symbol in symbols:
            try:
                # Get last 120 months of price data (10 years)
                cur.execute("""
                    SELECT date, close
                    FROM price_monthly
                    WHERE symbol = %s
                ...
                roc_6m = calculate_roc(prices, period=6) if len(prices) >= 7 else None
                roc_12m = calculate_roc(prices, period=12) if len(prices) >= 13 else None
                roc_24m = calculate_roc(prices, period=24) if len(prices) >= 25 else None
                roc_60m = calculate_roc(prices, period=60) if len(prices) >= 61 else None
        ...
        cur.execute("DELETE FROM technical_data_monthly WHERE date >= CURRENT_DATE - INTERVAL '30 days'")
        ...
        if indicators_data:
            execute_values(
                ...
                INSERT INTO technical_data_monthly (
                    ...
                    mom, roc, roc_6m, roc_12m, roc_24m, roc_60m, roc_120m,
                ...
                ON CONFLICT (symbol, date) DO UPDATE SET
                    ...
                    roc_24m = EXCLUDED.roc_24m,
                    roc_60m = EXCLUDED.roc_60m
                """,
                indicators_data,
                page_size=1000
            )
...
def main():
    ...
    try:
        ...
        logging.info(f"[OK] Technical indicators (MONTHLY) loaded successfully ({count} records)")
        log_mem("finished")
