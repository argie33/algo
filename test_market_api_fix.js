        ...
        try {
          ...
          if (res.statusCode === 200) {
            ...
            if (path === "/market/overview") {
              ...
              if (hasSentiment) {
                console.log(
                  `  🎯 Sentiment keys: ${Object.keys(jsonData.data.sentiment_indicators)}`
                );
              }
            }
...
async function runTests() {
  ...
  const tests = [
    ...
    {
      path: "/market/sentiment/history?days=7",
  ...
  if (failed.length > 0) {
    ...
    failed.forEach((test) => {
      console.log(
        `  ❌ ${test.description}: ${test.error || test.status || "Unknown error"}`
      );
    });
  }
  ...
  if (overviewTest && overviewTest.success) {
    ...
    console.log(
      "The frontend should now be able to load market data correctly!"
    );
  } else {
