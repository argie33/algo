...
class DatabaseConnectivityTest {
  ...
  async testSecretsManager() {
    ...
    try {
      ...
      this.logResult(
        'AWS Secrets Manager',
        success,
        success ? 'Secret retrieved and parsed successfully' : `Failed to parse secret: ${parseError}`,
        diagnostics
      );
  ...
  async testDatabaseConfig(secret) {
    ...
    for (const field of requiredFields) {
      ...
      config[field] = value ? (field === 'password' ? '***HIDDEN***' : value) : 'MISSING';
    ...
    this.logResult(
      'Database Config',
      allFieldsPresent,
      allFieldsPresent ? 'All database configuration fields present' : 'Some database configuration fields missing',
      config
    );
  ...
  async testDatabaseConnection(secret) {
    ...
    if (!secret) {
      this.logResult('Database Connection', false, 'No database configuration available');
      return null;
    }
    ...
    const sslConfigs = [
      ...
      { ssl: { rejectUnauthorized: false }, description: 'SSL without certificate validation' },
  ...
  async testDatabaseSchema(connectionInfo) {
    ...
    if (!connectionInfo) {
      this.logResult('Database Schema', false, 'No database connection available');
      return;
    }
    ...
    try {
      ...
      this.logResult(
        'Database Schema',
        allTablesExist,
        allTablesExist ? 'All critical tables exist' : 'Some critical tables missing',
        tableResults
      );
  ...
  async testApiKeyEncryption() {
    ...
    if (!secretArn) {
      ...
      this.logResult('API Key Encryption', false, 'No encryption secret available');
      return;
    }
    ...
    try {
      ...
      const hasEncryptionKey = secret && (secret.encryptionKey || secret.API_KEY_ENCRYPTION_SECRET);
      
      this.logResult(
        'API Key Encryption',
        hasEncryptionKey,
        hasEncryptionKey ? 'API key encryption secret available' : 'API key encryption secret missing',
        { 
        ...
        switch (test.test) {
          ...
          case 'Database Connection':
            recommendations.push({
              ...
              solution: 'Check database configuration, network connectivity, and SSL settings'
            });
            break;
          ...
          case 'Database Schema':
            recommendations.push({
              ...
              solution: 'Run database initialization scripts to create required tables'
            });
            break;
        }
      }
