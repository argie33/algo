...
async function testTradingFixes() {
  ...
  async function runTest(name, testFn) {
    ...
    try {
      ...
      console.log('   Details:', error.response?.body || error.stack?.split('\n')[0]);
    }
  }
  ...
  await runTest('Trading signals handles missing database columns', async () => {
    const response = await request(app)
      .get('/api/trading/signals/daily?limit=1');
    ...
    if (response.status === 500 && response.body.error && response.body.error.includes('column')) {
  ...
  await runTest('Trading positions endpoint handles auth correctly', async () => {
    const response = await request(app)
