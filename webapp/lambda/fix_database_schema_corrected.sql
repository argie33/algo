-- Corrected database schema fixes for all missing columns
...
-- Fix 4: Add missing transaction_id column to portfolio_transactions table (check if it exists first)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Fix 6: Add sample data to existing annual_balance_sheet table (using fiscal_year instead of date)
...
-- Fix 7: Add sample data to existing annual_income_statement table (using fiscal_year instead of date)
...
-- Fix 12: Add sample portfolio risk data
