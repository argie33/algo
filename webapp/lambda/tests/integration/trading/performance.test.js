...
describe("Performance Analytics API", () => {
  describe("Portfolio Performance", () => {
    test("should retrieve portfolio performance overview", async () => {
      ...
      if (response.status === 200) {
        ...
        const performanceFields = ["total_return", "annualized_return", "sharpe_ratio", "max_drawdown"];
    ...
    test("should analyze performance over time periods", async () => {
      const response = await request(app)
        .get("/api/performance/returns?period=1Y");
  ...
  describe("Benchmark Comparison", () => {
    test("should compare portfolio against benchmarks", async () => {
      const response = await request(app)
        .get("/api/performance/benchmark?indices=SPY,QQQ,VTI&period=1Y");
      ...
      if (response.status === 200) {
        ...
        if (comparison && Object.keys(comparison).length > 0) {
          const benchmarkFields = ["portfolio_return", "benchmark_return", "alpha", "beta"];
          const hasBenchmarkData = benchmarkFields.some(field => 
            Object.keys(comparison).some(key => key.toLowerCase().includes(field.replace("_", "")))
          );
  ...
  describe("Risk-Adjusted Metrics", () => {
    test("should calculate risk-adjusted performance metrics", async () => {
      const response = await request(app)
        .get("/api/performance/risk?period=1Y");
      ...
      if (response.status === 200) {
        ...
        const riskMetrics = ["sharpe_ratio", "sortino_ratio", "treynor_ratio", "information_ratio"];
        const hasRiskMetrics = riskMetrics.some(field => 
          Object.keys(metrics).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
  ...
  describe("Attribution Analysis", () => {
    test("should perform sector attribution analysis", async () => {
      const response = await request(app)
        .get("/api/performance/attribution?type=sector&period=6M");
      ...
      if (response.status === 200) {
        ...
        if (response.body.data.length > 0) {
          ...
          const hasAttributionData = attributionFields.some(field => 
            Object.keys(sectorAttribution).some(key => key.toLowerCase().includes(field))
          );
    ...
    test("should perform stock-level attribution analysis", async () => {
      const response = await request(app)
        .get("/api/performance/attribution?type=stocks&period=3M&top=10");
