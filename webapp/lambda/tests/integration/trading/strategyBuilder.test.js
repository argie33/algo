  ...
  describe("Strategy Creation", () => {
    test("should create a new trading strategy", async () => {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        expect(response.body.data).toHaveProperty("name", "Test Momentum Strategy");
      }
    });
  ...
  describe("Strategy Testing", () => {
    test("should backtest strategy", async () => {
      ...
      if (response.status === 200) {
        ...
        const backtestFields = ["total_return", "sharpe_ratio", "max_drawdown", "trades_count"];
        const hasBacktestData = backtestFields.some(field => 
          Object.keys(results).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
    ...
    test("should validate strategy against current market", async () => {
      ...
      if (response.status === 200) {
        ...
        const hasValidationData = validationFields.some(field => 
          Object.keys(validation).some(key => key.toLowerCase().includes(field))
        );
