...
describe("Technical Analysis", () => {
  describe("Technical Indicators", () => {
    test("should calculate RSI indicator", async () => {
      const response = await request(app)
        .get("/api/technical/daily/AAPL?indicators=rsi&period=14");
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 1) {
          ...
          const rsiKey = Object.keys(data).find(key => key.toLowerCase().includes("rsi"));
    ...
    test("should calculate MACD indicator", async () => {
      const response = await request(app)
        .get("/api/technical/daily/AAPL?indicators=macd");
    ...
    test("should calculate moving averages", async () => {
      const response = await request(app)
        .get("/api/technical/daily/AAPL?indicators=sma,ema&periods=20,50");
    ...
    test("should calculate Bollinger Bands", async () => {
      const response = await request(app)
        .get("/api/technical/daily/AAPL?indicators=bollinger&period=20&std=2");
    ...
    test("should support multiple indicators in single request", async () => {
      const response = await request(app)
        .get("/api/technical/daily/AAPL?indicators=rsi,macd,sma,bollinger");
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 1) {
          ...
          const hasAnyIndicator = allIndicators.some(indicator => 
            Object.keys(data).some(key => key.toLowerCase().includes(indicator))
          );
  ...
  describe("Trading Signals", () => {
    test("should generate trading signals based on technical analysis", async () => {
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 1) {
          ...
          const signalFields = ["signal", "strength", "recommendation", "confidence"];
          ...
          if (data.signal) {
            expect(["buy", "sell", "hold", "strong_buy", "strong_sell"]).toContain(data.signal.toLowerCase());
          }
        }
      }
    });
    ...
    test("should provide signal strength and confidence levels", async () => {
      const response = await request(app)
        .get("/api/technical/signals/MSFT?include_confidence=true");
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 1) {
          ...
          const confidenceFields = ["strength", "confidence", "probability", "score"];
    ...
    test("should identify price targets and stop losses", async () => {
      const response = await request(app)
        .get("/api/technical/signals/GOOGL?include_targets=true");
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 1) {
          ...
          const targetFields = ["price_target", "stop_loss", "support", "resistance"];
          const hasTargetData = targetFields.some(field => 
            Object.keys(data).some(key => key.toLowerCase().includes(field.replace("_", "")))
          );
  ...
  describe("Chart Data", () => {
    test("should provide OHLCV chart data", async () => {
      const response = await request(app)
        .get("/api/technical/chart/AAPL?period=1M&interval=1d");
    ...
    test("should support different chart intervals", async () => {
      ...
      for (const interval of intervals.slice(0, 3)) { // Test first 3 to avoid rate limits
        const response = await request(app)
          .get(`/api/technical/chart/SPY?period=1D&interval=${interval}`);
    ...
    test("should include volume data in charts", async () => {
      const response = await request(app)
        .get("/api/technical/chart/AAPL?period=1M&include_volume=true");
    ...
    test("should identify chart patterns", async () => {
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 0) {
          ...
          const patternFields = ["patterns", "bullish", "bearish", "formations"];
  ...
  describe("Multi-Timeframe Analysis", () => {
    test("should provide analysis across multiple timeframes", async () => {
      const response = await request(app)
        .get("/api/technical/multi-timeframe/AAPL?timeframes=1D,1W,1M");
    ...
    test("should aggregate signals across timeframes", async () => {
      const response = await request(app)
        .get("/api/technical/consensus/AAPL");
      ...
      if (response.status === 200) {
        ...
        if (data && Object.keys(data).length > 0) {
          ...
          const consensusFields = ["consensus", "overall", "aggregate", "summary"];
