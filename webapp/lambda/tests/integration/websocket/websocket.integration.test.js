...
describe("WebSocket Integration", () => {
  ...
  describe("WebSocket Subscription Management", () => {
    ...
    test("should handle subscription limits", async () => {
      ...
      const rejectedSubscriptions = subscriptions.filter((s) =>
        [400].includes(s.status)
      );

      if (rejectedSubscriptions.length > 0) {
        // Some should be rejected due to limits
        expect(successfulSubscriptions.length).toBeLessThanOrEqual(
          maxSubscriptions
        );
      }
    });
      ...
      const securityTests = [
        {
          message: {
            type: "subscribe",
            payload: { symbols: ["<script>alert('xss')</script>"] },
          },
  ...
  describe("WebSocket Performance and Scalability", () => {
    test("should handle concurrent WebSocket requests", async () => {
      ...
      expect(successfulConnections.length).toBeGreaterThan(
        concurrentConnections * 0.7
      );
  ...
  describe("WebSocket Integration with Authentication", () => {
        ...
        },
        {
          ...
          request: () =>
            request(app)
              ...
              .send({
                type: "authenticate",
                payload: { token: "dev-bypass-token" },
              }),
        },
        {
    ...
    test("should handle WebSocket session management", async () => {
      const sessionSteps = [
        {
          action: "create",
          endpoint: "/api/websocket/session",
          method: "post",
          data: {},
        },
        {
          action: "validate",
          endpoint: "/api/websocket/session/validate",
          method: "post",
          data: {},
        },
        {
          action: "refresh",
          endpoint: "/api/websocket/session/refresh",
          method: "post",
          data: {},
        },
      ];
