...
describe("AI Strategy Generator Streaming Integration Tests", () => {
  ...
  beforeEach(() => {
            ...
            chunk: {
              bytes: new TextEncoder().encode(
                JSON.stringify({
                  ...
                  delta: { text: '{"name": "AI Momentum Strategy",' },
                })
              ),
            },
          },
            ...
            chunk: {
              bytes: new TextEncoder().encode(
                JSON.stringify({
                  type: "content_block_delta",
                  delta: {
                    text: '"description": "Advanced momentum strategy using moving averages",',
                  },
                })
              ),
            },
    ...
    jest
      ...
      .mockImplementation(async (strategy) => ({
        ...
        validated: true,
        insights: [
          "Strategy uses momentum indicators",
  ...
  describe("Stream Management", () => {
    ...
    test("should enforce maximum concurrent streams", async () => {
      ...
      // Start first stream
      const stream1Promise = streamingGenerator.generateWithStreaming(
        "Strategy 1",
        mockSymbols
      );
  ...
  describe("Progress Analysis", () => {
    ...
    test("should determine generation stages correctly", () => {
      const stages = [
        ...
        {
          text: '{"name": "Strategy", "description": "A strategy that uses advanced technical indicators to generate trading signals based on market momentum and trend analysis"}',
          expected: "Creating strategy metadata",
        },
        {
          text: '{"name": "Strategy", "code": "# This is the beginning of the strategy code generation process", "description": "Strategy"}',
          expected: "Starting code generation",
        },
        {
          text: "import pandas as pd\\nimport numpy as np\\nimport talib\\nfrom datetime import datetime\\n# Setting up all required libraries",
          expected: "Setting up imports and parameters",
        },
        {
          text: "def generate_signals(data):\\n    signals = []\\n    for symbol in symbols:\\n        # Process each symbol individually",
          expected: "Implementing strategy logic",
        },
        {
          text: 'for symbol in symbols:\\n    if condition_met:\\n        signals.append("buy")\\n        portfolio.add_position(symbol)',
          expected: "Implementing strategy logic",
        },
        {
          text: 'signals.append("buy")\\nsignals.append("sell")\\nreturn processed_signals # Final signal processing complete',
          expected: "Finalizing trade signals",
        },
        {
          text: "return signals}}\\n# Strategy generation complete with all metadata and implementation ready for deployment",
          expected: "Completing strategy",
        },
        {
          text: "random text with more than 100 characters to make sure it is not considered initializing phase because the logic checks for length",
          expected: "Generating strategy",
        },
      ];
    ...
    test("should estimate time remaining correctly", () => {
      ...
      testCases.forEach(({ progress, expectedMin = 0, expectedMax }) => {
        const timeRemaining =
          streamingGenerator.estimateTimeRemaining(progress);
  ...
  describe("Integration with Base Class", () => {
    test("should inherit from AIStrategyGenerator", () => {
      expect(streamingGenerator.constructor.name).toBe(
        "AIStrategyGeneratorStreaming"
      );
