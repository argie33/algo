...
describe("Backtest Store Integration Tests", () => {
  ...
  describe("Strategy Management Operations", () => {
    ...
    test("should get strategy by ID", () => {
      ...
      expect(loadedStrategy.parameters.symbol).toBe(testStrategy.parameters.symbol);
    });
  ...
  describe("File System Integration", () => {
    ...
    test("should verify file persistence", () => {
      const storeFile = path.join(__dirname, "../../../user_backtest_store.json");
  ...
  describe("Error Handling", () => {
    test("should handle concurrent access safely", async () => {
      const promises = [];
      for (let i = 0; i < 5; i++) {
        promises.push(Promise.resolve(backtestStore.addStrategy({
          ...testStrategy,
    ...
    test("should generate unique IDs for strategies", async () => {
      const strategy1 = backtestStore.addStrategy({...testStrategy, name: "Strategy 1"});
      
      // Add delay to ensure different timestamp
      await new Promise(resolve => setTimeout(resolve, 2));
      
      const strategy2 = backtestStore.addStrategy({...testStrategy, name: "Strategy 2"});
      ...
      if (strategy1.id === strategy2.id) {
        console.warn('ID collision detected - this is a potential bug in backtestStore.addStrategy()');
  ...
  describe("Strategy Data Validation", () => {
    ...
    test("should maintain data integrity across operations", async () => {
      ...
      const strategy1 = backtestStore.addStrategy({ name: `Test A ${testId}`, value: 123 });
      await new Promise(resolve => setTimeout(resolve, 2));
      
      const strategy2 = backtestStore.addStrategy({ name: `Test B ${testId}`, value: "string" });
      await new Promise(resolve => setTimeout(resolve, 2));
      
      const strategy3 = backtestStore.addStrategy({ name: `Test C ${testId}`, value: [1, 2, 3] });
