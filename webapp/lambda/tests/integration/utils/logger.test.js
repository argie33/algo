...
describe("Logger Integration Tests", () => {
  ...
  describe("Specialized Logging Methods", () => {
    test("should log database operations", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes(operation))
        );
        expect(logEntry).toBeDefined();
      }
    });
    ...
    test("should log API calls", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes(`API call: ${method} ${url}`))
        );
        expect(logEntry).toBeDefined();
      }
    });
    ...
    test("should log performance metrics", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes(operation) && arg.includes(`${duration}ms`))
        );
        expect(logEntry).toBeDefined();
      }
    });
    ...
    test("should log user actions with anonymized user ID", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes(action))
        );
        expect(logEntry).toBeDefined();
  ...
  describe("Structured Logging", () => {
    ...
    test("should handle error objects with stack traces", () => {
      ...
      const logEntry = capturedLogs.find(log => 
        log.args.some(arg => typeof arg === 'string' && arg.includes("Database operation failed"))
      );
      expect(logEntry).toBeDefined();
    });
  });
  ...
  describe("Configuration Utilities", () => {
    ...
    test("should log configuration loaded event", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes("Configuration loaded"))
        );
        expect(logEntry).toBeDefined();
      }
    });
  ...
  describe("Application Lifecycle Logging", () => {
    test("should log application startup", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes("Application starting"))
        );
        expect(logEntry).toBeDefined();
      }
    });
    ...
    test("should log application shutdown", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes("Application shutting down"))
        );
        expect(logEntry).toBeDefined();
      }
    });
  ...
  describe("Performance Logging Edge Cases", () => {
    test("should warn on slow operations", () => {
      ...
      if (shouldLogWarn) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes(operation) && arg.includes("6000ms"))
        );
        expect(logEntry).toBeDefined();
      }
    });
    ...
    test("should log normal operations as info", () => {
      ...
      if (shouldLogInfo) {
        const logEntry = capturedLogs.find(log => 
          log.args.some(arg => typeof arg === 'string' && arg.includes(operation) && arg.includes("100ms"))
        );
        expect(logEntry).toBeDefined();
      }
    });
