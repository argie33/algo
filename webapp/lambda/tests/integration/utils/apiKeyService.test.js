...
describe("API Key Service Integration Tests", () => {
  ...
  describe("API Key Encryption", () => {
    ...
    test("should handle encryption with different salts", async () => {
      ...
      expect(await apiKeyService.decryptApiKey(encrypted1, salt1)).toBe(
        testKey
      );
      expect(await apiKeyService.decryptApiKey(encrypted2, salt2)).toBe(
        testKey
      );
    });
  ...
  describe("API Key Storage and Retrieval", () => {
    ...
    test("should store and retrieve API keys", async () => {
      // Store API key
      const storeResult = await apiKeyService.storeApiKey(
        testToken,
        testProvider,
        testApiKeyData
      );
      ...
      // Retrieve API key
      const retrievedData = await apiKeyService.getApiKey(
        testToken,
        testProvider
      );
