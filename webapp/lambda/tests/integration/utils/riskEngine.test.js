...
describe("Risk Engine Integration Tests", () => {
  ...
  async function setupTestData() {
    ...
    for (const position of testPortfolio) {
      ...
      `, ['test-user-123', position.symbol, position.quantity, position.cost_basis, position.current_price]);
    }
  ...
  async function cleanupTestData() {
    await query("DELETE FROM user_portfolio WHERE user_id = $1", ['test-user-123']);
    await query("DELETE FROM price_daily WHERE symbol = 'AAPL' AND date >= '2023-12-01'");
  }
  ...
  describe("Portfolio Risk Assessment with Database Integration", () => {
    test("should calculate portfolio-wide risk metrics from database", async () => {
      const portfolioRisk = await riskEngine.calculatePortfolioRisk('test-user-123');
    ...
    test("should identify concentration risk", async () => {
      ...
      expect(Array.isArray(portfolioRisk.concentrationRisk.topHoldings)).toBe(true);
      expect(portfolioRisk.concentrationRisk.topHoldings.length).toBeGreaterThan(0);
      
      // Top holdings should be sorted by allocation percentage
      const allocations = portfolioRisk.concentrationRisk.topHoldings.map(h => h.allocation);
  ...
  describe("Historical Volatility Analysis", () => {
    test("should calculate volatility from historical price data", async () => {
      const volatility = await riskEngine.calculateHistoricalVolatility('AAPL', 5);
    ...
    test("should handle insufficient data gracefully", async () => {
      const volatility = await riskEngine.calculateHistoricalVolatility('NONEXISTENT', 30);
  ...
  describe("Value at Risk (VaR) Calculations", () => {
    test("should calculate portfolio VaR using historical method", async () => {
      const var95 = await riskEngine.calculatePortfolioVaR('test-user-123', 0.95, 'historical');
    ...
    test("should calculate VaR for different confidence levels", async () => {
      const var99 = await riskEngine.calculatePortfolioVaR('test-user-123', 0.99, 'historical');
      const var95 = await riskEngine.calculatePortfolioVaR('test-user-123', 0.95, 'historical');
    ...
    test("should calculate parametric VaR", async () => {
      const parametricVaR = await riskEngine.calculatePortfolioVaR('test-user-123', 0.95, 'parametric');
  ...
  describe("Position Sizing and Risk Management", () => {
    test("should recommend optimal position sizes", async () => {
      ...
      expect(positionRecommendation.riskAdjustedShares).toBeLessThanOrEqual(positionRecommendation.maxShares);
      
      // Should include risk impact assessment
      expect(positionRecommendation.riskMetrics).toHaveProperty("portfolioImpact");
      expect(positionRecommendation.riskMetrics).toHaveProperty("diversificationEffect");
      expect(positionRecommendation.riskMetrics).toHaveProperty("correlationRisk");
    });
  ...
  describe("Correlation and Covariance Analysis", () => {
    test("should calculate correlation matrix for portfolio", async () => {
      const correlationMatrix = await riskEngine.calculateCorrelationMatrix('test-user-123');
      ...
      expect(correlationMatrix.matrix.length).toBe(correlationMatrix.symbols.length);
    ...
    test("should identify highly correlated positions", async () => {
      const correlationAnalysis = await riskEngine.analyzePortfolioCorrelations('test-user-123');
      ...
      expect(Array.isArray(correlationAnalysis.highCorrelationPairs)).toBe(true);
  ...
  describe("Stress Testing and Scenario Analysis", () => {
    test("should perform market crash stress test", async () => {
      const stressTest = await riskEngine.performStressTest('test-user-123', 'market_crash');
    ...
    test("should perform custom scenario analysis", async () => {
      ...
      const scenarioTest = await riskEngine.performCustomStressTest('test-user-123', customScenario);
      ...
      const teslaImpact = scenarioTest.positionImpacts.find(p => p.symbol === 'TSLA');
  ...
  describe("Risk Monitoring and Alerts", () => {
    test("should monitor portfolio risk limits", async () => {
      const riskMonitoring = await riskEngine.monitorRiskLimits('test-user-123');
    ...
    test("should generate risk alerts for violations", async () => {
      ...
      await query(`
        ...
        DO UPDATE SET 
          ...
          cost_basis = EXCLUDED.cost_basis,
          current_price = EXCLUDED.current_price
      `, ['test-user-123', 'RISKY_STOCK', 1000, 100, 95]); // Large position with recent loss
      ...
      await query("DELETE FROM user_portfolio WHERE user_id = $1 AND symbol = $2", 
    ...
    test("should handle large portfolios efficiently", async () => {
      ...
      for (const position of largePortfolio) {
        await query(`
          INSERT INTO user_portfolio (user_id, symbol, quantity, cost_basis, current_price) 
          VALUES ($1, $2, $3, $4, $5)
        `, ['large-portfolio-test', position.symbol, position.quantity, position.cost_basis, position.current_price]);
      }

      const startTime = Date.now();
      const portfolioRisk = await riskEngine.calculatePortfolioRisk('large-portfolio-test');
      ...
      // Cleanup
      await query("DELETE FROM user_portfolio WHERE user_id = $1", ['large-portfolio-test']);
    }, 10000);
  ...
  describe("Error Handling and Edge Cases", () => {
    test("should handle empty portfolio gracefully", async () => {
      const emptyPortfolioRisk = await riskEngine.calculatePortfolioRisk('empty-user');
    ...
    test("should handle invalid risk parameters", async () => {
      const invalidVaR = await riskEngine.calculatePortfolioVaR('test-user-123', 1.5, 'invalid');
