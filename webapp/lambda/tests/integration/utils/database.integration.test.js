...
describe("Database Real Site Functionality Tests", () => {
  ...
  afterAll(async () => {
    ...
    // Clean up test data
    try {
      await query("DELETE FROM market_data WHERE symbol LIKE 'T%' OR symbol LIKE 'U%' OR symbol LIKE 'A%' OR symbol LIKE 'B%' OR symbol LIKE 'N%' OR symbol LIKE 'E%' OR symbol LIKE 'TIME%'");
      await query("DELETE FROM stock_symbols WHERE symbol LIKE 'T%' OR symbol LIKE 'U%' OR symbol LIKE 'A%' OR symbol LIKE 'B%' OR symbol LIKE 'N%' OR symbol LIKE 'E%' OR symbol LIKE 'TIME%'");
    } catch (error) {
  ...
  describe("Real Database Query Operations", () => {
    ...
    test("should prevent SQL injection attacks with parameterized queries", async () => {
      ...
      const tableCheck = await query("SELECT COUNT(*) as count FROM stock_symbols");
    ...
    test("should work with complex JOIN queries on real tables", async () => {
      ...
      await query(
        ...
        [testSymbol, 'Test Company', testDate, 150.50, 1000000]
      );
      ...
      expect(result.rows[0].symbol).toBe(testSymbol);
      expect(parseFloat(result.rows[0].price)).toBe(150.50);
    });
    ...
    test("should handle aggregate functions and calculations", async () => {
      ...
      for (let i = 0; i < prices.length; i++) {
        const testDate = new Date(Date.now() - i * 86400000).toISOString().split('T')[0]; // Daily intervals
        await query(
          "INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4",
  ...
  describe("Real Database Schema Operations", () => {
    test("should initialize and verify schema tables", async () => {
      ...
      const stockSymbolsCheck = await query("SELECT COUNT(*) FROM stock_symbols LIMIT 1");
      const marketDataCheck = await query("SELECT COUNT(*) FROM market_data LIMIT 1");
  ...
  describe("Transaction Support", () => {
    test("should execute transactions with commit", async () => {
      ...
      const result = await transaction(async (client) => {
        ...
        await client.query(
          'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
          [`TXN${testId}`, 'Transaction Test Co', testDate, 100.00, 50000]
        );
    ...
    test("should rollback transactions on error", async () => {
      ...
      await expect(transaction(async (client) => {
        ...
        await client.query(
          ...
          [`ERR${testId}`, 'Error Test Co', testDate, 'invalid_price_string', 1000]
        );
      ...
      const result = await transaction(async (client) => {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          marketDataPromises.push(
            client.query(
              'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
              [`NEST${testId}`, 'Nested Transaction Co', testDate, 200 + i * 5, 75000 + i * 1000]
            )
          );
        }
  ...
  describe("Real-World Data Operations", () => {
    test("should handle bulk data insertions efficiently", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        bulkInsertPromises.push(
          query(
            'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
            [bulkTestSymbol, 'Bulk Test Company', testDate, price.toFixed(2), Math.floor(volume)]
          )
        );
      }
    ...
    test("should handle complex analytical queries", async () => {
      ...
      for (let i = 0; i < prices.length; i++) {
        const testDate = new Date(Date.now() - (prices.length - i) * 86400000).toISOString().split('T')[0];
        await query(
          'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
          [analyticsSymbol, 'Analytics Test Corp', testDate, prices[i], 15000 + i * 500]
        );
      }
    ...
    test("should handle time-based queries with proper indexing performance", async () => {
      ...
      for (let i = 0; i < 24; i++) { // 24 days of daily data
        const testDate = new Date(now.getTime() - i * 86400000).toISOString().split('T')[0];
        timeBasedPromises.push(
          query(
            'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
            [timeTestSymbol, 'Time Series Test Co', testDate, 100 + Math.sin(i / 4) * 10, 25000]
          )
        );
      }
      ...
      const timeRangeResult = await query(`
        ...
        WHERE symbol = $1 
          ...
          AND date <= $3
        GROUP BY symbol
      `, [timeTestSymbol, new Date(now.getTime() - 12 * 86400000).toISOString().split('T')[0], now.toISOString().split('T')[0]]); // Last 12 days
  ...
  describe("Advanced Transaction Integration Scenarios", () => {
    test("should handle concurrent transactions without conflicts", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const transactionPromise = transaction(async (client) => {
          ...
          await client.query(
            'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
            [symbol, `Concurrent Test ${i}`, new Date().toISOString().split('T')[0], 100 + i, 1000 * (i + 1)]
          );
    ...
    test("should handle transaction rollback with concurrent access", async () => {
      ...
      await expect(failingTransaction).rejects.toThrow('Simulated transaction failure');
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          insertPromises.push(
            client.query(
              ...
              client.query(
                'INSERT INTO market_data (symbol, name, date, price, volume) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (symbol, date) DO UPDATE SET price = $4',
                [symbol, `Bulk Test Stock ${i}`, new Date().toISOString().split('T')[0], price, volume]
              )
            )
          );
        }
