...
describe("News Analyzer Integration Tests", () => {
  ...
  describe("News Data Processing", () => {
    test("should analyze news sentiment", async () => {
      const testArticle = {
        ...
        content:
          "Apple Inc. reported strong quarterly earnings that beat analyst expectations, driven by robust iPhone sales and growing services revenue.",
        source: "test_source",
    ...
    test("should extract key topics from news", async () => {
      const testArticle = {
        ...
        content:
          "Tesla announced plans for a new electric vehicle manufacturing facility that will increase production capacity and create thousands of jobs.",
        symbol: "TSLA",
      };
      ...
      const newsFeed = [
        {
          ...
          content:
            "Stock markets opened higher today following positive economic indicators...",
          symbol: "SPY",
        },
        {
          title: "Tech Stocks Rally on AI News",
          content:
            "Technology stocks surged on artificial intelligence development announcements...",
          symbol: "QQQ",
        },
      ];
  ...
  describe("Symbol-Specific Analysis", () => {
    test("should analyze news for specific symbols", async () => {
      const symbol = "AAPL";
      const symbolNews = await newsAnalyzer.getSymbolNews(symbol, {
        limit: 10,
      });
  ...
  describe("Trend Analysis", () => {
    test("should identify trending topics", async () => {
      const trendingTopics = await newsAnalyzer.getTrendingTopics({
        period: "24h",
      });
  ...
  describe("News Source Quality", () => {
    test("should evaluate news source credibility", async () => {
      ...
      for (const source of sources) {
        const credibility =
          await newsAnalyzer.evaluateSourceCredibility(source);
    ...
    test("should weight analysis by source quality", async () => {
      ...
      const highQualitySentiment =
        await newsAnalyzer.analyzeSentiment(highQualityArticle);
      const lowQualitySentiment =
        await newsAnalyzer.analyzeSentiment(lowQualityArticle);

      expect(highQualitySentiment.confidence).toBeGreaterThan(
        lowQualitySentiment.confidence
      );
      expect(highQualitySentiment.weight).toBeGreaterThan(
        lowQualitySentiment.weight
      );
    });
  });
  ...
  describe("Language Processing", () => {
    ...
    test("should detect article language automatically", async () => {
      const article = {
        ...
        content:
          "Apple a annoncé des résultats trimestriels solides qui dépassent les attentes des analystes...",
      };
  ...
  describe("Market Impact Analysis", () => {
    test("should predict market impact from news", async () => {
      const breakingNews = {
        ...
        content:
          "Apple announced record-breaking quarterly revenue driven by strong iPhone sales and services growth...",
        symbol: "AAPL",
        publishedAt: new Date().toISOString(),
      };
    ...
    test("should correlate news with price movements", async () => {
      const symbol = "AAPL";
      const correlation = await newsAnalyzer.correlateSentimentWithPrice(
        symbol,
        {
          period: "30days",
          lag: "1hour",
  ...
  describe("Alert Integration", () => {
    ...
    test("should process real-time news alerts", async () => {
      const urgentNews = {
        ...
        content:
          "Apple CEO announced a revolutionary new product that could transform the industry...",
        symbol: "AAPL",
        publishedAt: new Date().toISOString(),
        urgency: "breaking",
      };
