...
describe("Sentiment API", () => {
  describe("Stock Sentiment", () => {
    test("should analyze sentiment for specific stock", async () => {
      ...
      if (response.status === 200) {
        ...
        const sentimentFields = ["sentiment_score", "bullish_percent", "bearish_percent"];
        const hasSentimentData = sentimentFields.some(field => 
          Object.keys(sentiment).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
    ...
    test("should provide sentiment trend over time", async () => {
      const response = await request(app)
        .get("/api/sentiment/stock/AAPL/trend?period=30d");
  ...
  describe("Market Sentiment", () => {
    test("should analyze overall market sentiment", async () => {
      ...
      if (response.status === 200) {
        ...
        const marketFields = ["overall_sentiment", "fear_greed_index", "vix_level"];
        const hasMarketData = marketFields.some(field => 
          Object.keys(marketSentiment).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
  ...
  describe("Social Media Sentiment", () => {
    test("should analyze social media sentiment for stock", async () => {
      ...
      if (response.status === 200) {
        ...
        const socialFields = ["twitter_sentiment", "reddit_sentiment", "mention_volume"];
        const hasSocialData = socialFields.some(field => 
          Object.keys(socialSentiment).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
    ...
    test("should provide trending stocks by social sentiment", async () => {
      const response = await request(app)
        .get("/api/sentiment/social/trending?limit=20");
  ...
  describe("News Sentiment", () => {
    test("should analyze news sentiment for stock", async () => {
      ...
      if (response.status === 200) {
        ...
        const newsFields = ["news_sentiment", "article_count", "recent_headlines"];
        const hasNewsData = newsFields.some(field => 
          Object.keys(newsSentiment).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
    ...
    test("should provide sentiment analysis of specific news articles", async () => {
      const response = await request(app)
        .get("/api/sentiment/news/articles?symbol=AAPL&limit=10");
  ...
  describe("Institutional Sentiment", () => {
    test("should analyze institutional investor sentiment", async () => {
      const response = await request(app)
        .get("/api/sentiment/institutional/AAPL");
      ...
      if (response.status === 200) {
        ...
        const instFields = ["ownership_change", "insider_activity", "fund_flows"];
        const hasInstData = instFields.some(field => 
          Object.keys(institutionalSentiment).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
    ...
    test("should track options sentiment indicators", async () => {
      const response = await request(app)
        .get("/api/sentiment/options/AAPL");
      ...
      if (response.status === 200) {
        ...
        const optionsFields = ["put_call_ratio", "implied_volatility", "unusual_activity"];
        const hasOptionsData = optionsFields.some(field => 
          Object.keys(optionsSentiment).some(key => key.toLowerCase().includes(field.replace("_", "")))
        );
