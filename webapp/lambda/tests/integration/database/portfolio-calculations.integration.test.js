...
describe("Portfolio Calculations Integration", () => {
  ...
  beforeAll(async () => {
    ...
    `,
      [
        testUserId,
        testPortfolioData.symbol,
        testPortfolioData.quantity,
        testPortfolioData.avgCost,
      ]
    );
  ...
  afterAll(async () => {
    ...
    await query("DELETE FROM stock_prices WHERE symbol = $1", [
      testPortfolioData.symbol,
    ]);
  });
  ...
  describe("Portfolio Value Calculations", () => {
    test("should calculate correct total portfolio value", async () => {
      ...
      const expectedValue =
        testPortfolioData.quantity * testPortfolioData.currentPrice;

      expect(data.summary.totalValue).toBeCloseTo(expectedValue, 2);
      expect(data.summary.totalValue).toBe(17550.0); // 100 * 175.50
    });
    ...
    test("should calculate accurate profit/loss metrics", async () => {
      ...
      const { data } = response.body;
      const expectedCost =
        testPortfolioData.quantity * testPortfolioData.avgCost;
      const expectedValue =
        testPortfolioData.quantity * testPortfolioData.currentPrice;
      ...
      expect(data.totalCost).toBe(15000.0); // 100 * 150.00
      expect(data.totalGainLoss).toBe(2550.0); // 17550 - 15000
      expect(data.totalGainLossPercent).toBeCloseTo(17.0, 1); // (2550/15000)*100
    });
    ...
    test("should handle zero and negative positions correctly", async () => {
      ...
      expect(data.holdings).not.toEqual(
        expect.arrayContaining([expect.objectContaining({ symbol: "ZERO" })])
      );
    });
    ...
    test("should calculate sector allocation correctly", async () => {
      ...
      const techSector = data.sectorAllocation.find(
  ...
  describe("Data Consistency Validation", () => {
    test("should ensure holdings sum equals total portfolio value", async () => {
      ...
      const holdingsSum =
        data.holdings?.reduce((sum, holding) => {
    ...
    test("should maintain referential integrity with price updates", async () => {
      ...
      if (aaplHolding) {
        expect(aaplHolding.currentPrice).toBeCloseTo(180.0, 2);
      }
    });
  });
});
