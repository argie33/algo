...
const query = async (text, params) => {
  if (!global.TEST_DATABASE) {
    throw new Error('Test database not available. Run tests with proper setup.');
  }
...
describe('Portfolio Database Calculations Integration', () => {
  ...
  beforeAll(async () => {
    ...
    if (!global.DATABASE_AVAILABLE()) {
      console.log('⚠️  Skipping database integration tests - no database available');
      return;
    }
  ...
  test('should calculate portfolio value from database data', async () => {
    ...
    expect(parseFloat(holding.total_cost)).toBe(15000.00);
  });
  ...
  test('should handle multiple holdings correctly', async () => {
    ...
    expect(parseFloat(result.rows[0].total_value)).toBe(25000.00); // 15000 + 10000
  });
  ...
  test('should handle precision correctly for fractional shares', async () => {
    ...
    const calculatedValue = Math.round(parseFloat(result.rows[0].calculated_value) * 100) / 100;
