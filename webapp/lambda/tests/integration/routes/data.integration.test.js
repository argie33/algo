...
describe("Data Routes", () => {
  ...
  describe("GET /api/data", () => {
    ...
    test("should include endpoint documentation", async () => {
      ...
      expect(endpoints).toContain("/:symbol - Get comprehensive data for a symbol");
      expect(endpoints).toContain("/historical/:symbol - Get historical data for a symbol");
      expect(endpoints).toContain("/realtime/:symbol - Get real-time data for a symbol");
  ...
  describe("GET /api/data/:symbol", () => {
    test("should handle valid symbol request", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(response.body.message).toContain("No data available for symbol AAPL");
      }
    });
    ...
    test("should return 404 for non-existent symbol", async () => {
      ...
      expect(response.body.error).toContain("No data available for symbol NONEXISTENT");
    });
    ...
    test("should validate response structure when data exists", async () => {
      ...
      if (response.status === 200) {
        ...
        if (response.body.price) {
          ...
          expect(["open", "high", "low", "close", "volume"].some(prop => 
  ...
  describe("Data API Error Handling", () => {
    ...
    test("should return consistent error format", async () => {
      const response = await request(app)
        .get("/api/data/DEFINITELY_NONEXISTENT_SYMBOL");
