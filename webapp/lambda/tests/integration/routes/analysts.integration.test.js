...
describe("Analysts Routes", () => {
  ...
  describe("GET /api/analysts", () => {
    ...
    test("should include expected endpoints in overview", async () => {
      ...
      expect(endpoints.some(ep => ep.includes("/recommendations"))).toBe(true);
      expect(endpoints.some(ep => ep.includes("/earnings-estimates"))).toBe(true);
      expect(endpoints.some(ep => ep.includes("/revenue-estimates"))).toBe(true);
  ...
  describe("GET /api/analysts/upgrades", () => {
    ...
    test("should handle invalid pagination parameters", async () => {
      const response = await request(app)
        .get("/api/analysts/upgrades?page=invalid&limit=abc");
    ...
    test("should handle zero and negative pagination values", async () => {
      const response = await request(app)
        .get("/api/analysts/upgrades?page=0&limit=-5");
    ...
    test("should validate upgrade data structure", async () => {
      ...
      if (response.status === 200 && response.body.data.length > 0) {
        ...
        expect(["Upgrade", "Downgrade", "Neutral"].includes(upgrade.action)).toBe(true);
      }
    });
  ...
  describe("GET /api/analysts/:ticker/earnings-estimates", () => {
    ...
    test("should validate earnings estimate structure", async () => {
      const response = await request(app)
        .get("/api/analysts/MSFT/earnings-estimates");
    ...
    test("should handle special characters in ticker", async () => {
      const response = await request(app)
        .get("/api/analysts/BRK.A/earnings-estimates");
  ...
  describe("GET /api/analysts/:ticker/revenue-estimates", () => {
    ...
    test("should validate revenue estimate structure", async () => {
      const response = await request(app)
        .get("/api/analysts/NVDA/revenue-estimates");
  ...
  describe("GET /api/analysts/:ticker/earnings-history", () => {
    ...
    test("should validate earnings history structure", async () => {
      const response = await request(app)
        .get("/api/analysts/MSFT/earnings-history");
    ...
    test("should handle case insensitive ticker", async () => {
      const response = await request(app)
        .get("/api/analysts/tesla/earnings-history");
  ...
  describe("GET /api/analysts/:ticker/eps-revisions", () => {
    ...
    test("should validate EPS revisions structure", async () => {
      const response = await request(app)
        .get("/api/analysts/MSFT/eps-revisions");
    ...
    test("should handle ticker case conversion", async () => {
      const response = await request(app)
        .get("/api/analysts/googl/eps-revisions");
  ...
  describe("GET /api/analysts/:ticker/eps-trend", () => {
    ...
    test("should validate EPS trend structure", async () => {
      const response = await request(app)
        .get("/api/analysts/TSLA/eps-trend");
  ...
  describe("GET /api/analysts/:ticker/growth-estimates", () => {
    ...
    test("should validate growth estimates structure", async () => {
      const response = await request(app)
        .get("/api/analysts/META/growth-estimates");
  ...
  describe("GET /api/analysts/:ticker/overview", () => {
    ...
    test("should include metadata with note", async () => {
      const response = await request(app)
        .get("/api/analysts/NFLX/overview");
  ...
  describe("GET /api/analysts/recent-actions", () => {
    ...
    test("should handle invalid limit parameter", async () => {
      const response = await request(app)
        .get("/api/analysts/recent-actions?limit=invalid");
  ...
  describe("GET /api/analysts/recommendations/:symbol", () => {
    test("should return analyst recommendations for valid symbol", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(response.body.data.rating_distribution).toHaveProperty("strong_buy");
        ...
        expect(response.body.data.rating_distribution).toHaveProperty("strong_sell");
      } else if (response.status === 404) {
        expect(response.body).toHaveProperty("success", false);
        expect(response.body).toHaveProperty("error", "No analyst recommendations found");
    ...
    test("should validate recent changes structure", async () => {
      const response = await request(app)
        .get("/api/analysts/recommendations/MSFT");

      if (response.status === 200 && response.body.data.recent_changes.length > 0) {
    ...
    test("should handle special characters in symbol", async () => {
      const response = await request(app)
        .get("/api/analysts/recommendations/BRK.A");
    ...
    test("should calculate consensus rating correctly", async () => {
      const response = await request(app)
        .get("/api/analysts/recommendations/GOOGL");
  ...
  describe("GET /api/analysts/targets/:symbol", () => {
    test("should return price targets for valid symbol", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(response.body.data.price_targets).toHaveProperty("std_deviation");
      }
    });
  ...
  describe("GET /api/analysts/downgrades", () => {
    test("should return 501 not implemented", async () => {
      ...
      expect(response.body).toHaveProperty("success", false);
      expect(response.body).toHaveProperty("error", "Analyst downgrades not implemented");
      expect(response.body).toHaveProperty("message");
      ...
      expect(Array.isArray(response.body.troubleshooting.required_tables)).toBe(true);
    });

    test("should handle limit parameter", async () => {
      const response = await request(app)
        .get("/api/analysts/downgrades?limit=15");
  ...
  describe("GET /api/analysts/consensus/:symbol", () => {
    test("should return consensus analysis for valid symbol", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(response.body.data.consensus_metrics).toHaveProperty("avg_rating");
        expect(response.body.data.consensus_metrics).toHaveProperty("total_analysts");
        expect(response.body.data.consensus_metrics).toHaveProperty("rating_strength");
        expect(response.body.data.consensus_metrics).toHaveProperty("revision_trend");
        
        // Validate estimate revisions
        expect(response.body.data.estimate_revisions).toHaveProperty("upgrades_last_30d");
        expect(response.body.data.estimate_revisions).toHaveProperty("downgrades_last_30d");
        expect(response.body.data.estimate_revisions).toHaveProperty("target_increases");
        expect(response.body.data.estimate_revisions).toHaveProperty("target_decreases");
      }
    });
    ...
    test("should validate revision trend values", async () => {
      const response = await request(app)
        .get("/api/analysts/consensus/META");
  ...
  describe("Error Handling", () => {
    ...
    test("should handle malformed URLs", async () => {
      const response = await request(app)
        .get("/api/analysts/AAPL%20invalid/earnings-estimates");
    ...
    test("should handle extremely long ticker symbols", async () => {
      ...
      const response = await request(app)
        .get(`/api/analysts/${longTicker}/earnings-estimates`);
    ...
    test("should handle special characters in ticker", async () => {
      const response = await request(app)
        .get("/api/analysts/A@PPL/earnings-estimates");
  ...
  describe("Edge Cases", () => {
    test("should handle empty ticker parameters", async () => {
      const response = await request(app)
        .get("/api/analysts//earnings-estimates");
    ...
    test("should handle numeric tickers", async () => {
      const response = await request(app)
        .get("/api/analysts/123/earnings-estimates");
    ...
    test("should handle very large pagination values", async () => {
      const response = await request(app)
        .get("/api/analysts/upgrades?page=999999&limit=1000");
    ...
    test("should handle URL encoded parameters", async () => {
      const response = await request(app)
        .get("/api/analysts/BRK%2EA/earnings-estimates");
