/**
 * Commodities Routes Integration Tests - REAL DATA ONLY
 * Tests commodities endpoints with REAL database connection and REAL loaded data
...
describe("Commodities Routes - Real Data Validation", () => {
  beforeAll(async () => {
  ...
  describe("GET /api/commodities (Root Endpoint)", () => {
    test("should return commodities system information", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(Array.isArray(response.body.data.available_endpoints)).toBe(
          true
        );
        expect(response.body.data.available_endpoints.length).toBeGreaterThan(
          0
        );
      }
    });
      ...
      for (const category of categories) {
        ...
        if (response.status === 200 && response.body.data.length > 0) {
          response.body.data.forEach((commodity) => {
            expect(commodity.category.toLowerCase()).toBe(
  ...
  describe("GET /api/commodities/correlations (Price Correlations)", () => {
    ...
    test("should validate correlation strength classifications", async () => {
      ...
      if (
        response.status === 200 &&
        response.body.data.correlations.length > 0
      ) {
        const validStrengths = ["strong", "moderate", "weak", "very_weak"];

        response.body.data.correlations.forEach((corr) => {
          if (corr.strength) {
            expect(validStrengths.includes(corr.strength.toLowerCase())).toBe(
              true
            );
          }
        });
