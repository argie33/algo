/**
 * Performance Routes Integration Tests - REAL DATA ONLY
 * Tests performance endpoints with REAL database connection and REAL loaded data
...
describe("Performance Routes - Real Data Validation", () => {
  beforeAll(async () => {
  ...
  describe("GET /api/performance/benchmark (Benchmark Comparison)", () => {
    ...
    test("should handle different benchmark symbols", async () => {
      ...
      for (const benchmark of benchmarks) {
        ...
        if (response.status === 200 && response.body.data.benchmark) {
          expect(response.body.data.benchmark).toHaveProperty(
            "symbol",
            benchmark
          );
        }
    ...
    test("should handle different time periods", async () => {
      ...
      for (const period of periods) {
        ...
        if (response.status === 200 && response.body.data.comparison) {
          expect(response.body.data.comparison).toHaveProperty(
            "period",
            period
          );
        }
    ...
    test("should validate benchmark comparison metrics", async () => {
      ...
      if (response.status === 200 && response.body.data.comparison) {
        ...
        if (
          comparison.outperformance !== null &&
          comparison.outperformance !== undefined
        ) {
    ...
    test("should validate portfolio performance metrics", async () => {
      ...
      if (response.status === 200 && response.body.data.performance) {
        ...
        if (
          ...
          performance.total_return !== undefined
        ) {
        ...
        if (
          performance.portfolio_value !== null &&
          performance.portfolio_value !== undefined
        ) {
        ...
        if (
          ...
          Array.isArray(performance.daily_returns)
        ) {
    ...
    test("should return comprehensive performance metrics", async () => {
      ...
      if (response.status === 200) {
        ...
        expectedMetrics.forEach((metric) => {
          if (
            response.body.data[metric] !== undefined &&
            response.body.data[metric] !== null
          ) {
  ...
  describe("Performance and Edge Cases", () => {
    ...
    test("should validate response content types", async () => {
      ...
      for (const endpoint of endpoints) {
        const authHeader =
          endpoint.includes("health") || endpoint === "/api/performance"
            ? {}
    ...
    test("should handle memory pressure with large data requests", async () => {
      ...
      expect(
        [200, 400, 401, 403, 413, 500, 503].includes(response.status)
      ).toBe(true);
    });
  });
});
