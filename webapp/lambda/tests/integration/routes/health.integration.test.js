...
describe("Health Routes", () => {
  ...
  describe("GET /api/health", () => {
    test("should return quick health check", async () => {
      ...
      expect(response.body.note).toBe("Quick health check - database not tested");
  ...
  describe("GET /api/health/database", () => {
    ...
    test("should provide table summary statistics", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(summary.healthy_tables + summary.empty_tables + summary.error_tables).toBeGreaterThanOrEqual(0);
      }
    });
  });
  ...
  describe("GET /api/health/database/diagnostics", () => {
    ...
    test("should provide actionable recommendations", async () => {
      ...
      if (response.status === 200) {
        expect(Array.isArray(response.body.diagnostics.recommendations)).toBe(true);
