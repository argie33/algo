...
describe('New Endpoints Integration Tests', () => {
  ...
  describe('Market Correlation API', () => {
    test('GET /api/market/correlation - should return correlation matrix', async () => {
      ...
      expect(Array.isArray(response.body.data.correlation_matrix.symbols)).toBe(true);
      expect(Array.isArray(response.body.data.correlation_matrix.matrix)).toBe(true);
    });
    ...
    test('GET /api/market/correlation with symbol filter - should filter results', async () => {
      ...
      expect(response.body).toHaveProperty('success', true);
      expect(response.body.data.correlation_matrix.symbols).toEqual(['AAPL', 'MSFT']);
    });
  ...
  describe('Data Quality Validation', () => {
    ...
    test('dashboard financial values should be realistic', async () => {
      ...
      const total = allocation.stocks + allocation.etfs + allocation.cash + allocation.crypto;
