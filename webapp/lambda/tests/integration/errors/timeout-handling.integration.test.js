...
describe("Timeout Handling Integration", () => {
  ...
  describe("Request Timeout Scenarios", () => {
    test("should handle client-side request timeouts gracefully", async () => {
      ...
      const timeoutTestEndpoints = [
        { endpoint: "/api/portfolio", method: "get", auth: true, timeout: 1000 },
        { endpoint: "/api/backtest/strategies", method: "get", auth: true, timeout: 1000 },
        { endpoint: "/api/calendar/earnings", method: "get", auth: false, timeout: 2000 }
      ];
    ...
    test("should return 408 or 504 for server-side timeouts", async () => {
      ...
      for (const test of serverTimeoutEndpoints) {
        ...
        if (response.status === 408 || response.status === 504) {
          expect(response.body).toHaveProperty("success", false);
          expect(response.body).toHaveProperty("error");
          expect(response.body.error).toMatch(/timeout|slow|response|processing/i);
  ...
  describe("Database Query Timeout Scenarios", () => {
      ...
      for (const test of databaseQueryEndpoints) {
        ...
        if (response.status >= 400) {
          ...
          if (response.status === 500 && response.body.error) {
            expect(response.body.error).not.toMatch(/query.*timeout|connection.*timeout|pg.*timeout/i);
          }
        }
      }
    });
    ...
    test("should handle connection pool timeout scenarios", async () => {
      ...
      expect(successfulRequests.length + timeoutRequests.length + errorRequests.length).toBe(concurrentConnections);
      ...
      if (timeoutRequests.length > 0) {
        expect(timeoutRequests.length).toBeLessThan(concurrentConnections * 0.5); // Less than 50% timeouts
      }
    });
  });
  ...
  describe("External Service Timeout Scenarios", () => {
    test("should handle external API timeouts properly", async () => {
      ...
      const externalServiceEndpoints = [
        { endpoint: "/api/market/overview", auth: false, expectedTimeout: 10000 },
        { endpoint: "/api/calendar/earnings", auth: false, expectedTimeout: 10000 }
      ];
      ...
      for (const test of externalServiceEndpoints) {
        ...
        if (response.status === 504 || response.status === 408) {
          expect(response.body).toHaveProperty("success", false);
          expect(response.body).toHaveProperty("error");
          expect(response.body.error).toMatch(/timeout|gateway|external|service/i);
          
          // Should not expose external service URLs or details
          expect(response.body.error).not.toMatch(/http:\/\/|https:\/\/|api\..*\.com/);
        }
      }
    });
    ...
    test("should implement circuit breaker patterns for repeated timeouts", async () => {
      ...
      if (timeoutRequests.length > rapidRequests * 0.5) {
        ...
        const avgLaterDuration = laterRequests.reduce((sum, r) => sum + r.duration, 0) / laterRequests.length;
  ...
  describe("Streaming and Long-Running Operation Timeouts", () => {
    ...
    test("should provide timeout warnings for slow operations", async () => {
      ...
      const slowOperationEndpoints = [
        { endpoint: "/api/backtest/run", method: "post", body: { strategy: "complex" }, auth: true }
      ];
      ...
      for (const test of slowOperationEndpoints) {
        ...
        if (response.status === 200 && duration > 5000) { // If took more than 5 seconds
  ...
  describe("Timeout Recovery and Resilience", () => {
    test("should recover gracefully after timeout scenarios", async () => {
      ...
      const recoveryResponse = await request(app).get(testEndpoint).timeout(10000);
      ...
      const successResults = stressResults.filter(r => !r.timeout && r.status === 200);
    ...
    test("should maintain system health during timeout scenarios", async () => {
      ...
      const slowRequestPromises = Array.from({ length: 3 }, () =>
        request(app).get("/api/calendar/earnings").timeout(1000).catch(() => ({ timeout: true }))
      );
      ...
      const [healthResponse, ...slowResults] = await Promise.all([healthPromise, ...slowRequestPromises]);
    ...
    test("should provide appropriate timeout configuration", async () => {
      ...
      for (const test of timeoutTests) {
        ...
        try {
          const response = await request(app).get(testEndpoint).timeout(test.timeout);
          const duration = Date.now() - startTime;
  ...
  describe("Timeout Error Response Consistency", () => {
    test("should return consistent timeout error responses", async () => {
      const timeoutTestScenarios = [
        { endpoint: "/api/portfolio/analyze", method: "post", body: { symbols: ["AAPL"] }, auth: true },
    ...
    test("should not expose internal timeout configurations", async () => {
      ...
      for (const test of sensitiveEndpoints) {
        ...
        if (response.status === 408 || response.status === 504) {
          ...
          expect(errorMessage).not.toMatch(/connection.*pool|database.*timeout/);
