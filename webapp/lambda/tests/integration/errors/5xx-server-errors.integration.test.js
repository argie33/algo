    ...
    test("should handle database connection failures gracefully", async () => {
      ...
      for (const test of databaseDependentEndpoints) {
        ...
        if (response.status === 500) {
          ...
          expect(response.body.error).not.toMatch(/connection|pool|timeout|database|pg|postgresql/i);
          expect(response.body.error).not.toMatch(/ECONNREFUSED|ETIMEDOUT|ENOTFOUND/);
        }
      }
    });
    ...
    test("should handle application errors with proper formatting", async () => {
      ...
      for (const test of errorProneEndpoints) {
        ...
        if (response.status === 500) {
          ...
          expect(response.body.error).not.toMatch(/stack trace|at Object\.|at Function\./);
          expect(response.body.error).not.toMatch(/node_modules|\/app\/|\/usr\/|\/var\//);
        }
      }
    });
    ...
    test("should handle memory or resource exhaustion", async () => {
      ...
      for (const test of stressTests) {
        ...
        if (response.status >= 500) {
          ...
          expect(response.body.error).not.toMatch(/memory|heap|cpu|process|system/i);
        }
      }
    });
    ...
    test("should handle upstream service failures", async () => {
      ...
      for (const test of externalServiceEndpoints) {
        ...
        if (response.status === 502) {
          expect(response.body).toHaveProperty("success", false);
          expect(response.body).toHaveProperty("error");
          expect(response.body.error).toMatch(/service|gateway|upstream|external/i);
          
          // Should not expose upstream service details
          expect(response.body.error).not.toMatch(/http:\/\/|https:\/\/|localhost|127\.0\.0\.1/);
        }
      }
    });
  });
    ...
    test("should handle service maintenance mode", async () => {
      ...
      for (const test of maintenanceTestEndpoints) {
        ...
        if (response.status === 503) {
          expect(response.body).toHaveProperty("success", false);
          expect(response.body).toHaveProperty("error");
          expect(response.body.error).toMatch(/unavailable|maintenance|overloaded/i);
    ...
    test("should handle database pool exhaustion", async () => {
      ...
      const concurrentRequests = Array.from({ length: 30 }, (_, i) =>
        request(app)
          ...
          .then(response => ({ id: i, status: response.status, body: response.body }))
    ...
    test("should handle cascading failures appropriately", async () => {
      ...
      cascadeResults.forEach(result => {
        if (result.hasError && result.status >= 500) {
          ...
          expect(result.errorMessage).not.toMatch(/stack|trace|node_modules|internal/i);
        }
      });
    ...
    test("should sanitize error messages from external sources", async () => {
      ...
      for (const test of externalDependentEndpoints) {
        ...
        if (response.status >= 500 && response.body?.error) {
          ...
          expect(errorMessage).not.toMatch(/http.*error|api.*key|endpoint.*not.*found/i);
          expect(errorMessage).not.toMatch(/timeout.*\d+ms|connection.*refused/i);
  ...
  describe("Cross-Route 5xx Error Consistency", () => {
    ...
    test("should provide consistent error codes across similar failures", async () => {
      ...
      const similarEndpoints = [
        { endpoint: "/api/portfolio/positions", category: "database-dependent" },
      ...
      for (const test of similarEndpoints) {
        ...
        if (test.endpoint.includes('portfolio') || test.endpoint.includes('backtest')) {
          requestBuilder = requestBuilder.set("Authorization", "Bearer dev-bypass-token");
        }
