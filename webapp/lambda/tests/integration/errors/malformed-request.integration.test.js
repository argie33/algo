    ...
    test("should handle malformed JSON gracefully", async () => {
      const malformedJsonTests = [
        ...
        },
        {
          ...
          description: "Escaped quotes with garbage",
        },
  ...
  describe("HTTP Header Malformation", () => {
    test("should handle malformed authorization headers", async () => {
      const malformedAuthHeaders = [
        ...
        {
          auth: "Bearer token\rcarriage",
          description: "Carriage return injection",
        },
        {
          auth: "Bearer " + "A".repeat(10000),
        ...
        { auth: "Bearer token\x80\x81\x82", description: "Non-ASCII bytes" },
        {
          ...
          description: "Basic auth on Bearer endpoint",
        },
      ];
    ...
    test("should handle malformed content-type headers", async () => {
      const malformedContentTypes = [
        {
          contentType: "application/json\x00",
          description: "Null byte in content type",
        },
        {
        ...
        {
          contentType: "application/json;" + "x".repeat(1000),
          description: "Extremely long parameters",
        },
        {
          contentType: "application\x80/json",
          description: "Non-ASCII in content type",
        },
        {
          contentType: "text/plain; boundary=" + "A".repeat(1000),
          description: "Long boundary parameter",
        },
      ];
    ...
    test("should handle custom header injection attempts", async () => {
      const headerInjectionTests = [
        ...
        },
        {
          ...
          description: "Header injection in value",
        },
        {
          headerName: "X-Custom-Header",
        ...
        {
          headerName: "X-Very-" + "Long-".repeat(100) + "Header",
          value: "test",
  ...
  describe("URL and Path Malformation", () => {
    test("should handle malformed URLs and paths", async () => {
      const malformedPaths = [
        {
          path: "/api/portfolio/../../../etc/passwd",
          description: "Directory traversal",
        },
        {
          path: "/api/portfolio/%2e%2e%2f%2e%2e%2fetc%2fpasswd",
          description: "URL encoded traversal",
        },
        { path: "/api/portfolio/\x00", description: "Null byte in path" },
        {
          path: "/api/portfolio/" + "A".repeat(1000),
          description: "Extremely long path",
        },
        {
          path: "/api/portfolio/\x80\x81\x82",
          description: "Non-ASCII in path",
        },
        {
          path: "/api/portfolio/<script>alert('xss')</script>",
          description: "XSS in path",
        },
      ];
    ...
    test("should handle malformed query parameters", async () => {
      const malformedQueryTests = [
        ...
        {
          query: "param\x00=value",
          description: "Null byte in parameter name",
        },
        {
          query: "param=value\x00",
    ...
    test("should sanitize error messages containing malformed input", async () => {
      ...
      for (const test of maliciousInputs) {
        ...
        if (response.status >= 400 && response.body?.error) {
          ...
          expect(errorMessage).not.toMatch(
            /<script>|DROP TABLE|\.\.\/|process\.exit/i
          );
