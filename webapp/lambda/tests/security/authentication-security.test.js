...
describe("Authentication Security Tests", () => {
  ...
  describe("Session Management Security", () => {
    test("should prevent session fixation attacks", async () => {
      const token1 = jwt.sign({ sub: validUserId }, jwtSecret, {
        expiresIn: "1h",
      });
      const token2 = jwt.sign({ sub: validUserId }, jwtSecret, {
        expiresIn: "1h",
      });
  ...
  describe("API Key Security", () => {
    ...
    test("should use different salts for different users", async () => {
      ...
      const user2Token = jwt.sign({ sub: "user-2" }, jwtSecret, {
        expiresIn: "1h",
      });
  ...
  describe("Input Validation Security", () => {
    ...
    test("should validate content-type for POST requests", async () => {
      ...
      expect(response.body.error).toMatch(
        /provider.*required|apikey.*required|invalid.*data/i
      );
    });
  });
  ...
  describe("Error Information Disclosure", () => {
    ...
    test("should use generic error messages for security-sensitive operations", async () => {
      ...
      expect(response.body.error).not.toMatch(
        /provider.*not.*found|invalid.*provider/i
      );
    });
  });
