...
const { initializeDatabase, closeDatabase, transaction, getPool } = require("../../utils/database");
...
describe("Concurrent Database Transaction Integration", () => {
    ...
    test("should handle multiple simultaneous read transactions", async () => {
      const concurrentReads = Array.from({ length: 10 }, (_, i) =>
        transaction(async (client) => {
          const result = await client.query("SELECT 1 as test_value, $1 as read_id", [i]);
  ...
  describe("Transaction Isolation Levels", () => {
    test("should maintain READ COMMITTED isolation", async () => {
      ...
      await transaction(async (client) => {
        ...
        await client.query("INSERT INTO test_isolation (id, value) VALUES (1, 100)");
      });
      ...
      const transaction1Promise = transaction(async (client) => {
        ...
        await client.query("UPDATE test_isolation SET value = 200 WHERE id = 1");
      ...
      const transaction2Promise = (async () => {
        ...
        await transaction(async (client) => {
          const result = await client.query("SELECT value FROM test_isolation WHERE id = 1");
    ...
    test("should prevent dirty reads", async () => {
      ...
      await transaction(async (client) => {
        ...
        await client.query("INSERT INTO test_dirty_read (id, value) VALUES (1, 500)");
      });
      ...
      const transaction1Promise = (async () => {
        ...
        try {
          ...
          // Update value
          await client.query("UPDATE test_dirty_read SET value = 999 WHERE id = 1");
      ...
      const transaction2Promise = (async () => {
        ...
        await transaction(async (client) => {
          const result = await client.query("SELECT value FROM test_dirty_read WHERE id = 1");
  ...
  describe("Deadlock Prevention and Recovery", () => {
    test("should handle potential deadlock scenarios gracefully", async () => {
      ...
      await transaction(async (client) => {
        ...
        await client.query("INSERT INTO test_deadlock_a (id, value) VALUES (1, 100), (2, 200)");
        await client.query("INSERT INTO test_deadlock_b (id, value) VALUES (1, 300), (2, 400)");
      });

      // Transaction 1: Lock A then B
      const transaction1Promise = transaction(async (client) => {
        await client.query("SELECT value FROM test_deadlock_a WHERE id = 1 FOR UPDATE");
        
        // Small delay
        await new Promise(resolve => setTimeout(resolve, 50));
        
        await client.query("SELECT value FROM test_deadlock_b WHERE id = 1 FOR UPDATE");
      ...
      const transaction2Promise = transaction(async (client) => {
        await client.query("SELECT value FROM test_deadlock_b WHERE id = 2 FOR UPDATE");
        
        // Small delay  
        await new Promise(resolve => setTimeout(resolve, 50));
        
        await client.query("SELECT value FROM test_deadlock_a WHERE id = 2 FOR UPDATE");
  ...
  describe("Connection Pool Stress Testing", () => {
    test("should handle connection pool exhaustion gracefully", async () => {
      ...
      const stressTransactions = Array.from({ length: maxConnections }, (_, i) =>
        transaction(async (client) => {
          // Hold connection for a bit
          await new Promise(resolve => setTimeout(resolve, 100 + Math.random() * 100));
          
          const result = await client.query("SELECT $1 as transaction_id", [i]);
          return result.rows[0].transaction_id;
        })
      );

      const results = await Promise.all(
        stressTransactions.map(t => t.catch(err => ({ error: err.message })))
      );
      ...
      expect(successfulResults.length + errorResults.length).toBe(maxConnections);
    });
    ...
    test("should recover from temporary connection issues", async () => {
      ...
      const quickTransactions = Array.from({ length: 50 }, (_, i) =>
        transaction(async (client) => {
          const result = await client.query("SELECT NOW() as timestamp, $1 as id", [i]);
