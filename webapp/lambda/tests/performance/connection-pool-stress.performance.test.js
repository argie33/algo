...
describe("Connection Pool Stress Integration", () => {
      ...
      for (let i = 0; i < poolSize; i++) {
        const connectionPromise = (async (id) => {
          ...
          try {
            ...
            const result = await client.query("SELECT $1 as connection_id", [
              id,
            ]);
          ...
          try {
            ...
            try {
              ...
              const result = await client.query(
                "SELECT $1 as id, NOW() as timestamp",
                [id]
              );
              return {
                id: result.rows[0].id,
  ...
  `postgresql://${process.env.DB_USER || 'postgres'}:${process.env.DB_PASSWORD || 'password'}@${process.env.DB_HOST || 'localhost'}:${process.env.DB_PORT || 5432}/${process.env.DB_NAME || 'stocks'}`,
      ...
      try {
        ...
        const avgFirstWaveStart =
          firstWave.reduce((sum, r) => sum + r.requestStart, 0) /
          firstWave.length;
        const avgSecondWaveStart =
          secondWave.reduce((sum, r) => sum + r.requestStart, 0) /
          secondWave.length;
  ...
  `postgresql://${process.env.DB_USER || 'postgres'}:${process.env.DB_PASSWORD || 'password'}@${process.env.DB_HOST || 'localhost'}:${process.env.DB_PORT || 5432}/${process.env.DB_NAME || 'stocks'}`,
      ...
      try {
                ...
                try {
                  ...
                  try {
                    await new Promise((resolve) =>
                      setTimeout(resolve, phase.holdTime)
                    );
                    const result = await client.query(
                      "SELECT $1 as id, $2 as phase",
        ...
        expect(recoverySuccess.length).toBeGreaterThan(
          recoveryPhase.length * 0.7
        );
      } finally {
        ...
        for (let i = 0; i < 4; i++) {
          postIdleConnections.push(
            (async (id) => {
              const client = await idleTestPool.connect();
              try {
                const result = await client.query("SELECT $1 as post_idle_id", [
                  id,
                ]);
  ...
  describe("Transaction Stress Testing", () => {
    test("should handle many concurrent transactions", async () => {
      ...
      expect(totalRecords).toBe(
        successfulTransactions.length * operationsPerTransaction
      );
    });
    ...
    test("should maintain transaction isolation under stress", async () => {
      ...
      const finalState = await transaction(async (client) => {
        const result = await client.query(
          "SELECT counter FROM test_isolation_stress WHERE id = 1"
        );
