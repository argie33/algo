...
const mockReturns = [
  0.5, 0.3, -0.2, 0.8, 0.1, -0.5, 1.2, 0.2, -0.1, 0.4, 0.6, -0.3, 0.9, 0.2, 0.5,
  1.1, -0.4, 0.7, 0.3, 0.6, -0.2, 0.8, 0.4, 0.2, 0.9, 0.5, -0.3, 1.0, 0.3, 0.6,
  0.5, 0.3, -0.2, 0.8, 0.1, -0.5, 1.2, 0.2, -0.1, 0.4, 0.6, -0.3, 0.9, 0.2, 0.5,
  1.1, -0.4, 0.7, 0.3, 0.6, -0.2, 0.8, 0.4, 0.2, 0.9, 0.5, -0.3, 1.0, 0.3, 0.6,
  0.5, 0.3, -0.2, 0.8, 0.1, -0.5, 1.2, 0.2, -0.1, 0.4, 0.6, -0.3, 0.9, 0.2, 0.5,
  1.1, -0.4, 0.7, 0.3, 0.6, -0.2, 0.8, 0.4, 0.2, 0.9, 0.5, -0.3, 1.0, 0.3, 0.6,
...
describe("Professional Metrics Calculations", () => {
  it("should calculate total return correctly", () => {
    ...
    assert.ok(
      totalReturn > 0,
      "Total return should be positive for this dataset"
    );
  });
  ...
  it("should calculate Sharpe Ratio", () => {
    ...
    const sharpeRatio =
      ((mean - riskFreeRate) * 252) / (stdDev * Math.sqrt(252));
    assert.ok(
  ...
  it("should calculate Sortino Ratio", () => {
    ...
    assert.ok(
      typeof sortinoRatio === "number",
      "Sortino Ratio should be a number"
    );
  });
  ...
  it("should calculate Skewness", () => {
    ...
    const skewness =
      mockReturns.reduce(
        (sum, x) => sum + Math.pow((x - mean) / stdDev, 3),
        0
      ) / mockReturns.length;
  ...
  it("should calculate Kurtosis", () => {
    ...
    const kurtosis =
      mockReturns.reduce(
        (sum, x) => sum + Math.pow((x - mean) / stdDev, 4),
        0
      ) /
        mockReturns.length -
      3;
  ...
  it("should calculate Conditional Value at Risk (CVaR)", () => {
    ...
    const var95Index = Math.floor(sortedReturns.length * 0.05);
    const cvar95 =
      sortedReturns.slice(0, var95Index + 1).reduce((a, b) => a + b, 0) /
      (var95Index + 1);
  ...
  it("should calculate concentration metrics", () => {
    ...
    assert.ok(
      herfindahl > 0 && herfindahl <= 1,
      "Herfindahl should be between 0-1"
    );
  ...
  it("should pass all metric boundary checks", () => {
    ...
    const metrics = {
      ...
      sharpeRatio:
        ((calculateMean(mockReturns) - 2 / 100 / 252) * 252) /
        (calculateStdDev(mockReturns) * Math.sqrt(252)),
      maxDrawdown: 45.5,
      winRate: 65.0,
      concentration: 0.85,
    };
    ...
    assert.ok(
      metrics.volatility > 0 && metrics.volatility < 200,
      "Volatility should be 0-200%"
    );
    assert.ok(
    ...
    assert.ok(
      metrics.concentration > 0 && metrics.concentration <= 1,
