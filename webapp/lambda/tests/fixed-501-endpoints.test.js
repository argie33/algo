...
async function testEndpoint(name, url, expectedStatus = 200, testDataShape = true) {
  try {
    ...
    if (success) {
      ...
      return { success: false, status, data, error: `Status mismatch: expected ${expectedStatus}, got ${status}` };
    }
  } catch (error) {
    if (error.code === 'ECONNREFUSED') {
      ...
      if (status === expectedStatus) {
        ...
        log(`❌ ${name}: Expected ${expectedStatus}, got ${status}`, 'red');
        return { success: false, status, data: error.response.data, error: `Status mismatch: expected ${expectedStatus}, got ${status}` };
      }
    } else {
...
async function waitForServer(maxRetries = 10) {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      const response = await axios.get(`${API_BASE}/health?quick=true`, { timeout: 2000 });
...
async function runTests() {
      ...
      if (symbolCount > 0) {
        ...
        if (firstSymbolData.metrics && Array.isArray(firstSymbolData.metrics)) {
          ...
          if (firstSymbolData.metrics.length > 0) {
            ...
            const requiredFields = ['symbol', 'eps_reported', 'eps_estimate', 'report_date'];
            const hasRequiredFields = requiredFields.every(field => firstMetric[field] !== undefined);
        ...
        log('   ⚠️  No symbol data found (might be expected in test environment)', 'yellow');
      }
  ...
  if (healthDebugTest.success && healthDebugTest.data) {
    ...
    if (data.success && data.debugInfo && data.debugInfo.environment === 'local_development') {
      log('   ✅ Health debug shows correct local development configuration', 'green');
  ...
  log('\n============================================================', 'bold');
  log(`🎯 FINAL RESULTS: ${successCount}/${totalTests} endpoints working (${successRate}%)`, 'bold');
  log('============================================================', 'bold');
