...
describe('AI Scan Endpoint', () => {
  ...
  describe('GET /ai-scan', () => {
    ...
    it('respects limit parameter', async () => {
      ...
      await request(app)
        .get('/api/screener/ai-scan?limit=25')
        .expect(200);
    ...
    it('calculates AI scores correctly', async () => {
      ...
      const aaplResult = response.body.data.results.find(r => r.symbol === 'AAPL');
      const tslaResult = response.body.data.results.find(r => r.symbol === 'TSLA');
    ...
    it('generates appropriate trading signals', async () => {
      ...
      expect(aaplResult.signals).toEqual(
        expect.arrayContaining(['Strong Buy', 'High Volume', 'RSI Bullish', 'Breakout'])
      );
      
      expect(tslaResult.signals).toEqual(
        expect.arrayContaining(['Oversold', 'High Volume', 'Potential Reversal'])
      );
    });
    ...
    it('validates limit parameter bounds', async () => {
      ...
      // Test maximum limit
      await request(app)
        .get('/api/screener/ai-scan?limit=101')
        .expect(200);
      ...
      // Test minimum limit
      await request(app)
        .get('/api/screener/ai-scan?limit=0')
        .expect(200);
  ...
  describe('AI Scoring Algorithm', () => {
    ...
    it('penalizes extreme RSI values appropriately', async () => {
      ...
      expect(oversoldResponse.body.data.results[0].aiScore)
        .toBeGreaterThan(overboughtResponse.body.data.results[0].aiScore);
    });
  });
  ...
  describe('Signal Generation', () => {
    it('generates breakout signals correctly', async () => {
      ...
      expect(response.body.data.results[0].signals)
        .toContain('Breakout');
    });
    ...
    it('generates reversal signals for oversold conditions', async () => {
      ...
      expect(response.body.data.results[0].signals)
        .toEqual(expect.arrayContaining(['Oversold', 'Potential Reversal']));
    });
  });
});