...
describe('Scores Routes', () => {
  ...
  describe('GET /scores/', () => {
    ...
    test('should handle search filter', async () => {
      ...
      expect(query).toHaveBeenCalledWith(
        expect.stringContaining('AND (ss.symbol ILIKE $1 OR ss.security_name ILIKE $1)'),
  ...
  describe('GET /scores/:symbol', () => {
    ...
    test('should return detailed scores for valid symbol', async () => {
      ...
      expect(response.body.interpretation.overall).toContain('Exceptional investment opportunity');
      expect(response.body.interpretation.recommendation).toContain('BUY');

      expect(consoleSpy).toHaveBeenCalledWith('Getting detailed scores for AAPL');
      consoleSpy.mockRestore();
    });
    ...
    test('should return 404 for non-existent symbol', async () => {
      ...
      expect(response.body.error).toBe('Symbol not found or no scores available');
    ...
    test('should include detailed breakdown with descriptions', async () => {
      ...
      expect(response.body.detailedBreakdown.quality.description).toContain('financial statement quality');
      expect(response.body.detailedBreakdown.value.description).toContain('P/E, P/B, EV/EBITDA');
      expect(response.body.detailedBreakdown.growth.description).toContain('revenue growth');
  ...
  describe('GET /scores/top/:category', () => {
    ...
    test('should enforce maximum limit', async () => {
      ...
      await request(app)
        .get('/scores/top/growth?limit=500')
        .expect(200);
    ...
    test('should test all valid categories', async () => {
      const categories = ['composite', 'quality', 'value', 'growth', 'momentum', 'sentiment', 'positioning'];
  ...
  describe('Score interpretation logic', () => {
    ...
    const mockSectorData = {
      rows: [{ avg_composite: 75.5, avg_quality: 80.2, avg_value: 70.1, peer_count: 50 }]
    };
    ...
    test('should generate BUY recommendation for high scores', async () => {
      ...
      expect(response.body.interpretation.overall).toContain('Exceptional investment opportunity');
      expect(response.body.interpretation.recommendation).toContain('BUY');
      expect(response.body.interpretation.strengths).toContain('High-quality financial statements and management');
    ...
    test('should generate SELL recommendation for low scores', async () => {
      ...
      expect(response.body.interpretation.overall).toContain('Poor investment profile');
      expect(response.body.interpretation.recommendation).toContain('SELL');
      expect(response.body.interpretation.concerns).toContain('Weak financial quality');
  ...
  describe('Response format consistency', () => {
    test('should return consistent timestamp format across endpoints', async () => {
      ...
      const sectorsResponse = await request(app).get('/scores/sectors/analysis');
