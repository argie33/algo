...
describe('Calendar Routes Unit Tests', () => {
  ...
  describe('GET /calendar/economic', () => {
    ...
    test('should return economic calendar with custom parameters', async () => {
      const response = await request(app)
        .get('/calendar/economic?country=EU&importance=high&days_ahead=7&limit=10')
        .expect(200);
    ...
    test('should filter by importance correctly', async () => {
      ...
      const highImportanceEvents = response.body.data.economic_events.filter(e => e.importance === 'high');
      expect(highImportanceEvents.length).toBe(response.body.data.economic_events.length);
    });
    ...
    test('should include available filters', async () => {
      ...
      expect(response.body.data.available_filters).toHaveProperty('importance_levels');
      expect(response.body.data.available_filters).toHaveProperty('categories');
      expect(Array.isArray(response.body.data.available_filters.countries)).toBe(true);
