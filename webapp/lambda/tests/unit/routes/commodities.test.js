...
describe('Commodities Routes', () => {
  ...
  describe('GET /api/commodities/', () => {
    ...
    test('should return all expected endpoints', async () => {
      ...
      expect(endpoints).toContain('GET /commodities/categories - Commodity categories');
      expect(endpoints).toContain('GET /commodities/prices - Current commodity prices');
      expect(endpoints).toContain('GET /commodities/market-summary - Market overview');
      expect(endpoints).toContain('GET /commodities/correlations - Price correlations');
      expect(endpoints).toContain('GET /commodities/news - Latest commodity news');
    });
  });
  ...
  describe('GET /api/commodities/categories', () => {
    ...
    test('should include energy category with correct structure', async () => {
      ...
      const energyCategory = response.body.data.find(cat => cat.id === 'energy');
    ...
    test('should include precious metals category', async () => {
      ...
      const preciousMetals = response.body.data.find(cat => cat.id === 'precious-metals');
    ...
    test('should include base metals category', async () => {
      ...
      const baseMetals = response.body.data.find(cat => cat.id === 'base-metals');
    ...
    test('should include agriculture category', async () => {
      ...
      const agriculture = response.body.data.find(cat => cat.id === 'agriculture');
    ...
    test('should calculate total weight correctly', async () => {
      ...
      const totalWeight = response.body.data.reduce((sum, cat) => sum + cat.weight, 0);
  ...
  describe('GET /api/commodities/prices', () => {
    ...
    test('should include required price fields', async () => {
      ...
      if (response.status === 200) {
        const goldPrice = response.body.data.find(item => item.symbol === 'GC');
    ...
    test('should limit results when limit parameter provided', async () => {
      const response = await request(app)
        .get('/api/commodities/prices?limit=3');
  ...
  describe('GET /api/commodities/market-summary', () => {
    ...
    test('should include top gainers and losers', async () => {
      ...
      if (response.status === 200) {
        ...
        if (response.body.data.performance && response.body.data.performance['1d']) {
          expect(response.body.data.performance['1d']).toHaveProperty('topGainer');
          expect(response.body.data.performance['1d']).toHaveProperty('topLoser');
        }
      }
    });
  ...
  describe('GET /api/commodities/correlations', () => {
    ...
    test('should filter correlations by minimum threshold', async () => {
      const response = await request(app)
        .get('/api/commodities/correlations?min_correlation=0.5');
  ...
  describe('GET /api/commodities/news', () => {
    ...
    test('should filter by category when provided', async () => {
      const response = await request(app)
        .get('/api/commodities/news?category=energy');
  ...
  describe('Response Format Validation', () => {
    ...
    test('should use consistent success field for API responses', async () => {
      const apiEndpoints = ['/categories', '/prices', '/market-summary', '/correlations'];
  ...
  describe('Edge Cases and Error Handling', () => {
    test('should handle malformed query parameters gracefully', async () => {
      const response = await request(app)
        .get('/api/commodities/prices?limit=invalid&category=123');
    ...
    test('should handle very large limit values', async () => {
      const response = await request(app)
        .get('/api/commodities/prices?limit=999999');
