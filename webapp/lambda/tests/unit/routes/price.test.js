...
describe("Price Route - Unit Tests", () => {
  ...
  describe("GET /api/price/:symbol", () => {
    ...
    test("should handle database errors", async () => {
      ...
      const response = await request(app).get("/api/price/AAPL").expect(500);
  ...
  describe("GET /api/price/futures/:symbol", () => {
    test("should return futures pricing data", async () => {
      ...
      expect(response.body.data.contract_details).toHaveProperty(
        "underlying_symbol"
      );
      expect(response.body.data.pricing_analysis).toHaveProperty(
        "theoretical_price"
      );
  ...
  describe("GET /api/price/:symbol/prediction", () => {
    test("should return price prediction analysis", async () => {
      ...
      expect(response.body.data.current_analysis).toHaveProperty(
        "current_price"
      );
