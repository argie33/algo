...
describe('Risk Routes Unit Tests', () => {
  ...
  describe('GET /risk/health', () => {
    test('should return health status without authentication', async () => {
      ...
      expect(response.body).toHaveProperty('message', 'Risk Analysis service is running');
  ...
  describe('GET /risk', () => {
    test('should return risk API information without authentication', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty('message', 'Risk Analysis API - Ready');
      expect(response.body).toHaveProperty('status', 'operational');
      expect(response.body).toHaveProperty('timestamp');
  ...
  describe('GET /risk/analysis (authenticated)', () => {
    test('should return risk analysis with default parameters', async () => {
      ...
      const mockPriceData = {
        rows: [
          { symbol: 'AAPL', date: '2023-01-01', close: 150.00 },
          { symbol: 'AAPL', date: '2023-01-02', close: 152.00 },
          { symbol: 'GOOGL', date: '2023-01-01', close: 2800.00 },
          { symbol: 'GOOGL', date: '2023-01-02', close: 2750.00 }
        ]
      };
      ...
      expect(response.body.data.portfolio_summary).toHaveProperty('holdings_count', 2);
