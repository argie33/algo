...
describe('Dividend Route - Comprehensive Unit Tests', () => {
  ...
  describe('GET /api/dividend/history/:symbol', () => {
    ...
    test('should handle query parameters gracefully', async () => {
      ...
      expect(response.body.data.symbol).toBe('AAPL');
      expect(response.body.success).toBe(true);
      expect(response.body.data.dividend_history.length).toBeLessThanOrEqual(100);
    });
  ...
  describe('GET /api/dividend/calendar', () => {
    ...
    test('should handle query parameters correctly', async () => {
      ...
      const response = await request(app)
        .get('/api/dividend/calendar?days=7&symbol=AAPL&min_yield=2.0&max_yield=5.0&limit=25&sort_by=yield')
        .expect(200);
    ...
    test('should handle string dividend amounts and yields correctly', async () => {
      ...
      expect(response.body.data.dividend_calendar[0].dividend_amount).toBe(0.24);
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    test('should handle extreme query parameters gracefully', async () => {
      ...
      const response = await request(app)
        .get('/api/dividend/calendar?days=99999&limit=99999&min_yield=-100&max_yield=100')
        .expect(501);
    ...
    test('should handle SQL injection attempts safely', async () => {
      ...
      await request(app)
        .get('/api/dividend/calendar?symbol=AAPL\'; DROP TABLE dividend_calendar; --')
        .expect(501);
