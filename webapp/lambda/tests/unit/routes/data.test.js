...
describe("Data Routes Unit Tests", () => {
  ...
  describe("GET /data", () => {
    ...
    test("should handle errors gracefully", async () => {
      ...
      expect(response.body).toHaveProperty(
        "error",
        "Failed to get data API information"
      );
  ...
  describe("GET /data/:symbol", () => {
    test("should return comprehensive data for a symbol", async () => {
      ...
      const response = await request(app).get("/data/aapl");
    ...
    test("should handle symbol case conversion", async () => {
      ...
      const response = await request(app).get("/data/tsla");
    ...
    test("should return 404 when no data found", async () => {
      ...
      const response = await request(app).get("/data/INVALID");
    ...
    test("should return data when only price data exists", async () => {
      const mockPriceData = { symbol: "AAPL", close: 150.0 };
  ...
  describe("GET /data/historical/:symbol", () => {
    test("should return historical data for a symbol", async () => {
      ...
      const response = await request(app).get("/data/historical/aapl");
    ...
    test("should handle query parameters", async () => {
      ...
      const response = await request(app).get("/data/historical/aapl").query({
        start: "2024-01-01",
        end: "2024-01-31",
        limit: "25",
      });
    ...
    test("should return 404 when no historical data found", async () => {
      ...
      const response = await request(app).get("/data/historical/INVALID");

      expect(response.status).toBe(404);
      expect(response.body).toHaveProperty(
        "error",
        "No historical data available for symbol INVALID"
      );
    });
  ...
  describe("GET /data/realtime/:symbol", () => {
    test("should return real-time data for a symbol", async () => {
      ...
      const response = await request(app).get("/data/realtime/aapl");
    ...
    test("should return 404 when no real-time data found", async () => {
      ...
      const response = await request(app).get("/data/realtime/INVALID");

      expect(response.status).toBe(404);
      expect(response.body).toHaveProperty(
        "error",
        "No real-time data available for symbol INVALID"
      );
    });

    test("should add real_time flag and last_updated timestamp", async () => {
      const mockData = { symbol: "AAPL", close: 150.0 };
    ...
    test("should handle database errors", async () => {
      ...
      expect(response.body).toHaveProperty(
        "error",
        "Failed to retrieve real-time data for AAPL"
      );
  ...
  describe("GET /data/bulk", () => {
    test("should return bulk data for multiple symbols", async () => {
      const mockBulkData = [
        ...
        { symbol: "GOOGL", close: 145.3, date: "2024-01-15" },
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty("requested_symbols", [
        "AAPL",
        "GOOGL",
        "MSFT",
      ]);
      expect(response.body).toHaveProperty("found_symbols", [
        "AAPL",
        "GOOGL",
        "MSFT",
      ]);
    ...
    test("should handle symbols with spaces and case conversion", async () => {
      ...
      expect(response.body.requested_symbols).toEqual([
        "AAPL",
        "GOOGL",
        "MSFT",
      ]);
    ...
    test("should return 400 when symbols parameter is missing", async () => {
      const response = await request(app).get("/data/bulk");
    ...
    test("should handle database errors", async () => {
      ...
      expect(response.body).toHaveProperty(
        "error",
        "Failed to retrieve bulk data"
      );
      expect(response.body.details).toHaveProperty("requested_symbols", [
        "AAPL",
        "GOOGL",
      ]);
  ...
  describe("Edge cases and error handling", () => {
    test("should handle special characters in symbols", async () => {
      ...
      const response = await request(app).get("/data/TEST@123");
    ...
    test("should handle non-numeric limit parameters", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.any(String), ["AAPL", NaN]);
    });
  ...
  describe("Response format validation", () => {
    ...
    test("should preserve data structure for symbol endpoint", async () => {
      const mockPrice = { symbol: "AAPL", close: 150.0 };
