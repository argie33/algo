...
describe('Screener Routes Unit Tests', () => {
  ...
  describe('GET /screener', () => {
    test('should return screener API overview without authentication', async () => {
      ...
      expect(response.body.data.available_endpoints).toContain('GET /screener/screen - Main stock screening with filters');
      expect(response.body.data.available_endpoints).toContain('GET /screener/templates - Pre-built screening templates');
  ...
  describe('GET /screener/screen (authenticated)', () => {
    ...
    test('should handle multiple filters combined', async () => {
      ...
      expect(response.status).toBe(200);
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringMatching(/pd\.close >= .* AND pd\.close <= .* AND cp\.market_cap >= .* AND cp\.sector = .* AND km\.trailing_pe >= .* AND km\.trailing_pe <=/),
