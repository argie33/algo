...
describe("Trading Routes Unit Tests", () => {
  ...
  describe("GET /trading/orders", () => {
    test("should return user orders with pagination", async () => {
      const response = await request(app).get("/trading/orders").expect(200);
    ...
    test("should filter orders by status", async () => {
      ...
      if (response.body.data.orders.length > 0) {
        response.body.data.orders.forEach((order) => {
          expect(["pending", "open", "partially_filled"]).toContain(
            order.status
          );
        });
      }
  ...
  describe("Trading Error Handling", () => {
    ...
    test("should handle market closed scenarios", async () => {
      const response = await request(app).post("/trading/orders").send({
        symbol: "AAPL",
        quantity: 100,
        side: "buy",
        type: "market",
  ...
  afterAll(async () => {
    try {
      ...
      await query(`DELETE FROM trade_history WHERE user_id = $1`, [
        "test-user-123",
      ]);
      await query(`DELETE FROM portfolio_summary WHERE user_id = $1`, [
        "test-user-123",
      ]);
      await query(`DELETE FROM trading_strategies WHERE user_id = $1`, [
        "test-user-123",
      ]);
