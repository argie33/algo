...
const mockResponseFormatter = (req, res, next) => {
  ...
  res.error = (message, statusCode, details) => {
    res.status(statusCode || 500).json({ success: false, error: message, details });
  };
  next();
};
...
describe('New Endpoints Unit Tests', () => {
  ...
  describe('Market Correlation Endpoint', () => {
    ...
    test('GET /correlation - should return correlation matrix with default parameters', async () => {
      ...
      expect(response.body.data.correlation_matrix).toHaveProperty('statistics');
    ...
    test('GET /correlation with period parameter - should use specified period', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.correlation_matrix.period_analysis.period).toBe('1W');
    });
    ...
    test('GET /correlation - should include analysis and recommendations', async () => {
      ...
      expect(response.body.data.analysis).toHaveProperty('diversification_score');
  ...
  describe('Momentum Scoring Endpoint', () => {
    ...
    test('GET /momentum - should return momentum scores with default parameters', async () => {
      ...
      if (scores.length > 0) {
        ...
        expect(['Strong', 'Moderate', 'Weak']).toContain(score.signals.strength);
  ...
  describe('Active Orders Endpoint', () => {
    ...
    test('GET /active - should return active orders with default parameters', async () => {
      ...
      if (orders.length > 0) {
        ...
        expect(['PENDING', 'PARTIALLY_FILLED', 'PENDING_CANCEL', 'PENDING_REPLACE']).toContain(order.status);
        expect(['LIMIT', 'STOP', 'STOP_LIMIT', 'MARKET', 'TRAILING_STOP']).toContain(order.order_type);
  ...
  describe('Symbol Financial Ratios Endpoint', () => {
    ...
    test('GET /ratios/:symbol - should include peer comparison', async () => {
      ...
      expect(response.body.data.peer_comparison).toHaveProperty('industry_averages');
      expect(response.body.data.peer_comparison).toHaveProperty('percentile_ranking');
      expect(response.body.data.peer_comparison).toHaveProperty('relative_performance');
    ...
    test('GET /ratios/:symbol - should include analysis and recommendations', async () => {
      ...
      expect(response.body.data.analysis.overall_score).toBeGreaterThanOrEqual(0);
      expect(response.body.data.analysis.overall_score).toBeLessThanOrEqual(100);
    });
