...
describe("Sentiment Engine", () => {
  describe("Market Sentiment Calculation", () => {
    ...
    test("should return error structure on failure", () => {
      ...
      sentimentEngine.calculateMarketSentiment = jest.fn().mockImplementation(() => {
  ...
  describe("Text Sentiment Analysis", () => {
    test("should analyze positive text correctly", async () => {
      const positiveText = "This is excellent news! The company shows great growth and bullish trends with huge profits.";
      
      const result = await sentimentEngine.analyzeSentiment(positiveText, "AAPL");
    ...
    test("should analyze negative text correctly", async () => {
      const negativeText = "This is terrible news. The company shows poor performance and bearish trends with massive losses.";
      
      const result = await sentimentEngine.analyzeSentiment(negativeText, "MSFT");
    ...
    test("should analyze neutral text correctly", async () => {
      const neutralText = "The company announced quarterly earnings today. The market opened at normal levels.";
      
      const result = await sentimentEngine.analyzeSentiment(neutralText, "GOOGL");
    ...
    test("should handle mixed sentiment text", async () => {
      const mixedText = "The company shows excellent growth but faces terrible challenges. Good profits despite poor market conditions.";
    ...
    test("should handle empty or invalid text inputs", async () => {
      ...
      expect(undefinedResult.error).toBe("Text is required for sentiment analysis");
      ...
      expect(numberResult.error).toBe("Text is required for sentiment analysis");
      
      const objectResult = await sentimentEngine.analyzeSentiment({});
      expect(objectResult.error).toBe("Text is required for sentiment analysis");
    });
    ...
    test("should handle very long text", async () => {
      const longText = "excellent ".repeat(100) + "terrible ".repeat(49) + "terrible";
    ...
    test("should handle special characters and formatting", async () => {
      const textWithSpecialChars = "This is EXCELLENT news!!! The stock went UP 📈 and profits are GREAT!!!";
      
      const result = await sentimentEngine.analyzeSentiment(textWithSpecialChars);
    ...
    test("should handle case insensitive analysis", async () => {
      const mixedCaseText = "EXCELLENT news! Great PROFITS and POSITIVE trends!";
    ...
    test("should calculate confidence based on sentiment word density", async () => {
      ...
      const lowDensityText = "the company announced quarterly earnings results today and provided excellent guidance";
      
      const highDensityResult = await sentimentEngine.analyzeSentiment(highDensityText);
      const lowDensityResult = await sentimentEngine.analyzeSentiment(lowDensityText);
      
      expect(highDensityResult.confidence).toBeGreaterThan(lowDensityResult.confidence);
    });
    ...
    test("should round numerical values appropriately", async () => {
      const result = await sentimentEngine.analyzeSentiment("excellent good positive");
      
      expect(result.score).toEqual(Math.round(result.score * 100) / 100);
      expect(result.confidence).toEqual(Math.round(result.confidence * 100) / 100);
    });
  ...
  describe("Integration Tests", () => {
    test("should work with real financial news text", async () => {
      ...
      const result = await sentimentEngine.analyzeSentiment(financialNews, "AAPL");
    ...
    test("should work with scoreToLabel integration", async () => {
      ...
      const positiveResult = await sentimentEngine.analyzeSentiment(positiveText);
      const negativeResult = await sentimentEngine.analyzeSentiment(negativeText);
      
      expect(positiveResult.label).toBe("positive");
      expect(negativeResult.label).toBe("negative");
      
      // Test direct scoreToLabel calls
      expect(sentimentEngine.scoreToLabel(positiveResult.score)).toBe("positive");
      expect(sentimentEngine.scoreToLabel(negativeResult.score)).toBe("negative");
    });
  });
});