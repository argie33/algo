...
const createTestApp = (userId = "test-user-id") => {
  ...
  app.use((req, res, next) => {
    ...
    res.error = (message, status = 500, data = {}) => res.status(status).json({ success: false, error: message, ...data });
    res.unauthorized = (message, data = {}) => res.status(401).json({ success: false, error: message, ...data });
    next();
  });
...
describe("Onboarding Flow Persistence Issues", () => {
  ...
  describe("API Key Storage and Retrieval Edge Cases", () => {
    ...
    it("should handle API key service circuit breaker", async () => {
      ...
      expect(response.body.message).toContain("API key service is experiencing issues");
    });
  });
