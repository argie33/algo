...
describe('Database Utility Unit Tests', () => {
  ...
  describe('Database Query Operations', () => {
    ...
    test('should handle query errors gracefully', async () => {
      ...
      await expect(database.query('INVALID SQL')).rejects.toThrow('Query execution failed');
    });
    ...
    test('should throw error when database unavailable during query', async () => {
      ...
      await expect(freshDatabase.query('SELECT 1')).rejects.toThrow('Database not available - running in fallback mode');
    });
  });
  ...
  describe('Database Transactions', () => {
    ...
    test('should rollback transaction on error', async () => {
      const mockCallback = jest.fn(() => Promise.reject(new Error('Transaction failed')));
    ...
    test('should throw error when pool not available', async () => {
      ...
      await expect(freshDatabase.transaction(mockCallback)).rejects.toThrow('Database not initialized');
    });
  });
  ...
  describe('Edge Cases and Error Handling', () => {
    test('should handle undefined query parameters', async () => {
      ...
      await expect(database.query('SELECT 1', undefined)).resolves.not.toThrow();
    ...
    test('should handle AWS region configuration properly', async () => {
      ...
      process.env.DB_SECRET_ARN = 'arn:aws:secretsmanager:us-west-2:123456789:secret:test';
