...
describe("Advanced Portfolio Analytics Tests", () => {
  ...
  beforeAll(() => {
    ...
    app.use((req, res, next) => {
      res.success = (data, message) => res.json({ success: true, data, message });
  ...
  describe("Advanced Risk Metrics", () => {
    it("should calculate Value at Risk (VaR)", async () => {
      ...
      expect(riskMetrics.value_at_risk.var_99).toBeGreaterThan(riskMetrics.value_at_risk.var_95);
    });
    ...
    it("should calculate concentration risk (Herfindahl-Hirschman Index)", async () => {
      ...
      const concentrationRisk = response.body.data.risk_metrics.concentration_risk;
  ...
  describe("Portfolio Beta Calculation", () => {
    ...
    it("should handle unknown sectors gracefully", async () => {
      const mockMixedPortfolio = {
        rows: [
          { symbol: 'UNKNOWN1', percentage_allocation: 50, sector: 'Unknown Sector' },
      ...
      expect(response.body.success).toBe(true);
      expect(typeof response.body.data.risk_metrics.portfolio_beta).toBe('number');
    });
  });
  ...
  describe("Performance Metrics Integration", () => {
    ...
    it("should calculate maximum drawdown correctly", async () => {
      ...
      expect(maxDrawdown).toBeGreaterThan(0.10); // Should be around 13.6%
      expect(maxDrawdown).toBeLessThan(0.20);
    });
  });

  describe("Benchmark Integration", () => {
    it("should support multiple benchmark symbols", async () => {
      const mockPortfolioData = { rows: [{ total_pnl_percent: 5, date: "2024-01-01" }] };
