...
describe('Live Data Manager', () => {
  ...
  describe('Connection Management', () => {
    ...
    test('should enforce global connection limits', () => {
      // Fill up to limit
      for (let i = 0; i < liveDataManager.globalLimits.maxTotalConnections; i++) {
      ...
      // Try to exceed limit
      const result = liveDataManager.addConnection('conn-overflow', 'alpaca', ['GOOGL']);
  ...
  describe('Subscription Management', () => {
    ...
    test('should remove symbol when no subscribers left', () => {
      liveDataManager.addSubscription('MSFT', 'alpaca', 'conn-123', 'user-only');
    ...
    test('should get subscription list for user', () => {
      ...
      const userSubscriptions = liveDataManager.getUserSubscriptions('user-123');
  ...
  describe('Performance Metrics', () => {
    ...
    test('should update success rate based on errors', () => {
      ...
      // Track errors
      liveDataManager.trackError('polygon', { message: 'Error 1', timestamp: Date.now() });
      liveDataManager.trackError('polygon', { message: 'Error 2', timestamp: Date.now() });
  ...
  describe('Alerting and Monitoring', () => {
    ...
    test('should generate alerts for rate limit violations', () => {
      ...
      const alerts = liveDataManager.generateAlerts();
      const rateLimitAlert = alerts.find(alert => alert.type === 'rate_limit');
    ...
    test('should trigger connection limit alert', () => {
      ...
      const connectionsNearLimit = Math.ceil(liveDataManager.globalLimits.maxTotalConnections * 0.8);
      ...
      const alerts = liveDataManager.generateAlerts();
      const connectionAlert = alerts.find(alert => alert.type === 'connection_limit');
