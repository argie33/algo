...
describe("Alert System", () => {
  ...
  describe("Coverage Enhancement Tests", () => {
    ...
    test("should handle configuration updates for uncovered paths", () => {
      // Test webhook configuration that might be uncovered
    ...
    test("should handle configuration validation edge cases", () => {
    ...
    test("should handle complex configuration merging", () => {
      // Test complex configuration updates to cover uncovered paths
      const complexConfig = {
        thresholds: {
    ...
    test("should handle alert deduplication and timing", () => {
      ...
      if (timingAlerts.length > 0) {
        expect(timingAlerts[0].createdAt).toBeGreaterThanOrEqual(startTime);
      }
    });
