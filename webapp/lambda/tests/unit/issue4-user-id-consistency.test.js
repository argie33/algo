...
describe('Issue #4: User ID Type Consistency', () => {
  ...
  describe('manual-trades routes', () => {
    ...
    it('should use req.user.sub (UUID string) for user_id in POST requests', async () => {
      query.mockResolvedValue({
        rows: [{ id: 1, symbol: 'AAPL', side: 'BUY', quantity: 100, execution_price: 150 }],
        rowCount: 1,
      });
      ...
      const insertCall = query.mock.calls.find(call =>
        call[0] && call[0].includes('INSERT INTO trades')
      );
  ...
  describe('trades routes', () => {
    it('should use req.user.sub for user_id in GET /trades', async () => {
      ...
      const call = query.mock.calls.find(call =>
        call[0] && call[0].includes('FROM trades') && !call[0].includes('alpaca')
      );
    ...
    it('should filter /summary endpoint by authenticated user', async () => {
      ...
      const summaryCall = query.mock.calls.find(call =>
        call[0] && call[0].includes('FROM trades') && call[0].includes('WHERE user_id')
      );
