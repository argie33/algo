...
describe('schemaValidator', () => {
  ...
  describe('validateData', () => {
    ...
    test('should validate decimal precision', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Field "price" exceeds precision constraints (10,2)');
    });
    ...
    test('should validate invalid date format', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Field "date" must be a valid date format');
    });
  ...
  describe('validateTableExists', () => {
    ...
    test('should return false if table does not exist', async () => {
      ...
      const result = await schemaValidator.validateTableExists('nonexistent_table');
    ...
    test('should handle database errors', async () => {
      ...
      await expect(schemaValidator.validateTableExists('stocks')).rejects.toThrow('Database connection failed');
      expect(logger.error).toHaveBeenCalledWith('Error validating table existence:', error);
    });
  });
  ...
  describe('validateColumns', () => {
    ...
    test('should handle unknown table schema', async () => {
      await expect(schemaValidator.validateColumns('unknown_table')).rejects.toThrow('Unknown table schema: unknown_table');
    });
  });
  ...
  describe('generateCreateTableSQL', () => {
    ...
    test('should handle unknown table schema', () => {
      expect(() => schemaValidator.generateCreateTableSQL('unknown_table'))
