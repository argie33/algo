...
jest.mock('crypto', () => ({
  ...
  scryptSync: jest.fn(() => Buffer.from('mockencryptionkey123456789012345678901234567890')),
...
describe('API Key Service Unit Tests', () => {
  ...
  describe('JWT Token Validation', () => {
    ...
    test('should handle circuit breaker OPEN state', async () => {
      ...
      mockJwtVerifier.verify.mockRejectedValue(new Error('JWT verification failed'));
      ...
      await expect(apiKeyService.validateJwtToken(mockToken))
        .rejects.toThrow('JWT circuit breaker is OPEN');
    });
  });
  ...
  describe('Encryption and Decryption', () => {
    ...
    test('should handle encryption key from Secrets Manager', async () => {
      process.env.API_KEY_ENCRYPTION_SECRET_ARN = 'arn:aws:secretsmanager:us-east-1:123:secret:test';
      delete process.env.API_KEY_ENCRYPTION_SECRET;
  ...
  describe('API Key Storage Operations', () => {
    ...
    test('should store API key successfully', async () => {
      ...
      const result = await apiKeyService.storeApiKey(mockToken, provider, apiKeyData);
    ...
    test('should retrieve API key successfully', async () => {
      ...
      mockDecipher.update.mockReturnValue('{"apiKey":"test-key","apiSecret":"test-secret"}');
      mockDecipher.final.mockReturnValue('');
  ...
  describe('Circuit Breaker Functionality', () => {
    ...
    test('should open API key circuit breaker after max failures', async () => {
      ...
      // Next call should fail due to circuit breaker
      await expect(apiKeyService.getApiKey(mockToken, provider))
  ...
  describe('Error Handling', () => {
    ...
    test('should handle encryption/decryption errors', async () => {
      ...
      await expect(apiKeyService.storeApiKey(mockToken, provider, apiKeyData))
        .rejects.toThrow();
    });
  ...
  describe('Edge Cases', () => {
    test('should handle undefined token gracefully', async () => {
      await expect(apiKeyService.validateJwtToken(undefined))
        .rejects.toThrow();
    });
