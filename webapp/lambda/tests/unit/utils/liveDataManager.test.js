...
describe('Live Data Manager', () => {
  ...
  describe('data provider management', () => {
    ...
    test('should add new data provider', () => {
      ...
      expect(liveDataManager.providers.some(p => p.name === 'testProvider')).toBe(true);
    });
  ...
  describe('real-time data streaming', () => {
    ...
    test('should handle multiple symbol streams', async () => {
      ...
      const results = await Promise.all(
        symbols.map(symbol => liveDataManager.startStream(symbol, mockCallback))
      );
  ...
  describe('error handling and recovery', () => {
    ...
    test('should handle websocket disconnections', () => {
      ...
      const result = liveDataManager.handleWebSocketError(mockWs, new Error('Connection lost'));
    ...
    test('should implement circuit breaker pattern', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await liveDataManager.makeRequest('failingProvider', '/test').catch(() => {});
      }

      const result = await liveDataManager.makeRequest('failingProvider', '/test');
