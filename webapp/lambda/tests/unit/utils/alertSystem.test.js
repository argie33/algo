...
describe('Alert System', () => {
  ...
  describe('alert creation and management', () => {
    test('should create alert with proper parameters', () => {
      ...
      const result = alertSystem.createAlert(key, severity, title, message, metadata);
  ...
  describe('monitoring and lifecycle', () => {
    test('should start monitoring', () => {
      ...
      expect(() => alertSystem.startMonitoring(mockLiveDataManager)).not.toThrow();
    });
  ...
  describe('provider monitoring', () => {
    test('should check provider health', async () => {
      ...
      await expect(alertSystem.checkProviderHealth(providerId, provider)).resolves.not.toThrow();
    });
    ...
    test('should detect high latency', async () => {
      ...
      const alertExists = Array.from(alertSystem.activeAlerts.keys())
        .some(key => key.includes('provider-slow-provider'));
  ...
  describe('error handling and edge cases', () => {
    test('should handle invalid alert creation', () => {
      ...
      expect(() => alertSystem.createAlert('valid-key', 'warning', 'Title', 'Message')).not.toThrow();
    });
    ...
    test('should handle provider disconnection', async () => {
      ...
      const alertExists = Array.from(alertSystem.activeAlerts.keys())
        .some(key => key.includes('provider-disconnected-provider'));
      expect(alertExists).toBe(true);
    });
    ...
    test('should handle high error rate', async () => {
      ...
      const alertExists = Array.from(alertSystem.activeAlerts.keys())
        .some(key => key.includes('provider-error-provider-errors'));
      expect(alertExists).toBe(true);
    });
  });
  ...
  describe('configuration management', () => {
    ...
    test('should handle cost monitoring', async () => {
      ...
      const alertExists = Array.from(alertSystem.activeAlerts.keys())
        .some(key => key.includes('global-cost'));
      expect(alertExists).toBe(true);
    });
  });
  ...
  describe('provider health monitoring', () => {
    it('should check provider health with high latency', async () => {
      ...
      const latencyAlert = Array.from(alertSystem.activeAlerts.values())
        .find(alert => alert.id === 'provider-test-provider' && alert.title.includes('Latency'));
    ...
    it('should check provider health with high error rate', async () => {
      ...
      const errorAlert = Array.from(alertSystem.activeAlerts.values())
        .find(alert => alert.id === 'provider-error-provider-errors');
    ...
    it('should check provider health with disconnected status', async () => {
      ...
      const statusAlert = Array.from(alertSystem.activeAlerts.values())
        .find(alert => alert.id === 'provider-disc-provider-status');
    ...
    it('should resolve provider alerts when healthy', async () => {
      ...
      await alertSystem.checkProviderHealth('recovery-provider', unhealthyProvider);
      ...
      await alertSystem.checkProviderHealth('recovery-provider', healthyProvider);

      // Provider-specific alerts should be resolved
      const providerAlerts = Array.from(alertSystem.activeAlerts.values())
      ...
      expect(providerAlerts.filter(a => !a.resolved).length).toBeLessThanOrEqual(1);
    });
  });
  ...
  describe('global health monitoring', () => {
    it('should check global cost thresholds - warning level', async () => {
      ...
      const costAlert = Array.from(alertSystem.activeAlerts.values())
        .find(alert => alert.id === 'global-cost');
    ...
    it('should check global connection thresholds - critical level', async () => {
      ...
      const connectionAlert = Array.from(alertSystem.activeAlerts.values())
    ...
    it('should resolve global alerts when metrics are healthy', async () => {
      ...
      const globalAlerts = Array.from(alertSystem.activeAlerts.values())
        .filter(alert => alert.id.startsWith('global-'));
      
      // Should have resolved the problematic global alerts
      expect(globalAlerts.filter(a => a.resolved).length).toBeGreaterThanOrEqual(0);
    });
  });
  ...
  describe('notification methods', () => {
    it('should send email notification', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('📧 Email notification: [critical] Test Email Alert')
      );
    ...
    it('should send webhook notification when URL is configured', async () => {
      ...
      alertSystem.config.notifications.webhook.url = 'http://example.com/webhook';
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('🔗 Webhook notification: [warning] Test Webhook Alert')
      );
  ...
  describe('alert lifecycle and management', () => {
    ...
    it('should resolve specific alert types', async () => {
      ...
      alertSystem.createAlert('test-alert-1', 'warning', 'Test Alert 1', 'Message 1');
      alertSystem.createAlert('test-alert-2', 'critical', 'Test Alert 2', 'Message 2');
    ...
    it('should filter alerts by severity manually', () => {
      ...
      alertSystem.createAlert('critical-test', 'critical', 'Critical Test', 'Critical message');
      alertSystem.createAlert('warning-test', 'warning', 'Warning Test', 'Warning message');
      ...
      expect(warningAlerts.find(a => a.id === 'warning-test').severity).toBe('warning');
    });

    it('should cleanup old resolved alerts', async () => {
      // Create alert
      alertSystem.createAlert('cleanup-test', 'info', 'Cleanup Test', 'Test cleanup');
      expect(alertSystem.activeAlerts.size).toBeGreaterThan(0);
    ...
    it('should handle provider health with warning level latency', async () => {
      ...
      const latencyAlert = Array.from(alertSystem.activeAlerts.values())
        .find(alert => alert.id === 'provider-warning-provider' && alert.title.includes('Latency'));
    ...
    it('should handle provider health with warning level error rate', async () => {
      ...
      const errorAlert = Array.from(alertSystem.activeAlerts.values())
        .find(alert => alert.id === 'provider-warning-error-provider-errors');
