...
describe("FactorScoringEngine", () => {
  ...
  describe("calculatePercentileScore", () => {
    ...
    test("should calculate percentile score correctly", () => {
      const result = engine.calculatePercentileScore(
        20.0,
        "pe_ratio",
        universeData,
        false
    ...
    test("should invert percentile for inverse factors", () => {
      const normal = engine.calculatePercentileScore(
        20.0,
        "pe_ratio",
        universeData,
        false
      );
      const inverse = engine.calculatePercentileScore(
        20.0,
        "pe_ratio",
        universeData,
        true
  ...
  describe("scoreUniverse", () => {
    const universeData = [
      ...
      { symbol: "GOOGL", pe_ratio: 20.0, roe: 0.18, revenue_growth_1y: 0.2 },
      { symbol: "MSFT", pe_ratio: 25.0, roe: 0.3, revenue_growth_1y: 0.1 },
    ];
    test("should score and rank universe", () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(result[i - 1].factorScore.compositeScore).toBeGreaterThanOrEqual(
          result[i].factorScore.compositeScore
        );
      }
    ...
    test("should apply custom weights", () => {
      ...
      expect(engine.factors.value.weight).toBeGreaterThan(
        engine.factors.growth.weight
      );
    });
  ...
  describe("applyCustomWeights", () => {
    test("should apply and normalize custom weights", () => {
      ...
      const totalWeight = Object.values(engine.factors).reduce(
  ...
  describe("interpretFactorScore", () => {
    test("should provide correct interpretations", () => {
      ...
      expect(engine.interpretFactorScore("pe_ratio", 20, 65)).toBe(
        "Above Average"
      );
      expect(engine.interpretFactorScore("pe_ratio", 25, 45)).toBe("Average");
      expect(engine.interpretFactorScore("pe_ratio", 30, 35)).toBe(
        "Below Average"
      );
  ...
  describe("createCustomProfile", () => {
    test("should create custom profile with metadata", () => {
      ...
      const result = engine.createCustomProfile(
        "Growth Focus",
        weights,
        "Emphasizes growth metrics"
      );
      expect(result).toEqual({
        name: "Growth Focus",
  ...
  describe("rankUniverse", () => {
    const universeData = [
      ...
      { symbol: "MSFT", pe_ratio: 25.0, roe: 0.3 },
    ];
  ...
  describe("calculateUniverseStats", () => {
    const universeData = [
      { pe_ratio: 10.0, roe: 0.2 },
      { pe_ratio: 15.0, roe: 0.25 },
      { pe_ratio: 20.0, roe: 0.3 },
    ];
  ...
  describe("analyzeFactors", () => {
    const universeData = [
      { pe_ratio: 10.0, roe: 0.2, revenue_growth_1y: 0.1 },
      { pe_ratio: 15.0, roe: 0.25, revenue_growth_1y: 0.15 },
      { pe_ratio: 20.0, roe: 0.3, revenue_growth_1y: 0.2 },
    ];
  ...
  describe("calculateCorrelation", () => {
    ...
    test("should calculate perfect positive correlation", () => {
      const result = engine.calculateCorrelation(
        universeData,
        "factor1",
        "factor2"
      );
  ...
  describe("identifyTopFactors", () => {
    const universeData = [
      { pe_ratio: 10.0, roe: 0.2, revenue_growth_1y: 0.1 },
      { pe_ratio: 20.0, roe: 0.3, revenue_growth_1y: 0.2 },
      { pe_ratio: 30.0, roe: 0.4, revenue_growth_1y: 0.3 },
    ];
    ...
    test("should handle single value factors", () => {
      const uniformData = [{ pe_ratio: 15.0 }, { pe_ratio: 15.0 }];
