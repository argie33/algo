...
describe('Performance Monitor', () => {
  ...
  describe('metrics collection', () => {
    test('should record custom metrics', () => {
      ...
      expect(Object.keys(metrics)).toEqual(['system', 'api', 'database', 'external']);
    });
  ...
  describe('specialized metrics', () => {
    ...
    test('should track external API metrics', () => {
      monitor.recordMetric({
        ...
        success: true,
        timestamp: Date.now(),
        metadata: { service: 'yahoo-finance', endpoint: '/quote', statusCode: 200 }
      });
