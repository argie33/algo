...
const { query, closeDatabase, initializeDatabase, getPool, transaction, healthCheck } = require("../../../utils/database");
  ...
  describe("sentiment analysis", () => {
    test("should analyze positive sentiment", () => {
      const positiveArticle = {
        title:
          "Apple Reports Strong Quarterly Growth and Beats Profit Expectations",
        description:
          "Apple shows excellent growth with strong earnings performance.",
      };
    ...
    test("should analyze negative sentiment", () => {
      const negativeArticle = {
        ...
        description:
          "Tesla reports significant decline with bearish outlook and weak performance.",
      };
    ...
    test("should analyze neutral sentiment", () => {
      const neutralArticle = {
        ...
        description:
          "The company reports stable earnings that remain unchanged with neutral outlook.",
      };
  ...
  describe("topic extraction", () => {
    test("should extract topics from articles", () => {
      const articles = [
        {
          ...
          description:
            "Strong iPhone sales drive revenue growth with excellent profit margins",
        },
        {
          title: "Tesla Stock Analysis",
          description:
            "Electric vehicle sales show growth potential despite market challenges",
        },
      ];
      ...
      const hasFinancialTopics = topicWords.some((word) =>
        ["earnings", "sales", "revenue", "profit", "stock"].includes(
          word.toLowerCase()
        )
      );
    ...
    test("should handle articles without descriptions", () => {
      const articles = [{ title: "News Title Only" }];
      const topics = analyzer.extractTopics(articles);
      expect(topics).toBeInstanceOf(Array);
    });
  });
  describe("impact calculation", () => {
    ...
    test("should consider content length in impact", () => {
      ...
      const longArticle = {
        ...
        description:
          "This is a very long and detailed analysis that provides comprehensive coverage of the topic with extensive information, analysis, and context that would typically indicate higher impact news content.",
      };
      const shortImpact = analyzer.calculateImpact(shortArticle);
      const longImpact = analyzer.calculateImpact(longArticle);
      expect(longImpact.factors.contentLength).toBeGreaterThan(
        shortImpact.factors.contentLength
      );
    });
  });
  describe("source reliability", () => {
    ...
    test("should detect unreliable sources", () => {
      const unreliableScore =
        analyzer.calculateReliabilityScore("fake-news-site.com");
  ...
  describe("sentiment keywords management", () => {
    ...
    test("should handle invalid keyword updates", () => {
      ...
      expect(() =>
        analyzer.updateSentimentKeywords({ positive: "not an array" })
      ).not.toThrow();
    });
  });
  describe("error handling and edge cases", () => {
    ...
    test("should handle very long content", () => {
      const longContent =
        "financial market analysis stock earnings profit growth ".repeat(1000);
      const longArticle = {
