...
describe('Risk Engine', () => {
  ...
  describe('portfolio risk assessment', () => {
    ...
    test('should compute correlation matrix', async () => {
      ...
      const mockReturns = {
        ...
        GOOGL: [0.02, -0.01, 0.02, 0.00],
        TSLA: [0.05, -0.08, 0.10, -0.03]
      };

      query.mockResolvedValue({ rows: mockReturns });

      const correlationMatrix = await riskEngine.calculateCorrelationMatrix(symbols);
  ...
  describe('individual stock risk metrics', () => {
    test('should calculate beta coefficient', async () => {
      ...
      query.mockResolvedValue({ 
        rows: [{ stock_returns: mockStockReturns, market_returns: mockMarketReturns }] 
      });
    ...
    test('should assess liquidity risk', async () => {
      ...
      query.mockResolvedValue({ rows: mockVolumeData.map(volume => ({ volume })) });
  ...
  describe('risk limit monitoring', () => {
    test('should check position size limits', () => {
      ...
      const limits = { maxSinglePosition: 0.10 };
      ...
      expect(check.limitValue).toBe(0.10);
    });
    ...
    test('should validate correlation limits', async () => {
      ...
      const check = await riskEngine.checkCorrelationLimits(portfolio, correlationMatrix, limits);
  ...
  describe('stress testing and scenario analysis', () => {
    test('should run market crash scenario', async () => {
      ...
      const scenario = { marketDrop: -0.20 }; // 20% market drop
    ...
    test('should analyze interest rate sensitivity', async () => {
      ...
      const sensitivity = await riskEngine.analyzeInterestRateSensitivity(portfolio, rateShock);
  ...
  describe('risk reporting and alerts', () => {
    ...
    test('should create risk alerts', () => {
      ...
      expect(alerts.some(alert => alert.type === 'LEVERAGE_EXCEEDED')).toBe(true);
    });
    ...
    test('should generate compliance report', async () => {
      ...
      const compliance = await riskEngine.generateComplianceReport(portfolio, riskLimits);
  ...
  describe('dynamic risk adjustment', () => {
    test('should calculate optimal position sizes', async () => {
      const assets = [
        ...
        { symbol: 'GOOGL', expectedReturn: 0.15, volatility: 0.30 },
        { symbol: 'JNJ', expectedReturn: 0.08, volatility: 0.15 }
      ];
      const riskTolerance = 0.20;

      const positions = await riskEngine.optimizePositionSizes(assets, riskTolerance);

      expect(positions.AAPL).toBeDefined();
      expect(positions.GOOGL).toBeDefined();
      expect(positions.JNJ).toBeDefined();
      
      const totalWeight = Object.values(positions).reduce((sum, weight) => sum + weight, 0);
    ...
    test('should rebalance portfolio for risk control', async () => {
      ...
      const rebalancing = await riskEngine.calculateRebalancing(currentPortfolio);
    ...
    test('should adjust risk based on market conditions', () => {
      ...
      const adjustment = riskEngine.adjustRiskForMarketConditions(marketConditions);

      expect(adjustment.leverageMultiplier).toBeLessThan(1);
      expect(adjustment.concentrationLimit).toBeLessThan(0.10);
