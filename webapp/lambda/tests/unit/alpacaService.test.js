...
describe('AlpacaService', () => {
  ...
  describe('constructor', () => {
    ...
    test('should throw error without API key', () => {
      expect(() => new AlpacaService(null, 'secret')).toThrow('Alpaca API key and secret are required');
    });

    test('should throw error without API secret', () => {
      expect(() => new AlpacaService('key', null)).toThrow('Alpaca API key and secret are required');
    });
  ...
  describe('checkRateLimit', () => {
    ...
    test('should throw error when rate limit exceeded', () => {
      ...
      expect(() => alpacaService.checkRateLimit()).toThrow('Rate limit exceeded. Please try again in a minute.');
    });
  ...
  describe('createOrder', () => {
    ...
    test('should create limit order with price', async () => {
      const limitOrderResponse = { ...mockOrderResponse, type: 'limit', limit_price: '150.00' };
    ...
    test('should validate required parameters', async () => {
      await expect(alpacaService.createOrder()).rejects.toThrow('Symbol is required');
      await expect(alpacaService.createOrder('AAPL')).rejects.toThrow('Quantity must be a positive number');
      await expect(alpacaService.createOrder('AAPL', -5)).rejects.toThrow('Quantity must be a positive number');
      await expect(alpacaService.createOrder('AAPL', 10)).rejects.toThrow('Side must be buy or sell');
      await expect(alpacaService.createOrder('AAPL', 10, 'invalid')).rejects.toThrow('Side must be buy or sell');
    });

    test('should require limit price for limit orders', async () => {
      await expect(alpacaService.createOrder('AAPL', 10, 'buy', 'limit')).rejects.toThrow('Limit price is required for limit orders');
    });
  });
  ...
  describe('error handling', () => {
    ...
    test('should handle API rate limit errors', async () => {
      ...
      await expect(alpacaService.getAccount()).rejects.toThrow('Too many requests');
    });
  });
});