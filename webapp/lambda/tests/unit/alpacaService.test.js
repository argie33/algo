...
describe('AlpacaService', () => {
  ...
  describe('getAccount', () => {
    ...
    test('should successfully get account information', async () => {
      ...
      expect(result).toEqual({
        ...
        lastEquity: 14500.00,
        dayTradeCount: 2,
        dayTradingBuyingPower: 60000.00,
        regtBuyingPower: 10000.50,
        initialMargin: 0.00,
        maintenanceMargin: 0.00,
        longMarketValue: 10000.50,
  ...
  describe('getPositions', () => {
    ...
    test('should handle API errors', async () => {
      mockAlpacaClient.getPositions.mockRejectedValue(new Error('Network error'));
      await expect(service.getPositions()).rejects.toThrow('Failed to fetch positions: Network error');
    });
  });
  ...
  describe('getPortfolioHistory', () => {
    ...
    test('should handle API errors', async () => {
      mockAlpacaClient.getPortfolioHistory.mockRejectedValue(new Error('Network error'));
      await expect(service.getPortfolioHistory()).rejects.toThrow('Failed to fetch portfolio history: Network error');
    });
  });
  ...
  describe('getAsset', () => {
    ...
    test('should handle asset fetch errors', async () => {
      ...
      await expect(service.getAsset('INVALID')).rejects.toThrow('Failed to fetch asset information: Asset not found');
    });
  });
  ...
  describe('getLatestQuote', () => {
    ...
    test('should handle quote fetch errors gracefully', async () => {
      mockAlpacaClient.getLatestQuote.mockRejectedValue(new Error('Quote not available'));
  ...
  describe('getLatestTrade', () => {
    ...
    test('should handle trade fetch errors gracefully', async () => {
      mockAlpacaClient.getLatestTrade.mockRejectedValue(new Error('Trade not available'));
  ...
  describe('getBars', () => {
    ...
    test('should handle bars fetch errors gracefully', async () => {
      mockAlpacaClient.getBars.mockRejectedValue(new Error('Bars not available'));
      const result = await service.getBars('INVALID');
      expect(result).toEqual([]);
    });
  });
  ...
  describe('getMarketClock', () => {
    ...
    test('should handle clock fetch errors gracefully', async () => {
      mockAlpacaClient.getClock.mockRejectedValue(new Error('Clock not available'));
  ...
  describe('createOrder', () => {
    ...
    test('should successfully create limit order', async () => {
      ...
      await service.createOrder('AAPL', 10, 'buy', 'limit', 149.50);
    ...
    test('should validate required parameters', async () => {
      await expect(service.createOrder('', 10, 'buy')).rejects.toThrow('Symbol is required');
      await expect(service.createOrder('AAPL', 0, 'buy')).rejects.toThrow('Quantity must be a positive number');
      await expect(service.createOrder('AAPL', 10, 'invalid')).rejects.toThrow('Side must be buy or sell');
      await expect(service.createOrder('AAPL', 10, 'buy', 'limit')).rejects.toThrow('Limit price is required for limit orders');
    });

    test('should handle order creation errors', async () => {
      mockAlpacaClient.createOrder.mockRejectedValue(new Error('Insufficient buying power'));
      await expect(service.createOrder('AAPL', 10, 'buy')).rejects.toThrow('Failed to create order: Insufficient buying power');
    });
  });
  ...
  describe('Risk Metrics Calculation', () => {
    test('should calculate risk metrics with sufficient data', () => {
      const positions = [
        { marketValue: 1000, unrealizedPL: 100 }
      ];
  ...
  describe('Error Handling', () => {
    ...
    test('should check rate limit before all API calls', async () => {
      ...
      await expect(service.getPositions()).rejects.toThrow('Rate limit exceeded');
      await expect(service.getPortfolioHistory()).rejects.toThrow('Rate limit exceeded');
    });
  });
});