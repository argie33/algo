...
describe("System Integration Tests", () => {
  describe("API Response Formats", () => {
    test("portfolio optimization endpoint returns required fields", () => {
      ...
      expect(
        mockResponse.data.portfolio_analysis.current_state.total_value
      ).toBeGreaterThan(0);
      expect(mockResponse.data.recommendations[0].action).toMatch(
        /BUY|SELL|REDUCE/
      );
    });
    ...
    test("stock detail endpoint returns real data fields", () => {
      ...
      expect(mockResponse.data.technical_analysis.trend).toMatch(
    ...
    test("market data endpoint returns healthy structure", () => {
      ...
      expect(
        mockResponse.data.market_internals.breadth.McClellanOscillator
      ).not.toBeNull();
    });
  });
  ...
  describe("Data Validation Throughout System", () => {
    ...
    test("rebalance preserves portfolio integrity", () => {
      ...
      const newQuantity =
        currentPortfolio.holdings[0].quantity - recommendation.quantity;
      const newMarketValue =
        newQuantity * currentPortfolio.holdings[0].current_price;

      expect(newQuantity).toEqual(75);
      expect(newMarketValue).toEqual(11250);
      expect(newMarketValue).toBeLessThan(
        currentPortfolio.holdings[0].market_value
      );
    });
  ...
  describe("Performance Boundaries", () => {
    test("correlation calculation completes in reasonable time", () => {
      ...
      const prices1 = Array(100)
        .fill(0)
        .map(() => Math.random() * 100);
      const prices2 = Array(100)
        .fill(0)
        .map(() => Math.random() * 100);

      const returns1 = prices1
        .slice(1)
        .map((p, i) => (p - prices1[i]) / prices1[i]);
      const returns2 = prices2
        .slice(1)
        .map((p, i) => (p - prices2[i]) / prices2[i]);
