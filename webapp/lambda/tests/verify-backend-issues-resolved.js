...
test(
  ...
  dbCode.includes('ALTER TABLE portfolio_holdings ADD COLUMN user_id_new VARCHAR(255)') &&
...
test(
  ...
  !dbCode.includes('DEFAULT_USER_ID = 1') && !dbCode.includes('DEFAULT_USER_ID = 2'),
...
const viteConfigFile = path.join(__dirname, '../../../webapp/frontend/vite.config.js');
...
const errorBoundaryFile = path.join(__dirname, '../../../webapp/frontend/src/components/ErrorBoundary.jsx');
test(
...
if (fs.existsSync(errorBoundaryFile)) {
  ...
  test(
    ...
    ebCode.includes('getDerivedStateFromError') && ebCode.includes('componentDidCatch'),
...
test(
  ...
  indexCode.includes('process.env.DB') || indexCode.includes('getDbConfig') || dbCode.includes('process.env'),
...
test(
  ...
  dbCode.includes('console.warn') && dbCode.includes('schema') && dbCode.includes('catch'),
