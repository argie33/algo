...
const PAGES_TO_TEST = [
  ...
  { path: '/portfolio', name: 'Portfolio', dataIndicators: ['position', 'value'] },
  { path: '/settings', name: 'Settings', dataIndicators: ['setting', 'preference'] },
  { path: '/backtest', name: 'Backtest', dataIndicators: ['result', 'performance'] }
];
...
describe('F12 Browser Console Validation', () => {
  ...
  afterAll(async () => {
    ...
    const report = {
      ...
      consoleWarnings: results.reduce((sum, r) => sum + r.consoleWarnings.length, 0),
      pages: results
    };
    ...
    console.log(`Console errors: ${report.consoleErrors}  Warnings: ${report.consoleWarnings}`);
    console.log('='.repeat(80));
    ...
    results.forEach(page => {
      ...
      if (page.consoleWarnings.length > 0) {
        ...
        if (page.consoleWarnings.length > 3) {
          console.log(`      ... and ${page.consoleWarnings.length - 3} more warnings`);
        }
      }
      console.log(`    - Logs: ${page.consoleLogs.length}`);
      console.log(`  Data indicators found: ${page.dataIndicatorsFound.join(', ') || 'none'}`);
    });
  ...
  PAGES_TO_TEST.forEach(pageConfig => {
    it(`should load ${pageConfig.name} with clean F12 console`, async () => {
      ...
      page.on('close', () => {
        if (page.isClosed()) {
          consoleMessages.errors.push('Page crashed or was closed unexpectedly');
        }
      });
      ...
      try {
        ...
        const passed = consoleMessages.errors.length === 0 &&
                       domElements > 30 &&
