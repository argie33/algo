...
const inputSchemas = {
  contact: {
    ...
    subject: v => v === null || v === undefined || (typeof v === 'string' && v.trim().length <= 200),
    message: v => typeof v === 'string' && v.trim().length > 0 && v.trim().length <= 5000,
  },

  manualTrade: {
    symbol: v => typeof v === 'string' && v.trim().length > 0,
    trade_type: v => typeof v === 'string' && ['buy', 'sell'].includes(v.toLowerCase()),
    quantity: v => typeof v === 'number' && v > 0,
    price: v => typeof v === 'number' && v > 0,
    execution_date: v => typeof v === 'string' && !isNaN(Date.parse(v)) && new Date(v) <= new Date(),
    commission: v => v === null || v === undefined || (typeof v === 'number' && v >= 0),
  },
};
  ...
  res.json = function(data) {
    ...
    if (dataSchema && data && data.success) {
      ...
      if (Array.isArray(data.items)) {
        ...
        if (!validation.valid) {
          console.warn(`Response validation failed for ${req.path}:`, validation.errors);
        }
      }
