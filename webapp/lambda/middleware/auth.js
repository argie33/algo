...
const authenticateToken = (req, res, next) => {
  ...
  const nodeEnv =
    process.env.NODE_ENV || (isLambda ? "production" : "development");
  ...
  if (process.env.DEBUG_AUTH) {
    console.log(
      `[AUTH DEBUG] isLambda=${isLambda}, nodeEnv=${nodeEnv}, isProd=${isProd}, isTest=${isTest}, isDev=${isDev}`
    );
  }
  ...
  if (isTest && !isProd) {
    if (process.env.DEBUG_AUTH)
      console.log("[AUTH DEBUG] Using handleTestAuth");
  ...
  if (process.env.DEBUG_AUTH)
    console.log("[AUTH DEBUG] Using authenticateTokenAsync (production mode)");
...
const handleDevAuth = (req, res, next) => {
  ...
  if (!authHeader) {
    ...
    console.log(
      "[DEV AUTH] No auth header provided - using default dev user with admin role"
    );
    return next();
  }
  ...
  if (process.env.DEBUG_AUTH) {
    console.log(
      `[DEV AUTH DEBUG] Token: "${token}", Valid tokens: ${Object.keys(devTokens).join(", ")}`
    );
  }
  ...
  if (devTokens[token]) {
    ...
    req.token = token;
    console.log(
      `[DEV AUTH] Authenticated as ${tokenConfig.role}: ${tokenConfig.username}`
    );
    return next();
  }
...
const handleTestAuth = (req, res, next) => {
  ...
  try {
    ...
    if (!decoded.sub && !decoded.id) {
      return sendError(res, "Invalid token - missing required claims", 401, {
        code: "MISSING_CLAIMS",
      });
    }
...
const authenticateTokenAsync = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return sendError(res, "Authorization missing from request headers", 401, {
        code: "MISSING_AUTHORIZATION",
      });
    }
    ...
    try {
      ...
      if (
        error.message &&
        error.message.includes("Cognito environment variables not configured")
      ) {
        ...
        return sendError(res, "Authentication service not configured", 500, {
    ...
    return sendError(res, "Could not verify authentication", 401, {
      code: "AUTH_FAILED",
    });
  }
};
...
const rateLimitAuth = (attemptsPerWindow = 5, windowMinutes = 15) => {
  ...
  return (req, res, next) => {
    ...
    if (attempts.length >= attemptsPerWindow) {
      return sendError(
        res,
        `Too many authentication attempts. Please try again in ${windowMinutes} minutes.`,
        429,
        {
...
const logApiAccess = async (req, res, next) => {
  ...
  if (process.env.NODE_ENV !== "test") {
    const { hashedId, sanitizedPath } = sanitizeForLogging(
      req.user?.sub,
      req.path
    );
    logger.debug(
