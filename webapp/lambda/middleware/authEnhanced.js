  ...
  try {
    ...
    if (error.message.includes('circuit breaker')) {
      return res.status(503).json({
        ...
        message: 'Authentication service is experiencing issues. Please try again shortly.',
...
const validateSession = async (req, res, next) => {
  try {
    ...
    if (timeSinceIssue > 86400) { // 24 hours
...
const logApiAccess = async (req, res, next) => {
  ...
  // Log request
  console.log(`${req.method} ${req.path} - User: ${req.user?.sub || 'anonymous'} - IP: ${req.ip}`);
