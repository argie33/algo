...
class FactorScoringEngine {
  ...
  calculateQuartiles(universeData, factorName) {
    ...
    if (values.length < 4) {
      // Insufficient data for meaningful quartiles - return null
      // NO fallback to 0 which corrupts distribution analysis
      return {
        q1: values.length > 0 && values[0] !== null ? values[0] : null,
        q2: values.length > 0 && values[Math.floor(values.length / 2)] !== null ? values[Math.floor(values.length / 2)] : null,
        q3: values.length > 0 && values[values.length - 1] !== null ? values[values.length - 1] : null,
      };
    }
