...
class ApiKeyService {
  ...
  async initializeJwtVerifier() {
    try {
      if (process.env.COGNITO_USER_POOL_ID && process.env.COGNITO_CLIENT_ID) {
        ...
        console.warn('Cognito environment variables not set - JWT verification disabled');
      }
    } catch (error) {
  ...
  async validateJwtToken(token) {
    ...
    try {
      ...
      if (cachedSession && Date.now() - cachedSession.timestamp < this.cacheTimeout) {
  ...
  checkJwtCircuitBreaker() {
    ...
    if (this.jwtCircuitBreaker.state === 'OPEN') {
      if (now - this.jwtCircuitBreaker.lastFailureTime > this.jwtCircuitBreaker.timeout) {
        this.jwtCircuitBreaker.state = 'HALF_OPEN';
        console.log('JWT circuit breaker entering HALF_OPEN state');
      } else {
        throw new Error('JWT circuit breaker is OPEN - authentication temporarily unavailable');
      }
    }
  }
  ...
  recordJwtFailure(error) {
    ...
    if (this.jwtCircuitBreaker.failures >= this.jwtCircuitBreaker.maxFailures) {
      ...
      console.error(`JWT circuit breaker OPENED after ${this.jwtCircuitBreaker.failures} failures:`, error.message);
    }
  }
  ...
  async getEncryptionKey() {
    ...
    try {
      ...
      if (!secretArn) {
        ...
        if (envKey) {
          console.warn('Using environment variable for encryption key - consider using AWS Secrets Manager');
      ...
      this.encryptionKey = secret.encryptionKey || secret.API_KEY_ENCRYPTION_SECRET;
      return this.encryptionKey;
  ...
  checkCircuitBreaker() {
    ...
    if (this.circuitBreaker.state === 'OPEN') {
      if (now - this.circuitBreaker.lastFailureTime > this.circuitBreaker.timeout) {
        ...
        throw new Error('API key circuit breaker is OPEN - service temporarily unavailable');
      }
    }
  }
  ...
  recordFailure(error) {
    ...
    if (this.circuitBreaker.failures >= this.circuitBreaker.maxFailures) {
      ...
      console.error(`API key circuit breaker OPENED after ${this.circuitBreaker.failures} failures:`, error.message);
    }
  }
  ...
  async storeApiKey(token, provider, apiKeyData) {
    ...
    try {
      ...
      // Log audit event
      await this.logAuditEvent(userId, 'API_KEY_STORED', provider, user.sessionId);
  ...
  async getApiKey(token, provider) {
    ...
    try {
      ...
      // Log audit event
      await this.logAuditEvent(userId, 'API_KEY_ACCESSED', provider, user.sessionId);
  ...
  async logAuditEvent(userId, action, provider, sessionId) {
    try {
      ...
      `, [userId, action, provider, sessionId, null]); // IP address can be added later
    } catch (error) {
  ...
  async validateApiKey(token, provider, testConnection = false) {
    try {
      ...
      if (testConnection) {
        ...
        await this.logAuditEvent(user.sub, 'API_KEY_TESTED', provider, user.sessionId);
        return testResult;
      }
  ...
  async deleteApiKey(token, provider) {
    ...
    try {
      ...
      // Log audit event
      await this.logAuditEvent(userId, 'API_KEY_DELETED', provider, user.sessionId);
...
module.exports = {
  ...
  storeApiKey: (token, provider, apiKeyData) => apiKeyService.storeApiKey(token, provider, apiKeyData),
  getApiKey: (token, provider) => apiKeyService.getApiKey(token, provider),
  validateApiKey: (token, provider, testConnection) => apiKeyService.validateApiKey(token, provider, testConnection),
  deleteApiKey: (token, provider) => apiKeyService.deleteApiKey(token, provider),
