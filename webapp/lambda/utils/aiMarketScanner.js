...
class AIMarketScanner {
  ...
  async momentumAlgorithm(limit, filters) {
    ...
    return results.rows.map((row) => ({
      ...
      price_change: row.price_change !== null && row.price_change !== undefined ? parseFloat(row.price_change).toFixed(2) : null,
      volume: row.volume !== null && row.volume !== undefined ? parseInt(row.volume) : null,
      volume_ratio: row.volume_ratio !== null && row.volume_ratio !== undefined ? parseFloat(row.volume_ratio).toFixed(2) : null,
  ...
  async unusualActivityAlgorithm(limit, filters) {
    const baseQuery = `
      ...
      WHERE pd.volume IS NOT NULL
        AND (
          (pd_prev.volume IS NOT NULL AND pd.volume > pd_prev.volume * 2.5) OR
          (pd_avg.avg_volume IS NOT NULL AND pd.volume > pd_avg.avg_volume * 2)
        )
        AND pd.close > 1
    ...
    return results.rows.map((row) => ({
      ...
      price_change: row.price_change !== null && row.price_change !== undefined ? parseFloat(row.price_change).toFixed(2) : null,
      volume: row.volume !== null && row.volume !== undefined ? parseInt(row.volume) : null,
      prev_volume: row.prev_volume !== null && row.prev_volume !== undefined ? parseInt(row.prev_volume) : null,
      avg_volume: row.avg_volume !== null && row.avg_volume !== undefined ? parseInt(row.avg_volume) : null,
      volume_ratio: row.volume_ratio !== null && row.volume_ratio !== undefined ? parseFloat(row.volume_ratio).toFixed(2) : null,
      volume_vs_avg: row.volume_vs_avg !== null && row.volume_vs_avg !== undefined ? parseFloat(row.volume_vs_avg).toFixed(2) : null,
      market_cap: row.market_cap,
      sector: row.sector,
      ai_score: parseInt(row.ai_score),
      confidence: this.getConfidenceLevel(row.ai_score),
