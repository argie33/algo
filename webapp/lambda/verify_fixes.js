...
async function verifyFixes() {
  ...
  try {
    ...
    if (tableExists.rows[0].exists) {
      ...
      const hasStoplevel = columns.rows.some(
        (r) => r.column_name === "stoplevel"
      );
    ...
    try {
      ...
      console.log(
        "✅ SQL execution successful, rows found:",
        testResult.rows.length
      );
      ...
      console.log(
        "⚠️  SQL test query failed (expected if no data):",
        error.message.split("\n")[0]
      );
    }
    ...
    console.log(
      "🔧 Trading routes now handle missing database columns gracefully"
    );
