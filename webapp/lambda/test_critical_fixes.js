...
async function testCriticalFixes() {
  ...
  async function runTest(name, testFn) {
    ...
    try {
      ...
      console.log('   Details:', error.response?.body?.error || error.stack?.split('\n')[0]);
    }
  }
  ...
  await runTest('Database column issues resolved in technical indicators', async () => {
    const response = await request(app)
      .get('/api/trading/AAPL/technicals?timeframe=daily');
    
    // Should return either data or proper error (not column existence error)
    if (response.status === 500 && response.body.error && response.body.error.includes('column') && response.body.error.includes('does not exist')) {
  ...
  await runTest('Trading signals SQL syntax errors resolved', async () => {
    const response = await request(app)
      .get('/api/trading/signals?limit=5');
    
    // Should return either data or proper error (not SQL syntax error)
    if (response.status === 500 && response.body.error && response.body.error.includes('syntax error')) {
  ...
  await runTest('Trading positions endpoint handles auth properly', async () => {
    const response = await request(app)
  ...
  await runTest('Trading simulator handles missing database columns', async () => {
    const response = await request(app)
      .get('/api/trading/simulator?symbols=AAPL&portfolio=10000');
    
    // Should return either data or proper error (not column existence error)
    if (response.status === 500 && response.body.error && response.body.error.includes('column') && response.body.error.includes('does not exist')) {
      throw new Error('Still has database column existence errors in simulator');
    }
