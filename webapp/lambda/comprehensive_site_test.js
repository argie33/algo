...
const apiEndpoints = [
  ...
  { path: '/api/screener/screen?criteria=market_cap>1B', name: 'Stock Screener' }
];
...
async function testEndpoint(url, endpoint) {
  try {
    ...
    if (response.status === 200 && hasData) {
      ...
      return { success: false, status: response.status, duration, issue: 'Empty data' };
    } else {
      console.log(`❌ ${response.status} - ${duration}ms - ${endpoint.name}`);
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      return { success: false, status: 'ECONNREFUSED', issue: 'Service unavailable' };
    } else if (error.response) {
      console.log(`❌ ${error.response.status} - ${endpoint.name} - ${error.response.data?.error || error.message}`);
...
async function testFrontendPage(url, page) {
  try {
    ...
    if (response.status === 200) {
      console.log(`✅ ${response.status} - ${duration}ms - Frontend ${page.name}`);
      return { success: true, status: response.status, duration };
    } else {
      console.log(`❌ ${response.status} - ${duration}ms - Frontend ${page.name}`);
      return { success: false, status: response.status, duration };
...
async function main() {
  ...
  console.log(`\n🔧 Backend APIs:`);
  console.log(`✅ Working: ${apiSuccessful.length}/${apiResults.length} (${(apiSuccessful.length/apiResults.length*100).toFixed(1)}%)`);
  console.log(`❌ Issues: ${apiFailed.length}/${apiResults.length} (${(apiFailed.length/apiResults.length*100).toFixed(1)}%)`);

  const frontendSuccessful = frontendResults.filter(r => r.success);
  const frontendFailed = frontendResults.filter(r => !r.success);

  console.log(`\n🌐 Frontend Pages:`);
  console.log(`✅ Working: ${frontendSuccessful.length}/${frontendResults.length} (${(frontendSuccessful.length/frontendResults.length*100).toFixed(1)}%)`);
  console.log(`❌ Issues: ${frontendFailed.length}/${frontendResults.length} (${(frontendFailed.length/frontendResults.length*100).toFixed(1)}%)`);
  ...
  if (apiFailed.length > 0) {
    ...
    apiFailed.forEach(f => {
      console.log(`   ${f.status} - ${f.name}: ${f.issue || f.error || 'Unknown issue'}`);
    });
  }
  ...
  // Performance summary
  const avgApiTime = apiSuccessful.reduce((sum, r) => sum + (r.duration || 0), 0) / (apiSuccessful.length || 1);
  const avgFrontendTime = frontendSuccessful.reduce((sum, r) => sum + (r.duration || 0), 0) / (frontendSuccessful.length || 1);
