...
async function testAlertsFixes() {
  ...
  try {
    ...
    const activeTestPassed = activeResponse.status === 501 && 
                            activeResponse.body.error === "Active alerts not implemented" &&
                            activeResponse.body.user_id === "dev-user-bypass";
    console.log(`   ✅ Active alerts test: ${activeTestPassed ? 'PASSED' : 'FAILED'}\n`);
    ...
    const priceTestPassed = priceResponse.status === 501 && 
                           priceResponse.body.error === "Price alerts not implemented" &&
                           priceResponse.body.user_id === "dev-user-bypass";
    console.log(`   ✅ Price alerts test: ${priceTestPassed ? 'PASSED' : 'FAILED'}\n`);
    ...
    const jsonTestPassed = jsonResponse.status === 400 && 
                          jsonResponse.body.success === false &&
                          jsonResponse.body.error === "Invalid JSON format";
    console.log(`   ✅ JSON parsing test: ${jsonTestPassed ? 'PASSED' : 'FAILED'}\n`);

    // Summary
    const allTestsPassed = activeTestPassed && priceTestPassed && jsonTestPassed;
    ...
    if (allTestsPassed) {
      ...
      console.log('   - All responses include success:false and proper user_id');
    }
