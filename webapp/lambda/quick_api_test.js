...
const criticalEndpoints = [
  ...
  {
    path: "/api/sentiment/analysis?symbol=AAPL",
    method: "GET",
  ...
  {
    path: "/api/financials/ratios?symbol=AAPL",
    method: "GET",
    name: "Financial Ratios",
  },
  { path: "/api/price/AAPL", method: "GET", name: "Price Data AAPL" },
  {
    path: "/api/stocks/AAPL/financials",
    method: "GET",
    name: "Stock Financials AAPL",
  },
...
async function testEndpoint(endpoint) {
  try {
    ...
    if (response.status === 200 && hasData) {
      ...
      console.log(
        `❌ ${response.status} - ${duration}ms - ${endpoint.name} - ${response.data?.error || response.data?.message || "Unknown error"}`
      );
      return {
        success: false,
        status: response.status,
    ...
    if (error.code === "ECONNREFUSED") {
      ...
      console.log(
        `❌ ${error.response.status} - ${endpoint.name} - ${error.response.data?.error || error.response.data?.message || error.message}`
      );
      return {
        success: false,
...
async function main() {
  ...
  console.log(
    `✅ Working: ${successful.length}/${results.length} (${((successful.length / results.length) * 100).toFixed(1)}%)`
  );
  console.log(
    `❌ Failing: ${failed.length}/${results.length} (${((failed.length / results.length) * 100).toFixed(1)}%)`
  );
