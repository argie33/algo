...
router.get("/earnings", async (req, res) => {
  try {
    ...
    if (isNaN(parsedDaysAhead) || parsedDaysAhead < 1 || parsedDaysAhead > 365) {
    ...
    console.log(`📅 Earnings calendar requested - symbol: ${symbol || 'all'}, days_ahead: ${parsedDaysAhead}`);
...
router.get("/debug", async (req, res) => {
  try {
    ...
    return res.status(500).json({success: false, error: "Debug check failed"});
  }
});
...
router.get("/summary", async (req, res) => {
  try {
    ...
    return res.status(500).json({success: false, error: "Failed to fetch calendar summary"});
  }
});
...
router.get("/dividends", async (req, res) => {
  try {
    ...
    const generateDividendCalendar = (daysAhead, targetSymbol, maxResults) => {
      const events = [];
      const now = new Date();
      const endDate = new Date(now.getTime() + parseInt(daysAhead) * 24 * 60 * 60 * 1000);
      
      const dividendStocks = [
        { symbol: 'AAPL', company: 'Apple Inc.', amount: 0.24, yield: 0.55, frequency: 'Quarterly' },
        { symbol: 'MSFT', company: 'Microsoft Corp.', amount: 0.75, yield: 0.80, frequency: 'Quarterly' },
        { symbol: 'JNJ', company: 'Johnson & Johnson', amount: 1.19, yield: 2.89, frequency: 'Quarterly' },
        { symbol: 'JPM', company: 'JPMorgan Chase & Co.', amount: 1.00, yield: 2.58, frequency: 'Quarterly' },
        { symbol: 'PG', company: 'Procter & Gamble Co.', amount: 0.91, yield: 2.36, frequency: 'Quarterly' },
        { symbol: 'KO', company: 'The Coca-Cola Co.', amount: 0.46, yield: 3.16, frequency: 'Quarterly' },
        { symbol: 'PEP', company: 'PepsiCo Inc.', amount: 1.15, yield: 2.68, frequency: 'Quarterly' },
        { symbol: 'WMT', company: 'Walmart Inc.', amount: 0.57, yield: 1.36, frequency: 'Quarterly' },
        { symbol: 'VZ', company: 'Verizon Communications', amount: 0.665, yield: 6.89, frequency: 'Quarterly' },
        { symbol: 'T', company: 'AT&T Inc.', amount: 0.2775, yield: 7.42, frequency: 'Quarterly' },
        { symbol: 'XOM', company: 'Exxon Mobil Corp.', amount: 0.95, yield: 5.87, frequency: 'Quarterly' },
        { symbol: 'CVX', company: 'Chevron Corp.', amount: 1.51, yield: 3.21, frequency: 'Quarterly' },
        { symbol: 'IBM', company: 'International Business Machines', amount: 1.67, yield: 4.56, frequency: 'Quarterly' },
        { symbol: 'MMM', company: '3M Co.', amount: 1.51, yield: 5.89, frequency: 'Quarterly' },
        { symbol: 'CAT', company: 'Caterpillar Inc.', amount: 1.44, yield: 2.76, frequency: 'Quarterly' }
      ];

      let stocksToProcess = dividendStocks;
      if (targetSymbol) {
        stocksToProcess = dividendStocks.filter(stock => stock.symbol === targetSymbol.toUpperCase());
      ...
      stocksToProcess.forEach(stock => {
        ...
        for (let dayOffset = 1; dayOffset <= parseInt(daysAhead); dayOffset += Math.floor(Math.random() * 15) + 10) {
          ...
          const recordDate = new Date(eventDate.getTime() + 2 * 24 * 60 * 60 * 1000);
          const paymentDate = new Date(eventDate.getTime() + 30 * 24 * 60 * 60 * 1000);
          ...
          events.push({
            ...
            dividend_amount: Math.round((stock.amount + (Math.random() * 0.1 - 0.05)) * 100) / 100,
            dividend_yield: Math.round((stock.yield + (Math.random() * 0.2 - 0.1)) * 100) / 100,
            frequency: stock.frequency,
            currency: 'USD',
            dividend_type: 'Regular Cash',
            announcement_date: new Date(eventDate.getTime() - 21 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
          });
        }
      });
      
      return events.sort((a, b) => new Date(a.ex_dividend_date) - new Date(b.ex_dividend_date));
    };
    ...
    const totalDividendAmount = dividendEvents.reduce((sum, event) => sum + event.dividend_amount, 0);
    const uniqueCompanies = new Set(dividendEvents.map(event => event.symbol)).size;
      ...
      data: {
        ...
        summary: {
          ...
          date_range: {
            ...
            to: new Date(Date.now() + parseInt(_days_ahead) * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
          }
        },
        filters: {
          symbol: symbol || null,
...
router.get("/economic", async (req, res) => {
  try {
    ...
    const generateEconomicEvents = (country, daysAhead, limit, importanceFilter) => {
      const events = [];
      ...
      const economicEvents = {
        'US': [
          ...
          },
          {
            ...
            description: 'Gross Domestic Product growth rate - quarterly preliminary',
            category: 'gdp',
            importance: 'high',
            frequency: 'quarterly',
            typical_time: '08:30',
      ...
      for (let dayOffset = 0; dayOffset <= daysAhead; dayOffset++) {
        ...
        eventsToday.forEach(event => {
          ...
          const generateValue = (category) => {
            switch (category) {
              case 'monetary_policy': return (Math.random() * 3 + 2).toFixed(2) + '%';
              case 'inflation': return (Math.random() * 4 + 1).toFixed(1) + '%';
              case 'employment': return (Math.random() * 200 + 150).toFixed(0) + 'K';
              case 'gdp': return (Math.random() * 4 + 0.5).toFixed(1) + '%';
              case 'consumption': return (Math.random() * 3 + (-1)).toFixed(1) + '%';
              case 'manufacturing': return (Math.random() * 20 + 45).toFixed(1);
              case 'sentiment': return (Math.random() * 40 + 90).toFixed(1);
              case 'housing': return (Math.random() * 200 + 1300).toFixed(0) + 'K';
              default: return (Math.random() * 5).toFixed(1);
            }
          };
          ...
          events.push({
            ...
            unit: event.category === 'employment' ? 'jobs' : event.category.includes('rate') || event.category === 'inflation' ? 'percent' : 'index',
            source: country === 'US' ? 'Bureau of Labor Statistics' : country === 'EU' ? 'Eurostat' : 'ONS',
            is_tentative: Math.random() > 0.9, // 10% are tentative
            volatility_expected: event.importance === 'high' ? 'high' : event.importance === 'medium' ? 'medium' : 'low'
          });
        });
    ...
    const economicEvents = generateEconomicEvents(country, parsedDaysAhead, parsedLimit, importance);
    ...
    const summary = {
      ...
      date_range: {
        from: new Date().toISOString().split('T')[0],
        to: new Date(Date.now() + parsedDaysAhead * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      ...
      upcoming_high_impact: economicEvents.filter(e => e.impact === 'high').length,
      next_24h: economicEvents.filter(e => {
        const eventTime = new Date(e.time);
        const now = new Date();
        const hours24 = 24 * 60 * 60 * 1000;
        return eventTime >= now && eventTime <= new Date(now.getTime() + hours24);
      }).length,
    ...
    res.json({
      ...
      data: {
        ...
        available_filters: {
          ...
          categories: ["monetary_policy", "inflation", "employment", "gdp", "consumption", "manufacturing", "sentiment", "housing"]
        }
      },
      metadata: {
        ...
        timezone: "UTC",
        currency_focus: economicEvents.length > 0 ? economicEvents[0].currency : country === 'US' ? 'USD' : 'EUR'
      },
      timestamp: new Date().toISOString()
    });
...
router.get("/upcoming", async (req, res) => {
  try {
    ...
    console.log(`📅 Upcoming calendar events requested - days: ${days}, country: ${country}, importance: ${importance}`);

    // Generate upcoming events for the next N days
    const generateUpcomingEvents = (numDays, targetCountry, targetImportance, targetCategory, maxResults) => {
      const events = [];
      const now = new Date();
      ...
      if (targetImportance !== 'all') {
        filteredTemplates = filteredTemplates.filter(t => t.importance === targetImportance);
      }
      if (targetCategory !== 'all') {
        filteredTemplates = filteredTemplates.filter(t => t.category === targetCategory);
      }
    ...
    const events = generateUpcomingEvents(days, country, importance, category, parseInt(limit));
