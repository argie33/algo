  ...
  try {
    ...
    if (marketResult.rows.length > 0) {
      ...
      currentPrice = parseFloat(
        marketData.close || marketData.bid_price || marketData.ask_price
      );
      spread =
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const tradingValidation = await validateTradingOperation(
      userId,
      side === "buy" ? "buy" : "sell",
      {
        amount: orderValue,
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.notFound("No account data found for user", {
        ...
        suggestion:
          "Please ensure your account is properly set up with broker integration",
      });
    }
...
router.get("/recent", async (req, res) => {
  try {
    ...
    console.log(
      `📋 Recent orders requested - limit: ${limit}, days: ${days}, status: ${status}`
    );
...
router.get("/active", async (req, res) => {
  try {
    ...
    const summary = {
      ...
      pending_orders: activeOrders.filter((order) => order.status === "PENDING")
        .length,
      order_types: {
        limit: activeOrders.filter((order) =>
          order.order_type.includes("LIMIT")
        ).length,
        market: activeOrders.filter((order) => order.order_type === "MARKET")
          .length,
        stop: activeOrders.filter((order) => order.order_type.includes("STOP"))
          .length,
      },
