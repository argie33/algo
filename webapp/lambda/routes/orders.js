...
router.post('/preview', async (req, res) => {
  const userId = req.user.sub;
  const { symbol, side, quantity, orderType, limitPrice, stopPrice, timeInForce, extendedHours } = req.body;
  ...
  try {
    ...
    if (marketResult.rows.length > 0) {
      ...
      currentPrice = parseFloat(marketData.close_price || marketData.bid_price || marketData.ask_price);
      spread = parseFloat(marketData.ask_price) - parseFloat(marketData.bid_price);
    }
    ...
    if (orderType === 'limit') {
      ...
      estimatedPrice = side === 'buy' ? currentPrice + (spread / 2) : currentPrice - (spread / 2);
    }
    ...
    if (orderType === 'market' && extendedHours) {
      warnings.push('Market orders during extended hours may have higher volatility');
    }
    
    if (Math.abs(estimatedPrice - currentPrice) / currentPrice > 0.1) {
      warnings.push('Order price is significantly different from current market price');
    }
    ...
    const mockPreview = {
      ...
      orderType: orderType,
      currentPrice: 185.50,
      estimatedPrice: orderType === 'market' ? 185.50 : parseFloat(limitPrice || stopPrice || 185.50),
      estimatedValue: parseFloat(quantity) * (orderType === 'market' ? 185.50 : parseFloat(limitPrice || stopPrice || 185.50)),
      estimatedCommission: 0,
      buyingPowerRequired: parseFloat(quantity) * (orderType === 'market' ? 185.50 : parseFloat(limitPrice || stopPrice || 185.50)),
...
router.post('/', async (req, res) => {
  const userId = req.user.sub;
  const { 
    symbol, side, quantity, orderType, limitPrice, stopPrice, 
    timeInForce, extendedHours, allOrNone, notes 
  } = req.body;
  ...
  try {
    ...
    const estimatedValue = parseFloat(quantity) * parseFloat(limitPrice || stopPrice || 0);
    ...
    const orderResult = await query(insertOrderQuery, [
      orderId, userId, symbol, side, parseFloat(quantity), orderType,
      limitPrice ? parseFloat(limitPrice) : null,
      stopPrice ? parseFloat(stopPrice) : null,
      timeInForce, 'pending', new Date().toISOString(),
      0, 0, estimatedValue, 0, broker, clientOrderId, notes,
      extendedHours || false, allOrNone || false
    ]);
    ...
    await logOrderActivity(userId, orderId, 'submitted', 'Order submitted successfully');
    ...
    res.json({
      ...
      data: {
        ...
        status: 'pending',
        submittedAt: new Date().toISOString(),
        estimatedValue: parseFloat(quantity) * parseFloat(limitPrice || stopPrice || 185.50),
...
router.post('/:orderId/cancel', async (req, res) => {
  ...
  try {
    ...
    // Log cancellation
    await logOrderActivity(userId, orderId, 'cancelled', 'Order cancelled by user');
...
router.patch('/:orderId', async (req, res) => {
  ...
  try {
    ...
    // Log modification
    await logOrderActivity(userId, orderId, 'modified', 'Order modified by user');
...
function validateOrder(orderData) {
  ...
  if ((orderType === 'limit' || orderType === 'stop_limit') && (!limitPrice || isNaN(limitPrice) || parseFloat(limitPrice) <= 0)) {
    return { valid: false, message: 'Limit price is required for limit orders' };
  }
  
  if ((orderType === 'stop' || orderType === 'stop_limit') && (!stopPrice || isNaN(stopPrice) || parseFloat(stopPrice) <= 0)) {
...
function isMarketOpen() {
  ...
  return currentDay >= 1 && currentDay <= 5 && currentHour >= 9 && currentHour < 16;
}
...
async function logOrderActivity(userId, orderId, activityType, description) {
  try {
    ...
    await query(logQuery, [userId, orderId, activityType, description, new Date().toISOString()]);
  } catch (error) {
