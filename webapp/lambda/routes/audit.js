...
router.get("/", (req, res) => {
  return sendSuccess(res, {
    message:
      "Audit API - available endpoints: /trades, /config-changes, /safeguard-activations",
  });
});
...
router.get("/trades", async (req, res) => {
  try {
    ...
    const countResult = await query(countSql, symbol ? [symbol] : []);
...
router.get("/config", async (req, res) => {
  try {
    ...
    const countResult = await query(
      "SELECT COUNT(*) as total FROM algo_config_audit"
    );
...
router.get("/safeguards", async (req, res) => {
  try {
    ...
    logger.error("Error fetching safeguard audit logs:", {
      error: err.message,
    });
    return sendError(
      res,
      "Failed to fetch safeguard audit logs",
      500,
      err.message
    );
  }
