...
router.get("/trades/:symbol", async (req, res) => {
  try {
    ...
    const result = await query(insiderQuery, [symbol.toUpperCase(), isNaN(parseInt(limit)) ? 50 : parseInt(limit)]);
    ...
    const buyValue = trades.filter(t => t.direction === 'BUY').reduce((sum, t) => sum + t.value, 0);
    const sellValue = trades.filter(t => t.direction === 'SELL').reduce((sum, t) => sum + t.value, 0);
...
router.get("/trades", async (req, res) => {
  try {
    ...
    console.log(`👥 General insider trades requested - limit: ${limit}, days: ${days}, type: ${trade_type}`);
    ...
    if (!result.rows || result.rows.length === 0) {
      return res.status(404).json({
        ...
        message: "No insider trading data available for the specified criteria. Please ensure the insider_transactions table is populated.",
        filters: {
    ...
    const trades = result.rows.map(row => ({
      ...
      trade_direction: row.transaction_type.includes('buy') || row.transaction_type.includes('purchase') ? 'BUY' : 'SELL',
    ...
    res.json({
      ...
      summary: {
        ...
        avg_trade_value: trades.length > 0 ? 
          trades.reduce((sum, trade) => sum + trade.value, 0) / trades.length : 0
      },
      metadata: {
