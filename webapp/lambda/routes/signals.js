...
const getStocksSignals = async (req, res) => {
  try {
    ...
    const actualColumns = useTechJoin ? `
      ...
      NULL::numeric AS r_multiple_t1, NULL::numeric AS r_multiple_t2, NULL::numeric AS r_multiple_t3,
      bsd.exit_trigger_1_price AS target_t1_price, bsd.exit_trigger_2_price AS target_t2_price, bsd.exit_trigger_3_price AS target_t3_price,
      NULL::text AS stop_method,
      NULL::boolean AS exit_21ema_break, NULL::boolean AS exit_50dma_light_volume, NULL::boolean AS exit_50dma_heavy_volume, NULL::boolean AS exit_power_trend_flag,
      NULL::numeric AS distance_to_pivot_pct, NULL::int AS base_count, NULL::int AS distribution_days_4w,
    ...
    const formattedData = signalsResult.rows.map(row => ({
      ...
      r_multiple_t1: sf(row.r_multiple_t1),
      r_multiple_t2: sf(row.r_multiple_t2),
      r_multiple_t3: sf(row.r_multiple_t3),
      target_t1_price: sf(row.target_t1_price),
      target_t2_price: sf(row.target_t2_price),
      target_t3_price: sf(row.target_t3_price),
      stop_method: row.stop_method,
      exit_21ema_break: row.exit_21ema_break,
      exit_50dma_light_volume: row.exit_50dma_light_volume,
      exit_50dma_heavy_volume: row.exit_50dma_heavy_volume,
      exit_power_trend_flag: row.exit_power_trend_flag,
      distance_to_pivot_pct: sf(row.distance_to_pivot_pct),
      base_count: row.base_count || 1,
      distribution_days_4w: row.distribution_days_4w || 0,
