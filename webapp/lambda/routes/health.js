...
router.get("/database/diagnostics", async (req, res) => {
  ...
  try {
    ...
    overallStatus = "unhealthy";
    diagnostics.recommendations.push(
      "Fatal error in diagnostics. Check backend logs."
    );
    res.status(500).json({
      status: "error",
      overallStatus,
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Health update: Failed to initialize database:",
          dbInitError.message
        );
        return res.status(503).json({
          status: "error",
    ...
    try {
      ...
      try {
        ...
        console.log(
          `Health_status table created and populated with ${monitoredTables.length} tables`
        );
      } catch (createError) {
        console.error(
          "Failed to create health_status table:",
          createError.message
        );
        return res.status(500).json({
          status: "error",
    ...
    try {
      ...
      const tablesToCheck = await query(
        "SELECT table_name, expected_update_frequency FROM health_status"
      );
      ...
      for (const tableRow of tablesToCheck.rows) {
            ...
            try {
              ...
              if (recordCount === 0) {
                  ...
                  try {
                    ...
                    if (colCheck.rowCount > 0) {
                      const tsResult = await query(
                        `SELECT MAX("${col}") as last_update FROM "${tableName}"`
                      );
                ...
                if (lastUpdated && tableRow.expected_update_frequency) {
                  ...
                  const expectedHours = expectedInterval.includes("hour")
                    ? 2
                    : expectedInterval.includes("day")
                      ? 48
                      : expectedInterval.includes("week")
                        ? 336
                        : expectedInterval.includes("month")
                          ? 1440
                          : 48;
        ...
        try {
          ...
          console.error(
            `Failed to update health status for table ${tableName}:`,
            updateError.message
          );
        }
      ...
      console.error(
        "Error during comprehensive health check:",
