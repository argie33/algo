...
const { authenticateToken, requireRole } = require('../middleware/authEnhanced');
 ...
 */
router.get('/database-connectivity', requireRole(['admin']), async (req, res) => {
  try {
...
router.post('/external-services', async (req, res) => {
  ...
  try {
    ...
    const overallSuccess = results.aws.secretsManager.status === 'connected' && 
  ...
  try {
    const lambdaInfo = {
      ...
      remainingTime: req.context?.getRemainingTimeInMillis ? 
        req.context.getRemainingTimeInMillis() : 'not-lambda'
    };
...
router.get('/health', async (req, res) => {
  try {
    ...
    try {
      ...
      health.checks.apiKeyService = apiKeyHealth.apiKeyCircuitBreaker.state === 'CLOSED' ? 'healthy' : 'unhealthy';
    } catch (error) {
