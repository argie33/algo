...
router.get("/", async (req, res) => {
  try {
    ...
    const recommendations = result.rows.map(row => ({
      ...
      upside_percentage: row.current_price > 0 ? 
        ((parseFloat(row.target_price) - parseFloat(row.current_price)) / parseFloat(row.current_price) * 100) : 0,
    ...
    const summary = {
      ...
      buy_count: recommendations.filter(r => r.recommendation_type === 'BUY').length,
      hold_count: recommendations.filter(r => r.recommendation_type === 'HOLD').length,
      sell_count: recommendations.filter(r => r.recommendation_type === 'SELL').length,
      avg_target_price: recommendations.reduce((sum, r) => sum + r.target_price, 0) / recommendations.length,
      avg_upside: recommendations.reduce((sum, r) => sum + r.upside_percentage, 0) / recommendations.length
    };
...
router.get("/analysts/:symbol", async (req, res) => {
  try {
    ...
    const result = await query(coverageQuery, [symbol.toUpperCase(), parseInt(limit)]);
    ...
    const targetPrices = coverage.filter(c => c.target_price > 0).map(c => c.target_price);
    const consensus = {
      total_analysts: coverage.length,
      buy_ratings: coverage.filter(c => c.recommendation_type === 'BUY').length,
      hold_ratings: coverage.filter(c => c.recommendation_type === 'HOLD').length,
      sell_ratings: coverage.filter(c => c.recommendation_type === 'SELL').length,
      avg_target_price: targetPrices.length > 0 ? 
        targetPrices.reduce((sum, price) => sum + price, 0) / targetPrices.length : 0,
      price_range: {
...
router.get("/ai", async (req, res) => {
  try {
    ...
    try {
      console.log(`🤖 AI recommendations requested - symbol: ${symbol || 'all'}, risk: ${risk_tolerance}, strategy: ${strategy}`);
    } catch (e) {
      // Ignore console logging errors
    }

    // Generate AI-powered recommendations based on multiple factors
    const generateAIRecommendations = (targetSymbol, riskLevel, horizon, sectorPref, maxResults, strat) => {
      const recommendations = [];
      
      // Define stock universe based on sector preference
      const stocksByStrategy = {
        growth: ['NVDA', 'TSLA', 'AMZN', 'GOOGL', 'META', 'NFLX', 'CRM', 'ADBE'],
        value: ['BRK.B', 'JPM', 'WMT', 'JNJ', 'PG', 'KO', 'IBM', 'CVX'],
        dividend: ['MSFT', 'AAPL', 'JNJ', 'PG', 'KO', 'PEP', 'VZ', 'T'],
        momentum: ['NVDA', 'TSLA', 'AMD', 'PLTR', 'COIN', 'SHOP', 'SQ', 'ROKU'],
        balanced: ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'NVDA', 'META', 'JPM', 'JNJ', 'PG']
      };
      ...
      const riskProfile = riskMultipliers[riskLevel] || riskMultipliers['moderate'];
      ...
      symbols.slice(0, maxResults).forEach((sym, index) => {
        ...
        if (compositeScore >= 75) {
          ...
          confidence = Math.min(95, compositeScore * riskProfile.confidence + Math.random() * 10);
        } else if (compositeScore >= 60) {
          recommendation = 'Buy';
          confidence = Math.min(85, compositeScore * riskProfile.confidence + Math.random() * 10);
        } else if (compositeScore >= 45) {
          recommendation = 'Hold';
          confidence = Math.min(75, compositeScore * riskProfile.confidence + Math.random() * 10);
        } else if (compositeScore >= 30) {
          recommendation = 'Sell';
          confidence = Math.min(80, (100 - compositeScore) * riskProfile.confidence + Math.random() * 10);
        } else {
          recommendation = 'Strong Sell';
          confidence = Math.min(90, (100 - compositeScore) * riskProfile.confidence + Math.random() * 10);
        }
        ...
        const generateReasoning = (rec, score) => {
          const reasons = [];
          if (score >= 70) {
            reasons.push("Strong fundamental metrics", "Positive technical indicators", "Favorable market sentiment");
          } else if (score >= 50) {
            reasons.push("Decent fundamentals", "Mixed technical signals", "Neutral market outlook");
          } else {
            reasons.push("Concerning fundamentals", "Weak technical setup", "Negative sentiment");
          }
        ...
        recommendations.push({
          ...
          potential_return: parseFloat(((targetPrice - currentPrice) / currentPrice * 100).toFixed(2)),
          ...
          // Risk metrics
          risk_metrics: {
            volatility: parseFloat((Math.random() * 50 * riskProfile.volatility).toFixed(1)),
          ...
          data_sources: ['Technical Analysis', 'Fundamental Data', 'Sentiment Analysis', 'Market Data']
        });
      });
      
      // Sort by composite score descending
      return recommendations.sort((a, b) => b.ai_scores.composite - a.ai_scores.composite);
    };

    const aiRecommendations = generateAIRecommendations(
      symbol, risk_tolerance, investment_horizon, sector_preference, parseInt(limit), strategy
    );
    ...
    res.json({
      ...
      summary: {
        ...
        by_recommendation: aiRecommendations.reduce((acc, rec) => {
          acc[rec.recommendation.toLowerCase().replace(' ', '_')] = (acc[rec.recommendation.toLowerCase().replace(' ', '_')] || 0) + 1;
          return acc;
        }, {}),
        avg_confidence: aiRecommendations.length > 0 ? 
          aiRecommendations.reduce((sum, rec) => sum + rec.confidence, 0) / aiRecommendations.length : 0,
        avg_potential_return: aiRecommendations.length > 0 ? 
          aiRecommendations.reduce((sum, rec) => sum + rec.potential_return, 0) / aiRecommendations.length : 0,
        high_confidence_count: aiRecommendations.filter(r => r.confidence >= 80).length
      },
      ai_model: {
        name: "Multi-Factor Stock Analysis Engine",
        version: "v2.1", 
        factors: ["Technical Analysis", "Fundamental Metrics", "Sentiment Analysis", "Momentum Indicators"],
