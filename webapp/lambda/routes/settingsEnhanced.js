...
const { authenticateToken, requireRole, rateLimitByUser, logApiAccess } = require('../middleware/authEnhanced');
const { 
  storeApiKey, 
  getApiKey, 
  validateApiKey, 
...
router.get('/api-keys/:provider', async (req, res) => {
  ...
  try {
    ...
    const maskedData = {
      ...
      // Mask sensitive data
      apiKey: apiKeyData.apiKey ? `${apiKeyData.apiKey.substring(0, 4)}${'*'.repeat(apiKeyData.apiKey.length - 4)}` : undefined,
  ...
  try {
    ...
    if (error.message.includes('circuit breaker')) {
      return res.status(503).json({
        ...
        message: 'API key service is experiencing issues. Please try again shortly.'
      });
    }
...
router.put('/api-keys/:provider', async (req, res) => {
  ...
  try {
    ...
    const updatedData = {
      ...
      isSandbox: isSandbox !== undefined ? Boolean(isSandbox) : existingData.isSandbox,
...
router.post('/api-keys/:provider/validate', async (req, res) => {
  ...
  try {
    const validation = await validateApiKey(req.token, provider, testConnection);
...
router.post('/api-keys/test-all', async (req, res) => {
  try {
    ...
    for (const provider of providers) {
      try {
        const validation = await validateApiKey(req.token, provider.provider, true);
        testResults.push({
