...
router.get("/intraday/:symbol", async (req, res) => {
  try {
    ...
    const generateIntradayData = (stockSymbol, intervalStr) => {
      ...
      let currentPrice =
        symbolPrices[stockSymbol.toUpperCase()] || Math.random() * 200 + 50;
    ...
    const closePrice =
      intradayPrices.length > 0
        ? intradayPrices[intradayPrices.length - 1].close
        : null;
      ...
      data: {
        ...
        summary: {
          ...
          market_session_summary: {
            regular_hours: intradayPrices.filter(
              (d) => d.market_session === "regular"
            ).length,
            pre_market: intradayPrices.filter(
              (d) => d.market_session === "pre_market"
            ).length,
            after_hours: intradayPrices.filter(
              (d) => d.market_session === "after_hours"
            ).length,
          },
          price_summary: {
            open: openPrice,
            ...
            change:
              openPrice && closePrice
                ? Math.round((closePrice - openPrice) * 100) / 100
                : 0,
            change_percent:
              openPrice && closePrice
                ? Math.round(((closePrice - openPrice) / openPrice) * 10000) /
                  100
                : 0,
          },
          volume_summary: {
...
router.get("/futures/:symbol", async (req, res) => {
  try {
    ...
    const futuresQuery = `
      WITH underlying_data AS (
        SELECT 
          ...
          pd.close * (1 + (RANDOM() - 0.5) * 0.02) as futures_price,
      ...
      SELECT 
        ...
        ROUND(((ud.futures_price - ud.close) / ud.close * 100)::numeric, 2) as basis_percent,
        CASE 
    ...
    console.log(
      `📈 Futures data generated for ${symbol}: ${futuresData.length} contracts`
    );
...
router.get("/alerts", async (req, res) => {
  try {
    ...
    const alertsQuery = `
      WITH alert_status AS (
        SELECT 
          ...
          ROUND(((pa.current_price - pd.close) / pd.close * 100)::numeric, 2) as price_change_since_created
      ...
      enriched_alerts AS (
        SELECT 
          ...
          (SELECT close FROM price_daily pd2 WHERE pd2.symbol = als.symbol ORDER BY date DESC LIMIT 1) as latest_price
    ...
    const alerts = result.rows.map((row) => ({
      ...
      current_price:
        parseFloat(row.current_price) || parseFloat(row.latest_price),
      status: row.status,
      priority_level: row.priority_level,
      priority_score: row.priority_score,
      distance_percent: parseFloat(row.distance_percent) || 0,
      price_change_since_created:
        parseFloat(row.price_change_since_created) || 0,
    ...
    const summary = {
      ...
      avg_distance_to_trigger:
        alerts.length > 0
          ? Math.round(
              (alerts.reduce(
                (sum, a) => sum + Math.abs(a.distance_percent),
                0
              ) /
                alerts.length) *
                100
    ...
    console.log(
      `🚨 Price alerts retrieved: ${alerts.length} alerts for user ${user_id}`
    );
...
router.post("/alerts", async (req, res) => {
  try {
    ...
    console.log(
      `🚨 Created new price alert: ${alertId} for ${symbol.toUpperCase()}`
    );
...
// Daily price data with limit
router.get("/daily/:symbol/:limit?", async (req, res) => {
  try {
