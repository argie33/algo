...
router.get('/market-overview', async (req, res) => {
  try {
    console.log('🚀 [CRYPTO-MARKET] Fetching market overview data from database');
    ...
    const marketOverview = {
      market: {
        ...
        avgChange24h: Math.round((parseFloat(overview.avg_price_change_24h || 0)) * 100) / 100,
      ...
      topGainers: gainersResult.rows.map(row => ({
        symbol: row.symbol,
        change24h: Math.round(parseFloat(row.price_change_percentage_24h) * 100) / 100,
    ...
    res.status(500).json({
      ...
      suggestion: 'Ensure crypto_market_data table has been populated with current data',
      last_updated: new Date().toISOString()
    });
  }
});
...
router.get('/top-coins', async (req, res) => {
  try {
    ...
    console.log(`📊 [CRYPTO-TOP] Fetching top ${limit} cryptocurrencies from database`);
    ...
    res.status(500).json({
      ...
      details: error.message,
      suggestion: 'Ensure crypto_market_data and crypto_symbols tables are populated',
      last_updated: new Date().toISOString()
    });
  }
});
...
router.get('/prices/:symbol', async (req, res) => {
  try {
    ...
    console.log(`💰 [CRYPTO-PRICE] Fetching price data for ${symbol.toUpperCase()}`);
    ...
    console.error(`❌ [CRYPTO-PRICE] Error fetching price for ${req.params.symbol}:`, error);
...
router.get('/historical/:symbol', async (req, res) => {
  try {
    ...
    console.log(`📈 [CRYPTO-HISTORICAL] Fetching ${days} days of ${symbol.toUpperCase()} data`);
    ...
    console.error(`❌ [CRYPTO-HISTORICAL] Error fetching historical data for ${req.params.symbol}:`, error);
...
router.post('/portfolio/add', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`➕ [CRYPTO-PORTFOLIO] Adding ${quantity} ${symbol} for user ${user_id}`);
...
router.get('/news', async (req, res) => {
  try {
    ...
    console.log(`📰 [CRYPTO-NEWS] Fetching crypto news${symbol ? ` for ${symbol}` : ''}`);
...
async function generateTopCryptocurrencies(options) {
  ...
  for (let i = 0; i < options.limit; i++) {
    ...
    const price = rank === 1 ? 42000 + Math.random() * 8000 : // BTC: $42k-50k
                  rank === 2 ? 2500 + Math.random() * 800 : // ETH: $2.5k-3.3k
                  rank <= 10 ? 1 + Math.random() * 500 : // Top 10: $1-500
                  0.01 + Math.random() * 10; // Others: $0.01-10
    
    const marketCap = rank === 1 ? 850000000000 : // BTC: ~$850B
                     rank === 2 ? 320000000000 : // ETH: ~$320B
...
async function generateCryptoPriceData(symbol, options) {
  ...
  return {
    ...
    price_change_24h: Math.round((currentPrice * (-0.15 + Math.random() * 0.3)) * 100) / 100,
    price_change_percentage_24h: -15 + Math.random() * 30,
    market_cap: Math.round(currentPrice * (10000000 + Math.random() * 50000000)),
    volume_24h: Math.round(currentPrice * (500000 + Math.random() * 2000000)),
    high_24h: Math.round(currentPrice * (1.02 + Math.random() * 0.08) * 100) / 100,
    low_24h: Math.round(currentPrice * (0.92 + Math.random() * 0.08) * 100) / 100,
    ...
    atl_date: new Date(Date.now() - Math.random() * 1095 * 24 * 60 * 60 * 1000).toISOString(),
...
async function generateTrendingCryptos(options) {
  const trendingCoins = [
    'BTC', 'ETH', 'SOL', 'AVAX', 'MATIC', 'DOT', 'LINK', 'UNI', 'AAVE', 'CRV'
  ];
  ...
  return {
    trending: trendingCoins.slice(0, options.limit).map((symbol, index) => ({
      ...
      trending_score: Math.round((90 - index * 5 + Math.random() * 10) * 100) / 100
    })),
...
async function generateUserCryptoPortfolio(userId) {
  const holdings = [
    { symbol: 'BTC', quantity: 0.5 + Math.random() * 2, purchase_price: 35000 + Math.random() * 10000 },
    { symbol: 'ETH', quantity: 2 + Math.random() * 8, purchase_price: 2000 + Math.random() * 1000 },
    { symbol: 'SOL', quantity: 10 + Math.random() * 40, purchase_price: 80 + Math.random() * 40 }
  ];
  ...
  positions.forEach(position => {
    position.allocation_percentage = Math.round((position.current_value / totalValue) * 100 * 100) / 100;
  });
  ...
  return {
    ...
    portfolio_summary: {
      ...
      total_pnl_percentage: Math.round(((totalValue - totalCost) / totalCost) * 100 * 100) / 100,
...
async function generateCryptoNews(options) {
  ...
  const articles = [];
  for (let i = 0; i < options.limit; i++) {
    const headline = cryptoHeadlines[Math.floor(Math.random() * cryptoHeadlines.length)];
    ...
    articles.push({
      ...
      sentiment_label: sentiment_score > 0.1 ? 'positive' : sentiment_score < -0.1 ? 'negative' : 'neutral',
  ...
  return {
    ...
    summary: {
      ...
      avg_impact_score: articles.reduce((sum, a) => sum + a.impact_score, 0) / articles.length
    }
  };
}
...
function generateCryptoNewsSummary(headline) {
  const summaries = {
    'Bitcoin': 'Analysis of Bitcoin market dynamics and technical developments affecting price action.',
    'Ethereum': 'Ethereum network updates and ecosystem developments driving adoption metrics.',
    'DeFi': 'Decentralized finance protocol innovations and yield optimization strategies.',
    'Regulatory': 'Cryptocurrency regulatory developments and institutional adoption trends.',
...
function generateNewsSource() {
  const sources = ['CoinDesk', 'CoinTelegraph', 'The Block', 'Decrypt', 'CryptoSlate'];
  return sources[Math.floor(Math.random() * sources.length)];
}

function generateAuthorName() {
  const firstNames = ['Alex', 'Sarah', 'Michael', 'Jessica', 'David', 'Rachel'];
  const lastNames = ['Chen', 'Johnson', 'Williams', 'Davis', 'Miller', 'Wilson'];
...
function generateFallbackHistoricalData(symbol) {
  ...
  return {
    symbol: symbol.toUpperCase(),
    prices: [[now - 86400000, 44567.89], [now, 45234.56]],
    market_caps: [[now - 86400000, 875000000000], [now, 885000000000]],
    total_volumes: [[now - 86400000, 26000000000], [now, 28500000000]],
    vs_currency: 'usd'
  };
}
    ...
    articles: [
      {
        ...
        summary: 'Analysis of Bitcoin market dynamics and institutional adoption trends.',
        source: 'CoinDesk',
