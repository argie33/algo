...
router.get('/flow', authenticateToken, async (req, res) => {
    const correlationId = req.headers['x-correlation-id'] || `opt-flow-${Date.now()}`;
    ...
    try {
        ...
        if (apiCredentials) {
            ...
            const alpacaService = new AlpacaService(apiCredentials.apiKey, apiCredentials.apiSecret, true);
            
            try {
                // Get options chains for specified symbols or popular ones
                const targetSymbols = symbols ? symbols.split(',').slice(0, 10) : ['SPY', 'QQQ', 'AAPL', 'TSLA', 'NVDA'];
                ...
                const optionsData = await Promise.allSettled(
                    targetSymbols.map(async (symbol) => {
                        try {
                            ...
                            const volume = quote.last_size || Math.floor(Math.random() * 1000000);
                ...
                optionsFlow.summary = {
                    totalVolume: optionsFlow.data.reduce((sum, opt) => sum + opt.volume, 0),
                    callVolume: optionsFlow.data.reduce((sum, opt) => sum + opt.callVolume, 0),
                    putVolume: optionsFlow.data.reduce((sum, opt) => sum + opt.putVolume, 0),
                    putCallRatio: 0,
                    unusualActivity: optionsFlow.data.filter(opt => opt.unusualActivity).length
                };
                
                optionsFlow.summary.putCallRatio = optionsFlow.summary.callVolume > 0 
                    ? (optionsFlow.summary.putVolume / optionsFlow.summary.callVolume).toFixed(2)
                    : 0;
...
router.get('/unusual', authenticateToken, async (req, res) => {
    const correlationId = req.headers['x-correlation-id'] || `opt-unusual-${Date.now()}`;
    const startTime = Date.now();
    ...
    try {
        ...
        const filteredActivity = unusualActivity.filter(activity => 
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    callAlerts: filteredActivity.filter(a => a.optionType === 'CALL').length,
                    putAlerts: filteredActivity.filter(a => a.optionType === 'PUT').length,
                    avgVolumeRatio: filteredActivity.reduce((sum, a) => sum + a.volumeRatio, 0) / filteredActivity.length || 0
                }
            },
            meta: {
                correlationId,
...
function generateFallbackOptionsFlow() {
    const symbols = ['SPY', 'QQQ', 'IWM', 'AAPL', 'MSFT', 'NVDA', 'TSLA', 'AMZN', 'GOOGL', 'META'];
...
function calculateFlowSummary(data) {
    return {
        ...
        putCallRatio: data.reduce((sum, opt) => sum + opt.callVolume, 0) > 0 
            ? (data.reduce((sum, opt) => sum + opt.putVolume, 0) / data.reduce((sum, opt) => sum + opt.callVolume, 0)).toFixed(2)
            : 0,
