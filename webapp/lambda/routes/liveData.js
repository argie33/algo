...
router.get("/symbols", async (req, res) => {
  const correlationId =
    req.headers["x-correlation-id"] || `livedata-symbols-${Date.now()}`;
  const startTime = Date.now();

  try {
    logger.info("Processing symbols request", {
      correlationId,
      query: req.query,
    });
    ...
    if (category && category === 'stocks') {
      // Already filtering for stocks by default
    }

    if (search) {
      whereConditions.push(`symbol ILIKE $${queryParams.length + 1}`);
      queryParams.push(`%${search.toUpperCase()}%`);
    }

    if (whereConditions.length > 0) {
      symbolsQuery += ` WHERE ${whereConditions.join(' AND ')}`;
    }
    ...
    const result = await query(symbolsQuery, queryParams);

    const symbols = result.rows.map(row => ({
      symbol: row.symbol,
      name: row.symbol, // Use symbol as name fallback
    ...
    const duration = Date.now() - startTime;
    logger.info("Symbols request completed", {
      correlationId,
      duration,
    ...
    const duration = Date.now() - startTime;
    logger.error("Symbols request failed", {
      correlationId,
      duration,
      error: error.message,
      stack: error.stack,
...
router.get("/quotes", async (req, res) => {
  try {
    ...
    try {
      console.log(
        `📊 Live quotes requested for symbols: ${symbols || "popular"}`
      );
    } catch (e) {
      // Ignore console logging errors
    }
...
router.get("/stream", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    try {
      console.log(
        `📡 Starting live data stream for symbols: ${symbolList.join(", ")}`
      );
    } catch (e) {
      // Ignore console logging errors
    }
...
router.get("/latest/:symbols", async (req, res) => {
  try {
    ...
    try {
      console.log(
        `📊 Latest data requested for symbols: ${symbolList.join(", ")}`
      );
    } catch (e) {
      // Ignore console logging errors
    }
