    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (command.trim()) {
        try {
          ...
          console.log(
            `⚠️ [DEBUG] Warning on command ${i + 1}: ${error.message}`
          );
          results.push({
            command: i + 1,
            status: "WARNING",
  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        status[table] = {
          ...
          has_correct_columns:
            schemaResult.rows.some((col) => col.column_name === "symbol") &&
    ...
    res.json({
      ...
      summary: {
        ...
        tables_with_correct_schema: Object.values(status).filter(
          (t) => t.has_correct_columns
        ).length,
      },
      timestamp: new Date().toISOString(),
    });
