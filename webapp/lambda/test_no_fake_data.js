...
async function testNoFakeData() {
  ...
  await runTest(
    "Trading strategies use real database (not hardcoded array)",
    async () => {
      const response = await request(baseURL).get(
        "/api/trading/strategies?limit=5"
      );
      ...
      if (response.status === 200) {
        ...
        const hasHardcodedStrategy = data.some(
          (strategy) =>
  ...
  await runTest(
    "Database errors return proper errors (not fake fallback data)",
    async () => {
      // Test an endpoint that requires database
      const response = await request(baseURL).get("/api/scores/stocks?limit=5");
  ...
  await runTest(
    "Trading routes handle missing DB columns gracefully (no fake data)",
    async () => {
      const response = await request(baseURL).get(
      ...
      if (response.status === 200) {
        ...
        if (data && data.indicators) {
          ...
          if (data.indicators.rsi_14) {
            throw new Error(
              "Still has old column reference (rsi_14) - should be rsi"
            );
          }
        }
      }
  ...
  await runTest(
    "Stock routes return real database data (not hardcoded symbols)",
    async () => {
      const response = await request(baseURL).get("/api/stocks?limit=3");

      if (response.status === 200) {
        const data = response.body.data || [];

        // Should not be returning obvious hardcoded test data
        const hasTestData = data.some(
          (stock) =>
            stock.symbol === "TEST" ||
            stock.symbol === "FAKE" ||
            stock.symbol === "MOCK"
  ...
  await runTest(
    "Portfolio routes require real database (no mock portfolios)",
    async () => {
  ...
  console.log("\n" + "=".repeat(60));
  console.log(
    `📊 No Fake Data Test Results: ${passedTests}/${totalTests} tests passed`
  );
