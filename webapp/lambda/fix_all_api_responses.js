...
function fixAllApiResponses(content) {
  ...
  content = content.replace(/res\.error\(\s*"([^"]+)"\s*,\s*(\d+)\s*,?\s*(\{[^}]*\})?\s*\)/g, (match, message, code, options) => {
    const optionsStr = options ? `, ${options.replace(/^{/, '').replace(/}$/, '')}` : '';
    return `res.status(${code}).json({success: false, error: "${message}"${optionsStr}})`;
  });

  // Fix res.error() without status code (default to 500)
  content = content.replace(/res\.error\(\s*"([^"]+)"\s*,?\s*(\{[^}]*\})?\s*\)/g, (match, message, options) => {
    const optionsStr = options ? `, ${options.replace(/^{/, '').replace(/}$/, '')}` : '';
    return `res.status(500).json({success: false, error: "${message}"${optionsStr}})`;
  });

  // Fix res.success() patterns to proper res.json()
  content = content.replace(/res\.success\(\s*\{([^}]*)\}\s*,?\s*(\d+)?\s*,?\s*(\{[^}]*\})?\s*\)/g, (match, data, code, options) => {
...
routeFiles.forEach(fileName => {
  ...
  try {
    ...
    if (content !== originalContent) {
      ...
      const errorMatches = (originalContent.match(/res\.(error|success)\(/g) || []).length;
      totalFixed += errorMatches;
      filesModified++;
      console.log(`✅ Fixed ${errorMatches} API responses in routes/${fileName}`);
    }
  } catch (error) {
