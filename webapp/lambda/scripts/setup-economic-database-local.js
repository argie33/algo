...
/**
 * Setup Economic Database Schema and Initial Data - Local Development Version
 * Uses local database configuration instead of AWS Secrets Manager
 */
...
const { query, initLocalDatabase } = require('../utils/database-local-dev');

async function setupEconomicDatabaseLocal() {
  console.log('🚀 Setting up Economic Database Schema (Local Development)...');
...
if (require.main === module) {
  setupEconomicDatabaseLocal()
    .then((result) => {
      console.log('✅ Local setup completed:', result);
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = { setupEconomicDatabaseLocal };