...
async function testFinancialEndpoint() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:3001/api/financials/statements?symbol=AAPL"
    );

    if (response.data.meta && response.data.meta.source === "mock_data") {
      console.log(
        "❌ Financial endpoint is returning mock data - database schema issue confirmed"
      );
      return false;
    } else {
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.data.meta && response.data.meta.source === "mock_data") {
          console.log(
            `   ⚠️  Using mock data - indicates database schema issue`
          );
        }
      } catch (error) {
    ...
    console.log(
      "   Check the backend logs (stderr) for database error details."
    );
