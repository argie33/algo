...
const MarketExposure = ({ marketData, breadthData, distributionDaysData }) => {
  ...
  const exposureScore = useMemo(() => {
    ...
    // CRITICAL: Require real index data (no fake defaults)
    if (!indices || !Array.isArray(indices) || indices.length === 0) {
      return { score: null, level: "No Data", signals: [], breakdown: {}, error: "Missing market indices data" };
    }

    const spxIndex = indices.find((i) => i.symbol === "^GSPC");

    // CRITICAL: S&P 500 data is required (no fallbacks)
    if (!spxIndex || spxIndex.changePercent === null || spxIndex.changePercent === undefined) {
      return { score: null, level: "No Data", signals: [], breakdown: {}, error: "Missing S&P 500 data" };
    }
              ...
              <Box>
                ...
                >
                  {exposureScore.breakdown?.breadth !== null && exposureScore.breakdown?.breadth !== undefined
                    ? `${exposureScore.breakdown.breadth >= 0 ? "+" : ""}${exposureScore.breakdown.breadth.toFixed(1)}`
                    : "N/A"}
                </Typography>
              </Box>
            </Grid>
              ...
              <Box>
                ...
                >
                  {exposureScore.breakdown?.sentiment !== null && exposureScore.breakdown?.sentiment !== undefined
                    ? `${exposureScore.breakdown.sentiment >= 0 ? "+" : ""}${exposureScore.breakdown.sentiment.toFixed(1)}`
                    : "N/A"}
                </Typography>
              </Box>
            </Grid>
              ...
              <Box>
                ...
                >
                  {exposureScore.breakdown?.distribution !== null && exposureScore.breakdown?.distribution !== undefined
                    ? `${exposureScore.breakdown.distribution >= 0 ? "+" : ""}${exposureScore.breakdown.distribution.toFixed(1)}`
                    : "N/A"}
                </Typography>
              </Box>
            </Grid>
