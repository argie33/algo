...
const LiveDataMonitor = () => {
  ...
  useEffect(() => {
    ...
    if (isConnected && selectedSymbols.length > 0) {
      websocketService.subscribeMarketData(selectedSymbols);
    }
  ...
  const setupWebSocketListeners = () => {
    ...
    websocketService.on('subscribed', (data) => {
      if (data.channel === 'market_data' && data.symbols) {
  ...
  const handleSubscribeOptions = (symbol) => {
    websocketService.subscribeOptionsData(symbol);
        ...
        <DialogContent dividers>
          <List>
            <ListItem>
              ...
              <ListItemText 
                primary="Server URL" 
                secondary={websocketService.config.url}
              />
            </ListItem>
            <ListItem>
              ...
              <ListItemText 
                ...
                secondary={`${websocketService.config.pingInterval / 1000}s`}
              />
            </ListItem>
            <ListItem>
              ...
              <ListItemText 
                ...
                secondary={`${websocketService.config.maxMessageSize / 1024}KB`}
              />
            </ListItem>
            <ListItem>
