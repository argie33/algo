    ...
    return scanResults.map((stock) => {
      ...
      return {
        ...
        realtimeChange: realtimePrice
          ? ((realtimePrice.price - stock.price) / stock.price) * 100
          : 0,
  ...
  return (
    <Box>
        ...
        <CardContent>
          ...
          <Alert severity="info" sx={{ mt: 2 }}>
            <Typography variant="body2">
              <strong>{scanConfigs[scanType].name}:</strong>{" "}
              {scanConfigs[scanType].description}
            </Typography>
          </Alert>
        </CardContent>
      </Card>
            ...
            <Table size="small">
              ...
              <TableBody>
                {enhancedResults.slice(0, 25).map((stock) => {
                  const isPositive =
                    (stock.realtimeChange || stock.priceChange || 0) >= 0;
                  const totalChange =
                    (stock.priceChange || 0) + (stock.realtimeChange || 0);
                  ...
                  return (
                    <TableRow
                      ...
                      hover
                      sx={{
                        backgroundColor: stock.isRealTime
                          ? "rgba(76, 175, 80, 0.05)"
                          : "inherit",
          ...
          {enhancedResults.length === 0 && !loading && (
            <Box textAlign="center" py={4}>
              <Typography color="text.secondary">
                No stocks found matching the current scan criteria. Try running
                a scan.
              </Typography>
            </Box>
          )}
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>{selectedStock?.symbol} - AI Analysis Details</DialogTitle>
        <DialogContent>
          {selectedStock && (
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                ...
                <Typography>
                  Daily Change:{" "}
                  {formatPercentage(selectedStock.priceChange || 0)}
                </Typography>
                <Typography>
                  Volume Ratio: {selectedStock.volumeRatio?.toFixed(1)}x
                </Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="h6" gutterBottom>
