...
const CostOptimizer = ({ costData, onOptimize, onApplyRecommendation }) => {
  ...
  const mockCostBreakdown = [
    { provider: 'Polygon', current: 18.75, optimized: 14.20, savings: 4.55, confidence: 95 },
    { provider: 'Alpaca', current: 12.50, optimized: 11.80, savings: 0.70, confidence: 85 },
    { provider: 'Finnhub', current: 8.20, optimized: 6.90, savings: 1.30, confidence: 90 }
  ];
  ...
  const mockOptimizations = [
    {
      ...
      description: 'Switch to Alpaca for non-critical symbols during 8PM-6AM EST',
      impact: 'High',
      savings: '$4.55/day',
      confidence: 95,
      effort: 'Low',
    ...
    },
    {
      ...
      description: 'Extend cache TTL for stable symbols during low volatility periods',
      impact: 'Medium',
      savings: '$1.85/day',
      confidence: 92,
      effort: 'Low',
    ...
    },
    {
      ...
      description: 'Merge multiple low-usage connections into single connections',
      impact: 'Low',
      savings: '$0.95/day',
      confidence: 85,
      effort: 'High',
  ...
  const totalSavings = mockOptimizations.reduce((sum, opt) => 
    sum + parseFloat(opt.savings.replace('$', '').replace('/day', '')), 0
  );
            ...
            <CardContent>
              ...
              <Box mb={3}>
                <FormControl fullWidth size="small">
                  ...
                  >
                    <MenuItem value="aggressive">Aggressive (Max Savings)</MenuItem>
                    <MenuItem value="balanced">Balanced</MenuItem>
                    <MenuItem value="conservative">Conservative (Min Risk)</MenuItem>
                  </Select>
                </FormControl>
              </Box>

              <Alert severity="info" size="small">
                Current usage: {((39.45 / budgetLimit) * 100).toFixed(1)}% of budget
              </Alert>
            </CardContent>
          </Card>
        </Grid>
                  ...
                  <TableBody>
                    {mockCostBreakdown.map((row) => (
                      <TableRow key={row.provider}>
                        <TableCell fontWeight="medium">{row.provider}</TableCell>
          ...
          <Card elevation={2}>
            <CardContent>
              ...
              <Alert severity="success" sx={{ mb: 2 }}>
                <AlertTitle>Potential Monthly Savings: ${(totalSavings * 30).toFixed(2)}</AlertTitle>
                Implementing all recommendations could save you ${totalSavings.toFixed(2)} per day
              </Alert>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12}>
                        <Box display="flex" justifyContent="space-between" alignItems="center" mt={2}>
                          <Box display="flex" gap={2}>
                            <Chip label={`${optimization.confidence}% confidence`} size="small" />
                            <Chip label={`${optimization.effort} effort`} size="small" variant="outlined" />
                          </Box>
                          <Box display="flex" gap={1}>
                            <Button
                              ...
                              onClick={() => handleOptimizationPreview(optimization)}
                            >
                              Preview
                            </Button>
                            <Button
                              size="small"
                              variant="contained"
                              color={optimization.autoApply ? 'success' : 'primary'}
          ...
          {selectedOptimization && (
            <Box sx={{ pt: 2 }}>
              <Alert severity="info" sx={{ mb: 2 }}>
                ...
                This optimization will save {selectedOptimization.savings} with {selectedOptimization.confidence}% confidence
              </Alert>
