...
// Portfolio API functions - with proper authentication
export const getPortfolioData = async (accountType = 'paper') => {
  console.log(`📊 [API] Fetching portfolio data for account type: ${accountType}`);
  
  try {
    const response = await api.get(`/portfolio/holdings?accountType=${accountType}`);
    console.log('📊 [API] Portfolio data response:', response.data);
    
    return {
      success: true,
      holdings: response.data.data?.holdings || [],
      summary: response.data.data?.summary || {},
      dataSource: response.data.dataSource || 'api',
      timestamp: response.data.timestamp
    };
  } catch (error) {
    console.error('❌ [API] Portfolio data error:', error);
    const errorMessage = handleApiError(error, 'get portfolio data');
    return { 
      success: false, 
      holdings: [], 
      summary: {},
...
export const getAccountInfo = async (accountType = 'paper') => {
  console.log(`🏦 [API] Fetching account info for account type: ${accountType}`);
  
  try {
    const response = await api.get(`/portfolio/account?accountType=${accountType}`);
    console.log('🏦 [API] Account info response:', response.data);
    
    return {
      success: true,
      accountType: response.data.data?.accountType || accountType,
      balance: response.data.data?.totalValue || 0,
      equity: response.data.data?.totalValue || 0,
      dayChange: response.data.data?.dayChange || 0,
      dayChangePercent: response.data.data?.dayChangePercent || 0,
      timestamp: response.data.timestamp
    };
  } catch (error) {
    console.error('❌ [API] Account info error:', error);
    const errorMessage = handleApiError(error, 'get account info');
    return { 
      success: false, 
      accountType,
      balance: 0,
...
export const importPortfolioFromBroker = async (provider, accountType, keyId) => {
  console.log(`📥 [API] Importing portfolio from ${provider} (${accountType})`);
  
  try {
    const response = await api.post('/portfolio/import', {
      provider,
      accountType,
      keyId
    });
    console.log('📥 [API] Portfolio import response:', response.data);
    ...
    const errorMessage = handleApiError(error, `import portfolio from ${provider}`);
    return { 
      success: false, 
      error: errorMessage,
      timestamp: new Date().toISOString()
...
export const getAvailableAccounts = async () => {
  console.log('🔑 [API] Fetching available broker accounts...');
  
  try {
    const response = await api.get('/user/api-keys');
    console.log('🔑 [API] Available accounts response:', response.data);
    ...
    console.error('❌ [API] Available accounts error:', error);
    const errorMessage = handleApiError(error, 'get available accounts');
    return { 
      success: false, 
      accounts: [],
      error: errorMessage,
...
export const getApiKeys = async () => {
  ...
  try {
    const response = await api.get('/user/api-keys');
    console.log('🔑 [API] API keys response:', response.data);
    ...
    const errorMessage = handleApiError(error, 'get API keys');
    return { 
      success: false, 
      keys: [],
      error: errorMessage,
...
export const testApiConnection = async (provider, keyId) => {
  console.log(`🔍 [API] Testing API connection for ${provider}`);
  
  try {
    const response = await api.post('/user/api-keys/test', {
      provider,
      keyId
    });
    console.log('🔍 [API] API connection test response:', response.data);
    ...
    console.error('❌ [API] API connection test error:', error);
    const errorMessage = handleApiError(error, `test ${provider} API connection`);
    return { 
      success: false, 
...
export const addHolding = async (holding) => {
  console.log('➕ [API] Adding portfolio holding:', holding);
  
  try {
    const response = await api.post('/portfolio/holdings', holding);
    console.log('➕ [API] Add holding response:', response.data);
    ...
    console.error('❌ [API] Add holding error:', error);
    const errorMessage = handleApiError(error, 'add portfolio holding');
    return { 
      success: false, 
      error: errorMessage,
      timestamp: new Date().toISOString()
...
export const updateHolding = async (holdingId, updates) => {
  console.log(`✏️ [API] Updating portfolio holding ${holdingId}:`, updates);
  
  try {
    const response = await api.put(`/portfolio/holdings/${holdingId}`, updates);
    console.log('✏️ [API] Update holding response:', response.data);
    ...
    console.error('❌ [API] Update holding error:', error);
    const errorMessage = handleApiError(error, 'update portfolio holding');
    return { 
      success: false, 
      error: errorMessage,
      timestamp: new Date().toISOString()
...
export const deleteHolding = async (holdingId) => {
  console.log(`🗑️ [API] Deleting portfolio holding ${holdingId}`);
  
  try {
    const response = await api.delete(`/portfolio/holdings/${holdingId}`);
    console.log('🗑️ [API] Delete holding response:', response.data);
    ...
    console.error('❌ [API] Delete holding error:', error);
    const errorMessage = handleApiError(error, 'delete portfolio holding');
    return { 
      success: false, 
      error: errorMessage,
      timestamp: new Date().toISOString()
