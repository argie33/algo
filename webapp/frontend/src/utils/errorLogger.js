 ...
 * @param {object} context - Additional context
 */
export const logApiSuccess = (component, operation, result = null, context = {}) => {
  const timestamp = new Date().toISOString();
  ...
  console.log(`✅ ${component} - ${operation} succeeded`, {
    ...
    component,
    operation,
    resultSize: result ? (Array.isArray(result) ? result.length : Object.keys(result).length) : 'N/A',
    context
  });
};
...
export const createComponentLogger = (component) => ({
  error: (operation, error, context) => logApiError(component, operation, error, context),
  queryError: (queryKey, error, context) => logQueryError(component, queryKey, error, context),
  success: (operation, result, context) => logApiSuccess(component, operation, result, context)
});
