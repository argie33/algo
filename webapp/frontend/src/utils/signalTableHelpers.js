...
export const formatCellValue = (value, key) => {
  ...
  if (
    ...
    key === "range_low" ||
    key === "current_price" ||
    key === "exit_trigger_1_price" ||
    key === "exit_trigger_2_price" ||
    key === "exit_trigger_3_price" ||
    key === "exit_trigger_4_price" ||
    key === "macd" ||
    key === "signal_line"
  ) {
  ...
  if (key.includes("date") && typeof value === "string") {
    try {
      ...
      console.error(
        "[SignalTableHelpers] Failed to format signal table date:",
        err?.message || err
      );
      return value;
    }
...
export const getCellAlign = (key) => {
  if (
    ...
    key === "stage_number" ||
    key === "stage_confidence" ||
    key === "macd" ||
    key === "signal_line" ||
    key === "td_buy_setup_count" ||
    key === "td_sell_setup_count" ||
    key === "td_buy_countdown_count" ||
    key === "td_sell_countdown_count"
  ) {
    return "right";
  }
  return "left";
