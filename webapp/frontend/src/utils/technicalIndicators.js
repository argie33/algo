...
export const calculateEMA = (data, period, key = "close") => {
  ...
  let ema =
    data.slice(0, period).reduce((acc, item) => acc + (item[key] || 0), 0) /
    period;
...
export const calculateMACD = (
  ...
  for (let i = slowPeriod - 1; i < data.length; i++) {
    const macd =
      fastEMA[i][`ema${fastPeriod}`] - slowEMA[i][`ema${slowPeriod}`];
  ...
  if (result.length >= signalPeriod) {
    ...
    let signalEMA =
      result.slice(0, signalPeriod).reduce((acc, item) => acc + item.macd, 0) /
      signalPeriod;
...
export const calculateBollingerBands = (
  ...
  for (let i = period - 1; i < data.length; i++) {
    ...
    result.push({
      ...
      bbPercent:
        ((data[i][key] - (sma - standardDeviation * stdDev)) /
...
export const calculateStochastic = (data, kPeriod = 14, dPeriod = 3) => {
  ...
  for (let i = kPeriod - 1; i < data.length; i++) {
    ...
    const lowestLow = Math.min(
      ...slice.map((item) => item.low || Number.MAX_VALUE)
    );
...
export const calculateCCI = (data, period = 20) => {
  ...
  for (let i = period - 1; i < data.length; i++) {
    ...
    const typicalPrices = slice.map(
      (item) => ((item.high || 0) + (item.low || 0) + (item.close || 0)) / 3
    );
    ...
    const meanDeviation =
      typicalPrices.reduce((acc, tp) => acc + Math.abs(tp - smaTP), 0) / period;
...
export const calculateMFI = (data, period = 14) => {
  ...
  for (let i = 1; i < data.length; i++) {
    const typical =
      ((data[i].high || 0) + (data[i].low || 0) + (data[i].close || 0)) / 3;
    const prevTypical =
      ((data[i - 1].high || 0) +
        (data[i - 1].low || 0) +
        (data[i - 1].close || 0)) /
...
export const calculateVWAP = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    result[i].vwap =
      cumulativeVolume > 0 ? cumulativeVolumePrice / cumulativeVolume : 0;
  }
...
export const calculateFibonacci = (data, high = null, low = null) => {
  ...
  const dataHigh =
    high !== null ? high : Math.max(...data.map((item) => item.high || 0));
  const dataLow =
    low !== null
      ? low
      : Math.min(...data.map((item) => item.low || Number.MAX_VALUE));
    ...
    if (isUptrend) {
      ...
      if (low <= sar) {
        ...
        const prevLow = Math.min(
          data[i - 1].low || 0,
          i > 1 ? data[i - 2].low || 0 : data[i - 1].low || 0
        );
      ...
      if (high >= sar) {
        ...
        const prevHigh = Math.max(
          data[i - 1].high || 0,
          i > 1 ? data[i - 2].high || 0 : data[i - 1].high || 0
        );
