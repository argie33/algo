...
test.describe('Performance Tests', () => {
  ...
  test('Bundle size should be within budget', async ({ page }) => {
    ...
    page.on('response', async response => {
      ...
      if ((url.includes('.js') || url.includes('.css')) && !url.includes('node_modules')) {
    ...
    if (resources.length > 0) {
      expect(totalJsSize, `JavaScript bundle: ${Math.round(totalJsSize/1024)}KB`).toBeLessThan(10000 * 1024); // 10MB - realistic for financial platform with charting
      expect(totalCssSize, `CSS bundle: ${Math.round(totalCssSize/1024)}KB`).toBeLessThan(500 * 1024); // 500KB 
      expect(totalSize, `Total bundle: ${Math.round(totalSize/1024)}KB`).toBeLessThan(10500 * 1024); // 10.5MB total
    } else {
  ...
  test('Page should be responsive on mobile', async ({ page }) => {
    ...
    expect(mobileLoadTime, `Mobile load time: ${mobileLoadTime}ms`).toBeLessThan(8000);
  });
  ...
  test('API responses should be fast', async ({ page }) => {
    ...
    for (const api of apiTimes) {
      expect(api.responseTime, `${api.url}: ${api.responseTime}ms`).toBeLessThan(1000); // 1s
    }
  });
  ...
  test('Memory usage should be reasonable', async ({ page }) => {
    ...
    if (metrics) {
      ...
      expect(metrics.total, `Memory total: ${metrics.total}MB`).toBeLessThan(150); // 150MB
    }
  });
  ...
  test('Navigation should be instant', async ({ page }) => {
    ...
    const portfolioNav = page.locator('a[href="/portfolio"]').or(page.locator(':has-text("Portfolio")')).first();
    if (await portfolioNav.count() > 0) {
      ...
      expect(navigationTime, `Navigation time: ${navigationTime}ms`).toBeLessThan(2000);
    } else {
      console.log('ℹ️ Portfolio navigation not found - test passed as page loaded');
