...
test.describe('Edge Case Validation - Comprehensive Scenarios', () => {
  ...
  test('Should handle extremely large financial data sets', async ({ page }) => {
    ...
    const hasPortfolioData = pageContent.includes('$') || pageContent.includes('%');
    
    // Look for virtualization or pagination
    const virtualizationElements = await page.locator(
    ...
    expect(renderTime, `Render time for 1000 holdings: ${renderTime}ms`).toBeLessThan(15000);
    expect(hasPortfolioData, 'Should display portfolio data').toBe(true);
  });

  test('Should handle extreme financial values and precision', async ({ page }) => {
      ...
      if (url.includes('/portfolio')) {
        route.fulfill({
          json: {
            ...
            data: {
              ...
              holdings: [
                { symbol: 'BERKSHIRE', quantity: 1, currentPrice: 500000.99, totalValue: 500000.99, gainLoss: 0.01 },
                { symbol: 'PENNY', quantity: 1000000, currentPrice: 0.0001, totalValue: 100, gainLoss: -50 },
                { symbol: 'CRYPTO', quantity: 0.000001, currentPrice: 50000000, totalValue: 50, gainLoss: 25.5555 }
              ]
            }
          }
        });
        ...
        route.fulfill({
          json: {
            ...
            data: {
              indices: {
                EXTREME: { price: 9999999.999999, change: -0.000001, changePercent: -0.0000001 }
              }
            }
          }
        });
    ...
    for (const testPage of extremeValuePages) {
      ...
      const hasLargeNumbers = /$[0-9,]+/.test(pageContent) || /[0-9]+\.[0-9]{2}/.test(pageContent);
      const hasScientificNotation = /[0-9]+e[+-][0-9]+/i.test(pageContent);
      const hasInfinityOrNaN = pageContent.includes('Infinity') || pageContent.includes('NaN');
      ...
      console.log(`💰 ${testPage}: Large numbers: ${hasLargeNumbers}, Scientific: ${hasScientificNotation}, Invalid: ${hasInfinityOrNaN}`);
    }
    ...
    const validPages = extremeResults.filter(r => r.hasLargeNumbers && !r.hasInfinityOrNaN);
    
    console.log(`📊 Extreme values: ${validPages.length} pages handle properly, ${invalidPages.length} show invalid`);
    expect(invalidPages.length).toBe(0); // No pages should show Infinity/NaN
    expect(validPages.length).toBeGreaterThan(0); // At least one should show proper formatting
  });
  ...
  test('Should handle special characters and internationalization', async ({ page }) => {
        ...
        json: {
          ...
          data: {
            symbols: [
              ...
              'émoji🚀📈💰', // Emoji symbols
              'NULL', 'undefined', 'DROP TABLE;' // Potential injection
            ],
            currencies: ['$', '€', '£', '¥', '₹', '₽', '₩', '₦'],
            names: [
              'Company™ Inc.',
    ...
    const hasEmojis = /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]/u.test(pageContent);
    const hasSpecialSymbols = /[™®©]/.test(pageContent);
    const hasInjectionAttempts = pageContent.includes('DROP TABLE') || pageContent.includes('<script>');
    ...
    expect(hasUnicodeCharacters, 'Should support Unicode characters').toBe(true);
    expect(hasInjectionAttempts, 'Should not execute injection attempts').toBe(false);
  });

  test('Should handle rapid user input and interaction edge cases', async ({ page }) => {
    ...
    if (inputs > 0) {
      ...
      for (let i = 0; i < Math.min(inputs, 3); i++) {
        ...
        if (await inputField.isVisible() && await inputField.isEnabled()) {
          ...
          for (const testInput of testInputs.slice(0, 8)) { // Limit for speed
            try {
              const startTime = Date.now();
              ...
              inputTestResults.push({ inputIndex: i, input: testInput.slice(0, 20), responseTime, success: true });
              
              // Brief pause to prevent overwhelming
              await page.waitForTimeout(50);
              
            } catch (error) {
              inputTestResults.push({ inputIndex: i, input: testInput.slice(0, 20), error: error.message, success: false });
      ...
      const avgResponseTime = successfulInputs.length > 0 ? 
        successfulInputs.reduce((sum, r) => sum + r.responseTime, 0) / successfulInputs.length : 0;
      
      console.log(`⚡ Input testing results:`);
      console.log(`   Successful: ${successfulInputs.length}, Failed: ${failedInputs.length}`);
      console.log(`   Average response time: ${Math.round(avgResponseTime)}ms`);
      
      expect(successfulInputs.length).toBeGreaterThan(0);
      if (avgResponseTime > 0) {
        expect(avgResponseTime, `Average input response time: ${avgResponseTime}ms`).toBeLessThan(1000);
      }
    } else {
  ...
  test('Should handle browser limitation edge cases', async ({ page }) => {
    ...
    try {
      ...
      browserTests.push({ test: 'localStorage_1MB', success: false, error: error.message });
    }
    ...
    try {
      ...
      browserTests.push({ test: 'long_url', success: false, error: error.message });
    }
    ...
    try {
      ...
      browserTests.push({ test: 'many_dom_elements', success: elementCount > 5000 });
    } catch (error) {
      browserTests.push({ test: 'many_dom_elements', success: false, error: error.message });
    }
    ...
    try {
      ...
      browserTests.push({ test: 'memory_pressure', success: false, error: error.message });
    }
    ...
    try {
      ...
      browserTests.push({ test: 'cookie_overflow', success: false, error: error.message });
    }
    ...
    console.log(`   Successful: ${successfulTests.length}/${browserTests.length}`);
    console.log(`   Failed tests: ${failedTests.map(t => t.test).join(', ')}`);
    
    browserTests.forEach(test => {
      console.log(`   ${test.test}: ${test.success ? '✅' : '❌'} ${test.error ? test.error.slice(0, 30) : ''}`);
    });
    
    // Should handle most browser limitations gracefully
    expect(successfulTests.length).toBeGreaterThanOrEqual(Math.floor(browserTests.length * 0.6)); // 60% should pass
  });
  ...
  test('Should handle concurrent operations and race conditions', async ({ page }) => {
    ...
    await page.route('**/api/**', async route => {
      ...
      console.log(`🏃 API call ${callId} starting with ${Math.round(delay)}ms delay`);
    ...
    try {
      ...
      if (apiCallTimes.length > 0) {
        const avgApiTime = apiCallTimes.reduce((a, b) => a + b) / apiCallTimes.length;
        console.log(`   Average API response time: ${Math.round(avgApiTime)}ms`);
      }
      
      // Should handle concurrent operations without corruption
      expect(hasContent, 'Page should have content after concurrent operations').toBe(true);
      ...
      console.log(`🏃 Concurrent operations completed with some failures: ${error.message.slice(0, 50)}`);
  ...
  test('Should handle accessibility edge cases', async ({ page }) => {
    ...
    try {
      ...
      if (focusedElement) {
        console.log(`♿ Keyboard navigation completed, focused on: ${focusedElement}`);
      }
    } catch (error) {
    ...
    try {
      const ariaAttributes = await page.evaluate(() => {
        ...
        elements.forEach(el => {
          if (el.hasAttribute('aria-label') || el.hasAttribute('aria-labelledby') || el.hasAttribute('role')) {
            ariaCount++;
          }
      ...
      console.log(`♿ ARIA attributes: ${ariaAttributes.ariaCount}, Labels: ${ariaAttributes.labelCount}`);
    } catch (error) {
    ...
    try {
      ...
      const pageStillVisible = (await page.locator('#root').textContent()).length > 100;
      console.log(`♿ Low contrast simulation: Page readable: ${pageStillVisible}`);
    ...
    expect(accessibilityIssues.length, 'Should minimize accessibility issues').toBeLessThanOrEqual(2);
  });

  test('Should handle data corruption and validation edge cases', async ({ page }) => {
    ...
    await page.route('**/api/**', route => {
      const corruptedData = {
        ...
        data: {
          ...
          holdings: [
            { symbol: null, quantity: -1, currentPrice: undefined, totalValue: 'invalid' },
            { symbol: '', quantity: 0, currentPrice: NaN, totalValue: -0 },
            {}, // Empty object
            { symbol: 'TEST', extraField: 'unexpected_data', quantity: '100', currentPrice: '50.00' } // Mixed types
          ],
          metadata: {
    ...
    const validationResults = {
      ...
      showsErrorMessage: pageContent.toLowerCase().includes('error') || pageContent.toLowerCase().includes('invalid'),
      showsEmptyState: pageContent.toLowerCase().includes('no data') || pageContent.toLowerCase().includes('empty')
    };
    ...
    expect(validationResults.hasContent, 'Should display some content').toBe(true);
    expect(validationResults.showsInfinity, 'Should not display raw Infinity').toBe(false);
    expect(validationResults.showsNaN, 'Should not display raw NaN').toBe(false);
    expect(validationResults.showsNull, 'Should not display raw null').toBe(false);
    expect(validationResults.showsUndefined, 'Should not display raw undefined').toBe(false);
  });
});