...
test.describe('Financial Platform - Authentication Flows', () => {
  ...
  test('should handle login flow', async ({ page }) => {
    ...
    const signInButton = page.locator('button:has-text("Sign In"), [aria-label*="sign"], [data-testid*="auth"]').first();
    ...
    if (await signInButton.isVisible()) {
      ...
      const authForm = await page.locator('form, [role="dialog"], .auth, .login').first();
      ...
      console.log('ℹ️ No authentication UI found - may be already authenticated');
    }
    
    // Check for authentication state indicators
    const hasAuthState = await page.locator('[data-testid*="user"], .user-menu, .profile').count();
  ...
  test('should handle API key setup flow', async ({ page }) => {
    ...
    const apiKeyElements = await page.locator(
    ...
    const providerElements = await page.locator(
    ...
    expect(apiKeyElements + providerElements + setupElements).toBeGreaterThan(0);
  });
    ...
    for (const route of protectedRoutes) {
      try {
        ...
        const hasAuthPrompt = await page.locator(
          '.auth, .login, [role="dialog"]'
        ).count();
        
        // Check for text-based auth indicators
        const textBasedAuth = await page.locator('text("Sign In"), text("Login")').count();
        
        if (currentUrl.includes('login') || currentUrl.includes('auth') || hasAuthPrompt > 0 || textBasedAuth > 0) {
  ...
  test('should handle logout flow', async ({ page }) => {
    ...
    const logoutElements = await page.locator(
    ...
    const userMenu = await page.locator(
      '.user-menu, .profile, .avatar, [data-testid*="user"]'
    ).count();
