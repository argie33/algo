...
test.describe("Portfolio Optimizer Page", () => {
  test("loads and displays portfolio analysis", async ({ page }) => {
    ...
    const response = await page.goto(
      "http://localhost:5173/portfolio/optimize",
      {
  ...
  test("fetches portfolio analysis from API", async ({ page }) => {
    ...
    if (data.analysis) {
      ...
      console.log(
        `✅ Portfolio loaded: ${portfolio.holdings_count} holdings, $${portfolio.total_value}`
      );
      ...
      console.log(
        `✅ Metrics calculated: Sharpe=${portfolio.metrics.sharpeRatio}, Beta=${portfolio.metrics.beta}`
      );
      ...
      if (recommendations.length > 0) {
        ...
        console.log(
          `✅ First recommendation: ${firstRec.action} ${firstRec.symbol} - ${firstRec.reason}`
        );
      }
    }
  });
  ...
  test("displays recommendations with proper structure", async ({ page }) => {
    ...
    recommendations.forEach((rec, idx) => {
      ...
      console.log(
        `  Rec ${idx + 1}: ${rec.action} ${rec.symbol} (${rec.priority})`
      );
    });

    console.log(
      `✅ All ${recommendations.length} recommendations have proper structure`
    );
  });
  ...
  test("shows portfolio weaknesses and improvements", async ({ page }) => {
    ...
    if (portfolio) {
      ...
      issues.forEach((issue) => {
        ...
        console.log(
          `  - ${issue.metric}: ${issue.problem} (${issue.priority})`
        );
      });
      ...
      if (improvements) {
        ...
        Object.entries(improvements).forEach(([metric, data]) => {
          ...
          console.log(
            `  - ${metric}: ${imp.current} → ${imp.expected} (${imp.improved ? "✓" : "✗"})`
          );
        });
      }
    }
...
test.describe("Portfolio Optimizer Integration", () => {
  test("recommendation priorities are correct", async ({ page }) => {
    ...
    const lowPriorityRecs = recommendations.filter(
      (r: any) => r.priority === "low"
    );

    console.log(
      `✅ Found: ${highPriorityRecs.length} high, ${mediumPriorityRecs.length} medium, ${lowPriorityRecs.length} low priority recs`
    );
  ...
  test("sector diversification recommendations work", async ({ page }) => {
    ...
    const sectorRecs = recommendations.filter(
      (r: any) => r.reason && r.reason.includes("Sector diversification")
    );

    console.log(
      `✅ Found ${sectorRecs.length} sector diversification recommendations`
    );
  ...
  test("Sharpe ratio improvements are recommended", async ({ page }) => {
    ...
    const sharpeRecs = recommendations.filter(
      (r: any) => r.reason && r.reason.includes("Sharpe")
    );

    console.log(
      `✅ Found ${sharpeRecs.length} Sharpe ratio improvement recommendations`
    );
