...
async function globalSetup() {
  ...
  const testConfig = {
    baseURL: process.env.PLAYWRIGHT_BASE_URL || 'https://d1copuy2oqlazx.cloudfront.net',
    apiURL: process.env.VITE_API_URL || 'https://jh28jhdp01.execute-api.us-east-1.amazonaws.com/dev',
    timeout: 30000,
  };
  ...
  try {
    ...
    const apiHealthResponse = await page.request.get(`${testConfig.apiURL}/health`, {
      timeout: testConfig.timeout,
    });
    
    if (apiHealthResponse.ok()) {
      const healthData = await apiHealthResponse.json();
      console.log('✅ API health check passed:', healthData.status || 'healthy');
    } else {
    ...
    try {
      const authResponse = await page.request.get(`${testConfig.apiURL}/auth/status`);
      if (authResponse.ok()) {
