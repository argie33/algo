...
test.describe("Complete User Portfolio Workflow", () => {
  ...
  test("Complete user onboarding and portfolio setup flow", async () => {
    ...
    await test.step("User sets up API keys", async () => {
      ...
      await page.fill(
        '[data-testid="alpaca-key-id"]',
        TEST_CONFIG.testApiKeys.alpaca.keyId
      );
      await page.fill(
        '[data-testid="alpaca-secret"]',
        TEST_CONFIG.testApiKeys.alpaca.secret
      );
      ...
      // Wait for validation
      await page.waitForSelector('[data-testid="api-key-success"]', {
        timeout: 10000,
      });

      // Verify API key is validated
      const validationStatus = await page.textContent(
        '[data-testid="alpaca-status"]'
      );
    ...
    await test.step("User accesses portfolio page", async () => {
      ...
      if (isEmpty) {
        ...
        const emptyMessage = await page.textContent(
          '[data-testid="empty-portfolio-message"]'
        );
        expect(emptyMessage).toContain("No holdings");

        // Should show call-to-action
        expect(
          await page.isVisible('[data-testid="add-position-button"]')
        ).toBeTruthy();
      } else {
    ...
    await test.step("User views market data and research", async () => {
      ...
      expect(
        await page.isVisible('[data-testid="market-indices"]')
      ).toBeTruthy();
      ...
      const stockSymbol = await page.textContent(
    ...
    await test.step("User interacts with real-time features", async () => {
      ...
      if (await page.isVisible('[data-testid="real-time-toggle"]')) {
        ...
        await page.waitForSelector('[data-testid="real-time-connected"]', {
          timeout: 5000,
        });
        const connectionStatus = await page.textContent(
    ...
    await test.step("Application works on mobile devices", async () => {
      ...
      expect(
        await page.isVisible('[data-testid="mobile-portfolio-view"]')
      ).toBeTruthy();
      ...
      if (await page.isVisible('[data-testid="mobile-holdings-list"]')) {
        ...
        expect(
          await page.isVisible('[data-testid="holding-details-modal"]')
        ).toBeTruthy();
      }
    });
  ...
  test("Portfolio data accuracy and calculations", async () => {
    ...
    await test.step("Verify portfolio calculations", async () => {
      ...
      const holdings = await page.$$eval(
        '[data-testid="holding-row"]',
        (rows) => {
          return rows.map((row) => ({
            symbol: row.querySelector('[data-testid="holding-symbol"]')
              ?.textContent,
            quantity: parseFloat(
              row.querySelector('[data-testid="holding-quantity"]')
                ?.textContent || "0"
            ),
            price: parseFloat(
              row
                .querySelector('[data-testid="holding-price"]')
                ?.textContent?.replace(/[$,]/g, "") || "0"
            ),
            value: parseFloat(
              row
                .querySelector('[data-testid="holding-value"]')
                ?.textContent?.replace(/[$,]/g, "") || "0"
            ),
          }));
        }
      ...
      const displayedTotal = await page.textContent(
        '[data-testid="total-portfolio-value"]'
      );
    ...
    await test.step("Verify real-time price updates", async () => {
      ...
      const _initialPrices = await page.$$eval(
      ...
      const updatedPrices = await page.$$eval(
        '[data-testid="holding-price"]',
        (elements) => {
          return elements.map((el) => el.textContent);
        }
