...
test.describe("Data Validation - Verify All Pages Show Real Data", () => {
  ...
  test("Dashboard displays real portfolio data", async ({ page }) => {
    ...
    const portfolioSection = await page
      .locator('[data-testid="portfolio-summary"]')
      .isVisible()
      .catch(() => false);
  ...
  test("Market Overview displays market data", async ({ page }) => {
    ...
    const marketData = await page
      .locator("text=/market|breadth|sentiment/i")
      .count();
  ...
  test("Trading Signals displays buy/sell signals", async ({ page }) => {
    ...
    const symbolElements = await page
  ...
  test("Sector Analysis displays sectors", async ({ page }) => {
    ...
    const sectorNames = await page
      .locator("text=/technology|finance|healthcare|energy/i")
      .count();
  ...
  test("Analyst Insights displays ratings", async ({ page }) => {
    ...
    const analysts = await page
      .locator('[data-testid*="analyst"]')
      .count()
      .catch(() => 0);
  ...
  test("Economic Dashboard displays economy data", async ({ page }) => {
    ...
    const economicTerms = await page
      .locator("text=/gdp|inflation|unemployment|rate/i")
      .count();
  ...
  test("Earnings Calendar shows earnings", async ({ page }) => {
    ...
    const earningsItems = await page
      .locator('[data-testid*="earning"]')
      .count()
      .catch(() => 0);
  ...
  test("Sentiment page displays sentiment data", async ({ page }) => {
    ...
    const sentiments = await page
      .locator("text=/bullish|bearish|neutral|sentiment/i")
      .count();
  ...
  test("Watchlist displays monitored stocks", async ({ page }) => {
    ...
    const watchlistItems = await page
      .locator('[data-testid*="watchlist"]')
      .count()
      .catch(() => 0);
    const symbols = await page
      .locator("text=/[A-Z]{1,5}/")
  ...
  test("Trade History displays trades", async ({ page }) => {
    ...
    const tradeItems = await page
      .locator('[data-testid*="trade"]')
      .count()
      .catch(() => 0);
    const rows = await page
      .locator("tbody tr")
