...
test.describe("Visual Regression Tests", () => {
  ...
  test("Portfolio chart component visual comparison", async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="portfolio-chart"], .recharts-wrapper',
      { timeout: 10000 }
    );
    await page.waitForTimeout(2000);

    const chartElement = page
      .locator('[data-testid="portfolio-chart"], .recharts-wrapper')
      .first();
  ...
  test("Error state visual comparison", async ({ page }) => {
    ...
    const errorVisible =
      (await page
        .locator(
          '[data-testid*="error"], .error, [class*="error"], :has-text("error"), :has-text("failed"), :has-text("unavailable")'
        )
        .count()) > 0;
