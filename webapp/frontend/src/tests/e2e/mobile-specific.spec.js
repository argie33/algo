...
test.describe('Mobile-Specific Testing', () => {
  ...
  test.beforeEach(async ({ page, isMobile, browserName }) => {
    ...
    const timeout = browserName === 'firefox' ? 8000 : browserName === 'webkit' ? 10000 : 6000;
        ...
        route.fulfill({
          json: {
            ...
            data: {
              ...
              holdings: [
                { symbol: 'AAPL', quantity: 10, currentPrice: 195.20, totalValue: 1952.00 },
                { symbol: 'MSFT', quantity: 5, currentPrice: 385.75, totalValue: 1928.75 },
                { symbol: 'GOOGL', quantity: 2, currentPrice: 2650.75, totalValue: 5301.50 }
              ]
            }
          }
        });
  ...
  test('Mobile dashboard should load and display key metrics', async ({ page }) => {
    ...
    const mobileMenus = await page.locator('button[aria-label*="menu"], .mobile-menu, .hamburger').count();
  ...
  test('Mobile portfolio page should be touch-optimized', async ({ page }) => {
    ...
    const tapTargets = await page.locator('button, [role="button"], .clickable').all();
  ...
  test('Mobile market data should be readable and accessible', async ({ page }) => {
    ...
    console.log(`📱 Readable text elements: ${readableTextCount}/${Math.min(textElements.length, 20)}`);
    ...
    if (tables > 0) {
      ...
      const tableOverflow = await page.locator('table').first().evaluate(el => {
  ...
  test('Mobile navigation should work with touch gestures', async ({ page }) => {
    ...
    for (const route of mobileRoutes) {
      try {
        ...
        console.log(`❌ Mobile navigation to ${route} failed: ${e.message.slice(0, 50)}`);
      }
    }
    
    expect(successfulMobileNavigation).toBeGreaterThanOrEqual(2);
    console.log(`📱 Mobile navigation success: ${successfulMobileNavigation}/${mobileRoutes.length}`);
  });
  ...
  test('Mobile performance should meet Core Web Vitals', async ({ page }) => {
    ...
    for (const pagePath of mobilePages) {
      ...
      try {
        ...
        console.log(`📱 ${pagePath}: ${loadTime}ms load, ${metrics.domContentLoaded}ms DOM ready`);
        
      } catch (e) {
        console.log(`📱 Performance test failed for ${pagePath}: ${e.message.slice(0, 50)}`);
      }
    }
  ...
  test('Mobile accessibility should support assistive technologies', async ({ page }) => {
    ...
    try {
      ...
      console.log(`📱 Mobile keyboard navigation issue: ${e.message.slice(0, 50)}`);
    }
