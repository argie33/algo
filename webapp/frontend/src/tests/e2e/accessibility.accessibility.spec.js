...
test.describe("Accessibility Tests", () => {
  ...
  test("Keyboard navigation should work", async ({ page }) => {
    ...
    if (allButtons === 0) {
      console.log(
        `⚠️ No interactive elements found - page may not be fully loaded`
      );
      // Try waiting for specific elements that should be present
      try {
        await page.waitForSelector("button", { timeout: 5000 });
        const afterWaitButtons = await page
          .locator(
            'button, [role="button"], a[href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
          )
          .count();
        console.log(
          `🔍 After waiting for buttons: ${afterWaitButtons} interactive elements`
        );
      } catch (error) {
        console.log(
          `⚠️ No buttons appeared even after waiting: ${error.message.slice(0, 50)}`
        );
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        if (focusCount > 0) {
          ...
          try {
            ...
            console.log(
              `✅ Tab ${i + 1}: ${tagName.toLowerCase()} - "${elementText?.slice(0, 20) || ariaLabel?.slice(0, 20) || "no text"}"`
            );
          } catch (error) {
            console.log(
              `✅ Tab ${i + 1}: element focused but couldn't read details`
            );
          }
        }
      }
      ...
      console.log(
        `⚠️ Keyboard navigation error: ${error.message.slice(0, 50)}`
      );
    }

    console.log(
      `⌨️ Total keyboard accessible elements: ${focusableElements} (${allButtons} total interactive elements)`
    );
    ...
    if (allButtons > 0) {
      ...
      console.log(
        `ℹ️ No interactive elements found on page - may indicate loading issue`
      );
      ...
      try {
        ...
        if (isVisible && isEnabled) {
          ...
          if (hasFocusIndicator) {
            ...
            console.log(
              `✅ Focus indicator found: "${elementText?.slice(0, 25) || ariaLabel?.slice(0, 25) || "no text"}"`
            );
            console.log(
              `   Style: outline=${styles.outline}, boxShadow=${styles.boxShadow.slice(0, 50)}`
            );
          } else {
            const elementText = await element.textContent();
            const ariaLabel = await element.getAttribute("aria-label");
            console.log(
              `⚠️ No focus indicator: "${elementText?.slice(0, 25) || ariaLabel?.slice(0, 25) || "no text"}"`
            );
            console.log(
              `   Style: outline=${styles.outline}, boxShadow=${styles.boxShadow.slice(0, 50)}`
            );
        ...
        console.log(
          `⚠️ Could not test focus on element ${i}: ${error.message.slice(0, 50)}`
        );
      }
    }

    console.log(
      `🎯 Focus indicators: ${elementsWithFocusIndicators}/${totalElementsTested} elements`
    );
    ...
    for (const img of images) {
      try {
        ...
        const hasAccessibleName =
          alt || ariaLabel || ariaLabelledby || role === "presentation";

        if (hasAccessibleName) {
          imagesWithAltText++;
          console.log(
            `✅ Image has alt text: "${alt || role || "aria-labeled"}"`
          );
        } else {
          console.log(
            `⚠️ Missing alt text for image: ${src?.slice(0, 50) || "unknown"}`
          );
        }
      } catch (error) {
  ...
  test("Headings should have proper hierarchy", async ({ page }) => {
    ...
    if (headings.length > 0) {
      try {
        ...
        if (firstHeading === "h1") {
          console.log(
            `✅ Page starts with H1: "${firstHeadingText?.slice(0, 30)}"`
          );
        } else {
          console.log(
            `⚠️ Page should start with H1, found: ${firstHeading.toUpperCase()}`
          );
          hierarchyIssues++;
        }
        ...
        for (let i = 1; i < headings.length; i++) {
          const currentLevel = parseInt(
            (await headings[i].evaluate((el) => el.tagName)).slice(1)
          );
          const previousLevel = parseInt(
            (await headings[i - 1].evaluate((el) => el.tagName)).slice(1)
          );
          const currentText = await headings[i].textContent();

          if (currentLevel - previousLevel > 1) {
            console.log(
              `⚠️ Heading level skip: H${previousLevel} → H${currentLevel} ("${currentText?.slice(0, 30)}")`
            );
            hierarchyIssues++;
          }
        }
  ...
  test("Color contrast should be sufficient", async ({ page }) => {
    ...
    if (colorContrastViolations.length > 0) {
      console.log(
        `🎨 Found ${colorContrastViolations.length} color contrast issues:`
      );
  ...
  test("Forms should have proper labels", async ({ page }) => {
    ...
    for (const input of inputs) {
      try {
        ...
        const hasLabel =
          label || labelledby || placeholder || associatedLabel > 0;

        if (hasLabel) {
          inputsWithLabels++;
          console.log(
            `✅ Form input labeled: ${inputType || "unknown"} - "${label || placeholder || "associated label"}"`
          );
        } else {
          console.log(
            `⚠️ Form input missing label: ${inputType || "unknown"} input`
          );
        }
      } catch (error) {
        console.log(
          `⚠️ Could not check form input: ${error.message.slice(0, 30)}`
        );
      }
    }

    console.log(
      `📝 Form inputs with labels: ${inputsWithLabels}/${totalInputs}`
    );
  ...
  test("Interactive elements should be large enough", async ({ page }) => {
    ...
    for (const element of interactiveElements.slice(0, 10)) {
      ...
      try {
        if (await element.isVisible()) {
          ...
          if (boundingBox) {
            ...
            if (boundingBox.width >= 44 && boundingBox.height >= 44) {
              elementsLargeEnough++;
              console.log(
                `✅ Element large enough: ${boundingBox.width}x${boundingBox.height} - "${elementText?.slice(0, 20)}"`
              );
            } else {
              console.log(
                `⚠️ Element too small: ${Math.round(boundingBox.width)}x${Math.round(boundingBox.height)} - "${elementText?.slice(0, 20)}"`
              );
            }
          }
        }
        ...
        console.log(
          `⚠️ Could not check element size: ${error.message.slice(0, 30)}`
        );
      }
    }

    console.log(
      `📏 Elements large enough: ${elementsLargeEnough}/${elementsChecked}`
    );
