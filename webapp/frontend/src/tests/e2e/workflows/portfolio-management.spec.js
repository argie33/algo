...
test.describe("Portfolio Management Workflow", () => {
  ...
  test("should complete portfolio management workflow", async ({ page }) => {
    ...
    const holdingsElements = await page
      .locator("tbody tr, .holding, .position, .stock-item, .portfolio-item")
      .count();

    // Look for portfolio summary/totals
    const summaryElements =
      (await page
        .locator(".total-value, .portfolio-value, .balance, .summary")
        .count()) +
      (await page.locator(':has-text("Total"), :has-text("$")').count());

    // Look for P&L information
    const plElements =
      (await page.locator(".profit, .loss, .gain, .pnl").count()) +
      (await page
        .locator(
          ':has-text("P&L"), :has-text("Gain"), :has-text("Loss"), :has-text("%")'
        )
        .count());
    ...
    if (!addButtonFound) {
      ...
      const inlineElements = await page
        .locator('.editable, [contenteditable], input[type="text"]')
        .count();
    }
    ...
    const analyticsElements = await page
      .locator(
    ...
    const allocationElements =
      (await page
        .locator(".allocation, .breakdown, .sector, .distribution")
    ...
    const optimizationElements = await page
      .locator(
    ...
    const optimizeControls = await page
      .locator(
    ...
    const tradeHistoryElements = await page
      .locator("tbody tr, .trade, .transaction, .history-item")
      .count();

    // Look for trade filters
    const filterElements = await page
      .locator('select, input[type="date"], .filter, .date-picker')
      .count();
    ...
    const orderElements = await page
      .locator("tbody tr, .order, .pending, .order-item")
      .count();

    // Look for order controls
    const orderControls = await page
      .locator(
    ...
    const dashboardPortfolioElements =
      (await page
        .locator(".portfolio-widget, .portfolio-summary, .holdings-widget")
    ...
    const mobileElements = await page
      .locator("tbody tr, .holding, .position, .mobile-card")
      .count();
  ...
  test("should handle portfolio calculations correctly", async ({ page }) => {
    ...
    const calculationElements =
      (await page.locator(".total, .average, .sum, .calculated").count()) +
      (await page.locator(':has-text("Total")').count());
    ...
    expect(
      priceElements + percentageElements + calculationElements > 0 || hasContent
    ).toBe(true);
  });
  ...
  test("should handle portfolio data loading states", async ({ page }) => {
    ...
    const loadingElements =
      (await page
        .locator(".loading, .spinner, .skeleton")
        .count({ timeout: 500 })) +
      (await page
  ...
  test("should handle empty portfolio state", async ({ page }) => {
    ...
    const emptyStateElements = await page
      .locator(
  ...
  test("should handle portfolio performance metrics", async ({ page }) => {
    ...
    const metricsElements =
      ...
      (await page
        .locator(
          ':has-text("Return"), :has-text("Volatility"), :has-text("Sharpe"), :has-text("Beta"), :has-text("Alpha")'
        )
        .count());

    // Look for benchmark comparisons
    const benchmarkElements = await page
      .locator(
