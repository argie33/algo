...
test.describe('Financial Platform - Accessibility', () => {
  ...
  test.beforeEach(async ({ page, browserName }) => {
    ...
    const timeout = browserName === 'webkit' ? 15000 : browserName === 'firefox' ? 8000 : 5000;
  ...
  test('should have proper page titles for screen readers', async ({ page, browserName }) => {
    let properTitles = 0;
  ...
  test('should test keyboard shortcuts and navigation', async ({ page }) => {
    ...
    for (const { keys, description } of shortcuts) {
      try {
        ...
        if (newUrl !== initialUrl) {
          workingShortcuts++;
          console.log(`✅ ${description}: URL changed from ${initialUrl} to ${newUrl}`);
        } else {
          console.log(`ℹ️ ${description}: No navigation (may not be implemented)`);
        }
    ...
    console.log(`⌨️ Working keyboard shortcuts: ${workingShortcuts}/${shortcuts.length}`);
  ...
  test('should have proper ARIA attributes', async ({ page }) => {
    ...
    const ariaChecks = [
      ...
      { selector: '[aria-expanded]', attribute: 'aria-expanded', expected: null },
      { selector: '[aria-hidden]', attribute: 'aria-hidden', expected: null },
      { selector: 'button[aria-pressed]', attribute: 'aria-pressed', expected: null },
      ...
      try {
        ...
        if (elements > 0) {
          ...
          if (expected && attrValue !== expected) {
            console.log(`⚠️ Expected ${attribute}="${expected}", found "${attrValue}"`);
          }
        }
      } catch (error) {
  ...
  test('should have accessible form controls', async ({ page, browserName }) => {
    ...
    for (const pagePath of pagesWithForms) {
      try {
        ...
        const formChecks = [
          ...
          { selector: 'input[aria-label]', description: 'Inputs with aria-label' },
          { selector: 'input[placeholder]', description: 'Inputs with placeholders' },
          { selector: 'button[type="submit"]', description: 'Submit buttons' },
          { selector: '[required]', description: 'Required fields' },
          { selector: '[aria-required="true"]', description: 'ARIA required fields' }
        ];
        ...
        if (pageAccessibilityScore > 2) {
          accessibleForms++;
          console.log(`✅ ${pagePath}: Good form accessibility (${pageAccessibilityScore}/6)`);
        } else if (pageAccessibilityScore > 0) {
          console.log(`⚠️ ${pagePath}: Some accessibility features (${pageAccessibilityScore}/6)`);
        } else {
          console.log(`ℹ️ ${pagePath}: No forms detected or limited accessibility`);
        }
    ...
    console.log(`📊 Pages with accessible forms: ${accessibleForms}/${pagesWithForms.length}`);
  ...
  test('should test color contrast and visual accessibility', async ({ page }) => {
    ...
    for (const { selector, description } of colorContrastTests) {
      try {
        ...
        if (elements > 0) {
          ...
          console.log(`✅ ${description}: color=${styles.color}, bg=${styles.backgroundColor}`);
    ...
    console.log(`🎨 Color contrast: ${elementsChecked - contrastIssues}/${elementsChecked} elements appear accessible`);
  ...
  test('should test screen reader accessibility', async ({ page }) => {
    ...
    const screenReaderChecks = [
      ...
      { selector: '[aria-describedby]', description: 'Elements with descriptions' },
    ...
    for (const { selector, description } of screenReaderChecks) {
      try {
        ...
        console.log(`⚠️ Screen reader check failed: ${error.message.slice(0, 30)}`);
      }
    }
    
    console.log(`👁️ Screen reader features: ${screenReaderFeatures}/${screenReaderChecks.length}`);
