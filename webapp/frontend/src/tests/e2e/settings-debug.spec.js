...
test.describe('Settings Page Debug Analysis', () => {
  ...
  test.afterEach(async () => {
    ...
    const failedRequests = networkRequests.filter(req => req.url.includes('/api/'));
    console.log('\n📡 API REQUESTS:');
  ...
  test('Settings page login flow with comprehensive logging', async ({ page }) => {
    ...
    const settingsHeader = await page.locator('h1, h2, [data-testid="settings-header"]').textContent().catch(() => null);
    if (settingsHeader) {
    ...
    const apiKeySection = await page.locator('[data-testid="api-keys"], :has-text("API Keys")').count();
    console.log(`🔑 API Key sections found: ${apiKeySection}`);

    // Check for settings tabs/sections
    const settingsTabs = await page.locator('[role="tab"], .tab, [data-testid*="tab"]').count();
  ...
  test('Settings page API interactions deep dive', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(buttons.length, 5); i++) {
      ...
      if (buttonText.includes('Save') || buttonText.includes('Test') || buttonText.includes('Load')) {
        console.log(`🔥 Clicking potentially API-triggering button: "${buttonText}"`);
