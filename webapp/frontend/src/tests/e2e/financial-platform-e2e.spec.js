...
test.describe('Financial Platform - Core Functionality', () => {
  ...
  test('should load dashboard with financial data', async ({ page }) => {
    ...
    console.log(`📊 Page has text content: ${bodyText ? bodyText.length > 10 : false}`);
    ...
    console.log(`📊 React elements found: ${foundReactElements}/${reactElements.length}`);
  ...
  test('should navigate to portfolio page', async ({ page }) => {
    ...
    const _portfolioElements = page.locator('text*="portfolio", text*="holdings", text*="value", text*="balance"');
  ...
  test('should handle trading signals page', async ({ page }) => {
    ...
    const _tradingElements = page.locator('text*="trading", text*="signals", text*="buy", text*="sell"');
    const pageTitle = await page.title();
    
    console.log(`📈 Trading Signals page title: ${pageTitle}`);
    expect(pageTitle.toLowerCase()).toMatch(/(trading|signals|financial|stocks)/);
  });
  ...
  test('should load technical analysis page', async ({ page }) => {
    ...
    const _technicalElements = page.locator('text*="technical", text*="analysis", text*="chart", text*="indicator"');
    const pageTitle = await page.title();
    
    console.log(`📊 Technical Analysis page title: ${pageTitle}`);
    expect(pageTitle.toLowerCase()).toMatch(/(technical|analysis|financial|stocks)/);
  });
  ...
  test('should navigate between main financial pages', async ({ page }) => {
    ...
    console.log(`🧭 Navigation Summary: ${successfulNavigations}/${financialPages.length} pages loaded`);
  ...
  test('should handle responsive design on mobile', async ({ page }) => {
    ...
    if (!mobileNavFound) {
      console.log('📱 No specific mobile navigation found - checking if desktop nav is responsive');
    }
...
test.describe('Financial Platform - Error Handling', () => {
  ...
  test('should handle API failures gracefully', async ({ page }) => {
    ...
    for (const indicator of errorIndicators) {
      if (await indicator.count() > 0) {
        errorShown = true;
        console.log('❌ Error state detected (good - app handles failures gracefully)');
        break;
      }
    }
    
    if (!errorShown) {
      console.log('📊 No explicit error shown - checking if fallback data is displayed');
    }
  ...
  test('should handle missing authentication', async ({ page }) => {
    ...
    const finalTitle = await page.title();
    expect(finalTitle.toLowerCase()).toMatch(/(financial|dashboard|portfolio)/);
    console.log(`🔐 Auth handling: ${authPromptFound ? 'Prompts for login' : 'Shows demo/fallback data'}`);
  });
