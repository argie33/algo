...
test.describe("Financial Platform - Form Interactions", () => {
    ...
    for (const selector of searchSelectors) {
      ...
      if ((await searchInput.count()) > 0) {
        try {
          ...
          console.log(
            `✅ Stock search interaction successful with: ${selector}`
          );
          searchFound = true;
          ...
          console.log(
            `⚠️ Search interaction failed with ${selector}: ${error.message.slice(0, 50)}`
          );
        }
      }
    }
  ...
  test("should interact with portfolio forms", async ({ page }) => {
    ...
    for (const selector of portfolioControls) {
      ...
      if (elements > 0) {
        ...
        try {
          ...
          console.log(
            `   - Interaction test failed: ${error.message.slice(0, 30)}`
          );
        }
      }
    }
    ...
    console.log(
      `📊 Total portfolio interactive elements: ${interactiveElements}`
    );
  ...
  test("should interact with settings forms", async ({ page }) => {
    ...
    for (const selector of settingsControls) {
      ...
      if (elements > 0) {
        ...
        console.log(
          `✅ Found settings controls: ${selector} (${elements} elements)`
        );
        ...
        if (selector.includes("input")) {
          try {
            ...
            if (
              (await firstInput.isVisible()) &&
              (await firstInput.isEnabled())
            ) {
  ...
  test("should interact with trading forms", async ({ page }) => {
    ...
    for (const selector of tradingControls) {
      ...
      if (count > 0) {
        tradingElements++;
        console.log(
          `✅ Found trading controls: ${selector} (${count} elements)`
        );
        ...
        if (selector.includes("symbol") || selector.includes("Calculate")) {
          try {
            ...
            console.log(
              `   - Trading interaction failed: ${error.message.slice(0, 30)}`
            );
          }
        }
      }
  ...
  test("should test dropdown and select interactions", async ({ page }) => {
    ...
    for (const pagePath of pagesWithDropdowns) {
      try {
        ...
        for (const selector of dropdownSelectors) {
          ...
          if (count > 0) {
            ...
            try {
              ...
              if (
                (await firstDropdown.isVisible()) &&
                (await firstDropdown.isEnabled())
              ) {
              ...
              console.log(
                `⚠️ ${pagePath}: Dropdown test failed: ${error.message.slice(0, 30)}`
              );
            }
          }
        }
        ...
        console.log(
          `⚠️ ${pagePath}: Page load failed: ${error.message.slice(0, 50)}`
        );
      }
    }
  ...
  test("should test button and navigation interactions", async ({ page }) => {
        ...
        for (let i = 0; i < maxToTest; i++) {
          try {
            ...
            if (isVisible && isEnabled) {
              ...
              console.log(
                `✅ Interactable button found: "${buttonText?.slice(0, 30)}"`
              );
            ...
            console.log(
              `   - Button test error: ${error.message.slice(0, 30)}`
            );
          }
        }
      }
    ...
    console.log(
      `📊 Total buttons: ${totalButtons}, Interactable: ${interactableButtons}`
    );
