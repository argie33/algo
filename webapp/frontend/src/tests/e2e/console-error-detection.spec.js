...
const ERROR_PATTERNS = {
  ...
  critical: /Maximum call stack|RangeError|TypeError.*undefined|Cannot read prop/i,
...
class ConsoleErrorDetector {
  ...
  calculateSeverity(type, message, classification) {
    ...
    if (type === 'error' && (classification.react || classification.mui)) return 'high';
...
test.describe('Console Error Detection', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('pageerror', error => {
      detector.addConsoleMessage('error', `JavaScript Runtime Error: ${error.message}`);
    });
  ...
  PAGES_TO_TEST.forEach(pageInfo => {
    test(`should have no critical console errors on ${pageInfo.name}`, async ({ page }) => {
      ...
      try {
        ...
        const buttons = page.locator('button:not([type="submit"]):not(.delete):not(.remove)');
        ...
        console.log(`Note: Could not interact with ${pageInfo.name}: ${interactionError.message}`);
      }
      ...
      console.log(`   Errors: ${report.summary.totalErrors} (${report.summary.criticalErrors} critical)`);
      ...
      if (report.errors.length > 0) {
        ...
        report.errors.forEach((error, i) => {
          ...
          if (Object.keys(error.classification).length > 0) {
            console.log(`      Types: ${Object.keys(error.classification).join(', ')}`);
          }
        });
      }
      ...
      // Test assertions
      expect(report.summary.criticalErrors, 
      ...
      expect(report.summary.highErrors,
      ...
      expect(report.summary.totalNetworkErrors,
  ...
  test('should maintain low error count during full user journey', async ({ page }) => {
    const journeyPages = [
      '/', '/dashboard', '/portfolio', '/market', '/settings'
    ];
    ...
    console.log(`   Total Test Duration: ${finalReport.summary.testDurationMs}ms`);
