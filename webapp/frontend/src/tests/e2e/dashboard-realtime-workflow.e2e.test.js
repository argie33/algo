...
test.describe('Dashboard Real-time Workflow - E2E Tests', () => {
  ...
  test('should display dashboard with market data widgets', async ({ page }) => {
    // Navigate to Dashboard
    await page.click('text=Dashboard');
    await page.waitForURL('**/dashboard', { timeout: 10000 });
    
    // Verify main dashboard elements
    await expect(page.locator('h1:has-text("Dashboard"), h2:has-text("Dashboard"), text=Dashboard')).toBeVisible();
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            indices: {
              ...
              QQQ: { price: 380.50, change: -2.25, changePercent: -0.59 }
            },
            marketStatus: 'OPEN',
  ...
  test('should display portfolio summary widget', async ({ page }) => {
    ...
    const portfolioContent = page.locator('text=Portfolio').or(page.locator('text=Holdings')).or(page.locator('text=Positions'));
  ...
  test('should handle search functionality', async ({ page }) => {
    ...
    const searchInput = page.locator('.MuiAutocomplete-root input, input[placeholder*="search" i]');
    ...
    if (await searchInput.isVisible()) {
      ...
      const suggestions = page.locator('.MuiAutocomplete-listbox li, .search-results');
  ...
  test('should display market indices and movers', async ({ page }) => {
    ...
    const _marketSymbols = page.locator('text=SPY').or(page.locator('text=QQQ')).or(page.locator('text=DIA'));
    
    // Check for any market-related content
    const marketContent = page.locator('text=Market').or(page.locator('text=Index')).or(page.locator('text=Indices'));
    ...
    const moversContent = page.locator('text=Gainers').or(page.locator('text=Losers')).or(page.locator('text=Movers'));
  ...
  test('should display news and alerts', async ({ page }) => {
    ...
    const newsContent = page.locator('text=News').or(page.locator('text=Headlines')).or(page.locator('text=Market News'));
    ...
    const _notificationIcons = page.locator('[data-testid*="notification"], .notification');
  ...
  test('should handle interactive charts', async ({ page }) => {
    ...
    const chartElements = page.locator('.recharts-wrapper, .chart-container, [data-testid*="chart"]');
    ...
    if (await chartElements.count() > 0) {
      ...
      const tooltips = page.locator('.recharts-tooltip-wrapper, .chart-tooltip');
  ...
  test('should support quick actions and navigation', async ({ page }) => {
    ...
    const actionButtons = page.locator('button').filter({ hasText: /Portfolio|Trade|Buy|Sell|Add|View/ });
    ...
    if (buttonCount > 0) {
      ...
      if (isDialog) {
        const closeButton = page.locator('button[aria-label="close"], button:has-text("Close"), button:has-text("Cancel")');
  ...
  test('should handle table interactions on dashboard', async ({ page }) => {
    ...
    if (await tables.count() > 0) {
      ...
      const tableRows = firstTable.locator('.MuiTableRow-root').filter({ hasNot: page.locator('.MuiTableHead-root .MuiTableRow-root') });
  ...
  test('should display performance metrics and analytics', async ({ page }) => {
    ...
    const performanceContent = page.locator('text=Performance').or(page.locator('text=Analytics')).or(page.locator('text=Metrics'));
    ...
    const percentageElements = page.locator('text=/%/').or(page.locator('text=/\\+\\d+\\.\\d+%/'));
    const currencyElements = page.locator('text=/\\$[\\d,]+/');
    
    // Should have some financial data displayed
    const hasFinancialData = (await percentageElements.count() > 0) || (await currencyElements.count() > 0);
  ...
  test('should handle real-time data simulation', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      window.dispatchEvent(new CustomEvent('marketUpdate', { detail: updateData }));
      window.dispatchEvent(new CustomEvent('priceUpdate', { detail: updateData }));
      
      // Also test localStorage updates (if app uses them)
      if (window.localStorage) {
        window.localStorage.setItem('lastMarketUpdate', JSON.stringify(updateData));
      }
    });
  ...
  test('should maintain performance with multiple widgets', async ({ page }) => {
    ...
    const interactiveElements = page.locator('button, [role="tab"], .MuiTableRow-root');
  ...
  test('should handle error states gracefully', async ({ page }) => {
    ...
    const hasContent = await page.locator('.MuiCard-root, .MuiGrid-root, table, .recharts-wrapper').count() > 0;
  ...
  test('should support customization and settings', async ({ page }) => {
    ...
    const settingsButton = page.locator('button[aria-label*="settings"], button:has-text("Settings"), button:has-text("Customize")');
