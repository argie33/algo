...
test.describe("Positioning E2E Tests", () => {
  ...
  test("loads stock detail page with positioning data", async ({ page }) => {
    ...
    await expect(page.getByText("Institutional Ownership")).toBeVisible({
      timeout: 10000,
    });

    // Wait for positioning section to be visible
    await expect(
      page.getByText(/Ownership & Short Interest|Market Positioning/i)
  ...
  test("displays institutional ownership percentage", async ({ page }) => {
    ...
    const ownershipText = page
      .locator("text=/\\d+\\.\\d+%/")
      .filter({ has: page.locator("text=/Institutional Ownership/i") })
      .first();
  ...
  test("displays insider ownership percentage", async ({ page }) => {
    // Wait for positioning data to load
    await expect(page.getByText("Insider Ownership")).toBeVisible({
      timeout: 10000,
    });

    // Find the insider ownership value
    const ownershipSection = page
      .locator("text=/Insider Ownership/i")
      .locator("xpath=ancestor::*[2]");
    const ownershipValue = ownershipSection
      .locator("text=/\\d+\\.\\d+%/")
      .first();
  ...
  test("displays short interest percentage", async ({ page }) => {
    // Wait for positioning data to load
    await expect(page.getByText(/Short Interest|Short %/i)).toBeVisible({
      timeout: 10000,
    });

    // Find the short interest value
    const shortSection = page
      .locator("text=/Short Interest|Short %/i")
      .locator("xpath=ancestor::*[2]");
  ...
  test("displays positioning score", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Positioning Score|Position Score/i)
    ).toBeVisible({ timeout: 10000 });

    // Find the score value (should be 0-100)
    const scoreSection = page
      .locator("text=/Positioning Score|Position Score/i")
  ...
  test("shows institutional holders list", async ({ page }) => {
    ...
    const institutionalSection = page.locator(
      "text=/Top (Institutional )?Holders|Major Holders/i"
    );

    if (await institutionalSection.isVisible({ timeout: 5000 })) {
      // Check for holder names (like Vanguard, Blackrock, etc.)
      const holderNames = page.locator(
        "text=/Vanguard|Blackrock|State Street|Fidelity/i"
      );
      await expect(holderNames.first()).toBeVisible({ timeout: 5000 });
    }
  });

  test("displays positioning metrics without N/A values for stock AAL", async ({
    page,
  }) => {
    // Wait for positioning data to load
    await page.waitForTimeout(2000);
  ...
  test("API call returns correct positioning data structure", async ({
    page,
  }) => {
    // Intercept the positioning API call
    const apiResponsePromise = page.waitForResponse(
      (response) =>
        response.url().includes(`/api/positioning/stocks`) &&
        response.status() === 200,
      { timeout: 10000 }
    );
    ...
    expect(responseBody.positioning_metrics).toHaveProperty(
  ...
  test("handles missing positioning data for non-existent symbol", async ({
    page,
  }) => {
    ...
    const hasError = await page
      .getByText(/not found|error|invalid|no stock found/i)
      .isVisible()
      .catch(() => false);
    const hasNoData = await page
      .locator("text=/n\\/a/i")
  ...
  test("positioning data updates when navigating between different stocks", async ({
    page,
  }) => {
    ...
    const firstOwnership = await page
      .locator("text=/Institutional Ownership/i")
      .locator("xpath=ancestor::*[2]")
      .textContent();
    ...
    const secondOwnership = await page
      .locator("text=/Institutional Ownership/i")
      .locator("xpath=ancestor::*[2]")
      .textContent()
      .catch(() => "N/A");
  ...
  test("positioning data persists when switching tabs on stock detail page", async ({
    page,
  }) => {
    // Navigate to stock
    await page.goto(`/stocks/${testSymbol}`);
  ...
  test("verifies correct percentage formatting (not decimal)", async ({
    page,
  }) => {
    // Navigate to stock
    await page.goto(`/stocks/${testSymbol}`);
...
test.describe("Positioning Summary Page E2E Tests", () => {
  ...
  test.skip("loads positioning summary with market overview", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Market Overview|Positioning Summary/i)
    ).toBeVisible({ timeout: 10000 });

    // Check for institutional flow
    await expect(
      page.getByText(/Institutional Flow|Institutional Sentiment/i)
    ).toBeVisible();
