...
vi.mock("@mui/icons-material", () => {
  ...
  const iconMocks = {
    ...
    PictureAsPdf: createIconMock("PictureAsPdf"),
    Upload: createIconMock("Upload"),
    AccountBalanceWallet: createIconMock("AccountBalanceWallet"),
...
const MockWebSocketClass = vi.fn().mockImplementation((url) => {
  const ws = {
    addEventListener: vi.fn(),
    removeEventListener: vi.fn(),
    send: vi.fn(),
...
beforeEach(() => {
  console.error = (...args) => {
    if (
      ...
      args[0].includes(
        "Warning: An update to Portfolio inside a test was not wrapped in act"
      ) ||
      args[0].includes(
