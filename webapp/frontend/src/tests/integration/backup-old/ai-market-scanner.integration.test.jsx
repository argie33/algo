...
describe("AI Market Scanner Integration", () => {
  ...
  describe("Full Workflow Integration", () => {
    it("completes full scan workflow from UI to API", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "/api/screener/ai-scan?type=momentum&limit=50"
      );
    ...
    it("handles scan type changes and triggers correct API calls", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "/api/screener/ai-scan?type=reversal&limit=50"
      );

      await waitFor(() => {
        expect(
          screen.getByText("Oversold stocks with reversal indicators")
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Performance Integration", () => {
    ...
    it("handles rapid scan type changes without race conditions", async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          "/api/screener/ai-scan?type=unusual&limit=50"
        );
      });
    });
  });
