...
describe("Core Features Integration Tests", () => {
  ...
  describe("Trading Features", () => {
    it("should handle order placement workflow", async () => {
            ...
            {result && (
              <div data-testid="order-result">
                {result.error ? (
                  ...
                  <div>
                    Order {result.orderId} submitted: {result.orderType} {result.quantity} {result.symbol}
                  </div>
                )}
              </div>
            )}
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("order-result")).toHaveTextContent("Order ORD123 submitted");
      });
    ...
    it("should display portfolio performance metrics", async () => {
      ...
      const PerformanceWidget = () => {
        ...
        React.useEffect(() => {
          mockDataService.fetchData('/api/portfolio/performance')
            .then(data => {
              setPerformance(data);
              setLoading(false);
            })
        ...
        return (
          <div data-testid="performance-widget">
            ...
            <div data-testid="day-change">
              Day Change: ${performance.dayChange} ({performance.dayChangePercent}%)
            </div>
            <div data-testid="total-return">
              Total Return: ${performance.totalReturn} ({performance.totalReturnPercent}%)
            </div>
            <div data-testid="top-performer">
              Top: {performance.topPerformer.symbol} (+{performance.topPerformer.gain}%)
            </div>
            <div data-testid="worst-performer">
              Worst: {performance.worstPerformer.symbol} ({performance.worstPerformer.gain}%)
            </div>
          </div>
        );
      };
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("total-value")).toHaveTextContent("Total Value: $250,000");
        expect(screen.getByTestId("day-change")).toHaveTextContent("Day Change: $3500 (1.42%)");
        expect(screen.getByTestId("total-return")).toHaveTextContent("Total Return: $50000 (25%)");
      });

      expect(mockDataService.fetchData).toHaveBeenCalledWith('/api/portfolio/performance');
    });
  });
  ...
  describe("Market Analysis Features", () => {
    it("should display technical analysis indicators", async () => {
      ...
      const TechnicalAnalysis = () => {
        ...
        React.useEffect(() => {
          mockDataService.fetchData('/api/technical/AAPL')
            .then(data => {
              setAnalysis(data);
              setLoading(false);
            })
        ...
        return (
          <div data-testid="technical-analysis">
            ...
            <div data-testid="macd">
              MACD: {analysis.indicators.macd}
            </div>
            <div data-testid="moving-averages">
              SMA20: ${analysis.indicators.sma20} | SMA50: ${analysis.indicators.sma50}
            </div>
            <div data-testid="bollinger-bands">
              Bollinger: ${analysis.indicators.bollingerLower} - ${analysis.indicators.bollingerUpper}
            </div>
            <div data-testid="signals">
              Trend: {analysis.signals.trend} | Strength: {analysis.signals.strength} | Rec: {analysis.signals.recommendation}
            </div>
          </div>
        );
      };
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("symbol-price")).toHaveTextContent("AAPL: $175.5");
        expect(screen.getByTestId("rsi")).toHaveTextContent("RSI: 68.5");
        expect(screen.getByTestId("signals")).toHaveTextContent("Trend: bullish | Strength: strong | Rec: buy");
      });

      expect(mockDataService.fetchData).toHaveBeenCalledWith('/api/technical/AAPL');
    });
    ...
    it("should handle market news and sentiment", async () => {
      ...
      const NewsAndSentiment = () => {
        ...
        React.useEffect(() => {
          mockDataService.fetchData('/api/news/sentiment')
            .then(result => {
              setData(result);
              setLoading(false);
            })
            ...
            <div data-testid="news-list">
              {data.news.map(article => (
                <div key={article.id} data-testid={`news-${article.id}`}>
                  {article.headline} - {article.sentiment} ({article.impact} impact)
                </div>
              ))}
            </div>
          </div>
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("sentiment-overview")).toHaveTextContent("Sentiment: positive (Score: 0.75)");
        expect(screen.getByTestId("news-1")).toHaveTextContent("Apple Reports Strong Q4 Earnings - positive (high impact)");
      });

      expect(mockDataService.fetchData).toHaveBeenCalledWith('/api/news/sentiment');
    });
  });
  ...
  describe("Real-time Features", () => {
    it("should handle live price updates", async () => {
        ...
        if (url.includes('/realtime/prices')) {
          ...
          setTimeout(() => {
            if (priceUpdateCallback) {
              priceUpdateCallback([
                ...
                { symbol: "MSFT", price: 386.50, change: -0.75 }
              ]);
            }
          }, 100);
          ...
          return Promise.resolve({
            subscribe: (callback) => { 
              ...
              return () => { priceUpdateCallback = null; };
            }
          });
        }
      ...
      const LivePrices = () => {
        ...
        React.useEffect(() => {
          mockDataService.fetchData('/api/realtime/prices')
            .then(service => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("price-AAPL")).toHaveTextContent("AAPL: $176.25 (+2.75)");
        expect(screen.getByTestId("price-MSFT")).toHaveTextContent("MSFT: $386.5 (-0.75)");
      });
    });
    ...
    it("should handle market alerts and notifications", async () => {
      ...
      const AlertsPanel = () => {
        ...
        React.useEffect(() => {
          mockDataService.fetchData('/api/alerts')
            .then(data => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("alerts-count")).toHaveTextContent("Active Alerts: 2");
        expect(screen.getByTestId("alert-1")).toHaveTextContent("[PRICE] AAPL: AAPL reached target price of $175");
        expect(screen.getByTestId("alert-2")).toHaveTextContent("[VOLUME] MSFT: MSFT volume spike detected");
      });
  ...
  describe("User Settings and Configuration", () => {
    it("should handle user preferences and settings", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("settings-message")).toHaveTextContent("Settings saved successfully");
      });
