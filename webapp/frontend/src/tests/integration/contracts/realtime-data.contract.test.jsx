...
describe("Real-Time Data Contract Tests", () => {
  ...
  it("validates WebSocket stream endpoint requires symbols parameter", async () => {
    if (skipIfServerUnavailable(serverAvailable, "WebSocket contract test")) return;

    // Test the exact bug we fixed - empty symbols causing 404
    const emptySymbolsResponse = await fetch(`${API_BASE_URL}/api/websocket/stream/`, {
    ...
    const validSymbolsResponse = await fetch(`${API_BASE_URL}/api/websocket/stream/AAPL,GOOGL`, {
      headers: AUTH_HEADERS
    });
