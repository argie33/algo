...
describe("Settings API Keys Contract Tests", () => {
  ...
  it("should validate Settings component consumes real API keys endpoint structure", async () => {
    ...
    const settingsLogic = (responseData) => {
      ...
      return {
        hasCorrectStructure: !!(responseData.data && Array.isArray(responseData.data)),
        apiKeysCount: apiKeys.length,
        hasExpectedFields: apiKeys.length === 0 || (apiKeys[0].brokerName !== undefined)
      };
    };
  ...
  it("should validate API key creation contract", async () => {
    ...
    await fetch(`${API_BASE_URL}/api/portfolio/api-keys/${testApiKey.brokerName}`, {
      method: 'DELETE',
      headers: {
        'Authorization': 'Bearer mock-access-token'
      }
  ...
  it("should validate error handling contract", async () => {
    ...
    const response = await fetch(`${API_BASE_URL}/api/portfolio/api-keys/nonexistent`, {
      method: 'DELETE',
      headers: {
        'Authorization': 'Bearer mock-access-token'
      }
