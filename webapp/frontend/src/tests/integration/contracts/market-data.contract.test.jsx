...
describe("Market Data Contract Tests", () => {
  ...
  it("should validate market overview data structure", async () => {
    if (skipIfServerUnavailable(serverAvailable, "market overview test")) return;
  ...
  it("should validate sector analysis data structure", async () => {
    if (skipIfServerUnavailable(serverAvailable, "sector analysis test")) return;
  ...
  it("should validate real-time quotes data structure", async () => {
    if (skipIfServerUnavailable(serverAvailable, "real-time quotes test")) return;

    const testSymbols = 'AAPL,MSFT,GOOGL';
    const response = await fetch(`${API_BASE_URL}/api/market/quotes?symbols=${testSymbols}`, {
      headers: AUTH_HEADERS
    });
    ...
    console.log('Real-time quotes structure:', {
      ...
      dataType: typeof apiResponse.data,
      dataKeys: Object.keys(apiResponse.data || {}),
      quotesCount: Array.isArray(apiResponse.data) ? apiResponse.data.length : 'not array'
    });
  ...
  it("should validate WebSocket streaming contract for market data", async () => {
    if (skipIfServerUnavailable(serverAvailable, "WebSocket streaming test")) return;

    // Test WebSocket endpoint that was causing 404 errors
    const symbols = 'AAPL,MSFT,GOOGL';
    const response = await fetch(`${API_BASE_URL}/api/websocket/stream/?symbols=${symbols}`, {
      headers: AUTH_HEADERS
    });
    ...
    const emptyResponse = await fetch(`${API_BASE_URL}/api/websocket/stream/?symbols=`, {
      headers: AUTH_HEADERS
    });
  ...
  it("should validate market data API contract supports trading dashboard", async () => {
    if (skipIfServerUnavailable(serverAvailable, "trading dashboard contract test")) return;
  ...
  it("should validate sector data API contract supports portfolio analysis", async () => {
    if (skipIfServerUnavailable(serverAvailable, "portfolio analysis contract test")) return;

    const sectorsResponse = await fetch(`${API_BASE_URL}/api/sectors/analysis`, {
      headers: AUTH_HEADERS
    });
  ...
  it("should render MarketOverview component with real backend data and handle UI interactions", async () => {
    if (skipIfServerUnavailable(serverAvailable, "MarketOverview full-stack test")) return;
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/market/i) || screen.getByText(/overview/i)).toBeInTheDocument();
    });
    ...
    await waitFor(
      () => {
        ...
        expect(screen.queryByText(/error loading market data/i)).not.toBeInTheDocument();
    ...
    console.log('✅ MarketOverview component successfully integrates with backend API');
  });

  it("should render SectorAnalysis component with real backend data and display sectors", async () => {
    if (skipIfServerUnavailable(serverAvailable, "SectorAnalysis full-stack test")) return;
    ...
    await waitFor(() => {
      expect(screen.getByText(/sector/i) || screen.getByText(/analysis/i)).toBeInTheDocument();
    });
    ...
    await waitFor(
      () => {
        ...
        expect(screen.queryByText(/error loading sector data/i)).not.toBeInTheDocument();
        expect(screen.queryByText(/sectors service unavailable/i)).not.toBeInTheDocument();
      },
      { timeout: 12000 }
    );

    console.log('✅ SectorAnalysis component successfully displays backend sector data');
  });
});