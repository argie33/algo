...
describe("Complete Authentication Flow Integration Tests", () => {
  ...
  describe("User Registration Flow", () => {
    test("should complete full registration process", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Account confirmed! Please sign in.")
        ).toBeInTheDocument();
      });
    ...
    test("should handle registration errors", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("An account with the given email already exists.")
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("User Login Flow", () => {
    test("should complete successful login", async () => {
      const user = userEvent.setup();
      const { signIn, getCurrentUser, fetchAuthSession } = await import(
        "@aws-amplify/auth"
      );
  ...
  describe("Password Reset Flow", () => {
    test("should complete password reset process", async () => {
      const user = userEvent.setup();
      const { forgotPassword, forgotPasswordSubmit } = await import(
        "@aws-amplify/auth"
      );
