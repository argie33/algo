...
describe("Economic Modeling Integration Tests", () => {
  test("loads economic modeling page with real backend data", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Economic Modeling & Forecasting")
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /Advanced econometric models and real-time recession probability/i
        )
      ).toBeInTheDocument();
    });
  });
  ...
  test("displays recession forecast data from backend", async () => {
    ...
    await waitFor(
      () => {
        const forecastElements = screen.queryAllByText(
          /forecast|model|probability/i
        );
  ...
  test("displays leading indicators from backend", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.queryByText(/Loading economic data/i)
  ...
  test("displays AI insights from backend", async () => {
    ...
    await waitFor(
      () => {
        const insightElements = screen.queryAllByText(
          /confidence|insight|analysis/i
        );
    ...
    await waitFor(
      () => {
        const errorText = screen.queryByText(
          /Error loading economic data|Failed to fetch/i
        );
