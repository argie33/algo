...
describe('API Key and Portfolio Import Integration', () => {
  ...
  describe('API Key Management Workflow', () => {
    test('should add API key and display it in settings', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/api key added successfully/i)).toBeInTheDocument();
      });
  ...
  describe('Portfolio Import Integration', () => {
    ...
    test('should handle portfolio import', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/portfolio imported successfully/i)).toBeInTheDocument();
    ...
    test('should show no connections message when no API keys exist', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/no broker connections found/i)).toBeInTheDocument();
    ...
    test('should filter to only show supported brokers', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryByText('Unsupported-broker')).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe('End-to-End Integration', () => {
    test('should complete full workflow: add API key → test connection → import portfolio', async () => {
      ...
      global.fetch
        .mockImplementationOnce(async () => ({ // Initial load - empty
          ok: true,
          json: async () => ({ data: [] })
        }))
        .mockImplementationOnce(async () => ({ // Add API key
          ok: true,
          json: async () => ({ success: true })
        }))
        .mockImplementationOnce(async () => ({ // Reload after add
          ok: true,
          json: async () => ({
            data: [{
              brokerName: 'alpaca',
