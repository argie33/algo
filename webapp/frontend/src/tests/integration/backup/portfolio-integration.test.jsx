    ...
    it("should display actual portfolio holdings with real data", async () => {
      ...
      await waitFor(
        async () => {
          ...
          const appleHolding = await screen.findByText(/AAPL/, {
            timeout: 10000,
          });
      ...
      await waitFor(
        async () => {
          ...
          expect(content).toMatch(
            /(Total Value|Portfolio Value|Market Value)/i
          );
      ...
      await waitFor(
        async () => {
          ...
          expect(content).not.toContain(
            "requires portfolio holdings and market data"
          );
  ...
  describe("Data Consistency Validation", () => {
    it("should show consistent portfolio values across tabs", async () => {
      ...
      await waitFor(
        async () => {
          const valueMatch =
            document.body.textContent.match(/\$[\d,]+\.[\d]{2}/);
          if (valueMatch) {
