...
const renderWithAuth = (component) => {
  ...
  return render(
    <ThemeProvider theme={theme}>
      <AuthContext.Provider value={mockAuth}>{component}</AuthContext.Provider>
    </ThemeProvider>
  );
};
...
describe("Service Data Flow Integration", () => {
      ...
      if (url.includes("/market-overview")) {
        return Promise.resolve({
          data: {
            ...
            data: {
              indices: {
                SPY: { price: 425.5, change: 3.25, changePercent: 0.77 },
                QQQ: { price: 355.75, change: 4.5, changePercent: 1.28 },
  ...
  describe("API Service to Data Service Integration", () => {
    it("should properly route API requests through data service with caching", async () => {
      ...
      expect(
        mockDataService.cache.has(expect.stringContaining("/portfolio"))
      ).toBe(true);
    });
  ...
  describe("Service Error Propagation", () => {
    it("should properly propagate API errors through data service to components", async () => {
      ...
      // Component should display specific error state
      await waitFor(
        () => {
          // Test should know what specific error to expect based on the mock
    ...
    it("should handle network timeouts through service layer", async () => {
      ...
      // Component should show specific timeout error
      await waitFor(
        () => {
          // Since we're mocking a specific "Network timeout" error, test for that
  ...
  describe("Loading State Management Across Services", () => {
    it("should coordinate loading states between API service and data service", async () => {
      ...
      // Component should show loading UI - test for the specific indicator your component uses
      expect(screen.getByRole("progressbar")).toBeInTheDocument();

      // Loading should resolve
      await waitFor(
        () => {
          // Test the specific loading indicator that should be removed
      ...
      expect(
        mockDataService.loadingStates.get(expect.stringContaining("/portfolio"))
      ).toBe(false);
    });
  });
      ...
      mockDataService.fetchData.mockImplementation(
        async (url, options = {}) => {
          if (url.includes("/portfolio")) {
            ...
            const portfolioResponse = await mockApiService.get(
  ...
  describe("Service State Synchronization", () => {
    it("should synchronize state changes across multiple service instances", async () => {
      ...
      // Should reflect updated data with specific format
      await waitFor(
        () => {
          // Test should validate the exact format returned by our formatter
