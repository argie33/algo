...
describe("End-to-End Data Flow Integration", () => {
  ...
  beforeEach(() => {
    ...
    require("../../services/realTimeDataService.js").default =
      mockRealTimeService;
  });
      ...
      const mockMarketData = {
        data: {
          indices: [
            ...
            { symbol: "NASDAQ", name: "NASDAQ", value: 14200.5, change: -25.3 },
          ],
          sectors: [
    ...
    it("should load comprehensive stock data from multiple endpoints", async () => {
      ...
      const mockTechnicalData = {
        data: {
          ...
          movingAverages: { sma50: 152.3, sma200: 148.75 },
        },
      };
      ...
      mockApiService.get.mockImplementation((url) => {
        ...
        if (url.includes("/technical"))
          return Promise.resolve(mockTechnicalData);
        return Promise.resolve({ data: {} });
      });

      mockDataService.fetchData.mockImplementation((url) => {
        if (url.includes("/price")) return Promise.resolve(mockStockPrice.data);
        if (url.includes("/stock-info"))
          return Promise.resolve(mockStockInfo.data);
        if (url.includes("/technical"))
          return Promise.resolve(mockTechnicalData.data);
        return Promise.resolve({});
      });
      ...
      expect(mockApiService.get).toHaveBeenCalledWith(
        expect.stringContaining("/stock-info")
      );
      expect(mockApiService.get).toHaveBeenCalledWith(
  ...
  describe("Dashboard Integration Flow", () => {
    it("should coordinate data from multiple sources on dashboard", async () => {
      ...
      mockDataService.fetchData.mockImplementation((url) => {
        if (url.includes("/portfolio"))
          return Promise.resolve(mockDashboardData.portfolio);
        if (url.includes("/market"))
          return Promise.resolve(mockDashboardData.market);
        if (url.includes("/watchlist"))
          return Promise.resolve(mockDashboardData.watchlist);
        if (url.includes("/news"))
          return Promise.resolve(mockDashboardData.news);
        return Promise.resolve({});
      });
  ...
  describe("Error Handling Integration", () => {
    it("should handle API failures gracefully across components", async () => {
      ...
      mockDataService.fetchData.mockRejectedValue(
