...
describe('requestBatcher', () => {
  describe('batchRequests', () => {
    ...
    it('should work with different concurrency levels', async () => {
      const testConcurrency = async (concurrency) => {
        ...
        await batchRequests(items, async (item) => {
          active++;
  ...
  describe('batchPromises', () => {
    ...
    it('should preserve promise execution order', async () => {
      const execution = [];
      const fns = [
        async () => { execution.push('a'); return 'a'; },
        async () => { execution.push('b'); return 'b'; },
        async () => { execution.push('c'); return 'c'; },
      ];
  ...
  describe('Real-world scenarios', () => {
    it('should handle API-like request batching', async () => {
      ...
      const result = await batchRequests(symbols, async (symbol) => {
      ...
      expect(result.every(r => r.symbol && typeof r.price === 'number')).toBe(true);
