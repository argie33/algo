...
describe("AI Market Scanner API", () => {
  ...
  describe("AI Scan Functionality", () => {
    ...
    it("handles multiple filters", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        expect.stringMatching(/min_market_cap=5000000000.*sector=Healthcare|sector=Healthcare.*min_market_cap=5000000000/)
      );
    });
  });
  ...
  describe("AI Strategies API", () => {
    ...
    it("includes strategy metadata and descriptions", async () => {
      ...
      const momentumStrategy = result.data.strategies.find(s => s.type === 'momentum');
  ...
  describe("Error Handling", () => {
    it("handles AI scan API errors", async () => {
      ...
      await expect(aiScanner.scan('momentum', 50)).rejects.toThrow('AI scan failed: 500');
    });
    ...
    it("handles strategies API errors", async () => {
      ...
      await expect(aiScanner.getStrategies()).rejects.toThrow('Failed to get strategies: 503');
    });

    it("handles network errors gracefully", async () => {
      fetch.mockImplementationOnce(() => Promise.reject(new Error('Network error')));

      await expect(aiScanner.scan('momentum', 50)).rejects.toThrow('Network error');
    });
  });
