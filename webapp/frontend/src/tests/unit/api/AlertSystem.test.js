...
describe("Alert System API", () => {
  ...
  describe("Alert Management", () => {
    ...
    it("returns comprehensive alert data", async () => {
      ...
      const priceAlert = result.data.alerts.find(
        (a) => a.alert_type === "price_target"
      );
      expect(priceAlert.symbol).toBe("AAPL");
      expect(priceAlert.condition).toBe("above");
      expect(priceAlert.target_price).toBe(185.0);
      expect(priceAlert.current_price).toBe(182.5);
    ...
    it("includes alert metadata and statistics", async () => {
      ...
      const volumeAlert = result.data.alerts.find(
        (a) => a.alert_type === "volume"
      );
  ...
  describe("Alert Templates", () => {
    ...
    it("returns system and user templates", async () => {
      ...
      const priceTemplate = result.data.templates.find(
        (t) => t.template_type === "price"
      );
      ...
      expect(priceTemplate.alert_config.notification_methods).toContain(
        "email"
      );
    });
  ...
  describe("Webhook Management", () => {
    ...
    it("returns comprehensive webhook data", async () => {
      ...
      const slackWebhook = result.data.webhooks.find(
        (w) => w.webhook_type === "slack"
      );
  ...
  describe("Error Handling", () => {
    it("handles alerts API errors", async () => {
      ...
      await expect(alertsAPI.getAlerts()).rejects.toThrow(
        "Alerts API failed: 500"
      );
    });
    ...
    it("handles create alert errors", async () => {
      ...
      await expect(alertsAPI.createAlert(alertData)).rejects.toThrow(
        "Create alert failed: 400"
      );
    });
    ...
    it("handles webhook configuration errors", async () => {
      ...
      await expect(alertsAPI.getWebhooks()).rejects.toThrow(
        "Get webhooks failed: 503"
      );
    });
  });
