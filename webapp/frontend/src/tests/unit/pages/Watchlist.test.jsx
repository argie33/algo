...
describe("Watchlist Component", () => {
  ...
  it("allows adding stocks to watchlist", async () => {
    ...
    const addInput = screen.queryByPlaceholderText(/add stock/i) || 
  ...
  it("displays stock metrics", async () => {
    ...
    await waitFor(() => {
      // Volume
      expect(screen.getByText("52.0M") || screen.getByText("52,000,000")).toBeInTheDocument();
      // Market Cap
      expect(screen.getByText("$2.80T") || screen.getByText("2.8T")).toBeInTheDocument();
      // P/E Ratio
  ...
  it("shows percentage changes with proper styling", async () => {
    ...
    await waitFor(() => {
      // Positive change
      expect(screen.getByText("+1.24%") || screen.getByText("1.24%")).toBeInTheDocument();
      // Negative change  
      expect(screen.getByText("-0.85%") || screen.getByText("0.85%")).toBeInTheDocument();
    });
  });
  ...
  it("supports sorting by different columns", async () => {
    ...
    const sortHeaders = screen.getAllByRole("columnheader") ||
                       screen.getAllByRole("button").filter(btn => 
                         btn.textContent.match(/symbol|name|price|change/i)
                       );
  ...
  it("handles empty watchlist", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText(/no stocks/i) || 
             screen.queryByText(/empty/i) ||
             screen.queryByText(/add your first stock/i)).toBeInTheDocument();
    });
  });
  ...
  it("displays time when stocks were added", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/added/i) || 
             screen.getByText(/Jan/i) ||
             screen.getByText(/day/i)).toBeInTheDocument();
    });
  });
  ...
  it("shows watchlist performance summary", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/total value/i) || 
             screen.getByText(/daily change/i) ||
             screen.getByText(/performance/i)).toBeInTheDocument();
    });
  });
});