...
const mockRealTimeData = {
  portfolio: {
    ...
    positions: [
      ...
      { symbol: "GOOGL", price: 142.56, change: -1.44, changePercent: -1.00 },
      { symbol: "MSFT", price: 378.85, change: 3.22, changePercent: 0.86 },
    ],
  },
  market: {
    sp500: { price: 4756.50, change: 15.25, changePercent: 0.32 },
    nasdaq: { price: 14845.12, change: -23.44, changePercent: -0.16 },
    dow: { price: 37248.35, change: 45.87, changePercent: 0.12 },
  },
  alerts: [
    { id: 1, type: "price", symbol: "AAPL", message: "AAPL hit target price $185", time: "2024-01-25T15:30:00Z" },
  ],
};
...
describe("RealTimeDashboard Component", () => {
  ...
  it("renders real-time dashboard page", async () => {
    ...
    expect(screen.getByText(/real.?time dashboard|dashboard/i)).toBeInTheDocument();
  });
  ...
  it("handles WebSocket messages", async () => {
    ...
    if (messageHandler) {
      const mockMessage = {
        data: JSON.stringify({
          type: "price_update",
          data: { symbol: "AAPL", price: 186.00, change: 2.23 }
        })
      };
  ...
  it("handles API errors gracefully", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/error|failed to load|connection failed/i)).toBeInTheDocument();
    });
  });
  ...
  it("displays real-time timestamps", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/last updated|live|real.?time/i) ||
             screen.getByText(/\d{1,2}:\d{2}/) ||
             screen.getByText(/\d{1,2}\/\d{1,2}\/\d{4}/)).toBeInTheDocument();
    });
  });
  ...
  it("shows connection status indicator", () => {
    ...
    expect(screen.getByText(/connected|live|online/i) ||
           screen.getByTestId(/connection|status/)).toBeInTheDocument();
  });
  ...
  it("handles WebSocket disconnection", async () => {
    ...
    if (errorHandler) {
      ...
      await waitFor(() => {
        expect(screen.getByText(/disconnected|offline|connection lost/i)).toBeInTheDocument();
      });
    }
  });
