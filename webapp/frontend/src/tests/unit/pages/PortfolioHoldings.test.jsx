...
vi.mock("../../../services/api.js", async () => {
  ...
  return {
    ...
    getPerformanceAnalytics: vi.fn().mockResolvedValue({ data: { returns: 0.15, volatility: 0.18, sharpe_ratio: 0.85, portfolio_metrics: { total_value: 100000 }, performance_timeline: [] } }),
    getRiskAnalytics: vi.fn().mockResolvedValue({ data: { risk: { portfolio_metrics: { portfolio_volatility: 15, max_drawdown: -12, value_at_risk_95: -8, concentration_risk: "Medium" }, risk_assessment: { overall_risk: "Medium" } } } }),
    getAllocationAnalytics: vi.fn().mockResolvedValue({ data: { sectors: [], assets: [] } }),
    getCorrelationAnalytics: vi.fn().mockResolvedValue({ data: { correlations: { insights: { diversification_score: 75, average_correlation: 0.45, assets_analyzed: 10 } } } }),
    getVolatilityAnalytics: vi.fn().mockResolvedValue({ data: { volatility: { annualized_volatility: 18, risk_level: "Medium" } } }),
    getTrendsAnalytics: vi.fn().mockResolvedValue({ data: { trends: { trend_direction: "Upward", trend_strength: "Strong" } } }),
