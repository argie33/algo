...
const mockSectorData = {
  sectors: [
    { name: "Technology", performance: 15.2, allocation: 28.5, marketCap: 12500000000000, topStocks: ["AAPL", "MSFT", "GOOGL"]},
    { name: "Healthcare", performance: 8.7, allocation: 15.2, marketCap: 6800000000000, topStocks: ["JNJ", "PFE", "UNH"]},
    { name: "Finance", performance: 12.4, allocation: 18.9, marketCap: 8900000000000, topStocks: ["JPM", "BAC", "WFC"]},
    { name: "Consumer Discretionary", performance: 6.3, allocation: 12.1, marketCap: 5200000000000, topStocks: ["AMZN", "TSLA", "HD"]},
    { name: "Energy", performance: -2.1, allocation: 8.5, marketCap: 3100000000000, topStocks: ["XOM", "CVX", "COP"]},
    { name: "Utilities", performance: 4.2, allocation: 3.2, marketCap: 1800000000000, topStocks: ["NEE", "SO", "D"]},
  ],
  comparison: {
...
describe("SectorAnalysis Component", () => {
  ...
  it("shows sector rotation analysis", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/overweight growth sectors/i)).toBeInTheDocument();
      expect(screen.getByText(/underweight defensive sectors/i)).toBeInTheDocument();
    });
  });
  ...
  it("handles time period selection", async () => {
    ...
    const periodControls = screen.getAllByRole("button");
    const periodButton = periodControls.find(btn => 
      btn.textContent && (btn.textContent.includes("1Y") || btn.textContent.includes("6M"))
    );
  ...
  it("displays momentum indicators", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("85") || screen.getByText("78")).toBeInTheDocument(); // Momentum scores
    });
  });
  ...
  it("handles empty sector data", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/no sector data|no data/i) ||
             screen.getByText(/limited data available/i)).toBeInTheDocument();
    });
  });
  ...
  it("shows sector recommendations", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/overweight|underweight|neutral/i)).toBeInTheDocument();
    });
  });
  ...
  it("displays relative strength indicators", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/relative|vs market|outperform|underperform/i) ||
             screen.getByText(/strength/i)).toBeInTheDocument();
    });
  });
});
