...
vi.mock('../../../services/api', () => {
  ...
  const mockGetMarketOverview = vi.fn(() => Promise.resolve({
    data: {
      sentiment_indicators: {
        fear_greed: { value: 45, value_text: 'Neutral', timestamp: '2023-01-01' },
        aaii: { bullish: 35, neutral: 30, bearish: 35, date: '2023-01-01' },
        naaim: { average: 50, bullish_8100: 60, bearish: 40, week_ending: '2023-01-01' }
      },
      market_breadth: {
        advancing: 1500,
        declining: 1200,
  ...
  const mockGetMarketSentimentHistory = vi.fn(() => Promise.resolve({
    data: {
      fear_greed_history: [
  ...
  const mockGetMarketSectorPerformance = vi.fn(() => Promise.resolve({
    data: {
      sectors: [
        { sector: 'Technology', avg_change_percent: 2.5, stock_count: 300, sector_market_cap: 12000000000000 },
        { sector: 'Healthcare', avg_change_percent: 1.2, stock_count: 250, sector_market_cap: 8000000000000 },
        { sector: 'Financial Services', avg_change_percent: -0.5, stock_count: 200, sector_market_cap: 9000000000000 }
      ]
    }
  }));

  const mockGetMarketBreadth = vi.fn(() => Promise.resolve({
    data: {
      advancing: 1600,
      declining: 1100,
      unchanged: 300,
  ...
  const mockGetEconomicIndicators = vi.fn(() => Promise.resolve({
    data: [
      { name: 'Unemployment Rate', value: 3.7, unit: '%', previous_value: 3.8, change_percent: -2.6, timestamp: '2023-01-01' },
      { name: 'Consumer Price Index', value: 3.2, unit: '%', previous_value: 3.0, change_percent: 6.7, timestamp: '2023-01-01' },
      { name: 'Federal Funds Rate', value: 5.25, unit: '%', previous_value: 5.0, change_percent: 5.0, timestamp: '2023-01-01' }
    ]
  }));

  const mockGetSeasonalityData = vi.fn(() => Promise.resolve({
    data: {
      summary: {
      ...
      seasonalAnomalies: [
        { name: 'January Effect', period: 'Jan 1-31', description: 'Small cap outperformance', strength: 'Moderate' }
      ],
      holidayEffects: [
  ...
  const mockGetMarketResearchIndicators = vi.fn(() => Promise.resolve({
    data: {
      summary: {
      ...
      sectorRotation: [
        { sector: 'Technology', momentum: 'Strong', flow: 'Inflow', performance: 2.5 },
        { sector: 'Healthcare', momentum: 'Moderate', flow: 'Neutral', performance: 1.2 }
      ],
      economicCalendar: [
        { 
          event: 'FOMC Meeting', 
...
vi.mock('recharts', () => ({
  ...
  PieChart: ({ children, data }) => <div data-testid="pie-chart" data-chart-data={JSON.stringify(data)}>{children}</div>,
  Pie: ({ dataKey }) => <div data-testid={`pie-${dataKey}`} />,
  Cell: () => <div data-testid="cell" />,
  RadarChart: ({ children, data }) => <div data-testid="radar-chart" data-chart-data={JSON.stringify(data)}>{children}</div>,
...
describe('MarketOverview Page', () => {
  const mockMarketData = {
    indices: [
      ...
      { symbol: 'QQQ', name: 'NASDAQ', price: 375.80, change: -2.15, changePercent: -0.57 },
      { symbol: 'DIA', name: 'Dow Jones', price: 350.90, change: 3.20, changePercent: 0.92 },
    ],
    sectors: [
      { name: 'Technology', performance: 2.5, volume: '125M', topStock: 'AAPL' },
    ...
    movers: {
      gainers: [
        { symbol: 'AAPL', change: 8.5, changePercent: 5.2, price: 172.30 },
        { symbol: 'GOOGL', change: 15.2, changePercent: 3.1, price: 2805.60 },
      ],
      losers: [
        { symbol: 'TSLA', change: -12.4, changePercent: -4.8, price: 246.80 },
        { symbol: 'META', change: -8.9, changePercent: -2.1, price: 415.70 },
      ],
    },
    economicIndicators: [
      { name: 'VIX', value: 18.5, change: -1.2, description: 'Volatility Index' },
      { name: 'DXY', value: 103.8, change: 0.3, description: 'Dollar Index' },
      { name: '10Y Treasury', value: 4.25, change: 0.05, description: 'Treasury Yield' },
    ],
  };
  ...
  describe('Sector Performance Section', () => {
    ...
    it('allows sorting sectors by performance', async () => {
      ...
      await waitFor(() => {
        const sortButton = screen.getByRole('button', { name: /sort by performance/i });
  ...
  describe('Interactive Features', () => {
    it('navigates to detailed views', async () => {
      ...
      await waitFor(() => {
        const sectorButton = screen.getByRole('button', { name: /view sector details/i });
  ...
  describe('Accessibility', () => {
    ...
    it('provides alternative text for charts', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/s&p 500 trend chart/i)).toBeInTheDocument();
        expect(screen.getByLabelText(/sector performance chart/i)).toBeInTheDocument();
      });
    });
  });
});