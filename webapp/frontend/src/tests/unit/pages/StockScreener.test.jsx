...
const mockScreenerData = {
  results: [
    { symbol: "AAPL", name: "Apple Inc.", price: 185.92, marketCap: 2850000000000, pe: 28.5, div: 0.52, volume: 45678900, sector: "Technology" },
    { symbol: "MSFT", name: "Microsoft Corp", price: 378.85, marketCap: 2820000000000, pe: 32.1, div: 0.68, volume: 23456789, sector: "Technology" },
    { symbol: "JNJ", name: "Johnson & Johnson", price: 162.45, marketCap: 425000000000, pe: 15.8, div: 2.95, volume: 8765432, sector: "Healthcare" },
    { symbol: "JPM", name: "JPMorgan Chase", price: 168.24, marketCap: 485000000000, pe: 12.4, div: 4.20, volume: 12345678, sector: "Finance" },
  ],
  criteria: {
  ...
  presets: [
    { id: 1, name: "Large Cap Growth", description: "Large cap stocks with strong growth potential" },
    { id: 2, name: "Dividend Aristocrats", description: "High dividend yield with consistent payments" },
    { id: 3, name: "Value Stocks", description: "Undervalued stocks with low P/E ratios" },
    { id: 4, name: "Small Cap Momentum", description: "Small cap stocks with strong momentum" },
  ],
  totalResults: 156,
  page: 1,
  limit: 25,
...
describe("StockScreener Component", () => {
  ...
  it("displays sector selection", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole("combobox") || 
             screen.getByText(/technology|healthcare|finance/i)).toBeInTheDocument();
    });
  });
  ...
  it("handles running a custom screen", async () => {
    ...
    await waitFor(() => {
      const screenButton = screen.getByRole("button", { name: /screen|search|apply/i });
  ...
  it("shows results count and pagination", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/page|1-25|showing/i) ||
  ...
  it("allows saving custom screens", async () => {
    ...
    await waitFor(() => {
      const saveButton = screen.getByRole("button", { name: /save|save screen/i });
      if (saveButton) {
  ...
  it("shows loading state during screening", () => {
    ...
    expect(screen.getByRole("progressbar") || screen.getByText(/screening|searching/i)).toBeInTheDocument();
  });
  ...
  it("handles API errors gracefully", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/error|failed|screening failed/i)).toBeInTheDocument();
    });
  });
  ...
  it("handles empty results", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/no results|no stocks found/i)).toBeInTheDocument();
    });
  });
  ...
  it("displays advanced filter options", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/advanced|more filters/i) ||
             screen.getByRole("button", { name: /advanced/i })).toBeInTheDocument();
    });
  });
  ...
  it("shows stock detail links", async () => {
    ...
    await waitFor(() => {
      ...
      expect(stockLinks[0].closest('a') || 
             stockLinks[0].getAttribute('role')).toBeDefined();
    });
  });
});
