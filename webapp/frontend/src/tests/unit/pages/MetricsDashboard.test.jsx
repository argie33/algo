...
describe("MetricsDashboard Component", () => {
  ...
  it("renders metrics dashboard page", async () => {
    ...
    expect(screen.getByText(/metrics dashboard|dashboard/i)).toBeInTheDocument();
  });
  ...
  it("displays metrics in cards layout", () => {
    ...
    expect(container.querySelector('[class*="MuiCard"]') || 
           container.querySelector('[class*="card"]')).toBeInTheDocument();
  });
  ...
  it("handles empty metrics data", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/no data|unavailable/i) || 
             screen.getByText("0") ||
