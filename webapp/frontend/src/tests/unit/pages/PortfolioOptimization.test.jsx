...
describe("PortfolioOptimization Component", () => {
  ...
  it("runs optimization when button is clicked", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /optimize|run optimization/i })
      ).toBeInTheDocument();
    });
  ...
  it("displays optimized portfolio results", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/optimized portfolio|optimization results/i)
      ).toBeInTheDocument();
  ...
  it("displays improvement summary", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/better risk-adjusted returns/i)
      ).toBeInTheDocument();
  ...
  it("shows loading state during optimization", () => {
    ...
    expect(
      screen.getByRole("progressbar") || screen.getByText(/optimizing/i)
    ).toBeInTheDocument();
  });
  ...
  it("handles API errors gracefully", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/error|failed to load|optimization failed/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("allows changing risk tolerance", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByDisplayValue(/aggressive/i) ||
          screen.getByText(/aggressive/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays optimization steps/stepper", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/step 1|configure|settings/i)
      ).toBeInTheDocument() ||
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("WarningIcon") ||
          screen.getByText(/warning|risk|disclaimer/i)
      ).toBeInTheDocument();
    });
  });
});
