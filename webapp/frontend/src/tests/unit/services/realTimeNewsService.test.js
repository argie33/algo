...
const { default: mockRealTimeDataService } = await import('../../../services/realTimeDataService.js');
...
const mockSentimentData = {
  ...
  sentiment: {
    ...
    newsImpact: [
      { title: 'Apple earnings beat', impact: 0.8 }
    ]
  },
  timestamp: Date.now()
};
...
describe('RealTimeNewsService', () => {
  ...
  describe('News Subscriptions', () => {
    test('allows subscribing to news updates', () => {
      ...
      expect(realTimeNewsService.newsSubscribers.has(subscriptionId)).toBe(true);
    });
    ...
    test('allows unsubscribing from news updates', () => {
      ...
      expect(realTimeNewsService.newsSubscribers.has(subscriptionId)).toBe(false);
    });
  });
  ...
  describe('Sentiment Subscriptions', () => {
    test('allows subscribing to sentiment updates', () => {
      const callback = vi.fn();
      const subscriptionId = realTimeNewsService.subscribeToSentiment('AAPL', callback);
      
      expect(subscriptionId).toEqual(expect.any(Symbol));
      expect(realTimeNewsService.sentimentSubscribers.has('AAPL')).toBe(true);
      expect(realTimeNewsService.sentimentSubscribers.get('AAPL').has(subscriptionId)).toBe(true);
    });

    test('sends latest sentiment to new subscribers', () => {
      realTimeNewsService.latestSentiment['AAPL'] = { score: 0.8, label: 'positive' };
      const callback = vi.fn();
    ...
    test('allows unsubscribing from sentiment updates', () => {
      ...
      const result = realTimeNewsService.unsubscribeFromSentiment('AAPL', subscriptionId);
      
      expect(result).toBe(true);
      expect(realTimeNewsService.sentimentSubscribers.get('AAPL')?.has(subscriptionId)).toBe(false);
    });
  ...
  describe('Sentiment Update Handling', () => {
    test('processes incoming sentiment updates correctly', () => {
      ...
      expect(callback).toHaveBeenCalledWith(realTimeNewsService.latestSentiment['AAPL']);
    });
  ...
  describe('News Buffer Processing', () => {
    ...
    test('processes up to 10 articles at a time', async () => {
      ...
      realTimeNewsService.newsBuffer = Array(15).fill().map((_, i) => ({
  ...
  describe('Impact Calculation', () => {
    test('calculates impact based on source credibility', () => {
      const article = {
        source: 'Reuters',
        publishedAt: new Date().toISOString(),
        summary: 'A detailed analysis of market conditions with comprehensive data'
      };
    ...
    test('considers content length', () => {
      ...
      const longArticle = {
        source: 'Test',
        summary: 'A' + 'very '.repeat(50) + 'long summary with comprehensive details and analysis'
      };
  ...
  describe('API Methods', () => {
    test('fetches news sentiment from API', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/api/news/sentiment/AAPL?timeframe=1h');
    ...
    test('handles API errors for sentiment fetch', async () => {
      ...
      await expect(realTimeNewsService.fetchNewsSentiment('AAPL')).rejects.toThrow('API Error');
    ...
    test('handles API errors for breaking news fetch', async () => {
      ...
      await expect(realTimeNewsService.fetchBreakingNews()).rejects.toThrow('API Error');
      consoleSpy.mockRestore();
    });
  });
  ...
  describe('Data Access Methods', () => {
    test('returns latest news with limit', () => {
      realTimeNewsService.latestNews = Array(30).fill().map((_, i) => ({
        id: `news_${i}`,
        title: `Article ${i}`
      }));
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    test('maintains news history limit of 100 articles', async () => {
      ...
      realTimeNewsService.latestNews = Array(95).fill().map((_, i) => ({
        id: `existing_${i}`,
        title: `Existing ${i}`
      }));
      
      // Process 10 new articles
      realTimeNewsService.newsBuffer = Array(10).fill().map((_, i) => ({
        id: `new_${i}`,
        title: `New ${i}`,
        symbols: []
      }));
