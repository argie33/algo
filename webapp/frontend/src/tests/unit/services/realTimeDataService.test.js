...
describe('RealTimeDataService', () => {
  ...
  describe('Connection Management', () => {
    ...
    test('handles connection errors gracefully', async () => {
      ...
      try {
        ...
        expect(error.message).toBe('Failed to establish WebSocket connection: Connection failed');
      }
    });
  ...
  describe('Data Handling', () => {
    test('processes incoming data', () => {
      ...
      const mockData = {
        type: 'stock-prices',
        data: { AAPL: 150.00, GOOGL: 2500.00 }
      };
    ...
    test('handles message without subscribers', () => {
      ...
      expect(() => service.handleMessage(JSON.stringify(mockData))).not.toThrow();
    });
  });
