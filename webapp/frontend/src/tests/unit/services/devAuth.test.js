...
describe('DevAuthService', () => {
  ...
  beforeEach(async () => {
    ...
    devAuthService.users['argeropolos@gmail.com'] = devAuthService.users['devuser'];
  });
  ...
  describe('Initialization', () => {
    test('creates default dev user on initialization', () => {
      ...
      expect(devAuthService.users['devuser'].email).toBe('argeropolos@gmail.com');
  ...
  describe('Email Confirmation', () => {
    test('successfully confirms user with valid code', async () => {
      ...
      const result = await devAuthService.confirmSignUp('testuser', pending.verificationCode);
    ...
    test('fails confirmation with expired code', async () => {
      ...
      devAuthService.pendingVerifications['testuser'].createdAt = Date.now() - (6 * 60 * 1000);
  ...
  describe('User Authentication', () => {
    ...
    test('successfully signs in with email', async () => {
      const result = await devAuthService.signIn('test@example.com', 'TestPass123!');
  ...
  describe('Password Reset', () => {
    ...
    test('initiates password reset successfully', async () => {
      ...
      expect(result.success).toBe(true);
      expect(devAuthService.pendingVerifications['reset_testuser']).toBeDefined();
    });
    ...
    test('confirms password reset with valid code', async () => {
      ...
      const resetCode = devAuthService.pendingVerifications['reset_testuser'].resetCode;

      const result = await devAuthService.forgotPasswordSubmit('testuser', resetCode, 'NewPass123!');

      expect(result.success).toBe(true);
      expect(devAuthService.users['testuser'].password).toBe('NewPass123!');
      expect(devAuthService.pendingVerifications['reset_testuser']).toBeUndefined();
    });

    test('fails password reset with invalid code', async () => {
      await devAuthService.forgotPassword('testuser');

      const result = await devAuthService.forgotPasswordSubmit('testuser', 'wrongcode', 'NewPass123!');
    ...
    test('fails password reset with expired code', async () => {
      ...
      // Manually set creation time to 6 minutes ago
      devAuthService.pendingVerifications['reset_testuser'].createdAt = Date.now() - (6 * 60 * 1000);
      const resetCode = devAuthService.pendingVerifications['reset_testuser'].resetCode;
  ...
  describe('Utility Methods', () => {
    ...
    test('confirms MFA (dev bypass)', async () => {
      const result = await devAuthService.confirmMFA({ username: 'testuser' }, '123456');
