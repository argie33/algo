...
vi.mock('react-chartjs-2', () => ({
  Line: ({ data }) => <div data-testid="portfolio-chart">Portfolio Chart with {data?.datasets?.length || 0} datasets</div>,
  Doughnut: ({ data }) => <div data-testid="allocation-chart">Allocation Chart with {data?.labels?.length || 0} segments</div>
}));
...
describe('Portfolio Component - User Interface', () => {
  ...
  describe('Portfolio Loading and Display', () => {
    ...
    it('should display portfolio data when loaded successfully', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/125,750\.50/i) || screen.getByText(/\$125,750/i)).toBeTruthy();
      });

      // Should display today's P&L
      expect(screen.getByText(/2,500\.75/i) || screen.getByText(/\$2,500/i)).toBeTruthy();
    ...
    it('should handle empty portfolio gracefully', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/\$0\.00/i) || screen.getByText(/0\.00/)).toBeTruthy();
      });

      // Should show empty state message
      expect(screen.getByText(/no positions/i) || screen.getByText(/empty/i) || screen.getByText(/no holdings/i)).toBeTruthy();
    });
  });
  ...
  describe('Portfolio Calculations Display', () => {
    it('should display profit/loss with correct formatting and colors', async () => {
      ...
      await waitFor(() => {
        ...
        const todaysPnL = screen.getByText(/-1,250\.75/i) || screen.getByText(/-\$1,250/i);
        expect(todaysPnL).toBeTruthy();
        
        // Should display positive total P&L
        const totalPnL = screen.getByText(/5,750\.25/i) || screen.getByText(/\$5,750/i);
        expect(totalPnL).toBeTruthy();
      });

      // Should show loss position
      expect(screen.getByText('TSLA')).toBeTruthy();
      expect(screen.getByText(/-295/i) || screen.getByText(/-\$295/i)).toBeTruthy();
    });
    ...
    it('should calculate and display percentage returns correctly', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/100\.0%/i) || screen.getByText(/\+100%/i)).toBeTruthy();
      });
    });
  });
  ...
  describe('Error Handling', () => {
    it('should display error message when portfolio load fails', async () => {
      ...
      const { api } = await import('../../../services/api.js');
      api.getPortfolio.mockRejectedValue(new Error('API temporarily unavailable'));
  ...
  describe('User Interaction Features', () => {
    ...
    it('should show real-time price updates when available', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/450\.25/i) || screen.getByText(/\$450/i)).toBeTruthy();
      });
  ...
  describe('Accessibility and User Experience', () => {
    ...
    it('should handle large numbers formatting correctly', async () => {
      ...
      await waitFor(() => {
        ...
        const formattedNumber = screen.getByText(/5,750,000/i) || 
