...
describe("Settings Component - Trading Configuration", () => {
  ...
  describe("API Key Management", () => {
    it("should display existing API keys without exposing secrets", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("alpaca") || screen.getByText("Alpaca")
        ).toBeTruthy();
        expect(
          screen.getByText("interactive_brokers") ||
            screen.getByText("Interactive Brokers")
        ).toBeTruthy();
      });
      ...
      expect(
        screen.getByText(/active/i) || screen.getByText(/enabled/i)
      ).toBeTruthy();
      expect(
    ...
    it("should allow adding new API keys with validation", async () => {
      ...
      const providerSelect =
        ...
        screen.getByDisplayValue(/alpaca/i) ||
    ...
    it("should allow testing API key connectivity", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/50,000/i) || screen.getByText(/\$50,000/i)
        ).toBeTruthy();
        expect(
  ...
  describe("Trading Preferences", () => {
    it("should allow configuring trading preferences", async () => {
      ...
      const tradingSection =
        screen.getByText(/trading/i) || screen.getByText(/preferences/i);
      ...
      const orderTypeSelect =
        screen.queryByLabelText(/order type/i) ||
    ...
    it("should handle password changes securely", async () => {
      ...
      if (passwordSection) {
        const currentPasswordInput =
          screen.queryByLabelText(/current/i) ||
  ...
  describe("Data and Privacy", () => {
    ...
    it("should handle account deletion requests appropriately", async () => {
      ...
      const deleteOption =
        screen.queryByText(/delete account/i) ||
  ...
  describe("Error Handling", () => {
    it("should handle settings load failures gracefully", async () => {
      ...
      const { api } = await import("../../../services/api.js");
      api.getApiKeys.mockRejectedValue(
    ...
    it("should validate all form inputs before submission", async () => {
      ...
      const form =
        screen.queryByRole("form") || screen.getByTestId("settings-form");
      if (form) {
