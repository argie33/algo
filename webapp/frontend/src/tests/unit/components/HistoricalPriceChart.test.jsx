...
describe("HistoricalPriceChart", () => {
  ...
  describe("Interactive Features", () => {
    it("supports timeframe selection", async () => {
      ...
      const oneWeekButton = timeframeButtons.find(
        (btn) => btn.textContent === "1W"
      );
    ...
    it("allows zooming and panning", () => {
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} enableZoom={true} />
      );
  ...
  describe("Data Visualization Options", () => {
    it("supports different chart types", () => {
      const { rerender } = renderWithProviders(
        <HistoricalPriceChart {...defaultProps} chartType="line" />
      );
      expect(screen.getByTestId("chart-line")).toBeInTheDocument();

      rerender(
        <HistoricalPriceChart {...defaultProps} chartType="candlestick" />
      );
    ...
    it("displays volume overlay when enabled", () => {
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} showVolume={true} />
      );
  ...
  describe("Responsive Design", () => {
    it("adapts to container size", () => {
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} height={300} />
      );
  ...
  describe("Performance Optimization", () => {
    it("handles large datasets efficiently", () => {
      ...
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} data={largeDataset} />
      );
  ...
  describe("Accessibility", () => {
    it("provides chart description for screen readers", () => {
      ...
      expect(
        screen.getByLabelText(/historical price chart for AAPL/i)
      ).toBeInTheDocument();
    });

    it("includes data table alternative", () => {
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} showDataTable={true} />
      );
  ...
  describe("Error Handling", () => {
    it("handles malformed data gracefully", () => {
      ...
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} data={malformedData} />
      );

      expect(
        screen.getByText(/error processing chart data/i)
      ).toBeInTheDocument();
    });
    ...
    it("recovers from chart rendering errors", () => {
      ...
      renderWithProviders(
        <HistoricalPriceChart {...defaultProps} data={null} />
      );
  ...
  describe("Real-time Updates", () => {
    it("handles live price updates", async () => {
      ...
      const updatedData = [
        ...mockHistoricalData,
        { date: "2024-01-06", price: 158.0, volume: 1400000 },
      ];
      ...
      await waitFor(() => {
        ...
        expect(chartData[5].price).toBe(158.0);
      });
    });
