...
vi.mock("react-chartjs-2", () => ({
  Line: ({ data: _data }) => (
    <div data-testid="market-chart">
      Market Chart with {_data?.datasets?.length || 0} datasets
    </div>
  ),
  Bar: ({ data: _data }) => (
...
describe("Dashboard Component - Trading Overview", () => {
  ...
  describe("Dashboard Loading and Layout", () => {
    it("should render main dashboard sections", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/portfolio/i) || screen.getByText(/total/i)
        ).toBeTruthy();
      });

      // Should show market data section
      expect(
        screen.getByText(/market/i) || screen.getByText(/summary/i)
      ).toBeTruthy();
    ...
    it("should display portfolio value prominently", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/247,350\.75/i) || screen.getByText(/\$247,350/i)
        ).toBeTruthy();
      });

      // Should show today's P&L
      expect(
        screen.getByText(/3,250\.50/i) || screen.getByText(/\$3,250/i)
      ).toBeTruthy();
    });
  ...
  describe("Market Data Display", () => {
    it("should show current market indices", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/4,185\.50/i) || screen.getByText(/4185/i)
        ).toBeTruthy();
        expect(
          screen.getByText(/13,245\.75/i) || screen.getByText(/13245/i)
        ).toBeTruthy();
      });

      // Should show changes with appropriate color coding
      expect(
        screen.getByText(/-15\.25/i) || screen.getByText(/-0\.36/i)
      ).toBeTruthy();
      expect(
        screen.getByText(/\+45\.80/i) || screen.getByText(/\+0\.35/i)
      ).toBeTruthy();
    });
    ...
    it("should indicate market status (open/closed)", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/open/i) || screen.getByText(/closed/i)
        ).toBeTruthy();
      });
    });
  });
  ...
  describe("Quick Actions and Navigation", () => {
    it("should provide quick trading actions", async () => {
      ...
      const buyButton =
        screen.queryByText(/buy/i) ||
        screen.queryByRole("button", { name: /buy/i });
      const sellButton =
        screen.queryByText(/sell/i) ||
        screen.queryByRole("button", { name: /sell/i });
      const tradeButton =
        screen.queryByText(/trade/i) ||
    ...
    it("should show recent orders or trading activity", async () => {
      ...
      expect(
        screen.getByText(/filled/i) || screen.getByText(/complete/i)
      ).toBeTruthy();
      expect(
        screen.getByText(/pending/i) || screen.getByText(/open/i)
      ).toBeTruthy();
    });
  });
  ...
  describe("Watchlist and Alerts", () => {
    it("should display user watchlist", async () => {
      ...
      expect(
        screen.getByText(/-5\.25/i) || screen.getByText(/-2\.43/i)
      ).toBeTruthy();
      expect(
        screen.getByText(/\+12\.30/i) || screen.getByText(/\+2\.81/i)
      ).toBeTruthy();
    });
  ...
  describe("Performance and Charts", () => {
    ...
    it("should show key performance metrics", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/25,000/i) || screen.getByText(/\$25,000/i)
        ).toBeTruthy();
        expect(
          screen.getByText(/25\.0%/i) || screen.getByText(/25%/i)
        ).toBeTruthy();
      });

      // Should show day return
      expect(
        screen.getByText(/1,500/i) || screen.getByText(/\$1,500/i)
      ).toBeTruthy();
  ...
  describe("Error Handling and Resilience", () => {
    it("should handle partial data loading failures gracefully", async () => {
      ...
      api.getMarketSummary.mockRejectedValue(
      ...
      expect(
        screen.getByText(/unavailable/i) || screen.getByText(/error/i)
      ).toBeTruthy();
    });
  ...
  describe("Real-time Updates", () => {
    it("should handle real-time price updates", async () => {
      ...
      api.getPortfolio.mockResolvedValue({
        totalValue: 100000,
        positions: [{ symbol: "AAPL", price: 150.0, marketValue: 15000 }],
      });
