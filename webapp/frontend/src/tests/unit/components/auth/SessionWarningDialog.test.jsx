...
describe('SessionWarningDialog', () => {
  ...
  describe('Countdown Timer', () => {
    ...
    test('calls onLogout when countdown reaches zero', async () => {
      ...
      render(<SessionWarningDialog {...defaultProps} timeRemaining={2000} onLogout={onLogout} />);
    ...
    test('stops countdown when dialog is closed', async () => {
      const { rerender } = render(<SessionWarningDialog {...defaultProps} timeRemaining={5000} />);
      
      expect(screen.getByText('0:05')).toBeInTheDocument();
      
      rerender(<SessionWarningDialog {...defaultProps} open={false} timeRemaining={5000} />);
      ...
      rerender(<SessionWarningDialog {...defaultProps} open={true} timeRemaining={5000} />);
      expect(screen.getByText('0:05')).toBeInTheDocument();
    });
    ...
    test('updates countdown when timeRemaining prop changes', () => {
      ...
      rerender(<SessionWarningDialog {...defaultProps} timeRemaining={10000} />);
  ...
  describe('User Actions', () => {
    ...
    test('calls onExtend and onClose when Stay Signed In is clicked', async () => {
      ...
      const onClose = vi.fn();
      render(<SessionWarningDialog {...defaultProps} onExtend={onExtend} onClose={onClose} />);
    ...
    test('handles extend session error gracefully', async () => {
      ...
      await waitFor(() => {
        expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to extend session:', expect.any(Error));
      });
  ...
  describe('Component Lifecycle', () => {
    ...
    test('handles rapid open/close cycles', () => {
      const { rerender } = render(<SessionWarningDialog {...defaultProps} open={false} />);
  ...
  describe('Edge Cases', () => {
    ...
    test('handles very large time values', () => {
      render(<SessionWarningDialog {...defaultProps} timeRemaining={3600000} />);
