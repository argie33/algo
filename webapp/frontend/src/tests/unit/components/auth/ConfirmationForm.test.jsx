...
describe('ConfirmationForm', () => {
  ...
  beforeEach(() => {
    ...
    mockResendConfirmationCode.mockResolvedValue({ success: true, message: 'Code sent' });
  });
  ...
  test('submits verification code', async () => {
    ...
    await waitFor(() => {
      expect(mockConfirmRegistration).toHaveBeenCalledWith('testuser', '123456');
    });
  });

  test('calls onConfirmationSuccess on success', async () => {
    const onConfirmationSuccess = vi.fn();
    render(<ConfirmationForm {...defaultProps} onConfirmationSuccess={onConfirmationSuccess} />);
  ...
  test('switches back to login', () => {
    ...
    render(<ConfirmationForm {...defaultProps} onSwitchToLogin={onSwitchToLogin} />);
