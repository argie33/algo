...
const ThrowError = ({ shouldThrow, error, errorInfo }) => {
  if (shouldThrow) {
    if (errorInfo) {
      ...
      mockError.componentStack =
        errorInfo.componentStack || "Mock component stack";
      throw mockError;
    }
...
describe("ErrorBoundary", () => {
  ...
  describe("Environment-Specific Behavior", () => {
    test("shows error details in development mode", () => {
      ...
      expect(
        screen.getByText("Error: Development error details")
      ).toBeInTheDocument();
  ...
  describe("Professional UI Elements", () => {
    test("displays professional footer content", () => {
      ...
      const dashboardTextExact = screen.queryByText(
        "Edgebrooke Capital Financial Dashboard"
      );
      const dashboardTextRegex = screen.queryByText(
        /Edgebrooke Capital Financial Dashboard/
      );
      const dashboardTextSplit =
        screen.queryByText(/Edgebrooke Capital/) &&
        screen.queryByText(/Financial Dashboard/);

      const dashboardExists =
        dashboardTextExact || dashboardTextRegex || dashboardTextSplit;
      expect(dashboardExists).toBeTruthy();

      expect(
        screen.getByText(/Enterprise-grade financial data platform/)
      ).toBeInTheDocument();
    });
