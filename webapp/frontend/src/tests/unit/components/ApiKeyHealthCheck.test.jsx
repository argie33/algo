...
describe('ApiKeyHealthCheck Component', () => {
      ...
      const keysWithWarning = [
        {
          ...mockApiKeys[0],
          status: 'warning',
          lastValidated: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString() // 7 days ago
        }
      ];
  ...
  describe('Auto Refresh Functionality', () => {
    it('should auto-refresh when enabled', async () => {
      ...
      const { unmount } = render(<ApiKeyHealthCheck {...defaultProps} autoRefresh={true} refreshInterval={1000} />);
    ...
    it('should not auto-refresh when disabled', async () => {
      ...
      const { unmount } = render(<ApiKeyHealthCheck {...defaultProps} autoRefresh={false} />);
  ...
  describe('Data Validation', () => {
    ...
    it('should handle malformed API key data', () => {
      const malformedKeys = [
        { id: null, provider: '', status: undefined }
      ];
