...
vi.mock('react-syntax-highlighter', () => ({
  Prism: ({ children }) => React.createElement('pre', { 'data-testid': 'syntax-highlighter' }, children)
}));
...
beforeEach(() => {
  ...
  const internals = global.React?.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
  if (internals) {
...
vi.mock('../contexts/AuthContext.jsx', async () => {
  ...
  const AuthProvider = ({ children }) => {
    ...
    return React.default.createElement(mockAuthContext.Provider, { value: authValue }, children);
  };
...
vi.mock('../services/api.js', async (importOriginal) => {
  ...
  return {
    ...
    getTradingSignalsWeekly: vi.fn().mockResolvedValue({ success: true, data: [] }),
