    ...
    <div data-testid="performance-monitor">
      {isLoading ? (
        ...
        <div data-testid="performance-metrics">
          ...
          <div data-testid="memory-usage">Memory: {metrics.memoryUsage} bytes</div>
          <div data-testid="render-time">Render Time: {metrics.renderTime}ms</div>
          <div data-testid="api-response-time">API Response: {metrics.apiResponseTime}ms</div>
        </div>
      )}
    </div>
  );
...
const MockHeavyChart = ({ data = [], onRenderComplete = vi.fn() }) => {
  ...
  React.useEffect(() => {
    ...
    }, Math.random() * 200 + 50); // 50-250ms render time
...
const MockLazyComponent = ({ threshold = 0.1 }) => {
  ...
  return (
    <div ref={elementRef} data-testid="lazy-component" style={{ height: '200px' }}>
      {!isVisible && <div data-testid="placeholder">Placeholder</div>}
      {isVisible && !isLoaded && <div data-testid="loading">Loading...</div>}
      {isLoaded && <div data-testid="loaded-content">Heavy Component Loaded</div>}
    </div>
  );
};
...
describe('Performance Monitoring Tests', () => {
  ...
  describe('Performance Metrics Collection', () => {
    ...
    test('should track component render performance', async () => {
      ...
      const chartData = Array.from({ length: 1000 }, (_, i) => ({ x: i, y: Math.random() * 100 }));
      
      render(<MockHeavyChart data={chartData} onRenderComplete={onRenderComplete} />);
  ...
  describe('Core Web Vitals Monitoring', () => {
    test('should collect Largest Contentful Paint (LCP)', async () => {
      const { getCLS, _getFID, getFCP, getLCP, _getTTFB } = require('web-vitals');
      ...
      const MockFIDComponent = () => {
        ...
        return (
          <div data-testid="fid-component">
            ...
            {fid && (
              <div data-testid="fid-metric">FID: {fid}ms</div>
            )}
          </div>
        );
      };
  ...
  describe('Bundle Size and Loading Performance', () => {
    test('should track resource loading performance', async () => {
      ...
      expect(screen.getByText('main.js: 245000 bytes, 850ms')).toBeInTheDocument();
    });
      ...
      const MockBundleSizeWarning = () => {
        ...
        React.useEffect(() => {
          ...
          const newWarnings = resources
            ...
            .map(resource => `${resource.name} is ${(resource.size / 1000).toFixed(0)}KB`);
        ...
        return (
          <div data-testid="bundle-size-warnings">
            {warnings.map((warning, index) => (
              <div key={index} data-testid={`warning-${index}`} className="warning">
                ⚠️ {warning}
              </div>
            ))}
          </div>
  ...
  describe('Performance Budget Monitoring', () => {
    test('should enforce performance budgets', () => {
      ...
      expect(screen.getByText('❌ Load time exceeds budget')).toBeInTheDocument();
      expect(screen.getByText('❌ Bundle size exceeds budget')).toBeInTheDocument();
    });
  });
});