...
const MockOrderFormComponent = ({ onOrderSubmit, availableCash = 10000 }) => {
  ...
  const validateOrder = () => {
    ...
    if (!orderData.quantity || orderData.quantity <= 0) newErrors.quantity = 'Valid quantity is required';
    ...
    if (orderData.orderType === 'stop' && (!orderData.stopPrice || orderData.stopPrice <= 0)) {
      newErrors.stopPrice = 'Valid stop price is required';
    }

    // Buying power check
    const estimatedCost = (marketPrice || parseFloat(orderData.limitPrice) || 0) * parseFloat(orderData.quantity || 0);
  ...
  return (
    <form data-testid="order-form" onSubmit={handleSubmit}>
      ...
      <div>
        ...
        />
        {errors.symbol && <span data-testid="symbol-error" role="alert">{errors.symbol}</span>}
      </div>
      ...
      <div>
        ...
        <input
          ...
          min="1"
        />
        {errors.quantity && <span data-testid="quantity-error" role="alert">{errors.quantity}</span>}
      </div>
      ...
      {(orderData.orderType === 'limit' || orderData.orderType === 'stop_limit') && (
        <div>
          ...
          />
          {errors.limitPrice && <span data-testid="limit-price-error" role="alert">{errors.limitPrice}</span>}
        </div>
      )}
      ...
      {(orderData.orderType === 'stop' || orderData.orderType === 'stop_limit') && (
        <div>
          ...
          />
          {errors.stopPrice && <span data-testid="stop-price-error" role="alert">{errors.stopPrice}</span>}
        </div>
      )}
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => setOrderData(prev => ({ ...prev, timeInForce: e.target.value }))}
        >
      ...
      <div data-testid="order-estimate">
        Estimated Cost: ${((marketPrice || parseFloat(orderData.limitPrice) || 0) * parseFloat(orderData.quantity || 0)).toFixed(2)}
      </div>
      ...
      {errors.buyingPower && (
        <div data-testid="buying-power-error" role="alert">{errors.buyingPower}</div>
      )}

      {errors.submit && (
        <div data-testid="submit-error" role="alert">{errors.submit}</div>
      )}
      ...
      <div data-testid="orders-list">
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map(order => (
            <div key={order.id} data-testid={`order-${order.id}`} className="order-item">
              <div>
                <strong>{order.symbol}</strong> - {order.side.toUpperCase()} {order.qty} shares
              </div>
              <div>
                Type: {order.type} | Status: {order.status}
              </div>
              <div>
                Price: {order.type === 'market' ? 'Market' : `$${order.limit_price || order.stop_price}`}
              </div>
              <div>
              ...
              {order.filled_at && (
                <div>
                  Filled: {new Date(order.filled_at).toLocaleString()} @ ${order.filled_avg_price}
                </div>
              )}
              {order.status === 'new' && (
                <button
      ...
      <div data-testid="positions-section">
        ...
        {positions.length === 0 ? (
          ...
          positions.map(position => {
            ...
            const unrealizedPnL = marketValue - (position.avg_entry_price * Math.abs(position.qty));
            
            return (
              <div key={position.asset_id} data-testid={`position-${position.symbol}`}>
                ...
                <div className={unrealizedPnL >= 0 ? 'profit' : 'loss'}>
                  Unrealized P&L: ${unrealizedPnL.toFixed(2)} ({((unrealizedPnL / (position.avg_entry_price * Math.abs(position.qty))) * 100).toFixed(2)}%)
                </div>
                <button
...
const MockRiskManagementComponent = ({ positions = [], orders = [] }) => {
  ...
  const calculateRiskMetrics = React.useCallback(() => {
    const totalPositionValue = positions.reduce((sum, pos) => sum + Math.abs(pos.market_value || 0), 0);
    ...
    const concentration = positions.reduce((max, pos) => {
      const positionPercent = Math.abs(pos.market_value || 0) / totalCash * 100;
    ...
    return {
      ...
      pendingOrderValue,
      riskScore: Math.min(100, (leverage * 30) + (concentration * 0.7) + (pendingOrderValue / totalCash * 40))
    };
  }, [positions, orders]);
  ...
  const checkRiskAlerts = React.useCallback(() => {
    ...
    positions.forEach(position => {
      ...
      const unrealizedPercent = (unrealizedPnL / (position.avg_entry_price * Math.abs(position.qty))) * 100;
    ...
    <div data-testid="risk-management">
      ...
      {riskMetrics && (
        <div data-testid="risk-metrics">
          ...
          <div>Max Concentration: {riskMetrics.maxConcentration.toFixed(1)}%</div>
      ...
      <div data-testid="risk-settings">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleRiskSettingChange('maxLeverage', e.target.value)}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleRiskSettingChange('stopLossPercent', e.target.value)}
...
describe('Trading Workflow Tests', () => {
  ...
  describe('Order Form Component', () => {
    ...
    beforeEach(() => {
      api.get.mockImplementation((url) => {
        if (url.includes('/api/market/quote/')) {
          return Promise.resolve({ data: { success: true, data: mockMarketQuote } });
        }
    ...
    it('should validate required fields before submission', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('symbol-error')).toHaveTextContent('Symbol is required');
        expect(screen.getByTestId('quantity-error')).toHaveTextContent('Valid quantity is required');
      });
    ...
    it('should load market price when symbol is entered', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('market-price')).toHaveTextContent('Current Price: $150.25');
      });
    ...
    it('should show stop price field for stop orders', () => {
      ...
      fireEvent.change(screen.getByTestId('order-type-select'), { target: { value: 'stop' } });
    ...
    it('should calculate estimated cost correctly', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('order-estimate')).toHaveTextContent('Estimated Cost: $1502.50');
      });
    });

    it('should check buying power for buy orders', async () => {
      const mockSubmit = vi.fn().mockResolvedValue({ success: true });
      renderWithRouter(<MockOrderFormComponent onOrderSubmit={mockSubmit} availableCash={1000} />);
      ...
      await waitFor(() => {
        expect(screen.getByTestId('buying-power-error')).toHaveTextContent('Insufficient buying power');
      });
    ...
    it('should submit valid market order successfully', async () => {
      const mockSubmit = vi.fn().mockResolvedValue({ success: true });
      renderWithRouter(<MockOrderFormComponent onOrderSubmit={mockSubmit} availableCash={10000} />);

      fireEvent.change(screen.getByTestId('symbol-input'), { target: { value: 'AAPL' } });
      fireEvent.change(screen.getByTestId('quantity-input'), { target: { value: '10' } });
      fireEvent.change(screen.getByTestId('side-select'), { target: { value: 'buy' } });
    ...
    it('should submit valid limit order successfully', async () => {
      ...
      fireEvent.change(screen.getByTestId('quantity-input'), { target: { value: '10' } });
      fireEvent.change(screen.getByTestId('order-type-select'), { target: { value: 'limit' } });
      fireEvent.change(screen.getByTestId('limit-price-input'), { target: { value: '149.00' } });
    ...
    it('should handle order submission errors', async () => {
      const mockSubmit = vi.fn().mockRejectedValue(new Error('Order rejected by exchange'));
      renderWithRouter(<MockOrderFormComponent onOrderSubmit={mockSubmit} availableCash={10000} />);
      ...
      await waitFor(() => {
        expect(screen.getByTestId('submit-error')).toHaveTextContent('Order rejected by exchange');
      });
    });
  });
  ...
  describe('Order History Component', () => {
    ...
    it('should show filled order details', async () => {
      ...
      await waitFor(() => {
        ...
        expect(within(filledOrder).getByText(/Filled:.*\$150\.25/)).toBeInTheDocument();
      });
    });
    ...
    it('should handle empty order list', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('no-orders')).toHaveTextContent('No orders found');
      });
    });
  });
  ...
  describe('Trading Dashboard Integration', () => {
    const mockAccount = {
      ...
      cash: 25000.00,
      day_trade_buying_power: 2500.00
    };
    ...
    beforeEach(() => {
      api.get.mockImplementation((endpoint) => {
        if (endpoint === '/api/trading/account') {
          return Promise.resolve({ data: { success: true, data: mockAccount } });
        }
        if (endpoint === '/api/trading/positions') {
          return Promise.resolve({ data: { success: true, data: mockPositions } });
        }
        ...
        if (endpoint.includes('/api/market/quote/')) {
          ...
          return Promise.resolve({
            data: {
              success: true,
              data: { symbol, price: symbol === 'AAPL' ? 150.25 : 300.00 }
            }
          });
        }
        return Promise.reject(new Error('Unknown endpoint'));
      ...
      api.post.mockResolvedValue({
        data: { success: true, data: { id: 'new-order-123', status: 'accepted' } }
      });
    ...
    it('should display positions with current market values', async () => {
      ...
      expect(within(aaplPosition).getByText('Quantity: 100')).toBeInTheDocument();
      expect(within(aaplPosition).getByText('Avg Cost: $145.00')).toBeInTheDocument();
      expect(within(aaplPosition).getByText('Current Price: $150.25')).toBeInTheDocument();

      // Check for profit/loss indication
      expect(within(aaplPosition).getByText(/Unrealized P&L.*\$/)).toBeInTheDocument();
    });
    ...
    it('should integrate order form with account buying power', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('available-cash')).toHaveTextContent('Available Cash: $50000.00');
      });
    });
    ...
    it('should submit orders and refresh data', async () => {
      ...
      await waitFor(() => {
        expect(api.post).toHaveBeenCalledWith('/api/trading/orders', expect.objectContaining({
          symbol: 'AAPL',
          quantity: '10'
        }));
      });
  ...
  describe('Risk Management Component', () => {
    ...
    it('should calculate and display risk metrics', () => {
      renderWithRouter(
        <MockRiskManagementComponent positions={mockPositions} orders={mockOrders} />
      );
    ...
    it('should generate risk alerts for high leverage', async () => {
      ...
      renderWithRouter(
        <MockRiskManagementComponent positions={highLeveragePositions} orders={mockOrders} />
      );
    ...
    it('should generate stop loss alerts', async () => {
      ...
      expect(screen.getByText(/LOSING_STOCK loss.*exceeds stop loss threshold/)).toBeInTheDocument();
      expect(screen.getByTestId('close-position-LOSING_STOCK')).toBeInTheDocument();
    });
    ...
    it('should update metrics when settings change', async () => {
      ...
      fireEvent.change(screen.getByTestId('stop-loss-percent'), { target: { value: '2.0' } });
  ...
  describe('Trading Workflow Accessibility', () => {
    ...
    it('should announce form validation errors to screen readers', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('symbol-error')).toHaveAttribute('role', 'alert');
        expect(screen.getByTestId('quantity-error')).toHaveAttribute('role', 'alert');
      });
    });
