...
describe("AIAssistant Real Functionality Tests", () => {
  ...
  describe("Component Loading and Real AI Integration", () => {
      ...
      await waitFor(
        () => {
          ...
          const hasDescription =
            screen.queryByText(/personal AI-powered investment assistant/i) !==
            null;
          const hasHelpText =
            screen.queryByText(/Ask me anything about your portfolio/i) !==
            null;
    ...
    test("should display AI interface components", async () => {
      ...
      await waitFor(
        () => {
          expect(screen.getAllByText(/AI Assistant/i).length).toBeGreaterThan(
            0
          );
          expect(
            screen.getByText(
              /personal AI-powered investment assistant powered by Claude/i
            )
          ).toBeInTheDocument();
          expect(
    ...
    test("should provide investment guidance interface", async () => {
      ...
      await waitFor(
        () => {
          ...
          expect(
            screen.getByText(/market analysis, investment strategies/i)
          ).toBeInTheDocument();
        },
        { timeout: 10000 }
      );
    ...
    test("should handle real user queries", async () => {
      ...
      await waitFor(
        () => {
          ...
          expect(
            screen.getByText(
              /technical analysis, market insights, and personalized investment guidance/i
            )
          ).toBeInTheDocument();
        },
        { timeout: 10000 }
