...
export function useQuery(options) {
  const { queryKey, queryFn, enabled = true, staleTime, cacheTime, ...otherOptions } = options;
...
export function useInfiniteQuery(options) {
  const { queryKey, queryFn, getNextPageParam, enabled = true } = options;
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    try {
      ...
      const nextPageParam = getNextPageParam ? getNextPageParam(lastPage) : undefined;
      ...
      setState(prev => ({
        ...
        isFetchingNextPage: false,
        hasNextPage: getNextPageParam ? getNextPageParam(newPage) !== undefined : false
      }));
    } catch (error) {
      setState(prev => ({
        ...prev,
  ...
  const fetchFirstPage = useCallback(async () => {
    ...
    try {
      ...
      setState({
        ...
        isLoading: false,
        error: null,
        hasNextPage: getNextPageParam ? getNextPageParam(firstPage) !== undefined : false,
        isFetchingNextPage: false
      });
    } catch (error) {
      setState(prev => ({
