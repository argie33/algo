...
function CryptoDashboard() {
  ...
  const { 
    ...
    queryFn: () => apiService.get(`/crypto/top-coins?limit=50&order=market_cap_desc`),
  ...
  const { 
    ...
    queryFn: () => apiService.get(`/crypto/trending?timeframe=${selectedTimeframe}&limit=10`),
  ...
  const { 
    ...
    queryFn: () => apiService.get(`/crypto/news?limit=20&timeframe=${selectedTimeframe}`),
  ...
  const renderMarketOverview = () => {
    if (marketLoading) return <LoadingDisplay message="Loading market overview..." />;
    if (marketError) return <Alert severity="error">Failed to load market overview</Alert>;
    if (!marketOverview?.data) return <Alert severity="info">No market data available</Alert>;
    ...
    return (
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={3}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>BTC Dominance</Typography>
              <Typography variant="h4" color="primary">
        ...
        <Grid item xs={12} md={3}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>Fear & Greed</Typography>
              <Typography variant="h4" color="primary">
              ...
              <Chip 
                ...
                color={
                  overview.fear_greed_index?.value >= 70 ? 'error' :
                  overview.fear_greed_index?.value >= 50 ? 'warning' : 'success'
                }
                size="small"
                sx={{ mt: 1 }}
              />
  ...
  const renderTopCoins = () => {
    if (topCoinsLoading) return <LoadingDisplay message="Loading top cryptocurrencies..." />;
    if (topCoinsError) return <Alert severity="error">Failed to load cryptocurrency data</Alert>;
    if (!topCoins?.data?.coins) return <Alert severity="info">No cryptocurrency data available</Alert>;
              ...
              <TableRow key={coin.id} hover>
                ...
                <TableCell align="center">
                  <IconButton 
                    ...
                    color={watchlist.includes(coin.symbol) ? 'primary' : 'default'}
                  >
                    {watchlist.includes(coin.symbol) ? <Star /> : <StarBorder />}
                  </IconButton>
                </TableCell>
              </TableRow>
            ))}
  ...
  const renderTrending = () => {
    if (trendingLoading) return <LoadingDisplay message="Loading trending cryptocurrencies..." />;
    if (trendingError) return <Alert severity="error">Failed to load trending data</Alert>;
    if (!trending?.data?.trending) return <Alert severity="info">No trending data available</Alert>;
  ...
  const renderNews = () => {
    ...
    if (newsError) return <Alert severity="error">Failed to load crypto news</Alert>;
    if (!cryptoNews?.data?.articles) return <Alert severity="info">No news articles available</Alert>;
  ...
  const renderDeFi = () => {
    if (defiLoading) return <LoadingDisplay message="Loading DeFi protocols..." />;
    if (defiError) return <Alert severity="error">Failed to load DeFi data</Alert>;
    if (!defiData?.data?.protocols) return <Alert severity="info">No DeFi data available</Alert>;
  ...
  return (
    <ErrorBoundary fallback={<Alert severity="error">Failed to load crypto dashboard</Alert>}>
      <Box sx={{ width: '100%', p: 3 }}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
          <Tabs value={activeTab} onChange={handleTabChange} aria-label="crypto dashboard tabs">
            <Tab 
