...
export default function SwingCandidates() {
  ...
  const sectors = useMemo(() => {
    if (!itemsList) return [];
    return Array.from(
      new Set(itemsList.map((i) => i.sector).filter(Boolean))
    ).sort();
  }, [itemsList]);
    ...
    return (
      <div className="main-content">
        <div className="page-head">
          <div>
            <div className="page-head-title">Swing Candidates</div>
            <div className="page-head-sub">Loading...</div>
          </div>
        </div>
        <div style={{ padding: "var(--space-4)", textAlign: "center" }}>
          <div className="t-sm muted">Loading swing candidates...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="main-content">
        <div className="page-head">
          <div>
            <div className="page-head-title">Swing Candidates</div>
            <div className="page-head-sub">No candidates available</div>
          </div>
        </div>
        <div
          style={{
        ...
        >
          <Inbox
            size={40}
            style={{ margin: "0 auto var(--space-2)", opacity: 0.5 }}
          />
    ...
    <ErrorBoundary>
      <div className="main-content">
        ...
        <div className="grid grid-4">
          ...
          <Kpi
            label="Top-10 Avg"
            value={`${num(stats.top10Score, 1)}/100`}
            sub="composite score"
          />
        </div>
            ...
            <div className="card-head">
              <div>
                ...
                <div className="card-sub">
                  5 highest-graded names with 60-day price action
                </div>
              </div>
            </div>
          ...
          <div className="card" style={{ marginTop: "var(--space-4)" }}>
            <div className="card-head">
              <div>
                ...
                <div className="card-sub">
                  High-quality candidate count over the last 30 days
                </div>
              </div>
            </div>
          ...
          <div className="card-body">
            ...
            >
              ...
              <span className="t-xs muted" style={{ marginLeft: "auto" }}>
                ...
                {filtered.length} of {itemsList?.length || 0}
              </span>
            </div>
          </div>
        </div>
              ...
              >
                ...
                >
                  <select
                    ...
                    className="input"
                    style={{ padding: "6px 8px", fontSize: "var(--t-xs)" }}
                  >
        ...
        <div>
          ...
          <div className="card-sub">
            {selected
              ...
              : "Click a row to compare a name against benchmarks"}
          </div>
        </div>
      </div>
      <div className="card-body">
    ...
    <div className="card">
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Top-50 candidates by sector · click to filter
          </div>
        </div>
      </div>
      <div className="card-body">
          ...
          <div style={{ height: 260 }}>
            <ResponsiveContainer width="100%" height="100%">
              <Treemap
                ...
                fill="var(--brand)"
                content={
                  <TreemapCell colors={COLORS} onClick={onSectorClick} />
                }
              >
                <Tooltip
                  contentStyle={TOOLTIP_STYLE}
              ...
              {data.funnel.map((f, i) => {
                const pct =
                  data.funnel[0].count > 0
                    ? (f.count / data.funnel[0].count) * 100
                    : 0;
                return (
                  <div key={i}>
                    <div
                      className="flex justify-between"
                      style={{ fontSize: "var(--t-xs)", marginBottom: 2 }}
                    >
  ...
  if (!matrix) {
    return (
      <div className="card">
        ...
        <div className="card-body">
          <Empty title="Need ≥5 candidates for correlation" />
        </div>
      </div>
    );
  }
              ...
              <tr key={la}>
                ...
                {COMPONENTS.map(([__, lb], cIdx) => {
                  const cell = matrix.find(
                    (m) => m.rowIdx === rIdx && m.colIdx === cIdx
                  );
                  const r = cell?.r ?? 0;
                  return (
                    <td
                      key={lb}
                      title={`${la} × ${lb} = ${r.toFixed(2)}`}
                      style={{
                        padding: "8px 4px",
                        textAlign: "center",
                        background: shade(r),
                        ...
                        color:
                          Math.abs(r) > 0.5 ? "var(--text)" : "var(--text-2)",
                      }}
                    >
                      {r.toFixed(2)}
                    </td>
...
function Row({ c, rank, active, onClick, _onNavigate }) {
  ...
  return (
    ...
    >
      ...
      <td>
        <span className={`badge ${GRADE_CLASS[c.grade] || "badge"}`}>
          {c.grade || "—"}
        </span>
      </td>
