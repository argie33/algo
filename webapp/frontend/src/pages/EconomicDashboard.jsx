...
function deriveRegime(indicators, yieldData, recessionProb) {
  ...
  if (recessionProb != null && recessionProb >= 60)
    return {
      ...
      color: "var(--danger)",
      desc: "Multiple recession signals active — risk-off positioning warranted.",
    };
  if (recessionProb != null && recessionProb >= 35)
    return {
      label: "Late Cycle",
      color: "var(--amber)",
      desc: "Growth slowing, credit tightening. Monitor leading indicators closely.",
    };
  if (spread != null && spread < 0 && (unrateVal ?? 5) < 5)
    return {
      label: "Late Cycle",
      color: "var(--amber)",
      desc: "Curve inverted but labor still resilient. Historically within 6–18 months of peak.",
    };
  if (gdpVal != null && gdpVal > 0 && indproTrend !== "down")
    return {
      label: "Expansion",
      color: "var(--success)",
      desc: "Broad-based growth with improving fundamentals. Risk appetite supported.",
    };
  return {
    label: "Uncertain",
    color: "var(--text-muted)",
    desc: "Mixed signals — maintain balanced positioning.",
  };
}
...
function EconomicDashboardPage() {
  ...
  const leadQ = useApiQuery(
    ["economic-leading"],
    () => api.get("/api/economic/leading-indicators"),
    { refetchInterval: 120000 }
  );
  const yldQ = useApiQuery(
    ["economic-yield"],
    () => api.get("/api/economic/yield-curve-full"),
    { refetchInterval: 120000 }
  );
  const calQ = useApiQuery(
    ["economic-calendar"],
    () => api.get("/api/economic/calendar"),
    { refetchInterval: 120000 }
  );
  const naaimQ = useApiQuery(
    ["market-naaim"],
    () => api.get("/api/market/naaim"),
    { refetchInterval: 120000 }
  );

  const isLoading =
    leadQ.loading || yldQ.loading || calQ.loading || naaimQ.loading;
  const refetch = () => {
    leadQ.refetch();
    yldQ.refetch();
    calQ.refetch();
  ...
  const calRaw =
    calQ.data?.events ??
    calQ.data?.items ??
    (Array.isArray(calQ.data) ? calQ.data : []);
  ...
  const recession = useMemo(() => {
    ...
    if (unrateHist.length >= 12) {
      const hist = unrateHist
        .slice(-12)
        .map((p) => +p.value)
        .filter((v) => !isNaN(v));
      if (hist.length >= 3) {
        const sahm =
          hist.slice(-3).reduce((s, v) => s + v, 0) / 3 - Math.min(...hist);
        tiles.push({
          label: "Sahm Rule",
          value: `${sahm >= 0 ? "+" : ""}${sahm.toFixed(2)} pp`,
          threshold: `≥ ${thresholds.sahm_critical} triggers`,
          desc: "3-mo unemployment MA vs trailing 12-mo low",
          status:
            sahm >= thresholds.sahm_critical
              ? "red"
              : sahm >= thresholds.sahm_warning
                ? "amber"
                : "green",
          weight:
            sahm >= thresholds.sahm_critical
    ...
    if (spread != null) {
      ...
      tiles.push({
        ...
        threshold: `< ${thresholds.spread_warning} inverts`,
        desc: "Inversion preceded last 8 US recessions",
        status:
          spread < thresholds.spread_critical
            ? "red"
            : spread < thresholds.spread_warning
              ? "amber"
              : "green",
        weight:
          spread < thresholds.spread_critical
    ...
    if (hyVal != null) {
      tiles.push({
        ...
        value: `${(+hyVal).toFixed(2)}%`,
        threshold: `> ${thresholds.hy_spread_warning}% elevated`,
        desc: "ICE BofA US High Yield OAS — default risk barometer",
        status:
          +hyVal > thresholds.hy_spread_critical
            ? "red"
            : +hyVal > thresholds.hy_spread_warning
              ? "amber"
              : "green",
        weight:
          +hyVal > thresholds.hy_spread_critical
    ...
    if (vixVal != null) {
      tiles.push({
        ...
        threshold: `> ${thresholds.vix_warning} elevated`,
        desc: "CBOE Volatility Index — equity fear gauge",
        status:
          +vixVal > thresholds.vix_critical
            ? "red"
            : +vixVal > thresholds.vix_warning
              ? "amber"
              : "green",
        weight:
          +vixVal > thresholds.vix_critical
    ...
    if (igVal != null) {
      tiles.push({
        ...
        value: `${(+igVal).toFixed(2)}%`,
        threshold: `> ${thresholds.ig_spread_warning}% caution`,
        desc: "Investment grade OAS — financial stress indicator",
        status:
          +igVal > thresholds.ig_spread_critical
            ? "red"
            : +igVal > thresholds.ig_spread_warning
              ? "amber"
              : "green",
        weight:
          +igVal > thresholds.ig_spread_critical
    ...
    const stlfsiVal =
      stlfsiHist.at?.(-1)?.value ??
      indicators.find((i) => (i.name || "").includes("Financial Stress"))
        ?.rawValue;
    if (stlfsiVal != null) {
      const sv = +stlfsiVal;
      tiles.push({
        label: "Financial Stress (STL)",
        value: sv >= 0 ? `+${sv.toFixed(2)}σ` : `${sv.toFixed(2)}σ`,
        threshold: `> ${thresholds.stress_warning} = stressed`,
        desc: "St. Louis Fed Financial Stress Index — 18 financial market variables",
        status:
          sv > thresholds.stress_critical
            ? "red"
            : sv > thresholds.stress_warning
              ? "amber"
              : "green",
        weight:
          sv > thresholds.stress_critical
    ...
    const cfnaiInd2 = indicators.find((i) =>
      (i.name || "").includes("Chicago Fed")
    );
    if (cfnaiInd2?.rawValue != null) {
      const cv = +cfnaiInd2.rawValue;
      tiles.push({
        label: "CFNAI Composite",
        value: cv >= 0 ? `+${cv.toFixed(2)}` : cv.toFixed(2),
        threshold: `< ${thresholds.cfnai_critical} = recession`,
        desc: "Chicago Fed National Activity Index — 85-indicator composite of US economic activity",
        status:
          cv < thresholds.cfnai_critical
            ? "red"
            : cv < thresholds.cfnai_warning
              ? "amber"
              : "green",
        weight:
          cv < thresholds.cfnai_critical
  ...
  const fci = useMemo(() => {
    ...
    const vixSeries = Array.isArray(yieldData.credit.history["VIXCLS"])
      ? yieldData.credit.history["VIXCLS"]
      : [];
    const hySeries = Array.isArray(yieldData.credit.history["BAMLH0A0HYM2"])
      ? yieldData.credit.history["BAMLH0A0HYM2"]
      : [];
    const igSeries = Array.isArray(yieldData.credit.history["BAMLH0A0IG"])
      ? yieldData.credit.history["BAMLH0A0IG"]
      : [];
    const curveSeries = Array.isArray(yieldData.history?.T10Y2Y)
      ? yieldData.history.T10Y2Y
      : [];
    ...
    const arr = [...map.values()]
      .filter((d) => d.vix != null && d.hy != null)
      .sort((a, b) => new Date(a.date) - new Date(b.date));
    if (arr.length === 0) return [];
    const mean = (arr, k) =>
      arr.reduce((s, d) => s + (d[k] ?? 0), 0) /
      arr.filter((d) => d[k] != null).length;
    const sd = (arr, k, m) =>
      Math.sqrt(
        arr
          .filter((d) => d[k] != null)
          .reduce((s, d) => s + (d[k] - m) ** 2, 0) / arr.length
      ) || 1;
    const mV = mean(arr, "vix");
    const sV = sd(arr, "vix", mV);
    const mH = mean(arr, "hy");
    const sH = sd(arr, "hy", mH);
    const mI = mean(arr, "ig");
    const sI = sd(arr, "ig", mI);
    return arr.map((d) => ({
      date: d.date,
      fci:
        (d.vix - mV) / sV +
        (d.hy - mH) / sH +
        (d.ig != null ? (d.ig - mI) / sI : 0) -
        (d.curve ?? 0),
    }));
  }, [yieldData]);
  ...
  const yieldCurve = useMemo(() => {
    ...
    return order
      .filter((k) => yieldData.currentCurve[k] != null)
      .map((k) => ({
        maturity: k,
        yield: yieldData.currentCurve[k],
      }));
  }, [yieldData]);
    ...
    <div className="main-content">
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-head-sub">
            Recession models · Credit conditions · Leading indicators · Fed
            policy
          </div>
        </div>
      ...
      {(leadQ.error || yldQ.error) && (
        ...
        >
          ...
          <div>
            {leadQ.error?.message ||
              leadQ.error?.responseData?.message ||
              yldQ.error?.message ||
              yldQ.error?.responseData?.message ||
        ...
        >
          ...
          <div>
            <div style={{ fontWeight: "var(--w-semibold)" }}>
              Economic data not yet loaded
            </div>
            <div className="t-sm muted">
              The FRED loader populates this dashboard. Run the economic data
              loader or wait for the next scheduled pipeline run. All values
              will show &mdash; until data is available.
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            <RegimeStat
              ...
              value={
                recession?.composite != null ? `${recession.composite}%` : "—"
              }
              color={
                recession?.composite >= 60
                  ? "var(--danger)"
                  : recession?.composite >= 35
                    ? "var(--amber)"
                    : "var(--success)"
              }
            />
            ...
            <RegimeStat
              label="10Y−2Y Spread"
              value={
                yieldData?.spreads?.T10Y2Y != null
                  ? bps(yieldData.spreads.T10Y2Y)
                  : "—"
              }
            ...
            <RegimeStat
              ...
              value={(() => {
                const v =
                  yieldData?.credit?.history?.["BAMLH0A0HYM2"]?.at?.(-1)?.value;
      ...
      <div style={{ marginTop: "var(--space-5)" }}>
                  ...
                  <div className="card-head">
                    <div>
                      <div className="card-title">
                        Recession Nowcasting Model
                      </div>
                      <div className="card-sub">
                        Composite probability across {recession.tiles.length}{" "}
                        indicators — Sahm rule, yield curve, credit spreads,
                        labor stress, vol regime
                      </div>
                    </div>
                    <span
                      className={`badge ${recession.composite >= 60 ? "badge-danger" : recession.composite >= 35 ? "badge-amber" : "badge-success"}`}
                    >
                  ...
                  <div className="card">
                    <div className="card-head">
                      <div>
                        ...
                        <div className="card-sub">
                          Z-score: VIX + HY spread + IG spread − yield curve ·
                          higher = tighter
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="card">
                    <div className="card-head">
                      <div>
                        ...
                        <div className="card-sub">
                          Below zero = inverted · preceded last 8 recessions
                        </div>
                      </div>
                      <span
                        className={`badge ${yieldData?.isInverted ? "badge-danger" : "badge-success"}`}
                      >
          ...
          <>
            ...
            >
              ...
              {["3M", "2Y", "10Y", "30Y"].map((k) => (
                ...
                >
                  ...
                  >
                    {yieldData?.currentCurve?.[k] != null
                      ? pct(yieldData.currentCurve[k])
                      : "—"}
                  </div>
                </div>
              ))}
              ...
              <div className="card" style={{ marginBottom: "var(--space-4)" }}>
                <div className="card-head">
                  <div>
                    ...
                    <div className="card-sub">
                      Current snapshot across all maturities
                    </div>
                  </div>
                  <span
                    className={`badge ${yieldData?.isInverted ? "badge-danger" : "badge-success"}`}
            ...
            {fedRate && (
              <IndHistory
                ...
                sub="Target rate set by FOMC — primary monetary policy tool"
                color="var(--cyan)"
              />
            )}
            ...
            {spreadHist.length > 0 && (
              <div className="card" style={{ marginTop: "var(--space-4)" }}>
                <div className="card-head">
                  <div>
                    <div className="card-title">
                      10Y − 2Y Yield Spread (Historical)
                    </div>
                    <div className="card-sub">
                      Duration of inversions and recovery shape
                    </div>
                  </div>
                </div>
          ...
          <>
              ...
              <div className="card" style={{ marginBottom: "var(--space-4)" }}>
                <div className="card-head">
                  <div>
                    ...
                    <div className="card-sub">
                      Created by ex-Fed economist Claudia Sahm — triggers at
                      +0.50pp rise in unemployment 3-mo MA vs prior year low
                    </div>
                  </div>
                  <RecessionBadge
                    status={
                ...
                <div className="card-body">
                  <div className="stile" style={{ maxWidth: 240 }}>
                    ...
                    <div className="stile-sub muted t-xs">
                      ≥ 0.50pp historically signals recession has started
                    </div>
                  </div>
                </div>
              </div>
            ...
            {unrateInd && (
              <IndHistory
                ...
                sub="Bureau of Labor Statistics — monthly"
                color="var(--amber)"
              />
            )}
            <div style={{ marginTop: "var(--space-4)" }}>
              {claimsInd && (
                <IndHistory
                  ...
                  sub="Weekly new unemployment insurance filings — earliest labor market signal"
                  color="var(--cyan)"
                />
              )}
            </div>
            {payrollInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Monthly job additions (thousands)"
                  color="var(--success)"
                />
              </div>
            )}
              ...
              <div className="card" style={{ marginTop: "var(--space-4)" }}>
                <div className="card-head">
                  <div>
                    ...
                    <div className="card-sub">
                      Job openings and quit rate — forward-looking demand and
                  ...
                  >
                    {joltsInd && (
                      <div className="stile">
                        ...
                        <div className="stile-value">
                          {joltsInd.rawValue != null
                            ? `${(+joltsInd.rawValue / 1000).toFixed(1)}M`
                            : "—"}
                        </div>
                        <div className="stile-sub muted t-xs">
                          High openings = labor demand strong — Beveridge Curve
                          tightness
                        </div>
                      </div>
                    )}
                    {quitRateInd && (
                      <div className="stile">
                        ...
                        <div className="stile-value">
                          {quitRateInd.value ? `${quitRateInd.value}%` : "—"}
                        </div>
                        <div className="stile-sub muted t-xs">
                          Workers quitting = confidence in finding new jobs —
                          "Great Resignation" proxy
                        </div>
                      </div>
                    )}
                  </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <XAxis
                            ...
                            interval={Math.max(
                              0,
                              Math.floor(joltsInd.history.length / 6)
                            )}
                          />
            ...
            {aheInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Wage growth — sustained >4% historically inflationary; below 3.5% = labor cost normalization"
                  color="var(--purple)"
                />
              </div>
            )}
        ...
        {tab === "inflation" && (
          <>
            ...
            {(() => {
              ...
              const realRate =
                corePce != null && rate10Y != null ? rate10Y - corePce : null;
              return (
                <div
                  className="grid grid-4"
                  style={{ marginBottom: "var(--space-4)" }}
                >
                  ...
                  >
                    ...
                    >
                      Positive = restrictive policy · Fed target 2.0%
                    </div>
                  </div>
                </div>
              );
            ...
            {(t5yieInd?.history?.length > 0 ||
              t10yieInd?.history?.length > 0 ||
              ...
              <div className="card" style={{ marginBottom: "var(--space-4)" }}>
                <div className="card-head">
                  <div>
                    ...
                    <div className="card-sub">
                      Fed policy stance relative to inflation — gap = real rate
                      environment · Core PCE is Fed's actual 2% target
                    </div>
                  </div>
                </div>
                <div className="card-body" style={{ height: 300 }}>
                  <ErrorBoundary>
                    <InflationVsFedChart
                      cpiHist={cpiInd.history}
                      corePceHist={corePceInd?.history}
                      fedHist={fedRate.history}
                    />
                  </ErrorBoundary>
                </div>
              </div>
            ...
            {cpiInd && (
              <IndHistory
                ...
                sub="Consumer Price Index — all items, all urban consumers"
                color="var(--danger)"
              />
            )}
            {corePceInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Personal Consumption Expenditures ex-Food & Energy — Federal Reserve's primary inflation target"
                  color="var(--amber)"
                />
              </div>
            )}
            {aheInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ind={aheInd}
                  title="Average Hourly Earnings (YoY)"
                  sub="Total private sector wages — wage inflation drives services inflation persistence"
                  color="var(--purple)"
                />
              </div>
            )}
        ...
        {tab === "cycle" && (
          <>
            ...
            >
              <MacroKpi
                label="Philly Fed Mfg Index"
                ind={phillyfedInd}
                unit=""
              />
              <MacroKpi
                label="Chicago Fed Activity (CFNAI)"
                ind={cfnaiInd}
                unit=""
              />
            ...
            <ErrorBoundary>
              <EconomicRegimeClock
                ...
                yieldData={yieldData}
                phillyfed={phillyfedInd}
              />
            </ErrorBoundary>

            {/* Growth-Labor Barometer */}
            <ErrorBoundary>
              <GrowthLaborBarometer
                indicators={indicators}
                phillyfed={phillyfedInd}
              />
            </ErrorBoundary>
            ...
            {phillyfedInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Federal Reserve Bank of Philadelphia monthly survey — diffusion index, >0 = expansion, <0 = contraction"
                  color="var(--brand)"
                />
              </div>
            )}
            {cfnaiInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="85-indicator composite measuring US economic activity — above 0 = above historical trend"
                  color="var(--cyan)"
                />
              </div>
            )}
            {tcuInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Manufacturing, mining, and utilities capacity in use — above 80% signals inflationary pressure"
                  color="var(--purple)"
                />
              </div>
            )}
          ...
          <>
            ...
            {gdpInd && (
              <IndHistory
                ...
                sub="Quarterly annualized real GDP — Bureau of Economic Analysis"
                color="var(--success)"
              />
            )}
            {indproInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Federal Reserve industrial output index — manufacturing, mining, utilities"
                  color="var(--brand)"
                />
              </div>
            )}
            {tcuInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ind={tcuInd}
                  title="Capacity Utilization"
                  sub="Share of productive capacity in use — above 80% = inflationary pressure, below 75% = slack"
                  color="var(--cyan)"
                />
              </div>
            )}
            {cfnaiInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="85-indicator composite — above 0 = above-trend growth, below -0.70 = recession risk"
                  color="var(--purple)"
                />
              </div>
            )}
            {rdpiInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ind={rdpiInd}
                  title="Real Disposable Personal Income (YoY)"
                  sub="Income after taxes adjusted for inflation — consumer spending fuel"
                  color="var(--amber)"
                />
              </div>
            )}
          ...
          <>
            ...
            {permitInd && (
              <IndHistory
                ...
                sub="Leading indicator for housing construction — issued 1-2 months before starts"
                color="var(--brand)"
              />
            )}
            {housingInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="New residential construction — Census Bureau (thousands, SAAR)"
                  color="var(--cyan)"
                />
              </div>
            )}
            {mortgageInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Freddie Mac PMMS — key affordability constraint for housing demand"
                  color="var(--danger)"
                />
              </div>
            )}
            {michInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="Monthly consumer survey — leading indicator for spending, ranges 50-110"
                  color="var(--amber)"
                />
              </div>
            )}
            {savingsInd && (
              <div style={{ marginTop: "var(--space-4)" }}>
                <IndHistory
                  ...
                  sub="% of disposable income saved — high savings = future spending fuel, low = stretched consumer"
                  color="var(--success)"
                />
              </div>
            )}
          ...
          <>
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    marginTop: "var(--space-2)",
                    color:
                      stlfsiInd?.rawValue > 1.5
                        ? "var(--danger)"
                        : stlfsiInd?.rawValue > 0.5
                          ? "var(--amber)"
                          : "var(--success)",
                  }}
                >
                  {stlfsiInd?.rawValue != null
                    ? stlfsiInd.rawValue >= 0
                      ? `+${(+stlfsiInd.rawValue).toFixed(2)}σ`
                      : `${(+stlfsiInd.rawValue).toFixed(2)}σ`
                    : "—"}
                </div>
                <div
              ...
              >
                ...
                >
                  {anfciInd?.rawValue != null
                    ? anfciInd.rawValue >= 0
                      ? `+${(+anfciInd.rawValue).toFixed(3)}`
                      : `${(+anfciInd.rawValue).toFixed(3)}`
                    : "—"}
                </div>
                <div
                  ...
                  style={{ marginTop: "var(--space-1)" }}
                >
                  Chicago Fed ANFCI · positive = tight
                </div>
              </div>
              <div
                className="card"
              ...
              >
                ...
                <div
                  className={`mono ${dollarInd?.trend || "flat"}`}
                  style={{
                    fontSize: "var(--t-xl)",
                    fontWeight: "var(--w-bold)",
                    marginTop: "var(--space-2)",
                ...
                >
                  {dollarInd?.rawValue != null
                    ? (+dollarInd.rawValue).toFixed(1)
                    : "—"}
                </div>
                <div
                  className="t-xs muted"
              ...
              >
                ...
                <div
                  className={`mono ${oilInd?.trend || "flat"}`}
                  style={{
                    fontSize: "var(--t-xl)",
                    fontWeight: "var(--w-bold)",
                    marginTop: "var(--space-2)",
                ...
                >
                  {oilInd?.rawValue != null
                    ? `$${(+oilInd.rawValue).toFixed(1)}`
                    : "—"}
                </div>
                <div
                  className="t-xs muted"
                ...
                <div className="card">
                  <div className="card-head">
                    <div>
                      ...
                      <div className="card-sub">
                        Trade-weighted nominal index vs major currencies —
                ...
                <div className="card">
                  <div className="card-head">
                    <div>
                      ...
                      <div className="card-sub">
                        Cushing, Oklahoma spot price — leading indicator for
                        energy inflation, global demand, and margin pressure
                      </div>
                    </div>
                  </div>
                  <div className="card-body" style={{ height: 240 }}>
          ...
          <div className="card">
            <div className="card-head">
              <div>
                ...
                <div className="card-sub">
                  Upcoming releases &amp; key events — next 120 days
                </div>
              </div>
            ...
            <div className="card-body" style={{ padding: 0 }}>
              {isLoading ? (
                ...
                <Empty
                  ...
                  desc="Run the economic calendar loader."
                />
              ) : (
                    ...
                    <tbody>
                      {calendar.map((e, i) => (
                        <tr key={i}>
                          ...
                          <td>
                            <span className="strong">{e.event}</span>
                          </td>
                          <td className="t-xs muted">{e.category || "—"}</td>
                          <td>
                            <span
                              className={`badge ${e.importance?.toLowerCase() === "high" ? "badge-danger" : e.importance?.toLowerCase() === "medium" ? "badge-amber" : ""}`}
                            >
                              {e.importance || "—"}
                            </span>
                          </td>
      ...
      {tabs.map((t) => (
        <button
          ...
          style={{
            ...
            fontWeight:
              value === t.id ? "var(--w-semibold)" : "var(--w-medium)",
            fontSize: "var(--t-sm)",
            borderRadius: 0,
            whiteSpace: "nowrap",
  ...
  const badge =
    status === "red"
      ...
      : status === "amber"
        ...
        : "badge-success";
  const label2 =
    status === "red" ? "TRIGGERED" : status === "amber" ? "ELEVATED" : "NORMAL";
  return (
    <div className="stile">
      <div className="stile-label">{label}</div>
      <div className="stile-value" style={{ color }}>
        {value}
      </div>
      ...
      <div className="t-2xs muted" style={{ marginTop: "var(--space-1)" }}>
        {desc}
      </div>
    </div>
  );
...
function MacroKpi({ label, ind, _unit, invertGood }) {
  ...
  return (
    <div className="card" style={{ padding: "var(--space-5) var(--space-6)" }}>
      ...
      {ind.change != null && (
        <div
          className={`t-xs mono ${up(ind.change)}`}
          style={{ marginTop: "var(--space-1)" }}
        >
          {ind.change > 0 ? "+" : ""}
          {(+ind.change).toFixed(2)}% MoM
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="card">
      <div className="card-head">
        ...
        {ind.signal && (
          <span
            className={`badge ${ind.signal === "Positive" ? "badge-success" : ind.signal === "Negative" ? "badge-danger" : "badge-cyan"}`}
          >
            {ind.signal}
          </span>
        )}
    ...
    <div className="card">
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            High yield OAS vs investment grade OAS — widening signals stress
          </div>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(v, n) => [
                `${(+v).toFixed(2)}%`,
                n === "hy" ? "High Yield OAS" : "IG OAS",
              ]}
            />
            <Legend
              wrapperStyle={{ fontSize: 11, color: "var(--text-muted)" }}
              formatter={(n) => (n === "hy" ? "High Yield OAS" : "IG OAS")}
            />
            <Line
              type="monotone"
              dataKey="hy"
      ...
      <div className="card-body" style={{ paddingTop: 0 }}>
        <div className="t-xs muted">
          HY spread &gt;5%: stress zone · &gt;8%: systemic risk · IG spread
          &gt;1.5%: caution · widening divergence = credit deterioration
        </div>
      </div>
    </div>
  );
...
function NaaimPanel({ naaim }) {
  ...
  const current =
    naaim?.current?.value ?? naaim?.current ?? naaim?.naaim_number_mean ?? null;
  ...
  const zoneLabel =
    zone === "healthy"
      ...
      : zone === "extended"
        ? "Extended — Watch for Reversion"
        : zone === "defensive"
          ? "Defensive Positioning"
          : "Very Defensive — Capitulation Risk";
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Professional active manager equity exposure · 0 = fully out, 100 =
            fully invested, 200 = leveraged long
          </div>
        </div>
        <span
          className="badge"
      ...
      <div className="card-body">
        ...
        >
          <div className="stile">
            <div className="stile-label">Current Reading</div>
            <div className="stile-value" style={{ color: zoneColor }}>
              {val.toFixed(1)}
            </div>
          ...
          <div className="stile">
            ...
            <div
              className="t-sm"
              style={{ marginTop: 4, maxWidth: 340, color: "var(--text-2)" }}
            >
              {zone === "extended"
        ...
        <div className="t-xs muted" style={{ marginTop: "var(--space-3)" }}>
          Source: National Association of Active Investment Managers · Weekly
          survey · Long-run avg ~65-70
        </div>
      </div>
    </div>
  );
...
function EconomicRegimeClock({ indicators, _yieldData, phillyfed }) {
  const gdp = indicators?.find((i) =>
    (i.name || "").toLowerCase().includes("gdp")
  );
  const cpi = indicators?.find((i) =>
    (i.name || "").toLowerCase().includes("cpi")
  );
  const cfnai = indicators?.find((i) =>
    (i.name || "").toLowerCase().includes("chicago fed")
  );
  ...
  const phillyTrend =
    phillyRaw != null ? (phillyRaw > 0 ? 1 : phillyRaw < 0 ? -1 : 0) : 0;
  ...
  const regime =
    growthScore > 0 && inflationScore < 0.5
      ? "Goldilocks"
      : growthScore > 0 && inflationScore >= 0.5
        ? "Overheat"
        : growthScore <= 0 && inflationScore >= 0.5
  ...
  const regimeDesc =
    regime === "Goldilocks"
      ? "Strong growth, moderate inflation — ideal policy conditions"
      : regime === "Overheat"
        ? "High growth + high inflation — Fed tightening, erosion risk"
        : regime === "Stagflation"
          ? "Weak growth + high inflation — no policy solution, defensive positioning"
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Growth vs Inflation — positioning for current economic phase
          </div>
        </div>
        <span
          className="badge"
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div className="t-xs strong" style={{ color: "var(--amber)" }}>
                  Slowdown
                </div>
              </div>
              {/* Top-Right: Overheat */}
              ...
              >
                <div className="t-xs strong" style={{ color: "var(--danger)" }}>
                  Overheat
                </div>
              </div>
          ...
          <div>
            <div className="stile">
              ...
              <div
                className="t-sm"
                style={{ marginTop: 4, color: "var(--text-2)" }}
              >
                {cfnaiVal != null ? (
                  <div>
                    CFNAI:{" "}
                ...
                <div style={{ marginTop: 4 }}>
                  ...
                  <strong
                    className={
                      phillyRaw != null ? (phillyRaw > 0 ? "up" : "down") : ""
                    }
                  >
                    {phillyRaw != null
                      ? `${phillyRaw >= 0 ? "+" : ""}${phillyRaw.toFixed(1)}`
                      : "—"}
                  </strong>
                </div>
              </div>
...
function GrowthLaborBarometer({ indicators, phillyfed }) {
  ...
  const claimsNumeric = claimsHist
    .map((h) => +h.value)
    .filter((v) => !isNaN(v));
  const claimsMA3 =
    claimsNumeric.length >= 13
      ? claimsNumeric.slice(-13).reduce((a, b) => a + b) / 13
      : claimsVal;
  const claimsTrend =
    claimsVal < claimsMA3 ? "down" : claimsVal > claimsMA3 ? "up" : "flat";

  const interpretation =
    barometer > 65
      ? {
          label: "Expansion Strong",
          color: "var(--success)",
          desc: "Industrial activity robust, job market tight — rising barometer signals continued growth.",
        }
      : barometer > 50
        ? {
            label: "Expansion Moderate",
            color: "var(--cyan)",
            desc: "Mixed but leaning positive — growth present but not accelerating.",
          }
        : barometer > 35
          ? {
              label: "Contraction Risk",
              color: "var(--amber)",
              desc: "Weakening indicators — monitor for further deterioration.",
            }
          : {
              label: "Contraction Evident",
              color: "var(--danger)",
              desc: "Industrial slowdown + labor weakness — recession-like conditions.",
            };
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Philly Fed Manufacturing (growth proxy) vs Jobless Claims (labor
            stress) — expansion vs contraction composite signal
          </div>
        </div>
        <span
          className="badge"
          style={{
            ...
            border: `1px solid ${interpretation.color}50`,
          }}
        >
          {barometer.toFixed(0)}
        </span>
          ...
          <div className="stile">
            ...
            >
              <div>
                ...
                <strong
                  className={ismVal > 50 ? "up" : ismVal < 50 ? "down" : ""}
                >
                  {ismVal.toFixed(1)} (ISM equiv)
                </strong>
              </div>
              <div style={{ marginTop: 4 }}>
                Jobless Claims:{" "}
              ...
              >
                <defs>
                  ...
                  >
                    <stop
                      offset="0%"
                      stopColor={interpretation.color}
                      stopOpacity={0.4}
                    />
                    <stop
                      offset="100%"
                ...
                <ReferenceLine
                  ...
                  stroke="var(--border-2)"
                  strokeDasharray="4 4"
                  label={{ value: "Risk", fontSize: 10, fill: "var(--text-3)" }}
                />
                <Area
                  type="monotone"
                  dataKey="barometer"
...
function LEIPanel({ indicators }) {
  ...
  const unrate = indicators?.find((i) =>
    (i.name || "").toLowerCase().includes("unemployment")
  );
  const houst = indicators?.find((i) =>
    (i.name || "").toLowerCase().includes("housing")
  );
  const icsa = indicators?.find(
    (i) =>
      (i.name || "").toLowerCase().includes("jobless") ||
      (i.name || "").toLowerCase().includes("initial claims")
  );
  const _civpart = indicators?.find((i) =>
    (i.name || "").toLowerCase().includes("participation")
  );
  const sp500 = indicators?.find(
    (i) =>
      (i.name || "").toLowerCase().includes("sp500") ||
      (i.name || "").toLowerCase().includes("s&p")
  );
  ...
  const leiScore = (() => {
    let components = [];
    if (unrateVal != null)
      components.push(Math.max(0, Math.min(100, 100 - unrateVal * 10)));
    if (houstVal != null)
      components.push(Math.max(0, Math.min(100, Math.min(houstVal / 2, 100))));
    if (icsaVal != null)
      components.push(
        Math.max(0, Math.min(100, Math.max(0, 100 - icsaVal / 5000)))
      );
    if (sp500Val != null)
      components.push(Math.max(0, Math.min(100, Math.min(sp500Val / 50, 100))));
  ...
  const chartData = (() => {
    ...
    const hist = history
      .slice(-26)
      .map((h) => {
        const u = unrate?.history?.find(
          (uh) => String(uh.date).slice(0, 10) === String(h.date).slice(0, 10)
        );
        const h2 = houst?.history?.find(
          (hh) => String(hh.date).slice(0, 10) === String(h.date).slice(0, 10)
        );
        const s = sp500?.history?.find(
          (sh) => String(sh.date).slice(0, 10) === String(h.date).slice(0, 10)
        );
        ...
        if (hou != null)
          comps.push(Math.max(0, Math.min(100, Math.min(hou / 2, 100))));
        if (ics != null)
          comps.push(Math.max(0, Math.min(100, Math.max(0, 100 - ics / 5000))));
        if (sp != null)
          comps.push(Math.max(0, Math.min(100, Math.min(sp / 50, 100))));

        return {
          date: String(h.date).slice(0, 10),
          lei:
            comps.length > 0
              ? comps.reduce((a, b) => a + b) / comps.length
              : 50,
        };
      })
      .sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  const sixMonthAvg =
    chartData.length > 0
      ? chartData.slice(-26).reduce((s, d) => s + d.lei, 0) /
        Math.min(26, chartData.length)
      : 50;
  const sixMonthTrend =
    chartData.length >= 2
      ? chartData[chartData.length - 1].lei > sixMonthAvg
        ? "up"
        : chartData[chartData.length - 1].lei < sixMonthAvg
          ? "down"
          : "flat"
  ...
  const leiInterpretation =
    leiScore > 60
      ? {
          label: "Expansion Strong",
          color: "var(--success)",
          desc: "LEI pointing to continued growth — positive momentum across labor, housing, and equities",
        }
      : leiScore > 50
        ? {
            label: "Expansion Moderate",
            color: "var(--cyan)",
            desc: "Mixed signals but growth-leaning — monitor for deterioration",
          }
        : leiScore > 40
          ? {
              label: "Growth Risk",
              color: "var(--amber)",
              desc: "Leading indicators softening — recession risk rising, watch labor market",
            }
          : {
              label: "Contraction Risk",
              color: "var(--danger)",
              desc: "LEI signaling recession — defensive positioning warranted",
            };
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Composite 6-month forward signal — unemployment, housing, claims,
            equities
          </div>
        </div>
        <span
          className="badge"
          style={{
            background: `${leiInterpretation.color}20`,
            color: leiInterpretation.color,
            border: `1px solid ${leiInterpretation.color}50`,
          }}
        >
          {leiScore.toFixed(0)}
        </span>
      ...
      <div className="card-body">
        ...
        >
          <div className="stile">
            ...
            <div
              className="stile-value"
              style={{ color: leiInterpretation.color }}
            >
              {leiScore.toFixed(0)}
            </div>
            <div className="stile-sub muted t-xs">
              {leiInterpretation.label}
            </div>
          </div>
          <div className="stile">
            ...
            >
              ...
              <div style={{ marginTop: 4 }}>
                ...
                <strong>
                  {chartData.length > 0
                    ? chartData[chartData.length - 1].lei.toFixed(0)
                    : "—"}
                </strong>
              </div>
            </div>
              ...
              >
                <defs>
                  <linearGradient id="leiGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop
                      offset="0%"
                      stopColor={leiInterpretation.color}
                      stopOpacity={0.4}
                    />
                    <stop
                      offset="100%"
                ...
                <Area
                  type="monotone"
                  dataKey="lei"
                  stroke={leiInterpretation.color}
                  strokeWidth={2}
                  fill="url(#leiGrad)"
                />
              </AreaChart>
...
function TipsBreakevenPanel({ t5y, t10y, yieldData }) {
  const t5hist = Array.isArray(t5y?.history)
    ? t5y.history
    : yieldData &&
        yieldData.breakevens &&
        yieldData.breakevens.history &&
        Array.isArray(yieldData.breakevens.history.T5YIE)
      ? yieldData.breakevens.history.T5YIE
      : [];
  const t10hist = Array.isArray(t10y?.history)
    ? t10y.history
    : yieldData &&
        yieldData.breakevens &&
        yieldData.breakevens.history &&
        Array.isArray(yieldData.breakevens.history.T10YIE)
      ? yieldData.breakevens.history.T10YIE
      : [];
  const t5cur =
    t5y?.rawValue ??
    (yieldData && yieldData.breakevens && yieldData.breakevens.current
      ? yieldData.breakevens.current.T5YIE
      : null);
  const t10cur =
    t10y?.rawValue ??
    (yieldData && yieldData.breakevens && yieldData.breakevens.current
      ? yieldData.breakevens.current.T10YIE
      : null);
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Market-implied inflation expectations from TIPS vs nominal
            Treasuries — the Fed monitors these closely
          </div>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(v, n) => [
                `${(+v).toFixed(2)}%`,
                n === "t5y" ? "5Y Breakeven" : "10Y Breakeven",
              ]}
            />
            <Legend
      ...
      <div className="card-body" style={{ paddingTop: 0 }}>
        <div className="t-xs muted">
          Breakeven = nominal Treasury yield − TIPS real yield · above 2.5% =
          markets expect above-target inflation · above 3% = inflation
...
function FinancialStressPanel({ stlfsiInd, anfciInd, yieldData }) {
  const stlfsiHist = Array.isArray(stlfsiInd?.history)
    ? stlfsiInd.history
    : Array.isArray(yieldData?.stress?.history?.STLFSI4)
      ? yieldData.stress.history.STLFSI4
      : [];
  const anfciHist = Array.isArray(anfciInd?.history)
    ? anfciInd.history
    : Array.isArray(yieldData?.stress?.history?.ANFCI)
      ? yieldData.stress.history.ANFCI
      : [];
  ...
  const stressLevel =
    stlfsiLatest != null
      ? +stlfsiLatest > 1.5
        ? { label: "Severe Stress", color: "var(--danger)" }
        : +stlfsiLatest > 0.5
          ? { label: "Elevated Stress", color: "var(--amber)" }
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            St. Louis Fed STLFSI4 (18 variables) + Chicago Fed ANFCI — broader
            than credit spreads alone
          </div>
        </div>
        <span
          className="badge"
          style={{
            ...
            border: `1px solid ${stressLevel.color}50`,
          }}
        >
          {stressLevel.label}
        </span>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(v, n) => [
                `${(+v).toFixed(3)}σ`,
                n === "stlfsi" ? "STLFSI4 (STL)" : "ANFCI (Chicago Fed)",
              ]}
            />
            <Legend
              wrapperStyle={{ fontSize: 11 }}
              formatter={(n) =>
                n === "stlfsi" ? "STLFSI4 (STL Fed)" : "ANFCI (Chicago Fed)"
              }
            />
            <ReferenceLine
              y={0}
            ...
            <ReferenceLine
              ...
              stroke="var(--success)"
              strokeDasharray="3 5"
              label={{ value: "Easy", fontSize: 9, fill: "var(--success)" }}
            />
            {stlfsiHist.length > 0 && (
              <Line
                type="monotone"
      ...
      <div className="card-body" style={{ paddingTop: 0 }}>
        <div className="t-xs muted">
          STLFSI4: 18 financial market variables (interest rates, spreads, other
          indicators) · 0 = average conditions · positive = stress · ANFCI
          adjusts for current macro conditions — negative = loose, positive =
          tight
        </div>
      </div>
    </div>
