...
function ScoresDashboardPage() {
  ...
  const sectors = useMemo(() => {
    ...
    return Array.from(
      new Set(items.map((i) => i.sector).filter(Boolean))
    ).sort();
  }, [items]);
    ...
    if (!details[symbol]) {
      try {
        ...
        if (!item && Array.isArray(r.data?.items)) {
          console.error(
            `[ScoresDashboard] Detail fetch returned empty items array for ${symbol}`
          );
        }
        if (item) {
          ...
          console.warn(
            `[ScoresDashboard] No detail item returned for ${symbol}`,
            r.data
          );
          setDetails((d) => ({
            ...d,
  ...
  const detailStock = selectedSymbol
    ? details[selectedSymbol] ||
      (items || []).find((s) => s.symbol === selectedSymbol)
    : null;
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-head-sub">
            Multi-factor stock scoring · composite + 6 factors · S&amp;P 500
            universe · click row for full factor breakdown
          </div>
        </div>
        ...
        <div className="card-body">
          <div className="flex items-center gap-3" style={{ flexWrap: "wrap" }}>
            ...
            <span className="t-xs muted" style={{ marginLeft: "auto" }}>
              <Filter size={12} style={{ verticalAlign: "-2px" }} />{" "}
      ...
      <div className="card" style={{ marginTop: "var(--space-4)" }}>
        <div className="card-body">
          <Empty
            ...
            desc="Try widening filters or refreshing the universe."
          />
        </div>
      </div>
    );
    ...
    <>
      ...
      >
        <span className="t-xs muted">
          {pageStart + 1}–{Math.min(pageEnd, totalRows)} of{" "}
      ...
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            {stock.sector || "—"} · composite {num(stock.composite_score, 1)} ·
...
function FactorScoreCard({
  ...
  const data = [
    {
      name: symbol,
      value: stockScore != null ? Number(stockScore) : null,
      fill: factor.tone,
    },
    {
      name: `${sectorLabel.slice(0, 12)} avg`,
      value: sectorAvg != null ? Number(sectorAvg) : null,
      fill: "var(--cyan)",
    },
    {
      name: "Market avg",
      value: marketAvg != null ? Number(marketAvg) : null,
      fill: "var(--text-3)",
    },
    ...
    <div className="card" style={{ background: "var(--surface-2)" }}>
      <div className="card-body">
        ...
        >
          ...
          <span
            className={`badge ${scoreClass(stockScore)}`}
          ...
          <ResponsiveContainer width="100%" height={140}>
            ...
            >
              ...
              <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                {data.map((d) => (
                  <Cell key={d.name || JSON.stringify(d)} fill={d.fill} />
                ))}
                <LabelList
                  dataKey="value"
                  position="top"
...
function FactorInputs({ title, inputs, schema }) {
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div className="t-xs muted">
            No detailed metrics available (API did not return inputs object)
          </div>
        </div>
      </div>
    );
  ...
  if (!schema || !Array.isArray(schema)) {
    console.error(
      `[FactorInputs] ${title}: Invalid schema - must be an array`,
      schema
    );
    return (
      <div className="card">
        ...
        <div className="card-body">
          <div className="t-xs muted">Configuration error: invalid schema</div>
        </div>
      </div>
    );
  }
      ...
      <div className="card-head">
        <div className="flex items-center gap-2">
          <Icon
            size={16}
            style={{
              color: mode === "laggards" ? "var(--danger)" : factor.tone,
            }}
          />
          <div className="card-title">
            {factor.label} {mode === "laggards" ? "Laggards" : "Leaders"}
          </div>
        </div>
      </div>
      <div className="card-body" style={{ padding: 0 }}>
            ...
            <tbody>
              {rows.map((s, i) => (
                <tr key={s.symbol} onClick={() => onClick(s.symbol)}>
                  ...
                  <td className="num">
                    <span
                      className={`badge ${mode === "laggards" ? "badge-danger" : scoreClass(s[factor.scoreKey])}`}
                    >
    ...
    <div className="grid grid-2" style={{ marginTop: "var(--space-4)" }}>
      {sectors.map((sec) => {
        const rows = items
          ...
          .sort(
            (a, b) => Number(b.composite_score) - Number(a.composite_score)
          );
        return (
...
function MoversTab({ items, onClick }) {
  ...
  const valid = items.filter((s) => s.composite_score != null);
  const topComposite = [...valid].sort(
    (a, b) => Number(b.composite_score) - Number(a.composite_score)
  );
  const botComposite = [...valid].sort(
    (a, b) => Number(a.composite_score) - Number(b.composite_score)
  );
  const validPx = items.filter((s) => s.change_percent != null);
  const gainers = [...validPx].sort(
    (a, b) => Number(b.change_percent) - Number(a.change_percent)
  );
  const decliners = [...validPx].sort(
    (a, b) => Number(a.change_percent) - Number(b.change_percent)
  );
  ...
  return (
    <>
      <div className="alert alert-info" style={{ marginTop: "var(--space-4)" }}>
        <Activity size={16} />
        <div>
          Score-history is a snapshot table — these cards show{" "}
          <strong>composite leaders / laggards</strong> and{" "}
          <strong>1-day price gainers / decliners</strong> across the universe
          (proxy for momentum movers).
        </div>
      </div>

      <div className="grid grid-2" style={{ marginTop: "var(--space-4)" }}>
        <MoverCard
          title="Top Composite Leaders"
          rows={topComposite}
        ...
        />
        <MoverCard
          title="Today's Top Gainers (Price)"
          rows={gainers}
          field="change_percent"
          tone="up"
          onClick={onClick}
          fmt={(v) => `${Number(v) >= 0 ? "+" : ""}${num(v, 2)}%`}
        />
        <MoverCard
          title="Today's Top Decliners (Price)"
          rows={decliners}
          field="change_percent"
          tone="down"
          onClick={onClick}
            ...
            {rows.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={`num mono tnum t-xs ${tone}`}>
                    {fmt(s[field])}
                  </td>
                  <td>
                    <div className="bar" style={{ width: 90 }}>
                      <div
                        ...
                        style={{
                          width: `${w}%`,
                          background:
                            tone === "up" ? "var(--success)" : "var(--danger)",
                        }}
                      />
                    </div>
                  </td>
...
function LeaderboardTab({ items, sectorFilter, onClick }) {
  ...
  const rows = (
    activeSec ? eligible.filter((s) => s.sector === activeSec) : eligible
...
function HeatmapTab({ items, sectorFilter, onClick }) {
  ...
  const filtered = (
    sectorFilter ? arr.filter((s) => s.sector === sectorFilter) : arr
  ).sort((a, b) => Number(b.composite_score) - Number(a.composite_score));
    ...
    <div className="card" style={{ marginTop: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Cell color = factor score 0-100 · click row → stock detail
          </div>
        </div>
      </div>
      <div className="card-body" style={{ padding: 0 }}>
          ...
          <table className="data-table" style={{ tableLayout: "fixed" }}>
            <thead>
              <tr>
                ...
                {FACTORS.map((f) => (
                  <th key={f.key} className="num" style={{ width: 90 }}>
                    {f.label}
                  </th>
                ))}
              </tr>
    ...
    <div className="card" style={{ marginTop: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Pearson correlation across the universe · 1.0 = perfect, 0 = none,
            −1.0 = inverse
          </div>
        </div>
      </div>
    ...
    <div className="card" style={{ marginTop: "var(--space-4)" }}>
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Each factor scored 0-100 from underlying fundamentals · composite is
            research-weighted blend
          </div>
        </div>
      </div>
      <div className="card-body">
...
function LegendChip({ color, label }) {
  return (
    <div className="flex items-center gap-2">
      <div
        style={{ width: 28, height: 8, borderRadius: 4, background: color }}
      />
...
function topBy(items, field, count, sector, dir) {
  const arr = (
    sector ? items.filter((s) => s.sector === sector) : items
