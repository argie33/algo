...
export default function PerformanceTab({ performance, equityCurve = [] }) {
  ...
  const drawdownData = React.useMemo(() => {
    ...
    return equityCurve.map((pt) => {
      ...
      const dd =
        peak > 0 ? -(((peak - pt.total_portfolio_value) / peak) * 100) : 0;
            ...
            <div style={{ height: 140, width: "100%", minWidth: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <RechartTooltip
                    contentStyle={TOOLTIP_STYLE}
                    formatter={(v) => (v != null ? `${Number(v)}%` : "N/A")}
                  />
                  <Area
                    type="monotone"
                    dataKey="drawdown"
        ...
        <>
          ...
          >
            ...
            <PerfCard
              ...
              color={numColor(p.win_rate_pct, 50)}
              hint="of closed trades"
            />
            <PerfCard
              label="Expectancy"
              value={`${p.expectancy_r >= 0 ? "+" : ""}${p.expectancy_r}R`}
              color={numColor(p.expectancy_r)}
              hint="per trade"
            />
            <PerfCard
              label="Profit Factor"
              value={p.profit_factor || "∞"}
              color={numColor((p.profit_factor || 0) - 1)}
              hint="gross win / gross loss"
            />
            <PerfCard
              label="Avg Win"
              value={`${p.avg_win_r >= 0 ? "+" : ""}${p.avg_win_r}R`}
              color="var(--success)"
              hint={`${p.avg_win_pct}%`}
            />
            <PerfCard
              label="Avg Loss"
              value={`${p.avg_loss_r}R`}
              color="var(--danger)"
              hint={`${p.avg_loss_pct}%`}
            />
            <PerfCard
              label="Avg Hold"
              value={`${p.avg_hold_days}d`}
              hint="days per trade"
            />
            <PerfCard
              label="Total P&L"
              value={`$${(p.total_pnl_dollars || 0).toLocaleString()}`}
          ...
          >
            ...
            <PerfCard
              label="Sortino Ratio"
              value={p.sortino_annualized}
              color={numColor(p.sortino_annualized - 1)}
              hint="downside-only volatility"
            />
            <PerfCard
              label="Max Drawdown"
              value={`${p.max_drawdown_pct}%`}
              color={
                p.max_drawdown_pct > 20
                  ? "var(--danger)"
                  : p.max_drawdown_pct > 10
                    ? "var(--amber)"
                    : "var(--success)"
              }
              hint="peak-to-trough"
            ...
            <PerfCard
              label="Calmar Ratio"
              value={p.calmar_ratio}
              color={numColor(p.calmar_ratio - 1)}
              hint="return / max DD"
            />
          </div>
          ...
          <div className="grid grid-4" style={{ gap: "var(--space-3)" }}>
            <PerfCard
              ...
              value={
                p.current_streak >= 0
                  ? `+${p.current_streak} W`
                  : `${Math.abs(p.current_streak)} L`
              }
              color={p.current_streak >= 0 ? "var(--success)" : "var(--danger)"}
            />
            <PerfCard
              label="Best Win Streak"
              value={p.best_win_streak}
              color="var(--success)"
            />
            <PerfCard
              label="Worst Loss Streak"
              value={p.worst_loss_streak}
              color="var(--danger)"
            />
            <PerfCard
              label="Sample Size"
              value={`${p.portfolio_snapshots} snapshots`}
              hint={
                p.portfolio_snapshots < 30 ? "(too few for solid stats)" : ""
              }
            />
          </div>
        </>
