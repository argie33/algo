...
function RiskTab({ circuitBreakers, _markets, positions = [] }) {
  ...
  const anyTriggered = Array.isArray(circuitBreakers)
    ? false
    : circuitBreakers?.any_triggered;
  ...
  const sectorExposure = React.useMemo(() => {
    ...
    if (missingValuePositions.length > 0) {
      console.warn(
        `[RiskTab] ${missingValuePositions.length} positions missing position_value: ${missingValuePositions.join(", ")}`
      );
    }
  ...
  const statusColor = (triggered) =>
    triggered ? "var(--danger)" : "var(--success)";
  const breakerBg = (triggered) =>
    triggered ? "rgba(225, 29, 72, 0.1)" : "rgba(34, 197, 94, 0.1)";
  const breakerBorder = (triggered) =>
    triggered ? "var(--danger)" : "var(--success)";

  const getPct = (current, threshold) =>
        ...
        >
          <strong>
            {!Array.isArray(circuitBreakers)
              ? circuitBreakers.triggered_count
              : breakers.filter((b) => b.triggered).length}{" "}
            circuit breaker
            {(!Array.isArray(circuitBreakers)
              ? circuitBreakers.triggered_count
              : breakers.filter((b) => b.triggered).length) !== 1
              ? "s"
              : ""}{" "}
            triggered
          </strong>{" "}
      ...
      <div className="card" style={{ marginBottom: "var(--space-4)" }}>
        <div className="card-head">
          <div className="card-title">
            Circuit Breakers — Kill-Switch Status
          </div>
        </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <RechartTooltip
                      ...
                      formatter={(v, n, p) => [
                        `${p.payload.pct}% ($${p.payload.value?.toLocaleString()})`,
                        "Exposure",
                      ]}
                    />
