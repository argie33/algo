...
function AnalystInsights({ symbol, onClose }) {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api.get(`/api/sentiment/analyst/insights/${symbol}`).then((r) => r?.data || null).catch(() => null),
    enabled: !!symbol,
  });
...
function SocialInsights({ symbol, onClose }) {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api.get(`/api/sentiment/social/insights/${symbol}`).then((r) => r?.data || null).catch(() => null),
    enabled: !!symbol,
  });
