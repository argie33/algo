  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (!res.ok) {
          // If API fails, return mock data instead of throwing
        ...
        try {
          ...
          console.warn('Invalid JSON in technical signals response, using mock data');
          return {
            data: [
              { symbol: 'AAPL', signal: 'Buy', date: '2025-07-03', current_price: 195.12, performance_percent: 2.1 },
              { symbol: 'TSLA', signal: 'Sell', date: '2025-07-02', current_price: 710.22, performance_percent: -1.8 }
...
const Dashboard = () => {
  ...
  // Equity curve (price history) - with fallback
  ...
  // Buy/Sell signals - with fallback
  const { data: buySignals } = useQuery({
    queryKey: ['buy-signals'],
    queryFn: async () => {
      try {
        return await getBuySignals();
      } catch (err) {
        console.warn('Buy signals API failed, using mock data:', err);
        return { data: mockSignals.filter(s => s.action === 'Buy') };
      }
    },
    staleTime: 2 * 60 * 1000
  });
  ...
  const { data: sellSignals } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await getSellSignals();
      } catch (err) {
        console.warn('Sell signals API failed, using mock data:', err);
        return { data: mockSignals.filter(s => s.action === 'Sell') };
      }
    },
    staleTime: 2 * 60 * 1000
  });
