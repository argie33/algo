...
const RiskManagement = () => {
  ...
  const mockRiskData = {
    ...
    positionRisks: [
      { symbol: 'AAPL', weight: 0.25, var: 15000, beta: 1.12, volatility: 0.22, contribution: 0.28 },
      { symbol: 'MSFT', weight: 0.20, var: 12000, beta: 0.98, volatility: 0.19, contribution: 0.22 },
      { symbol: 'GOOGL', weight: 0.18, var: 11000, beta: 1.25, volatility: 0.24, contribution: 0.25 },
      { symbol: 'TSLA', weight: 0.15, var: 22000, beta: 1.8, volatility: 0.35, contribution: 0.35 },
      { symbol: 'NVDA', weight: 0.12, var: 18000, beta: 1.65, volatility: 0.32, contribution: 0.28 }
    ],
    riskAlerts: [
      { id: 1, symbol: 'TSLA', metric: 'Volatility', value: 35.2, threshold: 30, severity: 'high', timestamp: '2025-07-03T10:30:00Z' },
      { id: 2, symbol: 'PORTFOLIO', metric: 'Concentration', value: 35, threshold: 30, severity: 'medium', timestamp: '2025-07-03T09:15:00Z' },
      { id: 3, symbol: 'NVDA', metric: 'Beta', value: 1.65, threshold: 1.5, severity: 'medium', timestamp: '2025-07-03T08:45:00Z' }
    ],
    historicalVaR: [
    ...
    stressTests: [
      ...
      { scenario: 'COVID-19 Crash (Mar 2020)', loss: -187500, percentage: -15.0 },
      { scenario: 'Tech Bubble Burst (2000)', loss: -312500, percentage: -25.0 },
    ...
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Comprehensive portfolio risk analysis, monitoring, and stress testing with real-time alerts.
        </Typography>
      </Box>
          ...
          <Card>
            <CardContent>
              ...
              <LinearProgress 
                ...
                sx={{ mt: 1 }}
                color={getRiskColor((riskData?.portfolioMetrics.var95 / riskData?.portfolioMetrics.totalValue) * 100, { low: 3, medium: 8 })}
              />
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={3}>
          <Card>
            <CardContent>
              ...
              <Chip 
                label={riskData?.portfolioMetrics.sharpeRatio > 1 ? 'Good' : 'Needs Improvement'} 
        ...
        <Grid item xs={12} md={3}>
          <Card>
            <CardContent>
              ...
              <Chip 
                label={Math.abs(riskData?.portfolioMetrics.beta - 1) < 0.2 ? 'Market Neutral' : 'Market Sensitive'} 
                color={Math.abs(riskData?.portfolioMetrics.beta - 1) < 0.2 ? 'success' : 'warning'} 
                size="small" 
                sx={{ mt: 1 }}
              />
            </CardContent>
      ...
      <Card>
              ...
              <TableBody>
                {riskData?.positionRisks.map((position) => (
                  <TableRow key={position.symbol}>
                    ...
                    <TableCell align="right">{(position.weight * 100).toFixed(1)}%</TableCell>
                    <TableCell align="right">${position.var.toLocaleString()}</TableCell>
                    <TableCell align="right">
                      <Chip 
                        label={position.beta.toFixed(2)} 
                        color={getRiskColor(position.beta, { low: 0.8, medium: 1.2 })}
                        size="small"
                      />
                    </TableCell>
                    <TableCell align="right">
                    ...
                    <TableCell align="right">
                      <LinearProgress 
                        ...
                        sx={{ width: 60 }}
                        color={getRiskColor(position.contribution * 100, { low: 20, medium: 30 })}
                      />
                    </TableCell>
                  </TableRow>
                ))}
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                      ...
                      <Typography sx={{ color: 'error.main', fontWeight: 600 }}>
                        ${Math.abs(test.loss).toLocaleString()} ({Math.abs(test.percentage)}%)
                      </Typography>
                    </Box>
                    <LinearProgress 
                      variant="determinate" 
        ...
        <TabPanel value={activeTab} index={3}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Button variant="contained" startIcon={<Notifications />} onClick={() => setAlertDialogOpen(true)}>
              Create Alert
            </Button>
          </Box>
