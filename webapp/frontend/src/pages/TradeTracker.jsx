      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-head-sub">
            Every action the algo takes — entries, exits, stops, pyramids,
            halts, skipped signals
          </div>
        </div>
        <button
    ...
    <div style={{ display: "flex", borderBottom: "1px solid var(--border)" }}>
      {tabs.map((t) => (
        <button
          ...
          onMouseEnter={(e) => {
            if (value !== t.value) e.currentTarget.style.color = "var(--text)";
          }}
          onMouseLeave={(e) => {
            if (value !== t.value)
...
function TradesView() {
  ...
  const openPositions = Array.isArray(positions)
    ...
    : positions?.items || [];
  const closedTrades = (
    Array.isArray(trades) ? trades : trades?.items || []
  ...
  const rows = useMemo(() => {
    ...
    if (statusFilter === "open")
      list = openPositions.map((p) => ({ ...p, _kind: "open" }));
    else if (statusFilter === "closed")
      list = closedTrades.map((t) => ({ ...t, _kind: "closed" }));
    else
      list = [
        ...openPositions.map((p) => ({ ...p, _kind: "open" })),
        ...closedTrades.map((t) => ({ ...t, _kind: "closed" })),
      ];
  ...
  const missingClosedPnL = closedTrades.filter(
    (t) => t.profit_loss_dollars == null
  );
  if (missingOpenPnL.length > 0) {
    console.warn(
      `[TradeTracker] ${missingOpenPnL.length} open positions missing unrealized_pnl`
    );
  }
  if (missingClosedPnL.length > 0) {
    console.warn(
      `[TradeTracker] ${missingClosedPnL.length} closed trades missing profit_loss_dollars`
    );
  }
  ...
  const closedPnL = closedTrades.reduce(
    (s, t) =>
      s + (t.profit_loss_dollars != null ? Number(t.profit_loss_dollars) : 0),
    0
  );
  ...
  if (pe || te) {
    return (
      <div className="alert alert-danger" style={{ margin: "16px" }}>
        {pe?.message || te?.message || "Error loading data"}
      </div>
    );
  }
    ...
    <div className="card">
      ...
      <div className="card-body">
        <div className="grid grid-4" style={{ marginBottom: "var(--space-4)" }}>
          <div className="stile">
            <div className="stile-label">Open</div>
            <div className="stile-value up">{openPositions.length}</div>
          </div>
          <div className="stile">
            <div className="stile-label">Closed</div>
            <div className="stile-value">{closedTrades.length}</div>
          </div>
          <div className="stile">
            <div className="stile-label">Open P&L</div>
            <div className={`stile-value ${openPnL >= 0 ? "up" : "down"}`}>
              {openPnL >= 0 ? "+" : ""}${num(openPnL, 0)}
            </div>
          </div>
          <div className="stile">
            <div className="stile-label">Closed P&L</div>
            <div className={`stile-value ${closedPnL >= 0 ? "up" : "down"}`}>
              {closedPnL >= 0 ? "+" : ""}${num(closedPnL, 0)}
            </div>
          </div>
        </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {rows.map((row, i) => {
                  ...
                  const k =
                    row.position_id || row.trade_id || `${row.symbol}-${i}`;
                  ...
                  const plPct = isOpen
                    ...
                    : row.profit_loss_pct;
                  const days = isOpen
                    ? row.days_since_entry
                    : row.trade_duration_days;
                  ...
                  return (
                    <React.Fragment key={k}>
                      <tr onClick={() => setExpandedKey(expanded ? null : k)}>
                        ...
                        <td>
                          <span
                            className={`badge ${isOpen ? "badge-success" : ""}`}
                          >
                            {isOpen ? "OPEN" : "CLOSED"}
                          </span>
                        </td>
                        <td className="num">
                          {fmtMoney(row.entry_price || row.avg_entry_price)}
                        </td>
                        <td className="num">{fmtMoney(px)}</td>
                        <td className="num">
                          {row.quantity || row.entry_quantity || "—"}
                        </td>
                        <td className="num">
                          <Pnl value={pl} />
                        </td>
                        <td className="num">
                          <Pnl value={plPct} suffix="%" />
                        </td>
                        <td className="num">
                          {rMult != null ? (
                            <Pnl value={rMult} suffix="R" />
                          ) : (
                            "—"
                          )}
                        </td>
  ...
  return (
    <div className="grid grid-2 gap-4">
      <div>
        ...
        <Detail
          label="Entry $"
          value={fmtMoney(row.entry_price || row.avg_entry_price)}
        />
        <Detail
          label="Quantity"
          value={row.entry_quantity || row.initial_quantity || row.quantity}
        />
        <Detail
          label="Stop"
          value={fmtMoney(row.current_stop_price || row.initial_stop)}
        />
        ...
        {isOpen ? (
          <>
            <Detail
              ...
              value={row.distribution_day_count}
            />
            <Detail label="Targets hit" value={row.target_levels_hit} />
            <Detail
              label="Trail stop"
              value={fmtMoney(row.current_stop_price)}
            />
          ...
          <>
            ...
            <Detail
              label="R multiple"
              value={
                row.exit_r_multiple != null
                  ? `${num(row.exit_r_multiple)}R`
                  : "—"
              }
            />
            <Detail
              label="MFE / MAE"
              value={`${row.mfe_pct || "—"}% / ${row.mae_pct || "—"}%`}
            />
          </>
        )}
      </div>
...
function ActivityView() {
  ...
  const {
    ...
    () =>
      api.get(
        `/api/algo/audit-log?limit=300${filter ? "&action_type=" + filter : ""}`
      ),
    { refetchInterval: 60000, retry: false }
  );

  const items = useMemo(() => {
    let rows = (Array.isArray(log) ? log : log?.items) || [];
    if (search) {
      ...
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            algo_audit_log · every decision the algo makes
          </div>
        </div>
        <div className="card-actions">
          <button className="btn btn-icon btn-ghost" onClick={refetch}>
            <RefreshCw size={14} />
          </button>
        </div>
      </div>
      <div className="card-body">
        ...
        <div className="flex gap-3" style={{ marginBottom: "var(--space-4)" }}>
          ...
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <input
              className="input"
              placeholder="Search symbol or detail"
              value={search}
        ...
        {isLoading ? (
          ...
          <Empty
            ...
            desc="The audit log is restricted to admin accounts."
          />
        ) : items.length === 0 ? (
          <Empty
            title="No activity yet"
            desc="The algo logs every decision here. Entries appear after each orchestrator run."
          />
        ) : (
...
function ActivityRow({ item }) {
  ...
  const icon = Object.entries(ACTION_ICON).find(([k]) =>
    actionKey.includes(k)
  )?.[1] || <Info size={16} className="muted" />;
    ...
    <div className="panel" style={{ padding: 0 }}>
      ...
      >
        ...
        <div className="flex-1" style={{ minWidth: 0 }}>
          ...
          <div className="t-xs muted">
            ...
            {failed && (
              <span className="down" style={{ marginLeft: 8 }}>
                FAILED
              </span>
            )}
          </div>
      ...
      {open && (
        ...
        >
          {item.error && (
            <div
              className="alert alert-danger t-xs"
  ...
  if (notifError) {
    if (notifError?.status === 403 || notifError?.httpStatus === 403) {
      return (
        <Empty
          title="Admin access required"
          desc="Notifications are restricted to admin accounts."
        />
      );
    }
    return (
      <div className="alert alert-danger" style={{ margin: "16px" }}>
        {notifError?.message || "Error loading notifications"}
      </div>
    );
  }
    ...
    <div className="card">
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Circuit-breaker fires, sector rotation, exposure transitions
          </div>
        </div>
        <div className="card-actions">
          <button className="btn btn-icon btn-ghost" onClick={refetch}>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-2">
            {items.map((n) => (
              <div
                key={n.id}
                className={`alert ${n.severity === "error" || n.severity === "critical" ? "alert-danger" : n.severity === "warn" ? "alert-warn" : "alert-info"}`}
              >
                <Info size={16} />
                <div style={{ flex: 1 }}>
                  <div className="flex items-center gap-2">
                    <span
                      className={`badge badge-${n.severity === "error" || n.severity === "critical" ? "danger" : n.severity === "warn" ? "amber" : "brand"}`}
                    >
