...
const filterByRange = (rows, range) => {
  ...
  const out = rows.filter((r) => {
    try {
      ...
      console.debug(
        "[SectorAnalysis] Failed to parse date for filtering:",
        err?.message
      );
      return true;
    }
...
const TrendIcon = ({ trend, size = 16 }) => {
  ...
  if (t.includes("up"))
    return <TrendingUp size={size} style={{ color: "var(--success)" }} />;
  if (t.includes("down"))
    return <TrendingDown size={size} style={{ color: "var(--danger)" }} />;
...
function MansfieldRotation({ sectors }) {
  const data = useMemo(() => {
    ...
    const N =
      sectors.filter((s) => s.current_rank != null).length || sectors.length;
    return sectors
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 16, right: 24, bottom: 32, left: 0 }}>
          ...
          <XAxis
            ...
            fontSize={11}
            label={{
              value: "RS Rank Percentile (higher = stronger)",
          ...
          <YAxis
            ...
            width={48}
            label={{
              value: "1W rank change (+ = improving)",
              angle: -90,
              position: "insideLeft",
              fill: "var(--text-3)",
              fontSize: 11,
...
function SectorRelativeChart({ sectors }) {
  ...
  const {
    ...
    async () => {
      ...
      const batchData = await api
        .get(`/api/sectors/trends-batch?sectors=${sectorsList}&days=90`)
        .then((r) => {
          const result = r.data;
      ...
      return Promise.resolve(
        Object.values(all).sort((a, b) => a.date.localeCompare(b.date))
      );
    },
    {
  ...
  const merged = Array.isArray(responseData)
    ? responseData
    : responseData?.items || [];

  if (isLoading) return <Empty title="Loading…" />;
  if (error)
    return <Empty title={`Error loading sector trends: ${error?.message}`} />;
...
function SectorBreadthChart() {
  ...
  if (error)
    return <Empty title={`Error loading breadth: ${error?.message}`} />;
  const breadthData = (Array.isArray(data) ? data : data?.items) || [];
  if (!breadthData || breadthData.length === 0)
    return <Empty title="No breadth data" />;

  const sorted = [...breadthData].sort(
    (a, b) => b.pct_above_200d - a.pct_above_200d
  );

  return (
    <div
      className="chart-container"
      style={{ height: Math.max(300, sorted.length * 28 + 40) }}
    >
      <ResponsiveContainer width="100%" height="100%">
        <BarChart
          data={sorted}
        ...
        >
          ...
          <Legend
            ...
            formatter={(v) =>
              v === "pct_above_50d" ? "% > 50D MA" : "% > 200D MA"
            }
          />
          <Bar
            dataKey="pct_above_50d"
            fill="var(--cyan)"
            radius={[0, 2, 2, 0]}
          />
          <Bar
            dataKey="pct_above_200d"
            fill="var(--brand)"
            radius={[0, 2, 2, 0]}
          />
        </BarChart>
...
function Stage2LeadersChart() {
  const {
    ...
    loading: isLoading,
    error,
  } = useApiQuery(["sector-stage2"], () => api.get("/api/algo/sector-stage2"), {
    refetchInterval: 60000,
  });

  if (isLoading) return <Empty title="Loading…" />;
  if (error)
    return <Empty title={`Error loading stage-2 data: ${error?.message}`} />;
  const dataItems = Array.isArray(data) ? data : data?.items || [];
  if (!dataItems || dataItems.length === 0) {
    return (
      <Empty
        title="No stage data"
        desc="Requires trend_template_data coverage."
      />
    );
  }
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(v, n, p) => {
              if (n === "pct_stage_2") {
                return [
                  `${v}% (${p.payload.stage_2}/${p.payload.total} stocks)`,
                  "Stage-2",
                ];
              }
              return [v, n];
...
function DefensiveCyclicalChart() {
  ...
  if (isLoading) return <Empty title="Loading…" />;
  if (error)
    return <Empty title={`Error loading rotation data: ${error?.message}`} />;
  if (!rotationData || rotationData.length === 0) {
    return (
      <Empty
        title="Sector rotation signal not yet computed"
...
function DailyStrengthChart({ name, type, range }) {
  ...
  const {
    ...
    loading: isLoading,
    error,
  } = useApiQuery([`${type}-strength`, name], () => api.get(endpoint), {
    enabled: !!name,
    staleTime: 1000 * 60 * 10,
  });
  ...
  if (isLoading) return <Empty title="Loading…" />;
  if (error)
    return <Empty title={`Error loading ${type} data: ${error?.message}`} />;
  ...
  const raw = trendArr.map((d) => ({
    ...d,
    dailyStrengthScore:
      d.dailyStrengthScore != null
        ? Number(d.dailyStrengthScore)
        : firstAvgPrice
          ? (Number(d.avgPrice) / firstAvgPrice - 1) * 100
          : null,
  }));
  ...
  const aboveMA20 =
    last.price != null && last.ma_20 != null && last.price > last.ma_20;
  const aboveMA10 =
    last.price != null && last.ma_10 != null && last.price > last.ma_10;
  const priceColor = aboveMA20
    ? "var(--success)"
    : aboveMA10
      ? "var(--amber)"
      : "var(--danger)";
...
function RankingTrendChart({ name, type, range }) {
  ...
  const { data: resp, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        ...
        .catch((err) => {
          console.error(
            `[RankingTrendChart] Failed to fetch ${type} trend:`,
            err?.message || err
          );
          throw err;
        }),
  ...
  let history = (resp?.trendData || []).map((r) => ({
    ...
    rank: r.rank ?? null,
    momentum:
      r.dailyStrengthScore != null
        ? Number(r.dailyStrengthScore)
        : r.momentumScore != null
          ? Number(r.momentumScore)
          : r.momentum != null
            ? Number(r.momentum)
            : r.avgPrice != null
              ? Number(r.avgPrice)
...
function SparklineTrend({ name, type }) {
  ...
  const rows = (resp?.trendData || []).filter(
    (r) => r.dailyStrengthScore != null
  );
  ...
  return (
    ...
    >
      <div className="grid grid-4">
        ...
        <Stile
          label="Momentum"
          value={sector.current_momentum || sector.momentum || "—"}
        />
        <Stile
          label="Trend"
          value={sector.current_trend || sector.trend || "—"}
        />
        <Stile
          label="Trailing P/E"
          value={
            sector.pe?.trailing != null ? sector.pe.trailing.toFixed(2) : "—"
          }
        />
      </div>
      ...
      <div className="grid grid-3" style={{ marginTop: "var(--space-4)" }}>
        <Stile
          label="1W Ago"
          value={sector.rank_1w_ago != null ? `#${sector.rank_1w_ago}` : "—"}
        />
        <Stile
          label="4W Ago"
          value={sector.rank_4w_ago != null ? `#${sector.rank_4w_ago}` : "—"}
        />
        <Stile
          label="12W Ago"
          value={sector.rank_12w_ago != null ? `#${sector.rank_12w_ago}` : "—"}
        />
      </div>
        ...
        <div className="card-head">
          <div>
            ...
            <div className="card-sub">
              Price proxy with MA10 / MA20 (color: above MA20 = green)
            </div>
          </div>
        </div>
        <div className="card-body">
      ...
      <div className="card" style={{ marginTop: "var(--space-4)" }}>
        <div className="card-head">
          <div>
            ...
            <div className="card-sub">
              Sorted by RS rank · {matched.length} total
            </div>
          </div>
        </div>
        <div className="card-body">
              ...
              <tbody>
                {top10.map((ind) => (
                  <tr key={ind.industry}>
                    ...
                    <td className="num">
                      <span
                        className={`mono tnum ${pctClass(ind.performance_1d)}`}
                      >
                    ...
                    <td className="num">
                      <span
                        className={`mono tnum ${pctClass(ind.performance_5d)}`}
                      >
                    ...
                    <td className="num">
                      <span
                        className={`mono tnum ${pctClass(ind.performance_20d)}`}
                      >
    ...
    >
      ...
      <div className="grid grid-3" style={{ marginTop: "var(--space-4)" }}>
        <Stile
          label="1W Ago"
          value={
            industry.rank_1w_ago != null ? `#${industry.rank_1w_ago}` : "—"
          }
        />
        <Stile
          label="4W Ago"
          value={
            industry.rank_4w_ago != null ? `#${industry.rank_4w_ago}` : "—"
          }
        />
        <Stile
          label="12W Ago"
          value={
            industry.rank_12w_ago != null ? `#${industry.rank_12w_ago}` : "—"
          }
        />
      </div>
...
function SectorsView({ sectors, industries, isLoading, error }) {
  ...
  const performanceVal = (s, range) => {
    switch (range) {
      case "5d":
        return s.current_perf_5d ?? s.performance_5d;
      case "20d":
        return s.current_perf_20d ?? s.performance_20d;
      case "ytd":
        return (
          s.current_perf_ytd ??
          s.performance_ytd ??
      ...
      default:
        return s.current_perf_1d ?? s.performance_1d;
    }
  };
        ...
        <div className="card">
          <div className="card-head">
            <div>
              ...
              <div className="card-sub">
                Quadrant scatter · x: RS-rank %ile · y: 1-week rank Δ · size:
                stocks
              </div>
            </div>
          </div>
        ...
        <div className="card">
          <div className="card-head">
            <div>
              ...
              <div className="card-sub">
                Spider radar · 1D / 5D / 20D returns per top-ranked sector
              </div>
            </div>
          </div>
          <div className="card-body">
      ...
      <div className="card" style={{ marginBottom: "var(--space-4)" }}>
        <div className="card-head">
          <div>
            ...
            <div className="card-sub">
              All series indexed to 100 at start · top-8 ranked sectors
            </div>
          </div>
        </div>
        <div className="card-body">
        ...
        <div className="card">
          <div className="card-head">
            <div>
              ...
              <div className="card-sub">
                % of constituent stocks above 50D / 200D MA
              </div>
            </div>
          </div>
          <div className="card-body">
        ...
        <div className="card">
          <div className="card-head">
            <div>
              ...
              <div className="card-sub">
                % of stocks in Stage-2 (markup) phase
              </div>
            </div>
          </div>
          <div className="card-body">
      ...
      <div className="card" style={{ marginBottom: "var(--space-4)" }}>
        <div className="card-head">
          <div>
            ...
            <div className="card-sub">
              Sector rotation signal · risk-off when defensive lead persists
            </div>
          </div>
        </div>
        <div className="card-body">
      ...
      <div className="card" style={{ marginTop: "var(--space-4)" }}>
        <div className="card-head">
          <div>
            ...
            <div className="card-sub">
              Click a row to drill into trends, daily strength, top industries
            </div>
          </div>
        </div>
        <div className="card-body" style={{ padding: 0 }}>
          <ErrorBoundary>
            {error ? (
              ...
              >
                <AlertCircle size={16} />
                <div>Sector data not available.</div>
              </div>
            ) : isLoading ? (
                          ...
                          <tr onClick={() => setExpanded(isOpen ? null : key)}>
                            ...
                            <td className="num">
                              <span className="badge badge-brand mono tnum">
                                #
                                <SafeMetricValue
                                  value={s.current_rank || s.overall_rank}
                                  fallback="—"
                                />
                              </span>
                            </td>
                            ...
                            <td>
                              ...
                              >
                                {
                                  <SafeMetricValue
                                    value={s.current_momentum || s.momentum}
                                    fallback="—"
                                  />
                                }
                              </span>
                            ...
                            <td>
                              <TrendIcon trend={s.current_trend || s.trend} />
                            </td>
                            <td className="num">
                              <span className={`mono tnum ${pctClass(p1)}`}>
                                {fmtPct(p1)}
                              </span>
                            </td>
                            <td className="num">
                              <span className={`mono tnum ${pctClass(p5)}`}>
                                {fmtPct(p5)}
                              </span>
                            </td>
                            <td className="num">
                              <span className={`mono tnum ${pctClass(p20)}`}>
                                {fmtPct(p20)}
                              </span>
                            </td>
                            <td>
                              <SparklineTrend name={name} type="sector" />
                            </td>
                          </tr>
                          {isOpen && (
                            <tr>
    ...
    <div className="card">
      <div className="card-head">
        <div>
          ...
          <div className="card-sub">
            Click a row to drill into trends and top companies
          </div>
        </div>
        <input
          className="input"
                  ...
                  return (
                    <React.Fragment key={key}>
                      <tr onClick={() => setExpanded(isOpen ? null : key)}>
                        ...
                        <td>
                          <TrendIcon trend={ind.current_trend} />
                        </td>
                        <td className="num">
                          <span
                            className={`mono tnum ${pctClass(ind.performance_1d)}`}
...
export default function SectorAnalysis() {
  ...
  const sectors = sectorsQ.items || [];
  const industries = industriesQ.items || [];
    ...
    <div style={{ display: "flex", borderBottom: "1px solid var(--border)" }}>
      {tabs.map((t) => (
        ...
        >
          {t.label}
          {t.count != null && (
            <span className="badge mono tnum" style={{ marginLeft: 8 }}>
              {t.count}
            </span>
          )}
        </button>
