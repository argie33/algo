...
import {
  ...
  Paper, IconButton, Button, TextField, Chip, Alert,
  LinearProgress, Dialog, DialogTitle, DialogContent, DialogActions
} from '@mui/material';
import {
  Add, Delete, Refresh, TrendingUp, TrendingDown,
...
const Watchlist = () => {
  ...
  const loadWatchlist = async () => {
    try {
      ...
      const defaultWatchlist = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'NVDA', 'META'];
    ...
    try {
      ...
      if (response.data.success) {
        ...
        for (const symbol of watchlist) {
          ...
          if (symbolData && !symbolData.error) {
            ...
            transformedData[symbol] = {
              ...
              spreadPercent: ((symbolData.askPrice - symbolData.bidPrice) / midPrice) * 100,
        ...
        console.log('✅ Watchlist market data loaded successfully', {
          ...
          successful: Object.values(transformedData).filter(d => !d.error).length,
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box display="flex" alignItems="center" justifyContent="between" mb={4}>
        <Box>
          ...
          <Box display="flex" gap={1} mt={1}>
            <Chip label={`${watchlist.length} symbols`} color="primary" size="small" />
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 3 }}>
          <Typography variant="h6" gutterBottom>Failed to Load Watchlist Data</Typography>
          <Typography variant="body2">{error}</Typography>
          <Button variant="outlined" size="small" onClick={loadMarketData} sx={{ mt: 1 }}>
            Retry
          </Button>
        </Alert>
      )}
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="right"><strong>Change %</strong></TableCell>
                    <TableCell align="right"><strong>Bid/Ask</strong></TableCell>
                          ...
                          {hasError ? (
                            ...
                            <Box>
                              <Typography variant="body2">
                                {formatCurrency(data.bidPrice)} / {formatCurrency(data.askPrice)}
                              </Typography>
                            </Box>
                          )}
                        </TableCell>
