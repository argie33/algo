...
const AdvancedScreener = () => {
  ...
  const filteredAndSortedResults = useMemo(() => {
    let filtered = mockResults.filter((stock) => {
      ...
      return (
        stock.scores.quality >= screenCriteria.quality[0] &&
        stock.scores.quality <= screenCriteria.quality[1] &&
        stock.scores.growth >= screenCriteria.growth[0] &&
        stock.scores.growth <= screenCriteria.growth[1] &&
        stock.scores.value >= screenCriteria.value[0] &&
        stock.scores.value <= screenCriteria.value[1] &&
        stock.scores.momentum >= screenCriteria.momentum[0] &&
        stock.scores.momentum <= screenCriteria.momentum[1] &&
        stock.scores.sentiment >= screenCriteria.sentiment[0] &&
        stock.scores.sentiment <= screenCriteria.sentiment[1] &&
        stock.scores.positioning >= screenCriteria.positioning[0] &&
        stock.scores.positioning <= screenCriteria.positioning[1] &&
        (screenCriteria.sector === "any" ||
          stock.sector === screenCriteria.sector) &&
        (screenCriteria.exchange === "any" ||
          stock.exchange === screenCriteria.exchange) &&
        stock.pe >= screenCriteria.pe[0] &&
        stock.pe <= screenCriteria.pe[1] &&
        stock.pb >= screenCriteria.pb[0] &&
        stock.pb <= screenCriteria.pb[1] &&
        stock.roe >= screenCriteria.roe[0] &&
        stock.roe <= screenCriteria.roe[1] &&
        stock.dividendYield >= screenCriteria.dividendYield[0] &&
        stock.dividendYield <= screenCriteria.dividendYield[1] &&
        stock.price >= screenCriteria.price[0] &&
        stock.price <= screenCriteria.price[1] &&
        stock.beta >= screenCriteria.beta[0] &&
        stock.beta <= screenCriteria.beta[1] &&
        stock.esgScore >= screenCriteria.esgScore[0] &&
        stock.esgScore <= screenCriteria.esgScore[1]
      );
    });
  ...
  const loadUserScreens = async () => {
    try {
      ...
      const localScreens = JSON.parse(
        localStorage.getItem("screener_screens") || "[]"
      );
  ...
  const loadUserWatchlists = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/screener/watchlists`, {
        headers,
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const runScreen = async () => {
    ...
    try {
      ...
      if (positioning[0] > 0)
        params.append("minPositioning", positioning[0] / 100);
      if (positioning[1] < 100)
        params.append("maxPositioning", positioning[1] / 100);
      ...
      if (dividendYield[0] > 0)
        params.append("minDividendYield", dividendYield[0] / 100);
      if (dividendYield[1] < 20)
        params.append("maxDividendYield", dividendYield[1] / 100);
      ...
      if (response.success && response.data) {
        ...
        const transformedResults = stocks.map((stock) => ({
          symbol: stock.symbol,
          company:
            stock.company_name || stock.security_name || `${stock.symbol} Corp`,
          scores: {
      ...
      <Box display="flex" alignItems="center" justifyContent="between" mb={3}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            Professional-grade stock screening with multi-factor analysis and
            real-time scoring
          </Typography>
              ...
              <Box display="flex" alignItems="center" gap={4}>
                <Box>
                  ...
                  <Typography variant="body2" sx={{ opacity: 0.8 }}>
                    ...
                    {(
                      filteredAndSortedResults.reduce(
                        (sum, stock) => sum + stock.scores.composite,
                        0
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      Dividend Yield: {screenCriteria.dividendYield[0]}% -{" "}
                      {screenCriteria.dividendYield[1]}%
                    </Typography>
                    <Slider
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <Slider
                      ...
                      onChange={(e, newValue) =>
                        setScreenCriteria((prev) => ({ ...prev, pe: newValue }))
                      }
                      valueLabelDisplay="auto"
                      min={0}
                      max={50}
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <Slider
                      ...
                      onChange={(e, newValue) =>
                        setScreenCriteria((prev) => ({ ...prev, pb: newValue }))
                      }
                      valueLabelDisplay="auto"
                      min={0}
                      max={10}
              ...
              <Card>
                <CardContent>
                  ...
                  <Typography
                    ...
                    gutterBottom
                  >
                    Quality: {screen.criteria.quality[0]}-
                    {screen.criteria.quality[1]}
                  </Typography>
                  <Typography
                    variant="body2"
                    color="text.secondary"
                    gutterBottom
                  >
                    Growth: {screen.criteria.growth[0]}-
                    {screen.criteria.growth[1]}
                  </Typography>
                  <Button
                    variant="outlined"
                    size="small"
                        ...
                        <CardHeader
                          ...
                          avatar={
                            <Box
                              sx={{
                                bgcolor:
                                  getScoreColor(stock.scores.composite) +
                                  ".main",
                                color: "white",
                                borderRadius: "50%",
                                width: 40,
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            <Typography variant="body1" gutterBottom>
              You're about to add {selectedStocks.length} stock
              {selectedStocks.length > 1 ? "s" : ""} to your portfolio:
            </Typography>
                ...
                return (
                  ...
                  >
                    <Box
                      sx={{
                        bgcolor:
                          getScoreColor(stock.scores.composite) + ".main",
                        color: "white",
                        borderRadius: "50%",
                        width: 32,
                        height: 32,
            ...
            <Alert severity="info" sx={{ mt: 2 }}>
              {isAuthenticated ? (
                <Box>
                  <Typography variant="body2">
                    These stocks will be added to your portfolio for tracking.
                    You can manage position sizes and set alerts from your
                    Portfolio page.
                  </Typography>
                ...
                <Box>
                  <Typography variant="body2">
                    You're not signed in. These stocks will be saved locally and
                    won't sync across devices.
                  </Typography>
                  <Typography variant="body2" sx={{ mt: 1 }}>
                    <strong>Tip:</strong> Sign in to sync your portfolio across
                    all devices.
                  </Typography>
                </Box>
              )}
            ...
            {isAuthenticated && (
              <Box sx={{ mt: 2 }}>
                <FormControl fullWidth variant="outlined">
                  ...
                  <Select value="main" label="Portfolio">
