...
async function testAuthFlow() {
  ...
  try {
    ...
    const signUpResult = await devAuth.signUp('testuser', 'password123', 'test@example.com', 'Test', 'User');
    ...
    if (verificationCode) {
      ...
      const confirmResult = await devAuth.confirmSignUp('testuser', verificationCode);
