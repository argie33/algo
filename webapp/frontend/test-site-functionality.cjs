...
class SiteFunctionalityTester {
          ...
          try {
            ...
            if (res.statusCode === expectedStatus) {
              ...
              this.testResults.errors.push(
                `${endpoint} returned ${res.statusCode}, expected ${expectedStatus}`
              );
              this.log(
                `❌ ${endpoint} - Status ${res.statusCode}, expected ${expectedStatus}`,
                "error"
              );
              resolve(result);
            }
            ...
            this.testResults.failed++;
            this.testResults.errors.push(
              `${endpoint} - JSON parse error: ${error.message}`
            );
  ...
  async testConfigurationFiles() {
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (
        configContent.includes("qda42av7je.execute-api.us-east-1.amazonaws.com")
      ) {
      ...
      if (configContent.includes("1rn05nvf53cvmc0dsvbbkl3ng1")) {
        ...
        this.log(
          "❌ Config does not contain correct Cognito Client ID",
          "error"
        );
        this.testResults.failed++;
        this.testResults.errors.push(
          "Config file missing correct Cognito Client ID"
        );
      }
    } else {
  ...
  async testDatabaseConnectivity() {
    ...
    if (healthResult.data && healthResult.data.database) {
      if (healthResult.data.database.status === "connected") {
        ...
        if (
          healthResult.data.database.tables &&
          Object.keys(healthResult.data.database.tables).length > 0
        ) {
          this.log(
            `✅ Database has ${Object.keys(healthResult.data.database.tables).length} tables with data`,
            "success"
          );
          this.testResults.passed++;
        } else {
  ...
  async runAllTests() {
    ...
    try {
      ...
      if (this.testResults.failed === 0) {
        this.log(
          "🎉 All tests passed! Site is functioning correctly.",
          "success"
        );
        process.exit(0);
      } else {
        this.log(
          `⚠️ ${this.testResults.failed} tests failed. Issues need attention.`,
          "error"
        );
        process.exit(1);
      }
