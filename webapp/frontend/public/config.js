...
window.__CONFIG__ = {
  ...
  COGNITO: {
    USER_POOL_ID: window.__CLOUDFORMATION_CONFIG__?.UserPoolId || 'us-east-1_BbUwXRwKR', // Working development pool
    CLIENT_ID: window.__CLOUDFORMATION_CONFIG__?.UserPoolClientId || '7smspn7gup3v0t8p3e2q7fh88m', // Working development client
    REGION: 'us-east-1',
...
if (window.__CONFIG__.ENVIRONMENT === 'development') {
  ...
  // Use development Cognito pool if available, otherwise use working values
  if (!window.__CONFIG__.COGNITO.USER_POOL_ID) {
    console.warn('⚠️ No Cognito configuration found for development - using working values');
    // Set working Cognito values for development
    window.__CONFIG__.COGNITO.USER_POOL_ID = 'us-east-1_BbUwXRwKR';
    window.__CONFIG__.COGNITO.CLIENT_ID = '7smspn7gup3v0t8p3e2q7fh88m';
  }
}
