...
class AlertSystem extends EventEmitter {
  ...
  startMonitoring(liveDataManager) {
    ...
    this.liveDataManager.on('errorRecorded', (_data) => {
      this.checkErrorRates(_data.providerId);
    });
  ...
  async performHealthCheck() {
    try {
      ...
      // Check each provider
      for (const [providerId, provider] of Object.entries(dashboardStatus.providers || {})) {
      ...
      this.createAlert('system', 'error', 'Health Check Failed', `Health monitoring system error: ${error.message}`);
    }
  }
  ...
  async checkProviderHealth(providerId, provider) {
    ...
    if (provider.latency > this.config.thresholds.latency.critical) {
      this.createAlert(alertKey, 'critical', 'High Latency Critical', 
      ...
      this.createAlert(alertKey, 'warning', 'High Latency Warning', 
    ...
    if (errorRate > this.config.thresholds.errorRate.critical) {
      this.createAlert(`${alertKey}-errors`, 'critical', 'High Error Rate Critical', 
      ...
      this.createAlert(`${alertKey}-errors`, 'warning', 'High Error Rate Warning', 
    ...
    if (provider.status === 'disconnected' || provider.status === 'error') {
      this.createAlert(`${alertKey}-status`, 'critical', 'Provider Disconnected', 
  ...
  async checkGlobalHealth(global) {
    ...
    if (global.dailyCost > this.config.thresholds.costDaily.critical) {
      this.createAlert('global-cost', 'critical', 'Daily Cost Critical', 
      ...
      this.createAlert('global-cost', 'warning', 'Daily Cost Warning', 
    ...
    if (global.totalConnections >= this.config.thresholds.connectionCount.critical) {
      this.createAlert('global-connections', 'critical', 'Connection Limit Critical', 
    ...
    } else if (global.totalConnections >= this.config.thresholds.connectionCount.warning) {
      this.createAlert('global-connections', 'warning', 'Connection Limit Warning', 
  ...
  async checkResourceLimits(limits) {
    ...
    if (limits.connections?.usage > 90) {
      this.createAlert('limits-connections', 'critical', 'Connection Limit Near Max', 
      ...
      this.createAlert('limits-connections', 'warning', 'Connection Usage High', 
        `Connection usage: ${limits.connections.usage.toFixed(1)}% (${limits.connections.current}/${limits.connections.max})`, {
          usage: limits.connections.usage,
          current: limits.connections.current,
          max: limits.connections.max
    ...
    if (limits.cost?.usage > 90) {
      this.createAlert('limits-cost', 'critical', 'Cost Budget Near Max', 
      ...
      this.createAlert('limits-cost', 'warning', 'Cost Usage High', 
        `Cost usage: ${limits.cost.usage.toFixed(1)}% ($${limits.cost.current.toFixed(2)}/$${limits.cost.max})`, {
          usage: limits.cost.usage,
          current: limits.cost.current,
          max: limits.cost.max
...
Metadata: ${JSON.stringify(alert.metadata, null, 2)}
      ...
      */
      ...
      this.emit('notificationSent', { type: 'email', alert, success: false, error });
    }
  }
  ...
  async sendSlackNotification(alert) {
    try {
      ...
      const slackMessage = {
        ...
        attachments: [{
          ...
          fields: [
            ...
            { title: 'Time', value: new Date(alert.createdAt).toLocaleString(), short: false }
          ],
      ...
      */
      ...
      this.emit('notificationSent', { type: 'slack', alert, success: false, error });
    }
  }
  ...
  async sendWebhookNotification(alert) {
    try {
      ...
      */
      ...
      console.log(`🔗 Webhook notification: [${alert.severity}] ${alert.title}`);
      
      this.emit('notificationSent', { type: 'webhook', alert, success: true });
      
    } catch (error) {
      console.error('Webhook notification failed:', error);
      this.emit('notificationSent', { type: 'webhook', alert, success: false, error });
    }
  }
