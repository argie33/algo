...
async function getFinancialStatement(ticker, type, _period) {
  let tableName = type;
  if (_period === 'quarterly' && type !== 'balance_sheet') {
    tableName = `quarterly_${type}`;
  } else if (_period === 'ttm' && type !== 'balance_sheet') {
    tableName = `ttm_${type}`;
  } else if (_period === 'quarterly' && type === 'balance_sheet') {
    tableName = 'quarterly_balance_sheet';
  }
