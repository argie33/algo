...
router.post('/preview', async (req, res) => {
  const userId = req.user.sub;
  const { symbol, side, quantity, orderType, limitPrice, stopPrice, extendedHours } = req.body;
...
router.post('/', async (req, res) => {
  const userId = req.user.sub;
  const { 
    symbol, quantity, limitPrice, stopPrice 
    /*, notes */
  } = req.body;
  ...
  try {
    ...
    // Store order in database (commented out for now)
