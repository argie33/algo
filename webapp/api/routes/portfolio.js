  ...
  try {
    ...
    if (performance.length > 0) {
      ...
      metrics = {
        ...
        annualizedReturn: parseFloat(latest.total_pnl_percent || 0), // Using total return as fallback
...
async function importFromAlpaca(_apiKey, _apiSecret, _sandbox) {
  try {
    const AlpacaService = require('../utils/alpacaService');
    const alpaca = new AlpacaService(_apiKey, _apiSecret, _sandbox);
...
async function importFromTDAmeritrade(_apiKey, _apiSecret, _sandbox) {
  try {
...
async function syncAlpacaPortfolio(userId, _apiKey, _apiSecret, _sandbox) {
  try {
    const AlpacaService = require('../utils/alpacaService');
    const alpaca = new AlpacaService(_apiKey, _apiSecret, _sandbox);
...
async function getAlpacaTransactions(_apiKey, _apiSecret, _sandbox, options = {}) {
  try {
    const AlpacaService = require('../utils/alpacaService');
    const alpaca = new AlpacaService(_apiKey, _apiSecret, _sandbox);
...
async function getAlpacaRealTimeValuation(userId, _apiKey, _apiSecret, _sandbox) {
  try {
    const AlpacaService = require('../utils/alpacaService');
    const alpaca = new AlpacaService(_apiKey, _apiSecret, _sandbox);
