...
try:
    ...
    logger.info(f"Circuit breaker threshold: {loader._rate_limit_circuit_break_threshold}s")
    ...
    if result.get('circuit_breaker_triggered'):
        print(f"  [WARN] Circuit breaker triggered (this would be a problem for full load)")
    else:
        print(f"  [OK] No circuit breaker trigger")
