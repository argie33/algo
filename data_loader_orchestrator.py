...
class DataLoaderOrchestrator:
    ...
    def __init__(self, config_file: str = None, dry_run: bool = False):
        ...
        self.logger.info(f"Data Loader Orchestrator initialized with {len(self.loaders)} loaders")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            format=log_format,
            handlers=[
                logging.FileHandler(os.path.join(log_dir, f'data_loader_{datetime.now().strftime("%Y%m%d")}.log')),
    ...
    def run_single_loader(self, loader_config: LoaderConfig) -> LoaderResult:
        ...
        try:
            ...
            script_path = os.path.join(os.path.dirname(__file__), loader_config.script_file)
            ...
            try:
                stdout, stderr = process.communicate(timeout=loader_config.timeout_seconds)
                ...
                if return_code == 0:
                    ...
                    for line in stdout.split('\n'):
                        if 'records processed' in line.lower() or 'inserted' in line.lower():
                            try:
                                import re
                    ...
                    self.logger.info(f"Loader {loader_config.name} completed successfully in {result.duration_seconds:.1f}s")
                    
                else:
                    result.error_message = f"Process exited with code {return_code}: {stderr}"
                    self.logger.error(f"Loader {loader_config.name} failed: {result.error_message}")
                
                # Log any warnings from stderr
                if stderr and result.success:
                    result.warnings = [line.strip() for line in stderr.split('\n') if line.strip()]
                ...
                result.error_message = f"Loader timed out after {loader_config.timeout_seconds} seconds"
    ...
    def run_loader_with_retries(self, loader_config: LoaderConfig) -> LoaderResult:
        ...
        for attempt in range(loader_config.retry_count + 1):
            if self.shutdown_requested:
                self.logger.warning(f"Shutdown requested, skipping {loader_config.name}")
                break
                
            if attempt > 0:
                self.logger.info(f"Retrying {loader_config.name} (attempt {attempt + 1}/{loader_config.retry_count + 1})")
            ...
            self.logger.warning(f"Attempt {attempt + 1} failed for {loader_config.name}: {result.error_message}")
    ...
    def run_loaders_by_priority(self, loaders_to_run: List[str] = None, max_parallel: int = 3) -> List[LoaderResult]:
        """
        ...
        filtered_loaders = {k: v for k, v in self.loaders.items() if k in loaders_to_run}
        ...
        for priority in ['critical', 'high', 'medium', 'low']:
            ...
            self.logger.info(f"Running {priority} priority loaders: {priority_groups[priority]}")
            ...
            with ThreadPoolExecutor(max_workers=max_parallel) as executor:
                ...
                for loader_name in priority_groups[priority]:
                    ...
                    future = executor.submit(self.run_loader_with_retries, filtered_loaders[loader_name])
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if not result.success and filtered_loaders[loader_name].priority == 'critical':
                            self.logger.error(f"CRITICAL loader {loader_name} failed - this may affect dependent loaders")
                            
                    except Exception as e:
                        self.logger.error(f"Error getting result for {loader_name}: {e}")
                        # Create error result
            ...
            if priority == 'critical':
                critical_failures = [r for r in all_results if not r.success and r.name in priority_groups['critical']]
                if critical_failures:
                    self.logger.error(f"Critical failures detected: {[r.name for r in critical_failures]}")
    ...
    def generate_report(self, results: List[LoaderResult]) -> Dict:
        ...
        report = {
            ...
            'summary': {
                ...
                'failed': failed,
                'success_rate': (successful / total_loaders * 100) if total_loaders > 0 else 0,
            ...
            'performance': {
                'fastest_loader': min(results, key=lambda x: x.duration_seconds).name if results else None,
                'slowest_loader': max(results, key=lambda x: x.duration_seconds).name if results else None,
                'avg_duration_seconds': total_duration / total_loaders if total_loaders > 0 else 0
            }
        }
...
"""
                ...
                subprocess.run([sys.executable, '-c', update_script], 
                             cwd=os.path.dirname(__file__), 
                             capture_output=True, 
                             timeout=30)
    ...
    def run_all_loaders(self, loaders_to_run: List[str] = None, max_parallel: int = 3) -> Dict:
        """
        ...
        try:
            ...
            self.logger.info(f"Total: {summary['total_loaders']}, Success: {summary['successful']}, Failed: {summary['failed']}")
            self.logger.info(f"Duration: {summary['total_duration_minutes']:.1f} minutes")
            ...
            self.is_running = False
            end_time = datetime.now()
            self.logger.info(f"Total execution time: {(end_time - start_time).total_seconds():.1f} seconds")
...
def main():
    ...
    parser.add_argument('--loaders', type=str, help='Comma-separated list of loaders to run')
    parser.add_argument('--config', type=str, help='Configuration file path')
    parser.add_argument('--dry-run', action='store_true', help='Dry run mode')
    parser.add_argument('--parallel', type=int, default=3, help='Maximum parallel executions')
    ...
    if args.priority:
        if loaders_to_run is None:
            loaders_to_run = []
        priority_loaders = [name for name, config in orchestrator.loaders.items() 
