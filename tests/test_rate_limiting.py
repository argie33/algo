...
def test_admin_rate_limit_basic():
    ...
    for i in range(max_requests):
        allowed, msg = check_admin_rate_limit(
...
def test_admin_rate_limits_configured():
    ...
    for endpoint in critical_endpoints:
        assert (
            endpoint in ADMIN_RATE_LIMITS
        ), f"Endpoint {endpoint} missing from ADMIN_RATE_LIMITS"
...
def test_public_rate_limits_configured():
    ...
    for endpoint in critical_endpoints:
        assert (
            endpoint in PUBLIC_RATE_LIMITS
        ), f"Endpoint {endpoint} missing from PUBLIC_RATE_LIMITS"
...
def test_different_users_independent():
    ...
    for i in range(3):
        allowed, msg = check_admin_rate_limit(
            "user_1", endpoint, max_requests=3, window_seconds=60
        )
        assert allowed is True

    # User 2: Should not be rate limited (independent bucket)
    allowed, msg = check_admin_rate_limit(
        "user_2", endpoint, max_requests=3, window_seconds=60
    )
