...
def test_schema_healing() -> None:
    ...
    with DatabaseContext("write") as cur:
        ...
        assert all_exist, "Schema healing failed"
        if created:
...
def test_loader_metadata() -> None:
    ...
    for name, loader in loaders:
        ...
        assert hasattr(loader, "table_name"), f"{name} missing table_name"
        assert hasattr(loader, "primary_key"), f"{name} missing primary_key"
        assert hasattr(loader, "watermark_field"), f"{name} missing watermark_field"

        logger.info(f"✓ {name}: table={loader.table_name}, primary_key={loader.primary_key}")


def test_aws_readiness() -> None:
