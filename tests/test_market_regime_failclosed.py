...
def test_market_regime_no_data_failclosed():
    ...
    assert not regime["is_entry_allowed"], \
        f"Expected is_entry_allowed=False, got {regime['is_entry_allowed']}"
    assert regime["exposure_pct"] == 0, \
        f"Expected exposure_pct=0, got {regime['exposure_pct']}"
    assert regime["regime"] == "unknown", \
        f"Expected regime='unknown', got {regime['regime']}"
    assert len(regime["halt_reasons"]) > 0, \
        "halt_reasons should document why entries are halted"
...
def test_market_regime_database_error_failclosed():
    ...
    assert regime["regime"] == "unknown", \
        f"Expected regime='unknown', got {regime['regime']}"
    assert len(regime["halt_reasons"]) > 0, \
        "halt_reasons should document the error"
...
def test_market_regime_valid_data():
    ...
    assert regime["is_entry_allowed"], \
        f"Expected is_entry_allowed=True, got {regime['is_entry_allowed']}"
    assert regime["exposure_pct"] == 75.0, \
        f"Expected exposure_pct=75.0, got {regime['exposure_pct']}"
    assert regime["regime"] == "normal", \
        f"Expected regime='normal', got {regime['regime']}"
