...
class TestBalanceSheetIdentity:
    ...
    def test_temporary_equity_closes_identity(self) -> None:
        ...
        mezzanine-equity amount (live OBAI shape) must NOT be flagged once temporary_equity is
        populated."""
        cur = _mock_cursor(
            [
                [
                    {
                        "symbol": "OBAI",
                        "fiscal_year": 2025,
                        "total_assets": 2_501_000.0,
                        "total_liabilities": 13_775_000.0,
                        "stockholders_equity": -22_663_000.0,
                        "noncontrolling_interest": None,
                        "temporary_equity": 11_389_000.0,
                    }
                ]
            ]
        )
...
class TestRunAggregatesAllChecks:
    def test_run_calls_all_one_hundred_eight_checks(self) -> None:
        ...
        # this permanently guards against) + 10 (2026-09-15, tie_out_income_statement_
        # nonnegative.py - 5 income-statement fields x annual/quarterly, the income-statement
        # side of the same nonnegative-magnitude guard class the balance-sheet fields already
        # had). All of these log an INFO result even when clean, so a no-violations run no
        # longer yields an empty results list for every check - except
        # check_quarterly_cashflow_cumulative_duplicate itself, which (like its sibling
        # check_quarterly_revenue_annual_duplicate) logs nothing at all when clean, matching
        # the assertion below (it only requires every LOGGED result be INFO, not one per check
        # call).
        cur = _mock_cursor([[]] * 114)
        checker = _checker()
        results = checker.run(cur)
        assert cur.execute.call_count == 114
