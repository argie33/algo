...
findings from the most recent DataPatrol run - closing a gap where this module's own docstring
("ISSUE #6 FIX: Integrate DataPatrol checks to block Phase 1") and terraform's DataPatrol step
comment both described this behavior, but no code anywhere in algo/orchestrator/ or
algo/orchestration/ ever queried the table. See _check_data_patrol_results's docstring for the
live-confirmed evidence (4 CRITICAL staleness findings on a local dev run that Phase 1 ignored).
"""
...
class TestCheckDataPatrolResults:
    def test_no_rows_at_all_warns_and_does_not_halt(self):
        cur = MagicMock()
    ...
    def test_stale_patrol_run_warns_and_does_not_halt(self):
        cur = MagicMock()
        ...
        assert result is None
        assert any("stale" in str(call).lower() for call in log_fn.calls)
