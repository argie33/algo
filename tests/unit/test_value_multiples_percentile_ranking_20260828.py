...
class TestPeCurveScoreUnchanged:
    """_pe_curve_score/_pb_curve_score are Pass-1 PROVISIONAL SCAFFOLDING ONLY - always
    overwritten by update_value_multiples_percentiles()'s real MSCI z-score pass. REPLACED WITH
    A FLAT NEUTRAL PLACEHOLDER 2026-09-17 (factor-purity follow-up, live-audited before removing
    - see value_metrics.py's own NEUTRAL_PLACEHOLDER_SCORE docstring for the full evidence these
    hand-set curves never survive as a live score) - this flattening is independent of the
    MSCI/AQR pillar-construction question and was kept as-is when Value reverted from AQR back
    to MSCI the same day, so these pin the flat placeholder, not the old hand-set curve values.
    _ps_curve_score was deleted 2026-09-16 (factor-purity sweep, P/S dropped from scoring
    entirely - see value_score.py's VALUE_MIN_WEIGHT docstring), so its own pinning test is gone
    with it."""
