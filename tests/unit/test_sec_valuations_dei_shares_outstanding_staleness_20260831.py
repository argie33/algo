...
class TestDeiSharesOutstandingStalenessRejected:
    def test_stale_dei_value_rejected_falls_through_to_fpi_yfinance(self) -> None:
        ...
        income_rows = [
            (2024, 738_169_736_000.0, 61_254_079_000.0, 33.01, None, None, None, None, None, None, True),
        ]
        fetchone_results: list[tuple[Any, ...] | None] = [
            (30_000_000.0,),  # cash_and_equivalents
            (5_000_000.0, None, None, None),  # debt_row
            (None,),  # company_info_sec fallback (FPI-safe tier, empty)
            (49_092_772_762.0, 2017),  # shares_outstanding_dei - stale (FY2017 vs most recent FY2024)
    ...
    def test_recent_dei_value_still_accepted(self) -> None:
        ...
        income_rows = [
            (2024, 10_000_000.0, 1_000_000.0, 2.0, None, None, None, None, None, None, True),
        ]
        fetchone_results: list[tuple[Any, ...] | None] = [
        ...
        with (
            ...
            result = _run_fetch_incremental("RECENTFPI", income_rows, fetchone_results)
