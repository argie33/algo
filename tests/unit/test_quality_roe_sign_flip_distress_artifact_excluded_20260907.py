...
class TestRoeSignFlipDistressArtifactExcluded:
    def test_positive_roe_with_negative_roa_floored_like_negative_roe(self) -> None:
        ...
        component filled in (4 x 15.0 + 25.0 = 85.0 under the 2026-09-15 asset_turnover+ROCE
        removal - see vqg_quality_score.py's "ASSET_TURNOVER + ROCE REMOVED ENTIRELY"
        ...
        percentile 50.0. roce_pct=20.0 in the row below is still read but no longer
        ...
        # singleton pool has no variance to standardize against), same as the other 4
        # components (weight 15.0 each except margin_volatility at 25.0 = 85.0 total, all
        # solo in their pools -> neutral 50.0).
        expected = round((50.0 * 85.0) / 100.0, 2)
    ...
    def test_positive_roe_with_positive_roa_still_ranks_normally(self) -> None:
        ...
        ranking the way the distress-artifact case above is. Both rows carry identical
        values for the other 6 components (weight 11+18+72=101, clears the 2026-09-10
        40%-of-101 completeness floor) so those tie and z-score to the same neutral 50.0
        for both symbols - only roe/roa (which differ) can move the comparison."""
        common = (20.0, 10.0, 0.2, 3.0, 90.0, 35.0)  # roce, fcf_margin, d2e, margin_vol, at, gp
        rows = [
            ("LOWER_REAL_ROE", "Technology", None, 20.0, 10.0, *common, 0.0),
            ("HRB_SHAPED", "Technology", None, 624.40, 18.59, *common, 0.0),
        ]
