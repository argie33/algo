...
class TestExcludesDataUnavailableRows:
    def test_all_four_latest_fiscal_year_queries_exclude_data_unavailable_true(self) -> None:
        ...
        # "COALESCE(long_term_debt" uniquely identifies the tiered debt_row query -
        # _get_net_borrowing_for_dcf (2026-08-25) also queries long_term_debt/
        # data_unavailable IS NOT TRUE, via a plain WHERE clause with no COALESCE tier.
        debt_queries = [sql for sql in cursor.executed_sql if "COALESCE(long_term_debt" in sql]
