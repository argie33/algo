#!/usr/bin/env python3
"""Integration tests for Phase 2 loaders (institutional holdings).

Tests verify:
1. Loader properly aggregates institutional data
2. Loader returns explicit data_unavailable markers
3. Loader follows governance rules (fail-fast, explicit markers, no silent fallbacks)
...
dataset aggregation (utils/external/sec_form345_bulk.py, used by the now-removed
InsiderHoldingsSECLoader) and the SEC 13F bulk dataset + OpenFIGI crosswalk (used by
InstitutionalHoldings13FLoader) - see steering/DATA_LOADERS.md's "Institutional holdings"
section. Never cleaned up after the bulk approach shipped.

REMOVED 2026-08-24 (real-money-readiness goal session): the insider-holdings tests that
used to live here (InsiderHoldingsSECLoader) along with the loader itself - see migration
1219_drop_insider_ownership.sql for the full rationale (static/slow-moving metric, near-zero
information content at this system's weeks-scale swing timeframe, recurring source of real
bugs). Institutional-holdings coverage below is unaffected and unrelated to that removal.
"""
...
class TestPhase2LoadersGovernance(unittest.TestCase):
    ...
    def test_loaders_never_silent_fail(self):
        ...
        with patch("loaders.load_institutional_holdings_13f.DatabaseContext") as mock_db_ctx:
            ...
            result = institutional_loader.fetch_incremental("AAPL", None)
            ...
            if result[0]["data_unavailable"]:
                ...
                for key in ["institutional_ownership_pct"]:
                    if key in result[0]:
