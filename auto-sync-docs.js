...
class DocumentSyncManager {
  ...
  async run() {
    ...
    try {
      ...
      // Step 8: Prepare to start building the next todo item
      const buildInstructions = await this.prepareTodoBuildInstructions(nextActions);
      
      console.log('✅ Auto-Sync Documentation Routine completed successfully');
      console.log('🔄 Ready to start building next todo item in Claude Code');
      return { nextActions, buildInstructions };
  ...
  async prepareTodoBuildInstructions(nextActions) {
    if (!nextActions || nextActions.length === 0) {
      this.log('📋 No high-priority actions to build');
      return { 
        status: 'no_actions', 
        message: 'No high-priority todos found',
        claudeInstructions: 'Continue with current work or run auto-sync again after changes.'
      };
    }

    const topAction = nextActions[0];
    this.log(`🎯 Preparing build instructions for: ${topAction.content}`);

    try {
      // Determine the type of task and create instructions for Claude Code
      const taskType = this.categorizeTask(topAction.content);
      const instructions = await this.generateClaudeInstructions(taskType, topAction);
      
      // Update todo status to in_progress
      await this.updateTodoStatus(topAction.id, 'in_progress');
      
      this.log(`✅ Generated build instructions for: ${topAction.content}`);
      return {
      ...
      this.log(`❌ Failed to generate build instructions: ${error.message}`);
      return {
        status: 'failed',
        action: topAction,
        error: error.message,
        claudeInstructions: 'Review the error and retry the auto-sync process.',
        timestamp: new Date().toISOString()
      };
    }
  }

  async generateClaudeInstructions(taskType, action) {
    switch (taskType) {
      case 'implementation':
        return this.generateImplementationInstructions(action);
      case 'testing':
        return this.generateTestingInstructions(action);
      case 'documentation':
        return this.generateDocumentationInstructions(action);
      case 'deployment':
        return this.generateDeploymentInstructions(action);
      case 'bugfix':
        return this.generateBugfixInstructions(action);
      case 'research':
        return this.generateResearchInstructions(action);
      default:
        return this.generateGeneralInstructions(action);
    }
  }

  generateImplementationInstructions(action) {
    if (action.content.includes('centralized live data service')) {
      return `
## Next Action: Implement Centralized Live Data Service

**Task**: ${action.content}

**Implementation Steps**:
1. Create \`webapp/lambda/services/centralizedLiveDataService.js\` with:
   - Single websocket connections per symbol
   - Subscriber management for multiple users
   - Data caching and distribution
   - Admin interface for managing feeds

2. Update existing live data components to use centralized service

3. Create admin interface for managing data feeds

4. Test the implementation with real data

5. Update documentation with the new architecture

**Tools to Use**: Write, Edit, Read, Bash for testing
**Priority**: HIGH - This will significantly reduce costs and improve performance
`;
    } else if (action.content.includes('trading signals ai')) {
      return `
## Next Action: Implement Real Trading Signals AI

**Task**: ${action.content}

**Implementation Steps**:
1. Remove remaining mock data from TradingSignals.jsx
2. Create real AI model integration
3. Connect to external AI/ML services
4. Test with real market data
5. Update error handling for real API failures

**Tools to Use**: Edit, Write, Read, Bash for testing
**Priority**: MEDIUM - Replace mock AI responses with real implementation
`;
    } else {
      return `
...
**Tools to Use**: Edit, Write, Read, Bash
**Priority**: ${action.priority.toUpperCase()}
`;
    }
  }

  generateTestingInstructions(action) {
    return `
## Next Action: ${action.content}

**Testing Steps**:
1. Run end-to-end tests to validate system functionality
2. Test deployment blockers are resolved
3. Validate API endpoints are working
4. Test authentication and security
5. Create test report and update documentation

**Tools to Use**: Bash for running tests, Read for checking results, Edit for updating docs
**Priority**: ${action.priority.toUpperCase()}
`;
  }

  generateDocumentationInstructions(action) {
    return `
## Next Action: ${action.content}

**Documentation Steps**:
1. Update the specified documentation files
2. Ensure consistency across all 4 core documents
3. Remove outdated information
4. Add current implementation status
5. Commit documentation changes

**Tools to Use**: Read, Edit, Write, Bash for git operations
**Priority**: ${action.priority.toUpperCase()}
`;
  }

  generateDeploymentInstructions(action) {
    return `
## Next Action: ${action.content}
...
4. Validate deployed system
5. Update system status

**Tools to Use**: Bash for deployment, Read for checking configs, Edit for updates
**Priority**: ${action.priority.toUpperCase()}
`;
  }

  generateBugfixInstructions(action) {
    return `
## Next Action: ${action.content}

**Bugfix Steps**:
1. Identify and analyze the root cause
2. Create fix implementation
3. Test the fix thoroughly
4. Deploy the fix
5. Update documentation

**Tools to Use**: Edit, Read, Bash for testing
**Priority**: ${action.priority.toUpperCase()}
`;
  }

  generateResearchInstructions(action) {
    return `
## Next Action: ${action.content}
...
4. Update todo list with action items
5. Present findings

**Tools to Use**: Read, WebSearch, Edit for documentation
**Priority**: ${action.priority.toUpperCase()}
`;
  }

  generateGeneralInstructions(action) {
    return `
## Next Action: ${action.content}
...
**Tools to Use**: Appropriate tools based on task requirements
**Priority**: ${action.priority.toUpperCase()}
`;
  }
  ...
  async implementCentralizedLiveDataService() {
    // This would actually call Claude Code's tools to implement the service
    console.log('🔧 ACTUALLY implementing centralized live data service...');
    ...
    return { step: 'Create centralized live data service architecture', status: 'implementing', action: 'real_implementation_started' };
  }
